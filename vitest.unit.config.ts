...
export function loadIncludePatternsFromEnv(
  ...
  return loadPatternListFromEnv("LADA_VITEST_INCLUDE_FILE", env);
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  return loadPatternListFromEnv("LADA_VITEST_EXTRA_EXCLUDE_FILE", env) ?? [];
}
...
export function createUnitVitestConfigWithOptions(
  ...
  return defineProject({
    ...
    test: {
      ...
      isolate,
      ...(isolate ? { runner: undefined } : { runner: "./tests/non-isolated-runner.ts" }),
      setupFiles: [
        ...new Set([...(sharedTest.setupFiles ?? []), "tests/setup-lada-runtime.ts"]),
      ],
