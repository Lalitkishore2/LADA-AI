---
name: node-connect
description: Diagnose LADA node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify LADA is advertising that route, then fix pairing/auth.
...
- whether `lada devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `lada qr --json`. It uses the same setup-code payload Android scans.

```bash
lada config get gateway.mode
lada config get gateway.bind
lada config get gateway.tailscale.mode
lada config get gateway.remote.url
lada config get gateway.auth.mode
lada config get gateway.auth.allowTailscale
lada config get plugins.entries.device-pair.config.publicUrl
lada qr --json
lada devices list
lada nodes status
```

If this LADA instance is pointed at a remote gateway, also run:
...
If `lada qr --json` says `Gateway is only bound to loopback`:
...
- `lada devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run lada qr again, rescan, then approve the pending device pairing.`
