---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for LADA deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, LADA cron scheduling for periodic checks, or version status checks on a machine running LADA (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running LADA, then align it to a user-defined risk tolerance without breaking access. Use LADA security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim LADA changes the host firewall, SSH, or OS updates; it does not.
...
5. LADA gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if LADA can access sensitive data.
...
### 2) Run LADA security audits (read-only)

As part of the default read-only checks, run `lada security audit --deep`. Only offer alternatives if the user requests them:

1. `lada security audit` (faster, non-probing)
2. `lada security audit --json` (structured output)

Offer to apply LADA safe defaults (numbered):

1. `lada security audit --fix`

Be explicit that `--fix` only tightens LADA defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check LADA version/update status (read-only)

As part of the default read-only checks, run `lada update status`.
...
- Credential hygiene notes (location of LADA creds, prefer disk encryption)
...
After LADA install or first hardening pass, run at least one baseline audit and version check:

- `lada security audit`
- `lada security audit --deep`
- `lada update status`

Ongoing monitoring is recommended. Use the LADA cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `lada cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `lada update status`
...
Before creating, `lada cron list` and match on exact `name`. If found, `lada cron edit <id> ...`.
If not found, `lada cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `lada update status` (preferred for source checkouts and channels)
2. `npm view lada version` (published npm version)

## LADA command accuracy

Use only supported commands and flags:

- `lada security audit [--deep] [--fix] [--json]`
- `lada status` / `lada status --deep`
- `lada health --json`
- `lada update status`
- `lada cron add|list|runs|run`

Do not invent CLI flags or imply LADA enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by LADA compaction:
