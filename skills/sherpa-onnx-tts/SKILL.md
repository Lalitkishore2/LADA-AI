...
1. Download the runtime for your OS (extracts into `$LADA_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.lada/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$LADA_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.lada/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active LADA config file (`$LADA_CONFIG_PATH`, default `~/.lada/lada.json`):
