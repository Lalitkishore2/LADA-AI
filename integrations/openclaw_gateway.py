...
- LADA_OPENCLAW_GATEWAY_URL: Gateway WebSocket URL (default: ws://127.0.0.1:18789)
- LADA_OPENCLAW_RECONNECT: Auto-reconnect on disconnect (default: true)
- LADA_OPENCLAW_DEBUG: Enable debug logging (default: false)

Legacy env aliases are still honored:
...
class OpenClawConfig:
    ...
    def from_env(cls) -> "OpenClawConfig":
        ...
        url = os.getenv("LADA_OPENCLAW_GATEWAY_URL", "").strip() or \
            os.getenv("OPENCLAW_GATEWAY_URL", "ws://127.0.0.1:18789").strip()
        reconnect_raw = os.getenv("LADA_OPENCLAW_RECONNECT", "").strip() or \
            os.getenv("OPENCLAW_RECONNECT", "true").strip()
        debug_raw = os.getenv("LADA_OPENCLAW_DEBUG", "").strip() or \
            os.getenv("OPENCLAW_DEBUG", "false").strip()

        return cls(
            url=url,
            auto_reconnect=reconnect_raw.lower() in {"1", "true", "yes", "on"},
