"""Feature-flagged OpenClaw adapter.

This module intentionally keeps OpenClaw integration in adapter mode only:
- No eager imports of archived OpenClaw modules on normal startup
...
- LADA_OPENCLAW_ADAPTER_ENABLED: enable adapter mode (default: 0)
- OPENCLAW_GATEWAY_URL: gateway URL (default: ws://127.0.0.1:18789)
- LADA_OPENCLAW_TIMEOUT_SEC: adapter call timeout (default: 20)
"""
...
class OpenClawAdapter:
    """Lazy adapter around archived OpenClaw gateway module."""
    ...
    def __init__(
        ...
        if timeout_sec is None:
            timeout_sec = float(os.getenv("LADA_OPENCLAW_TIMEOUT_SEC", "20"))
    ...
    def _load_gateway_module(self):
        ...
        if not path.exists():
            raise FileNotFoundError(f"Archived OpenClaw gateway not found at: {path}")

        spec = importlib.util.spec_from_file_location("lada_archived_openclaw_gateway", path)
        if spec is None or spec.loader is None:
            raise RuntimeError("Failed to build import spec for archived OpenClaw gateway")
    ...
    def _run(self, coro):
        ...
        if self._loop is None:
            raise RuntimeError("OpenClaw adapter event loop is not available")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.warning("[OpenClawAdapter] connect failed: %s", e)
            return False
    ...
    def disconnect(self):
        ...
        try:
            ...
            logger.warning("[OpenClawAdapter] disconnect warning: %s", e)
        finally:
            self._stop_loop()
    ...
    def navigate(self, url: str) -> bool:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] navigate failed: %s", e)
            return False
    ...
    def click(self, selector: str) -> bool:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] click failed: %s", e)
            return False
    ...
    def type_text(self, selector: str, text: str) -> bool:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] type failed: %s", e)
            return False
    ...
    def scroll(self, direction: str = "down", amount: int = 500) -> bool:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] scroll failed: %s", e)
            return False
    ...
    def extract_text(self, selector: Optional[str] = None) -> str:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] extract text failed: %s", e)
            return ""
    ...
    def snapshot_summary(self) -> Optional[dict]:
        ...
        try:
            ...
            self._state = "error"
            self._last_error = str(e)
            logger.warning("[OpenClawAdapter] snapshot failed: %s", e)
            return None


_adapter_singleton: Optional[OpenClawAdapter] = None


def openclaw_adapter_enabled() -> bool:
    return _flag_enabled("LADA_OPENCLAW_ADAPTER_ENABLED", "0")


def get_openclaw_adapter(force: bool = False) -> Optional[OpenClawAdapter]:
    """Get singleton OpenClaw adapter only when feature-flag is enabled.
    ...
    if not force and not openclaw_adapter_enabled():
        return None

    if _adapter_singleton is None:
        _adapter_singleton = OpenClawAdapter(enabled=True)
