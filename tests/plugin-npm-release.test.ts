...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @lada/zalo, @lada/feishu  @lada/zalo "),
    ).toEqual(["@lada/feishu", "@lada/zalo"]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "lada-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@lada/demo-plugin",
      version: "2026.4.6",
      lada: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@lada/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@lada/demo-plugin",
        version: "2026.4.6",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@lada/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@lada/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @lada/missing.");
  });
});
