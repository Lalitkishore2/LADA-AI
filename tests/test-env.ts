...
const LIVE_EXTERNAL_AUTH_DIRS = [".lada", ".codex", ".gemini", ".minimax"] as const;
const LIVE_EXTERNAL_AUTH_FILES = [".lada.json"] as const;
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.LADA_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "LADA_TEST_FAST", value: process.env.LADA_TEST_FAST },
    {
      key: "LADA_STRICT_FAST_REPLY_CONFIG",
      value: process.env.LADA_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "LADA_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.LADA_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "LADA_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.LADA_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "LADA_STATE_DIR", value: process.env.LADA_STATE_DIR },
    { key: "LADA_CONFIG_PATH", value: process.env.LADA_CONFIG_PATH },
    { key: "LADA_GATEWAY_PORT", value: process.env.LADA_GATEWAY_PORT },
    { key: "LADA_BRIDGE_ENABLED", value: process.env.LADA_BRIDGE_ENABLED },
    { key: "LADA_BRIDGE_HOST", value: process.env.LADA_BRIDGE_HOST },
    { key: "LADA_BRIDGE_PORT", value: process.env.LADA_BRIDGE_PORT },
    { key: "LADA_CANVAS_HOST_PORT", value: process.env.LADA_CANVAS_HOST_PORT },
    { key: "LADA_TEST_HOME", value: process.env.LADA_TEST_HOME },
    { key: "LADA_AGENT_DIR", value: process.env.LADA_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.LADA_TEST_HOME = tempHome;
  process.env.LADA_TEST_FAST = "1";
  process.env.LADA_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.LADA_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.LADA_GATEWAY_PORT;
  delete process.env.LADA_BRIDGE_ENABLED;
  delete process.env.LADA_BRIDGE_HOST;
  delete process.env.LADA_BRIDGE_PORT;
  delete process.env.LADA_CANVAS_HOST_PORT;
...
function restoreLADAConfigFromBackupIfNeeded(tempHome: string): void {
  const targetPath = path.join(tempHome, ".lada.json");
  if (fs.existsSync(targetPath)) {
    return;
  }
  const backupsDir = path.join(tempHome, ".lada", "backups");
  if (!fs.existsSync(backupsDir)) {
    return;
  }
  const latestBackup = fs
    .readdirSync(backupsDir)
    .filter((entry) => entry.startsWith(".lada.json.backup."))
    .toSorted()
    .at(-1);
  if (!latestBackup) {
    return;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.LADA_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.LADA_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".lada");
  const priorIsolatedHome = params.env.LADA_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".lada")
  ) {
    realStateDir = path.join(params.realHome, ".lada");
  }
  const tempStateDir = path.join(params.tempHome, ".lada");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.LADA_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.LADA_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "lada.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "lada.json"),
  ...
  restoreLADAConfigFromBackupIfNeeded(params.tempHome);
}
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.LADA_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.LADA_LIVE_USE_REAL_HOME);
