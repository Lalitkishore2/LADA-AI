"""Live smoke test for OpenClaw compatibility endpoints.

This test is intentionally env-gated and runs only when a real API server and
OpenClaw gateway are available.
"""
...
def _enabled() -> bool:
    return os.getenv("LADA_RUN_LIVE_OPENCLAW_SMOKE", "0").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
@pytest.mark.skipif(
    not _enabled(),
    reason="Set LADA_RUN_LIVE_OPENCLAW_SMOKE=1 to run live OpenClaw smoke tests",
)
def test_live_openclaw_smoke_matrix():
    ...
    if not _gateway_reachable(gateway_url):
        pytest.skip(f"OpenClaw gateway not reachable at {gateway_url}")
    ...
    status = call("GET", "/openclaw/status").json()
    assert status.get("success") is True

    connect = call("POST", "/openclaw/connect").json()
    assert connect.get("success") is True

    navigate = call("POST", "/openclaw/navigate", {"url": "https://example.com"}).json()
    assert navigate.get("success") is True

    action = call("POST", "/openclaw/action", {"action": "scroll", "direction": "down", "amount": 200}).json()
    assert action.get("success") is True

    snapshot = call("GET", "/openclaw/snapshot").json()
    assert snapshot.get("success") is True

    disconnect = call("POST", "/openclaw/disconnect").json()
