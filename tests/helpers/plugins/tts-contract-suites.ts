...
function asLegacyTtsConfig(value: unknown): LADAConfig {
  return value as LADAConfig;
}

function asLegacyLADAConfig(value: Record<string, unknown>): LADAConfig {
  return value as unknown as LADAConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): LADAConfig {
  ...
  describe("tts summarization contract", () => {
    ...
    it("uses summaryModel override when configured", async () => {
      const cfg: LADAConfig = {
        agents: { defaults: { model: { primary: "anthropic/lada-opus-4-5" } } },
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    const baseCfg: LADAConfig = asLegacyLADAConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.LADA_TTS_PREFS;
      process.env.LADA_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.LADA_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: LADAConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
