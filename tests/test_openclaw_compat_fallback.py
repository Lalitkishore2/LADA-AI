"""Fallback tests for OpenClaw compatibility router when adapter is unavailable."""
...
def test_openclaw_falls_back_to_stealth_for_navigation_and_actions(monkeypatch):
    client = _build_client(monkeypatch)

    status_resp = client.get("/openclaw/status", headers={"X-Request-ID": "openclaw-fallback-req-1"})
    assert status_resp.status_code == 200
    assert status_resp.headers["x-request-id"] == "openclaw-fallback-req-1"
    ...
    click_resp = client.post("/openclaw/action", json={"action": "click", "selector": "#login"})
    ...
    extract_resp = client.post("/openclaw/action", json={"action": "extract", "selector": "h1"})
...
def test_openclaw_snapshot_uses_stealth_when_adapter_unavailable(monkeypatch):
    client = _build_client(monkeypatch)
