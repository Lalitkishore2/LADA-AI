"""Tests for LADA browser compatibility API router."""
...
class _FakeState:
    def __init__(self):
        self.lada_browser_adapter = _FakeAdapter()
...
def _build_client(state):
    app = FastAPI()
    app.include_router(create_lada_browser_compat_router(state))
    return TestClient(app)


def test_lada_browser_status_and_connect_flow():
    state = _FakeState()
    client = _build_client(state)

    status_resp = client.get("/lada/browser/status", headers={"X-Request-ID": "lada-browser-status-req-1"})
    assert status_resp.status_code == 200
    assert status_resp.headers["x-request-id"] == "lada-browser-status-req-1"
    payload = status_resp.json()
    assert payload["success"] is True
    assert payload["adapter"]["enabled"] is True

    connect_resp = client.post("/lada/browser/connect", headers={"X-Request-ID": "lada-browser-connect-req-1"})
    assert connect_resp.status_code == 200
    assert connect_resp.headers["x-request-id"] == "lada-browser-connect-req-1"
    assert connect_resp.json()["success"] is True


def test_lada_browser_navigate_and_action_extract():
    state = _FakeState()
    client = _build_client(state)
    ...
    assert nav_payload["mode"] == "lada_browser_adapter"

    extract_resp = client.post("/lada/browser/action", json={"action": "extract", "selector": "#content"})
    assert extract_resp.status_code == 200
    extract_payload = extract_resp.json()
    assert extract_payload["success"] is True
    assert extract_payload["mode"] == "lada_browser_adapter"
    assert "selected:#content" in extract_payload["content"]


def test_lada_browser_snapshot_from_adapter():
    state = _FakeState()
    client = _build_client(state)
