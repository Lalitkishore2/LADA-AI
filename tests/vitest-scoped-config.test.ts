...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ LADA_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ LADA_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ LADA_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
});
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from LADA_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-vitest-scoped-"));
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          LADA_VITEST_INCLUDE_FILE: includeFile,
        },
      });
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the lada runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the lada runtime setup", () => {
  ...
  it("loads channel include overrides from LADA_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "lada-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
