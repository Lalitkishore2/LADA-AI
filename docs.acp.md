...
This document describes how the LADA ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`lada acp` exposes an ACP agent over stdio and forwards prompts to a running
LADA Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing LADA Gateway
...
- ACP client spawns `lada acp` and speaks ACP messages over stdio.
...
`lada acp` resolves the Gateway URL and auth from CLI flags or config:
