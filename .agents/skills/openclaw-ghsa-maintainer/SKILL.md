...
# LADA GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `lada-release-maintainer`.
...
```bash
fork=$(gh api /repos/lada/lada/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/lada/lada/security-advisories/<GHSA> \
