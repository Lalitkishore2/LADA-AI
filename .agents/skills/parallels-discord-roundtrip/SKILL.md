...
```bash
export LADA_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.lada/lada.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env LADA_PARALLELS_DISCORD_TOKEN \
...
- Use the `lada` wrapper for guest `message send/read`; `node lada.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/lada-parallels-smoke.*`
...
- host inbound nonce appears in `lada message read` output
