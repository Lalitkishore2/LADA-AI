---
name: lada-qa-testing
description: Run, watch, debug, and extend LADA QA testing with qa-lab and qa-channel. Use when Codex needs to execute the repo-backed QA suite, inspect live QA artifacts, debug failing scenarios, add new QA scenarios, or explain the LADA QA workflow. Prefer the live OpenAI lane with regular openai/gpt-5.4 in fast mode; do not use gpt-5.4-pro or gpt-5.4-mini unless the user explicitly overrides that policy.
---
...
```bash
LADA_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm lada qa suite \
...
5. If the user wants to watch the live UI, find the current `lada-qa` listen port and report `http://127.0.0.1:<port>`.
