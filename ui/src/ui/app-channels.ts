...
export async function handleWhatsAppStart(host: LADAApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: LADAApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: LADAApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: LADAApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: LADAApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: LADAApp): string {
...
function resolveGatewayHttpAuthHeader(host: LADAApp): string | null {
...
function buildGatewayHttpHeaders(host: LADAApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: LADAApp) {
...
export function handleNostrProfileToggleAdvanced(host: LADAApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: LADAApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: LADAApp) {
  const state = host.nostrProfileFormState;
