...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://lada.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://lada.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://lada.ai/abc-123", baseHref)).toBe(
      "blob:https://lada.ai/abc-123",
    );
  });
