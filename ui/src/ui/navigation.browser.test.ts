...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/lada");
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/lada/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/lada");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/lada/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__LADA_CONTROL_UI_BASE_PATH__ = "/lada";
    const app = mountApp("/lada/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/lada");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/lada/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("lada.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "lada.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/lada" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("lada.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/lada",
    });
    expect(JSON.parse(localStorage.getItem("lada.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/lada";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/lada#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/lada");
    expect(app.settings.token).toBe("");
