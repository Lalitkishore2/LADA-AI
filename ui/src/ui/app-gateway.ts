...
export function connectGateway(host: GatewayHost, options?: ConnectGatewayOptions) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as LADAApp);
      void loadAssistantIdentity(host as unknown as LADAApp);
      void loadAgents(host as unknown as LADAApp);
      void loadHealthState(host as unknown as LADAApp);
      void loadNodes(host as unknown as LADAApp, { quiet: true });
      void loadDevices(host as unknown as LADAApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as LADAApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as LADAApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as LADAApp, payload);
