"""
LADA API - OpenClaw compatibility routes (/openclaw/*)

Provides a native compatibility surface for OpenClaw-style browser automation calls.
If OpenClaw adapter mode is disabled, endpoints fall back to stealth browser mode.
"""
...
def create_openclaw_compat_router(state):
    """Create OpenClaw compatibility router."""
    async def _trace_request(request: Request, response: Response):
        set_request_id_header(request, response, prefix="openclaw")

    r = APIRouter(prefix="/openclaw", tags=["openclaw_compat"], dependencies=[Depends(_trace_request)])

    def _get_adapter():
        state.load_components()
        adapter = getattr(state, "openclaw_adapter", None)
        ...
        try:
            ...
            state.openclaw_adapter = adapter
            return adapter
        except Exception as e:
            logger.warning(f"[OpenClawAPI] Adapter unavailable: {e}")
            return None
    ...
    async def openclaw_connect():
        ...
        if not adapter:
            raise HTTPException(
                status_code=503,
                detail="OpenClaw adapter is disabled. Set LADA_OPENCLAW_ADAPTER_ENABLED=true to enable it.",
            )
    ...
    @r.post("/navigate")
    async def openclaw_navigate(body: dict = Body(default={})):
        ...
        if adapter:
            ...
            if ok:
                return {"success": True, "mode": "openclaw_adapter", "url": url}
    ...
    @r.post("/action")
    async def openclaw_action(body: dict = Body(default={})):
        ...
        if action == "click":
            ...
            if adapter:
                ...
                if ok:
                    return {"success": True, "mode": "openclaw_adapter", "action": action, "selector": selector}
        ...
        if action == "scroll":
            ...
            if adapter:
                ...
                if ok:
                    return {"success": True, "mode": "openclaw_adapter", "action": action, "direction": direction, "amount": amount}
    ...
    @r.get("/snapshot")
    async def openclaw_snapshot(selector: Optional[str] = None):
        adapter = _get_adapter()
        if adapter:
            data = adapter.snapshot_summary()
            if data:
                return {"success": True, "mode": "openclaw_adapter", "snapshot": data}
