...
function isSystemThrottleDisabled(env: Record<string, string | undefined>): boolean {
  const normalized = env.LADA_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export type LADAVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: LADAVitestPool = resolveDefaultVitestPool(env),
): number {
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.LADA_VITEST_MAX_WORKERS ?? env.LADA_TEST_WORKERS);
  if (override !== null) {
...
export const sharedVitestConfig = {
  resolve: {
    alias: [
      {
        find: "lada/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `lada/plugin-sdk/${subpath}`,
  ...
  test: {
    ...
    forceRerunTriggers: [
      ...
      "tests/setup.extensions.ts",
      "tests/setup-lada-runtime.ts",
