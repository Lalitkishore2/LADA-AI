...
const EXPECTED_REPOSITORY_URL = "https://github.com/lada/lada";
...
const skipPackValidationEnv = "LADA_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "lada") {
    errors.push(`package.json name must be "lada"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.lada !== "lada.mjs") {
    errors.push(
      `package.json bin.lada must be "lada.mjs"; found "${pkg.bin?.lada ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`lada-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `lada-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
