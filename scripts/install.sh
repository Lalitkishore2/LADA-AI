...
# LADA Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one LADA."
...
GUM_VERSION="${LADA_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 LADA Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://lada.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$LADA_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.lada.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_lada_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.lada-* "$npm_root"/lada 2>/dev/null || true
}

extract_lada_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_lada_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/lada" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/lada"|"/usr/local/bin/lada")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/lada/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale lada symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing lada binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing LADA package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_lada_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "LADA npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*lada" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_lada_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_lada_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "LADA npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an lada binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${LADA_TAGLINE_INDEX:-}" ]]; then
        if [[ "${LADA_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((LADA_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${LADA_NO_ONBOARD:-0}
NO_PROMPT=${LADA_NO_PROMPT:-0}
DRY_RUN=${LADA_DRY_RUN:-0}
INSTALL_METHOD=${LADA_INSTALL_METHOD:-}
LADA_VERSION=${LADA_VERSION:-latest}
USE_BETA=${LADA_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/lada"
GIT_DIR=${LADA_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${LADA_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${LADA_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${LADA_VERBOSE:-0}"
VERIFY_INSTALL="${LADA_VERIFY_INSTALL:-0}"
LADA_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
LADA installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/lada)
...
Environment variables:
  LADA_INSTALL_METHOD=git|npm
  LADA_VERSION=latest|next|main|<semver>|<spec>
  LADA_BETA=0|1
  LADA_GIT_DIR=...
  LADA_GIT_UPDATE=0|1
  LADA_NO_PROMPT=1
  LADA_VERIFY_INSTALL=1
  LADA_DRY_RUN=1
  LADA_NO_ONBOARD=1
  LADA_VERBOSE=1
  LADA_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected LADA checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a LADA source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_lada_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"lada"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://lada.ai/install.sh | bash"
}
...
ensure_lada_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/lada" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/lada" ]]; then
        ln -sf "$npm_root/lada/dist/entry.js" "${npm_bin}/lada"
        ui_info "Created lada bin link at ${npm_bin}/lada"
    fi
    return 0
}

# Check for existing LADA installation
check_existing_lada() {
    if [[ -n "$(type -P lada 2>/dev/null || true)" ]]; then
        ui_info "Existing LADA installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make lada show as \"command not found\" in new terminals."
...
warn_lada_not_found() {
    ui_warn "Installed, but lada is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t lada 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named lada; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_lada_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P lada 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/lada" ]]; then
        echo "${npm_bin}/lada"
        return 0
    fi
...
install_lada_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/lada/lada.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing LADA from git checkout: ${repo_dir}"
    else
        ui_info "Installing LADA from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning LADA" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building LADA" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/lada" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/lada"
    ui_success "LADA wrapper installed to \$HOME/.local/bin/lada"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view lada dist-tags.beta 2>/dev/null || true)"
...
install_lada() {
    local package_name="lada"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            LADA_VERSION="$beta_version"
    ...
    if [[ -z "${LADA_VERSION}" ]]; then
        LADA_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${LADA_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${LADA_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing LADA v${resolved_version}"
    else
        ui_info "Installing LADA (${LADA_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${LADA_VERSION}")"

    if ! install_lada_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_lada_paths
        install_lada_npm "${install_spec}"
    fi

    if [[ "${LADA_VERSION}" == "latest" && "${package_name}" == "lada" ]]; then
        if ! resolve_lada_bin &> /dev/null; then
            ui_warn "npm install lada@latest failed; retrying lada@next"
            cleanup_npm_lada_paths
            install_lada_npm "lada@next"
        fi
    fi

    ensure_lada_bin_link || true

    ui_success "LADA installed"
}
...
run_doctor() {
    ...
    local claw="${LADA_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_lada_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (lada not on PATH yet)"
        warn_lada_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${LADA_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.lada/workspace-${profile}"
    else
        echo "${HOME}/.lada/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${LADA_CONFIG_PATH:-$HOME/.lada/lada.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run lada onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but lada not on PATH; skipping onboarding"
        warn_lada_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run lada onboard to retry"
        return
    }
}
...
if ! declare -F extract_lada_semver >/dev/null 2>&1; then
...
resolve_lada_version() {
    local version=""
    local raw_version_output=""
    local claw="${LADA_BIN:-}"
    if [[ -z "$claw" ]] && command -v lada &> /dev/null; then
        claw="$(command -v lada)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_lada_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/lada/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/lada/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: lada not on PATH yet"
        warn_lada_not_found
        return 1
    fi

    run_quiet_step "Checking LADA version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: lada gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_lada_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found LADA checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set LADA_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    LADA_BIN="$(resolve_lada_bin || true)"
    ...
    installed_version=$(resolve_lada_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 LADA installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 LADA installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/lada"
        ui_kv "Update command" "lada update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://lada.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running lada doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                LADA_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                LADA_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                LADA_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run lada doctor and lada plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run lada onboard later"
        else
            local config_path="${LADA_CONFIG_PATH:-$HOME/.lada/lada.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (lada not on PATH yet)"
                    warn_lada_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run lada onboard to finish setup"
            return 0
        fi
    fi

    if command -v lada &> /dev/null; then
        local claw="${LADA_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_lada_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (lada daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if LADA_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: lada daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${LADA_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
