...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const ladaMeta =
    ...
    "lada" in source.packageJson
      ? (source.packageJson.lada as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    ladaMeta &&
    typeof ladaMeta.channel === "object" &&
    ladaMeta.channel &&
    !Array.isArray(ladaMeta.channel)
      ? (ladaMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
