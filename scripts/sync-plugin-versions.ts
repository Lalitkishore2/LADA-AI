...
const LADA_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncLADADependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.lada;
  if (!current || current === "workspace:*" || !LADA_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncMinHostVersion(pkg: PackageJson, targetVersion: string): boolean {
  const installConfig = pkg.lada?.install;
  const current = installConfig?.minHostVersion;
  if (!current || !LADA_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildLADAVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.lada?.build;
  const current = build?.ladaVersion;
  if (!current) {
    return false;
  }
  ...
  build.ladaVersion = targetVersion;
  return true;
}
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core LADA release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncLADADependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncLADADependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildLADAVersionChanged = syncBuildLADAVersion(pkg, targetVersion);
