...
# Check auth status
termux-toast "Checking LADA auth..."

STATUS=$(ssh "$SERVER" '$HOME/lada/scripts/lada-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/lada/scripts/lada-auth-status.sh json' 2>&1)
        HOURS=$(echo "$DETAILS" | jq -r '.lada_code.status' | grep -oP '\d+(?=h)' || echo "?")
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/lada/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: LADA won't work until re-auth"
                ;;
        esac
        ;;
