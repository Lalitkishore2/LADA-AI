#!/usr/bin/env bash
# Reset LADA like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${LADA_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="LADA.app/Contents/MacOS/LADA"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/LADA"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/LADA"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/LADA"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.lada.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/lada-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${LADA_RESTART_LOG:-/tmp/lada-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${LADA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.lada/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  LADA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node lada.mjs daemon install --force --runtime node"
      log "  node lada.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.lada/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.lada.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing LADA instances"
kill_all_lada
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product LADA"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/LADA.app" ]]; then
    APP_BUNDLE="/Applications/LADA.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/LADA.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/LADA.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/LADA.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set LADA_APP_BUNDLE to your installed LADA.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node lada.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node lada.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".lada", "lada.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: LADA is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.lada.gateway.plist' | head -n 40 || true"
fi
