...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".lada-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `lada-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `lada-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
