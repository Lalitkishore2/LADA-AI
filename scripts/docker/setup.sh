...
EXTRA_MOUNTS="${LADA_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${LADA_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${LADA_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${LADA_DOCKER_SOCKET:-}"
TIMEZONE="${LADA_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$LADA_CONFIG_DIR/lada.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == LADA_GATEWAY_TOKEN=* ]]; then
      token="${line#LADA_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${LADA_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$LADA_GATEWAY_PORT" "$LADA_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$LADA_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$LADA_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network lada-cli service because it
  ...
  run_prestart_gateway --entrypoint node lada-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" lada-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "LADA_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "LADA_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
LADA_CONFIG_DIR="${LADA_CONFIG_DIR:-$HOME/.lada}"
LADA_WORKSPACE_DIR="${LADA_WORKSPACE_DIR:-$HOME/.lada/workspace}"

validate_mount_path_value "LADA_CONFIG_DIR" "$LADA_CONFIG_DIR"
validate_mount_path_value "LADA_WORKSPACE_DIR" "$LADA_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "LADA_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "LADA_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "LADA_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "LADA_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "LADA_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "LADA_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$LADA_CONFIG_DIR"
mkdir -p "$LADA_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$LADA_CONFIG_DIR/identity"
mkdir -p "$LADA_CONFIG_DIR/agents/main/agent"
mkdir -p "$LADA_CONFIG_DIR/agents/main/sessions"

export LADA_CONFIG_DIR
export LADA_WORKSPACE_DIR
export LADA_GATEWAY_PORT="${LADA_GATEWAY_PORT:-18789}"
export LADA_BRIDGE_PORT="${LADA_BRIDGE_PORT:-18790}"
export LADA_GATEWAY_BIND="${LADA_GATEWAY_BIND:-lan}"
export LADA_IMAGE="$IMAGE_NAME"
export LADA_DOCKER_APT_PACKAGES="${LADA_DOCKER_APT_PACKAGES:-}"
export LADA_EXTENSIONS="${LADA_EXTENSIONS:-}"
export LADA_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export LADA_HOME_VOLUME="$HOME_VOLUME_NAME"
export LADA_ALLOW_INSECURE_PRIVATE_WS="${LADA_ALLOW_INSECURE_PRIVATE_WS:-}"
export LADA_SANDBOX="$SANDBOX_ENABLED"
export LADA_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export LADA_TZ="$TIMEZONE"
...
if [[ -z "${LADA_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    LADA_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $LADA_CONFIG_DIR/lada.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      LADA_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      LADA_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      LADA_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${LADA_CONFIG_DIR}:/home/node/.lada"
    gateway_workspace_mount="${LADA_WORKSPACE_DIR}:/home/node/.lada/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${LADA_INSTALL_DOCKER_CLI:-}" ]]; then
    export LADA_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  LADA_EXTRA_MOUNTS \
  LADA_HOME_VOLUME \
  LADA_DOCKER_APT_PACKAGES \
  LADA_EXTENSIONS \
  LADA_SANDBOX \
  LADA_DOCKER_SOCKET \
  DOCKER_GID \
  LADA_INSTALL_DOCKER_CLI \
  LADA_ALLOW_INSECURE_PRIVATE_WS \
  LADA_TZ

if [[ "$IMAGE_NAME" == "lada:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "LADA_DOCKER_APT_PACKAGES=${LADA_DOCKER_APT_PACKAGES}" \
    --build-arg "LADA_EXTENSIONS=${LADA_EXTENSIONS}" \
    --build-arg "LADA_INSTALL_DOCKER_CLI=${LADA_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the LADA metadata subdirectory
# (.lada/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh lada-gateway -c \
  'find /home/node/.lada -xdev -exec chown node:node {} +; \
   [ -d /home/node/.lada/workspace/.lada ] && chown -R node:node /home/node/.lada/workspace/.lada || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from LADA_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $LADA_GATEWAY_BIND"
echo "Gateway token: $LADA_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm lada-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm lada-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm lada-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.lada.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d lada-gateway

# --- Sandbox setup (opt-in via LADA_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: lada-sandbox:bookworm-slim"
    run_docker_build \
      -t "lada-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker lada-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg LADA_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (LADA_IMAGE=lada:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: LADA_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.lada.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate lada-gateway
  fi
else
...
echo "Config: $LADA_CONFIG_DIR"
echo "Workspace: $LADA_WORKSPACE_DIR"
echo "Token: $LADA_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f lada-gateway"
echo "  ${COMPOSE_HINT} exec lada-gateway node dist/index.js health --token \"$LADA_GATEWAY_TOKEN\""
