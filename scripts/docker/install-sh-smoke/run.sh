...
SMOKE_PREVIOUS_VERSION="${LADA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${LADA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="lada"
PACKAGE_NAME="${LADA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (LADA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${LADA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${LADA_INSTALL_LATEST_OUT:-}"
fi
