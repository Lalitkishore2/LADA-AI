# LADA Installer for Windows (PowerShell)
# Usage: iwr -useb https://lada.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://lada.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\lada",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 LADA Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one LADA.$NC" -Level info
    Write-Host ""
}
...
function Install-LADAGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing LADA from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/lada/lada.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\lada\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\lada.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install LADA from git to $GitDir" -Level info
        } else {
            Install-LADAGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install LADA via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-LADANpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'lada onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 LADA installed successfully!" -Level success
}
