 ...
 * `lada update`. Use it to verify the completion installation flow
...
\`lada update\`, \`lada doctor\`, and \`lada onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# LADA Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`lada completion --install\` later to enable.`));
    return;
  }
