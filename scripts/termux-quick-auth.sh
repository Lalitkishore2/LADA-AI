...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/lada/scripts/lada-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "LADA Re-Auth" -c "After getting key, run: ssh $SERVER '~/lada/scripts/mobile-reauth.sh'" --id lada-auth
        ;;
    *)
