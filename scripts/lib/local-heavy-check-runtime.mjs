...
export function isLocalCheckEnabled(env) {
  const raw = env.LADA_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (nextEnv.LADA_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.LADA_TSGO_PPROF_DIR);
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const locksDir = path.join(commonDir, "lada-local-checks");
  const lockDir = path.join(locksDir, `${params.lockName ?? "heavy-check"}.lock`);
  const ownerPath = path.join(lockDir, "owner.json");
  const timeoutMs = readPositiveInt(
    env.LADA_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
  );
  const pollMs = readPositiveInt(env.LADA_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
  const progressMs = readPositiveInt(
    env.LADA_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
  );
  const staleLockMs = readPositiveInt(
    env.LADA_HEAVY_CHECK_STALE_LOCK_MS,
...
function readLocalCheckMode(env) {
  const raw = env.LADA_LOCAL_CHECK_MODE?.trim().toLowerCase();
