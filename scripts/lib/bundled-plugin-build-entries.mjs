...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@lada/${params.dirName}`) {
    return false;
  }
...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.lada?.extensions)
    ? packageJson.lada.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.lada?.setupEntry === "string" &&
    packageJson.lada.setupEntry.trim().length > 0
      ? packageJson.lada.setupEntry
      : undefined;
  if (setupEntry) {
...
function shouldStageBundledPluginRuntimeDependencies(packageJson) {
  return packageJson?.lada?.bundle?.stageRuntimeDependencies === true;
}
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "lada.plugin.json");
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "lada.plugin.json"));
    }
    if (hasPackageJson) {
