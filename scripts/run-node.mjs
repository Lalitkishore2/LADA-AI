...
const extensionRestartMetadataFiles = new Set(["lada.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.LADA_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by LADA_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.LADA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[lada] ${message}\n`);
};
...
const runLADA = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["lada.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.LADA_WATCH_MODE === "1";
