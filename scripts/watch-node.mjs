...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[lada] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.LADA_WATCH_MODE = "1";
  childEnv.LADA_WATCH_SESSION = watchSession;
  ...
  childEnv.LADA_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.LADA_WATCH_COMMAND = deps.args.join(" ");
  }
