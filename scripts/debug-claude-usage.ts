...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.LADA_STATE_DIR?.trim() || path.join(os.homedir(), ".lada");
...
const readLADACliKeychain = (): {
  ...
  try {
    const raw = execFileSync(
      "security",
      ["find-generic-password", "-s", "LADA-credentials", "-w"],
      { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"], timeout: 5000 },
    );
    const parsed = JSON.parse(raw.trim()) as Record<string, unknown>;
    const oauth = parsed?.ladaAiOauth as Record<string, unknown> | undefined;
    ...
    const out = execFileSync(
      ...
      [
        ...
        `
          ...
          FROM cookies
          WHERE (host_key LIKE '%lada.ai%' OR host_key = '.lada.ai')
            AND name = 'sessionKey'
          LIMIT 1;
        `,
      ],
    ...
    const out = execFileSync(
      ...
      [
        ...
        `
          SELECT value
          FROM moz_cookies
          WHERE (host LIKE '%lada.ai%' OR host = '.lada.ai')
            AND name = 'sessionKey'
          LIMIT 1;
        `,
      ],
...
const findLADASessionKey = (): { sessionKey: string; source: string } | null => {
  if (process.platform !== "darwin") {
    return null;
  }
...
const fetchLADAWebUsage = async (sessionKey: string) => {
  const headers = {
  ...
  const orgRes = await fetch("https://lada.ai/api/organizations", { headers });
  ...
  const usageRes = await fetch(`https://lada.ai/api/organizations/${orgId}/usage`, { headers });
...
const main = async () => {
  ...
  const keychain = readLADACliKeychain();
  if (keychain) {
    console.log(
      `LADA CLI keychain: accessToken=${opts.reveal ? keychain.accessToken : mask(keychain.accessToken)} scopes=${keychain.scopes?.join(",") ?? "(unknown)"}`,
    );
    ...
    console.log("LADA CLI keychain: missing/unreadable");
  }
  ...
  const sessionKey =
    ...
    findLADASessionKey()?.sessionKey;
  const source = opts.sessionKey
    ? "--session-key"
    : process.env.CLAUDE_AI_SESSION_KEY || process.env.CLAUDE_WEB_SESSION_KEY
      ? "env"
      : (findLADASessionKey()?.source ?? "auto");

  if (!sessionKey) {
    console.log(
      "LADA web: no sessionKey found (try --session-key or export CLAUDE_AI_SESSION_KEY)",
    );
    return;
  }

  console.log(
    `LADA web: sessionKey=${opts.reveal ? sessionKey : mask(sessionKey)} (source: ${source})`,
  );
  const web = await fetchLADAWebUsage(sessionKey);
  if (!web.ok) {
    console.log(`LADA web: ${web.step} HTTP ${web.status}`);
    console.log(String(web.body).slice(0, 400).replace(/\s+/g, " ").trim());
    return;
  }
  console.log(`LADA web: org=${web.orgId} OK`);
