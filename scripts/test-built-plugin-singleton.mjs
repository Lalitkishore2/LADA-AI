...
const warningFilterKey = Symbol.for("lada.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadLADAPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadLADAPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "lada.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'lada/plugin-sdk';",
...
const registry = loadLADAPlugins({
  ...
  env: {
    ...process.env,
    LADA_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
