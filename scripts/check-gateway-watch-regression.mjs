...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  LADA_SKIP_ACPX_RUNTIME: "1",
  LADA_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$LADA_WATCH_PID_FILE"',
    'mkdir -p "$LADA_HOME/.lada"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$LADA_HOME/.lada/lada.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    LADA_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    LADA_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-gateway-watch-"));
