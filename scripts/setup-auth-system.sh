#!/bin/bash
# Setup LADA Auth Management System
# Run this once to set up:
# 1. Long-lived LADA token
...
echo "  2. Subscribe to a topic (e.g., 'lada-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  LADA message: Send warning via LADA itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/lada-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/lada-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/lada-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now lada-auth-monitor.timer
...
echo "  - Auth status: $SCRIPT_DIR/lada-auth-status.sh"
echo "  - Mobile re-auth: $SCRIPT_DIR/mobile-reauth.sh"
echo "  - Auth monitor: systemctl --user status lada-auth-monitor.timer"
echo ""
echo "Quick commands:"
echo "  Check auth:  $SCRIPT_DIR/lada-auth-status.sh"
