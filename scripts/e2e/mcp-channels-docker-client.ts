...
import {
  assert,
  LADAChannelNotificationSchema,
  LADAPermissionNotificationSchema,
  connectGateway,
  connectMcpClient,
  extractTextFromGatewayPayload,
  type LADAChannelNotification,
...
async function main() {
  ...
  try {
    ...
    let helpNotification: LADAChannelNotification;
    try {
      helpNotification = await waitFor(
        "LADA channel notification",
        () =>
          mcpHandle.rawMessages
            .map((entry) => LADAChannelNotificationSchema.safeParse(entry))
            .find(
              (entry) =>
                entry.success &&
      ...
      throw new Error(
        `timeout waiting for LADA channel notification: ${JSON.stringify(
          {
    ...
    assert(helpNotification.content === channelMessage, "expected LADA channel content");
    ...
    const permission = await waitFor(
      "LADA permission notification",
      () =>
        mcpHandle.rawMessages
          .map((entry) => LADAPermissionNotificationSchema.safeParse(entry))
      ...
      JSON.stringify(
        {
          ...
          rawNotifications: mcpHandle.rawMessages.filter(
            (entry) =>
              LADAChannelNotificationSchema.safeParse(entry).success ||
              LADAPermissionNotificationSchema.safeParse(entry).success,
          ).length,
        },
        null,
        2,
