...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/lada-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: lada@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/lada-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/lada-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/lada warnings do not
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$LADAPath,
    [int]$Attempts = 10,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      Invoke-Logged 'lada gateway status' { & $LADAPath gateway status --deep --require-rpc }
      return
    } catch {
...
try {
  ...
  $tgz = Join-Path $env:TEMP 'lada-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'lada --version' { & $lada --version }
  ...
  Invoke-Logged 'lada models set' { & $lada models set $ModelId }
  ...
  Invoke-Logged 'lada gateway restart' { & $lada gateway restart }
  Write-ProgressLog 'update.gateway-status'
  Wait-GatewayRpcReady -LADAPath $lada
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'lada agent' { & $lada agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/lada-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"LADA \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="lada-update-$RANDOM-$RANDOM.ps1"
  log_name="lada-update-$RANDOM-$RANDOM.log"
  done_name="lada-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/lada-update-log-state.XXXXXX")"
  : >"$log_state_path"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/lada-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/lada-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/lada-main-update.tgz
version="\$(/opt/homebrew/bin/lada --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/lada models set "$MODEL_ID"
...
/opt/homebrew/bin/lada gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/lada gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
/opt/homebrew/bin/lada gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/lada agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/lada-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/lada-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/lada-main-update.tgz
npm install -g /tmp/lada-main-update.tgz --no-fund --no-audit
version="\$(lada --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
lada models set "$MODEL_ID"
lada agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/lada-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="lada@$LATEST_VERSION"
fi
