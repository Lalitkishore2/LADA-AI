...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://lada.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_LADA_BIN="/opt/homebrew/bin/lada"
GUEST_LADA_ENTRY="/opt/homebrew/lib/node_modules/lada/lada.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/lada-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://lada.ai/install.sh
...
resolve_guest_git_lada_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/lada/lada.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__LADA_READY__\\n'\r"
expect "__LADA_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/lada-prl.sh <<'__LADA_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__LADA_SCRIPT__\r"
send -- "/bin/bash /tmp/lada-prl.sh; rc=\$?; rm -f /tmp/lada-prl.sh; printf '__LADA_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
export LADA_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/lada-install.sh
bash /tmp/lada-install.sh${version_arg_q}
$GUEST_LADA_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/lada-smoke-pnpm-bootstrap"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/lada-smoke-pnpm-bootstrap/node_modules/.bin"
  guest_home="$(resolve_guest_current_user_home)"
  update_root="$guest_home/lada"
  update_entry="$update_root/lada.mjs"
  ensure_guest_pnpm_for_dev_update
  printf 'update-dev: run\n'
  guest_current_user_sh "$(cat <<EOF
rm -rf $(shell_quote "$update_root")
export PATH=$(shell_quote "$bootstrap_bin:$GUEST_EXEC_PATH")
$GUEST_NODE_BIN $GUEST_LADA_ENTRY update --channel dev --yes --json
EOF
)"
...
verify_dev_channel_update() {
  ...
  update_entry="$(resolve_guest_git_lada_entry)"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_LADA_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/lada-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_gateway() {
  guest_current_user_cli "$GUEST_LADA_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_LADA_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_LADA_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_LADA_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_LADA_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_LADA_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_LADA_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from lada dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
verify_dashboard_load() {
  ...
  # `lada dashboard --no-open` can hang under the Tahoe Parallels transport
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/lada-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>LADA Control</title>' /tmp/lada-dashboard-smoke.html >/dev/null; then
      if grep -F '<lada-app></lada-app>' /tmp/lada-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>LADA Control</title>' /tmp/lada-dashboard-smoke.html >/dev/null
grep -F '<lada-app></lada-app>' /tmp/lada-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/lada-discord-token <<'__LADA_TOKEN__'
$DISCORD_TOKEN_VALUE
__LADA_TOKEN__
cat >/tmp/lada-discord-guilds.json <<'__LADA_GUILDS__'
$guilds_json
__LADA_GUILDS__
token="\$(tr -d '\n' </tmp/lada-discord-token)"
guilds_json="\$(cat /tmp/lada-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_LADA_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_LADA_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_LADA_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_LADA_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_LADA_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_LADA_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_LADA_ENTRY channels status --probe --json
rm -f /tmp/lada-discord-token /tmp/lada-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "lada-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "lada-main-upgrade.tgz"
