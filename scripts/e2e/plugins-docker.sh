...
if [ -f dist/index.mjs ]; then
  LADA_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  LADA_ENTRY="dist/index.js"
else
...
home_dir=$(mktemp -d "/tmp/lada-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
LADA_PLUGIN_HOME="$HOME/.lada/$BUNDLED_PLUGIN_ROOT_DIR"
...
const configPath = path.join(process.env.HOME, ".lada", "lada.json");
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$LADA_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$LADA_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$LADA_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JSON
  node - <<'NODE' "$dir/lada.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$LADA_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/lada.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$LADA_ENTRY" plugins list --json > /tmp/plugins.json
node "$LADA_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/lada-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@lada/demo-plugin-tgz",
  "version": "0.0.1",
  "lada": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/lada.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$LADA_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$LADA_ENTRY" plugins list --json > /tmp/plugins2.json
node "$LADA_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/lada-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@lada/demo-plugin-dir",
  "version": "0.0.1",
  "lada": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/lada.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$LADA_ENTRY" plugins install "$dir_plugin"
node "$LADA_ENTRY" plugins list --json > /tmp/plugins3.json
node "$LADA_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/lada-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@lada/demo-plugin-file",
  "version": "0.0.1",
  "lada": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/lada.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$LADA_ENTRY" plugins install "file:$file_pack_dir/package"
node "$LADA_ENTRY" plugins list --json > /tmp/plugins4.json
node "$LADA_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
echo "Testing /plugin alias with LADA bundle restart semantics..."
bundle_plugin_id="lada-bundle-e2e"
bundle_root="$LADA_PLUGIN_HOME/$bundle_plugin_id"
mkdir -p "$bundle_root/.lada-plugin" "$bundle_root/commands"
cat > "$bundle_root/.lada-plugin/plugin.json" <<'JSON'
{
  "name": "lada-bundle-e2e"
}
JSON
...
gateway_log="/tmp/lada-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/lada-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@lada/slash-install-plugin",
  "version": "0.0.1",
  "lada": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/lada.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
const text = payload.text || "";
if (!text.includes("lada-bundle-e2e")) {
...
const text = payload.text || "";
if (!text.includes('Plugin "lada-bundle-e2e" enabled')) {
...
wait_for_gateway_health
run_gateway_chat_json "plugin-e2e-show" "/plugin show lada-bundle-e2e" /tmp/plugin-command-show.json
node - <<'NODE'
const fs = require("node:fs");
const payload = JSON.parse(fs.readFileSync("/tmp/plugin-command-show.json", "utf8"));
const text = payload.text || "";
if (!text.includes('"bundleFormat": "lada"')) {
  throw new Error(`expected LADA bundle inspect payload, got:\n${text}`);
}
if (!text.includes('"enabled": true')) {
...
if (!text.includes("BUNDLE_OK")) {
  throw new Error(`expected LADA bundle command reply, got:\n${text}`);
}
console.log("ok");
NODE
else
  echo "Skipping live LADA bundle command invocation (OPENAI_API_KEY not set)."
fi

echo "Testing marketplace install and update flows..."
marketplace_root="$HOME/.lada/plugins/marketplaces/fixture-marketplace"
mkdir -p "$HOME/.lada/plugins" "$marketplace_root/.lada-plugin"
write_fixture_plugin \
...
write_fixture_plugin \
  ...
  "Marketplace Direct"
cat > "$marketplace_root/.lada-plugin/marketplace.json" <<'JSON'
{
...
JSON
cat > "$HOME/.lada/plugins/known_marketplaces.json" <<JSON
{
  "lada-fixtures": {
    "installLocation": "$marketplace_root",
    "source": {
      "type": "github",
      "repo": "lada/fixture-marketplace"
    }
  }
}
JSON

node "$LADA_ENTRY" plugins marketplace list lada-fixtures --json > /tmp/marketplace-list.json
...
node "$LADA_ENTRY" plugins install marketplace-shortcut@lada-fixtures
node "$LADA_ENTRY" plugins install marketplace-direct --marketplace lada-fixtures
node "$LADA_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$LADA_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$LADA_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
for (const id of ["marketplace-shortcut", "marketplace-direct"]) {
  ...
  if (record.marketplaceSource !== "lada-fixtures") {
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$LADA_ENTRY" plugins update marketplace-shortcut --dry-run
node "$LADA_ENTRY" plugins update marketplace-shortcut
node "$LADA_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$LADA_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
