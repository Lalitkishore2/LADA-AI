...
export const LADAChannelNotificationSchema = z.object({
...
export const LADAPermissionNotificationSchema = z.object({
...
export type LADAChannelNotification = z.infer<typeof LADAChannelNotificationSchema>["params"];
...
export async function connectMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      LADA_ALLOW_INSECURE_PRIVATE_WS: "1",
      LADA_STATE_DIR: "/tmp/lada-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[lada mcp] ${String(chunk)}`);
  });
