...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/lada-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/lada-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/lada-install.sh
  guest_exec /usr/bin/env LADA_NO_ONBOARD=1 bash /tmp/lada-install.sh "${version_args[@]}" --no-onboard
  guest_exec lada --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec lada --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" lada onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "lada gateway run" >/dev/null 2>&1 || true
rm -f /tmp/lada-parallels-linux-gateway.log
setsid sh -lc 'exec env LADA_HOME=/root LADA_STATE_DIR=/root/.lada LADA_CONFIG_PATH=/root/.lada/lada.json ${API_KEY_ENV}=${api_key_value_q} lada gateway run --bind loopback --port 18789 --force >/tmp/lada-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec lada gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec lada gateway status --deep --require-rpc
    return
  fi
  guest_exec lada gateway status --deep
}

verify_local_turn() {
  guest_exec lada models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" lada agent \
    --local \
    --agent main \
    --message ping \
    --json
