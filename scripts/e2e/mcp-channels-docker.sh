...
IMAGE_NAME="${LADA_IMAGE:-lada-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="lada-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t lada-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "LADA_SKIP_GMAIL_WATCHER=1" \
  -e "LADA_SKIP_CRON=1" \
  -e "LADA_SKIP_CANVAS_HOST=1" \
  -e "LADA_STATE_DIR=/tmp/lada-state" \
  -e "LADA_CONFIG_PATH=/tmp/lada-state/lada.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "LADA_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
