...
type DriverMode = "token" | "webhook" | "lada";
...
function resolveStateDir(): string {
  const override = process.env.LADA_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.LADA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".lada");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver lada] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) LADA spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|lada> Driver transport mode (default: token)\n" +
    ...
    "  --lada-bin <path>        LADA CLI binary for driver=lada (default: lada)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  LADA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  LADA_DISCORD_SMOKE_DRIVER\n" +
    "  LADA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  LADA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  LADA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  LADA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  LADA_DISCORD_SMOKE_AGENT\n" +
    "  LADA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  LADA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  LADA_DISCORD_SMOKE_POLL_MS\n" +
    "  LADA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  LADA_DISCORD_SMOKE_LADA_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.LADA_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.LADA_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "lada"
        ? "lada"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.LADA_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.LADA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.LADA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.LADA_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.LADA_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.LADA_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.LADA_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.LADA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.LADA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.LADA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const ladaBin =
    resolveArg("--lada-bin") || process.env.LADA_DISCORD_SMOKE_LADA_BIN || "lada";
...
async function ladaCliJson<T>(params: { ladaBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.ladaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`lada ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await ladaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    ladaBin: params.ladaBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "lada") {
    return await readMessagesWithOpenclaw({
      ladaBin: params.args.ladaBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `lada-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("lada message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from LADA.`,
        diagnostics: {
          bindingCandidates: [
            {
