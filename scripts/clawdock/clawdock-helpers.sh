#!/usr/bin/env bash
# ClawDock - Docker helpers for LADA
# Inspired by Simon Willison's "Running LADA in Docker"
# https://til.simonwillison.net/llms/lada-docker
#
# Installation:
...
# Common paths to check for LADA
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found LADA at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/lada"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ LADA not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/lada/lada.git ~/lada"
    echo "  cd ~/lada && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^LADA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d lada-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart lada-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f lada-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.lada"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show lada.json
  if [[ -f "${config_dir}/lada.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/lada.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/lada.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No lada.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec lada-gateway \
    bash -c 'echo "alias lada=\"./lada.mjs\"" > /tmp/.bashrc_lada && bash --rcfile /tmp/.bashrc_lada'
}
...
clawdock-cli() {
  _clawdock_compose run --rm lada-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build lada-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d lada-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build lada-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "LADA_GATEWAY_TOKEN=$token" lada-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" lada-gateway \
    bash -c './lada.mjs config set gateway.remote.token "$TOKEN" && ./lada.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec lada-gateway \
    bash -c "./lada.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart lada-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm lada-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec lada-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'lada config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for LADA${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (lada alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to lada project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.lada)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lada channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lada status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.lada.ai${_CLR_RESET}"
  echo ""
}
