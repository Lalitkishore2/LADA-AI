...
- regenerates apps/ios/LADA.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${LADA_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid LADA_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid LADA_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid LADA_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid LADA_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing LADA_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(LADA_URL_SLASH)$(LADA_URL_SLASH)#g'
)"
...
LADA_CODE_SIGN_STYLE = Automatic
LADA_DEVELOPMENT_TEAM = ${TEAM_ID}
LADA_IOS_SELECTED_TEAM = ${TEAM_ID}
LADA_APP_BUNDLE_ID = ai.lada.client
LADA_SHARE_BUNDLE_ID = ai.lada.client.share
LADA_ACTIVITY_WIDGET_BUNDLE_ID = ai.lada.client.activitywidget
LADA_WATCH_APP_BUNDLE_ID = ai.lada.client.watchkitapp
LADA_WATCH_EXTENSION_BUNDLE_ID = ai.lada.client.watchkitapp.extension
LADA_APP_PROFILE =
LADA_SHARE_PROFILE =
LADA_PUSH_TRANSPORT = relay
LADA_PUSH_DISTRIBUTION = official
LADA_URL_SLASH = /
LADA_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
LADA_PUSH_APNS_ENVIRONMENT = production
EOF
