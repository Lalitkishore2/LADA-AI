#!/usr/bin/env bash
# One-time host setup for rootless LADA in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.lada by default, and uses the repo-local
# launch script at ./scripts/run-lada-podman.sh.
#
...
#   Or set LADA_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-lada-podman.sh launch
#   ./scripts/run-lada-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start lada.service
set -euo pipefail

REPO_PATH="${LADA_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-lada-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/lada.container.in"
LADA_USER="$(id -un)"
LADA_HOME="${HOME:-}"
LADA_CONFIG_DIR="${LADA_CONFIG_DIR:-}"
LADA_WORKSPACE_DIR="${LADA_WORKSPACE_DIR:-}"
LADA_IMAGE="${LADA_PODMAN_IMAGE:-${LADA_IMAGE:-lada:local}}"
LADA_CONTAINER_NAME="${LADA_PODMAN_CONTAINER:-lada}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${LADA_PODMAN_GATEWAY_HOST_PORT:-${LADA_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate LADA_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${LADA_PODMAN_QUADLET:-}" ]]; then
  case "${LADA_PODMAN_QUADLET,,}" in
...
if [[ "$LADA_IMAGE" == "lada:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set LADA_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$LADA_HOME" ]]; then
  LADA_HOME="$(resolve_user_home "$LADA_USER")"
fi
if [[ -z "$LADA_HOME" ]]; then
  echo "Unable to resolve HOME for user $LADA_USER." >&2
  exit 1
fi
if [[ -z "$LADA_CONFIG_DIR" ]]; then
  LADA_CONFIG_DIR="$LADA_HOME/.lada"
fi
if [[ -z "$LADA_WORKSPACE_DIR" ]]; then
  LADA_WORKSPACE_DIR="$LADA_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$LADA_HOME"
validate_mount_source_path "config directory" "$LADA_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$LADA_WORKSPACE_DIR"
validate_container_name "$LADA_CONTAINER_NAME"
validate_image_name "$LADA_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$LADA_CONFIG_DIR" "$LADA_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$LADA_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$LADA_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${LADA_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "LADA_DOCKER_APT_PACKAGES=${LADA_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${LADA_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "LADA_EXTENSIONS=${LADA_EXTENSIONS}")
fi

if [[ "$LADA_IMAGE" == "lada:local" ]]; then
  echo "Building image $LADA_IMAGE ..."
  podman build -t "$LADA_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$LADA_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $LADA_IMAGE"
  else
    echo "Pulling image $LADA_IMAGE ..."
    podman pull "$LADA_IMAGE"
  fi
fi

ENV_FILE="$LADA_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated LADA_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "LADA_PODMAN_CONTAINER" "$LADA_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "LADA_PODMAN_IMAGE" "$LADA_IMAGE"

CONFIG_JSON="$LADA_CONFIG_DIR/lada.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$LADA_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/lada.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  LADA_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$LADA_HOME")"
  LADA_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$LADA_CONFIG_DIR")"
  LADA_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$LADA_WORKSPACE_DIR")"
  LADA_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$LADA_IMAGE")"
  LADA_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$LADA_CONTAINER_NAME")"
  sed \
    -e "s|{{LADA_HOME}}|$LADA_HOME_ESCAPED|g" \
    -e "s|{{LADA_CONFIG_DIR}}|$LADA_CONFIG_ESCAPED|g" \
    -e "s|{{LADA_WORKSPACE_DIR}}|$LADA_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$LADA_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$LADA_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start lada.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start lada.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-lada-podman.sh launch"
echo "  ./scripts/run-lada-podman.sh launch setup"
echo "  lada --container $LADA_CONTAINER_NAME dashboard --no-open"
