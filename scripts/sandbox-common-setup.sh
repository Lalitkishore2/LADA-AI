...
BASE_IMAGE="${BASE_IMAGE:-lada-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-lada-sandbox-common:bookworm-slim}"
...
LADA_DOCKER_BUILD_USE_BUILDX="${LADA_DOCKER_BUILD_USE_BUILDX:-0}"
LADA_DOCKER_BUILD_CACHE_FROM="${LADA_DOCKER_BUILD_CACHE_FROM:-}"
LADA_DOCKER_BUILD_CACHE_TO="${LADA_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${LADA_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${LADA_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${LADA_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${LADA_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${LADA_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=lada.sandbox=1)
NOTE
