...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.lada) ? packageJson.lada : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.lada?.channel?.id) || left.name;
    const rightId = trimString(right.lada?.channel?.id) || right.name;
