...
elif [[ -n "${LADA_LIVE_PROVIDERS:-}" || -n "${LADA_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      lada_live_collect_auth_dirs_from_csv "${LADA_LIVE_PROVIDERS:-}"
      lada_live_collect_auth_dirs_from_csv "${LADA_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      lada_live_collect_auth_files_from_csv "${LADA_LIVE_PROVIDERS:-}"
      lada_live_collect_auth_files_from_csv "${LADA_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
docker run --rm -t \
  ...
  -e LADA_LIVE_TEST=1 \
  -e LADA_LIVE_MODELS="${LADA_LIVE_MODELS:-modern}" \
  -e LADA_LIVE_PROVIDERS="${LADA_LIVE_PROVIDERS:-}" \
  -e LADA_LIVE_MAX_MODELS="${LADA_LIVE_MAX_MODELS:-12}" \
  -e LADA_LIVE_MODEL_TIMEOUT_MS="${LADA_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e LADA_LIVE_REQUIRE_PROFILE_KEYS="${LADA_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e LADA_LIVE_GATEWAY_MODELS="${LADA_LIVE_GATEWAY_MODELS:-}" \
  -e LADA_LIVE_GATEWAY_PROVIDERS="${LADA_LIVE_GATEWAY_PROVIDERS:-}" \
  -e LADA_LIVE_GATEWAY_MAX_MODELS="${LADA_LIVE_GATEWAY_MAX_MODELS:-}" \
  -v "$ROOT_DIR":/src:ro \
