#!/usr/bin/env bash
# Rootless LADA in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-lada-podman.sh launch        # Start gateway
#   ./scripts/run-lada-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   lada --container lada dashboard --no-open
#   lada --container lada channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      LADA_GATEWAY_TOKEN|LADA_PODMAN_CONTAINER|LADA_PODMAN_IMAGE|LADA_IMAGE|LADA_PODMAN_PULL|LADA_PODMAN_GATEWAY_HOST_PORT|LADA_GATEWAY_PORT|LADA_PODMAN_BRIDGE_HOST_PORT|LADA_BRIDGE_PORT|LADA_GATEWAY_BIND|LADA_PODMAN_USERNS|LADA_BIND_MOUNT_OPTIONS|LADA_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-lada-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${LADA_CONFIG_DIR:-$EFFECTIVE_HOME/.lada}"
ENV_FILE="${LADA_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${LADA_CONFIG_DIR:-$EFFECTIVE_HOME/.lada}"
ENV_FILE="${LADA_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${LADA_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${LADA_PODMAN_CONTAINER:-lada}"
LADA_IMAGE="${LADA_PODMAN_IMAGE:-${LADA_IMAGE:-lada:local}}"
PODMAN_PULL="${LADA_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${LADA_PODMAN_GATEWAY_HOST_PORT:-${LADA_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${LADA_PODMAN_BRIDGE_HOST_PORT:-${LADA_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${LADA_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$LADA_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v lada >/dev/null 2>&1; then
    return 0
  fi
  LADA_CONTAINER="" LADA_CONFIG_DIR="$config_dir" \
    lada config get gateway.bind 2>/dev/null || true
}
...
# LADA_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${LADA_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v lada >/dev/null 2>&1; then
    echo "Warning: lada not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    LADA_CONTAINER="" LADA_CONFIG_DIR="$config_dir" \
      lada config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    LADA_CONTAINER="" LADA_CONFIG_DIR="$config_dir" \
      lada config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  LADA_CONTAINER="" LADA_CONFIG_DIR="$config_dir" \
    lada config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${LADA_GATEWAY_TOKEN:-}" ]]; then
  export LADA_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "LADA_GATEWAY_TOKEN" "$LADA_GATEWAY_TOKEN"
  echo "Generated LADA_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/lada.json"
...
PODMAN_USERNS="${LADA_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported LADA_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (LADA_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${LADA_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${LADA_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$LADA_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.lada/.npm \
    -e LADA_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.lada:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.lada/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$LADA_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME lada dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME lada devices approve --latest  # if pairing required"
