#!/bin/bash
# Mobile-friendly LADA re-authentication
...
# 2. Running lada setup-token for long-lived auth
...
echo "=== LADA Mobile Re-Auth ==="
echo ""

# Check current auth status
echo "Checking auth status..."
AUTH_STATUS=$("$SCRIPT_DIR/lada-auth-status.sh" simple 2>/dev/null || echo "ERROR")

case "$AUTH_STATUS" in
    OK)
        echo -e "${GREEN}Auth is valid!${NC}"
        "$SCRIPT_DIR/lada-auth-status.sh" full
        exit 0
        ;;
    CLAUDE_EXPIRING|LADA_EXPIRING)
...
echo "3. Create a new API key or use existing 'LADA' key"
...
echo ""
echo "Running 'lada setup-token'..."
echo "(Follow the prompts and paste your API key when asked)"
echo ""

if lada setup-token; then
    echo ""
    echo -e "${GREEN}Authentication successful!${NC}"
    echo ""
    "$SCRIPT_DIR/lada-auth-status.sh" full

    # Restart lada service if running
    if systemctl --user is-active lada >/dev/null 2>&1; then
        echo ""
        echo "Restarting lada service..."
        systemctl --user restart lada
        echo -e "${GREEN}Service restarted.${NC}"
    fi
else
    echo ""
    echo -e "${RED}Authentication failed.${NC}"
    echo "Please try again or check the LADA documentation."
    exit 1
fi
