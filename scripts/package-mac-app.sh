...
# Build and bundle LADA into a minimal .app we can open.
# Outputs to dist/LADA.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/LADA.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="LADA"
BUNDLE_ID="${BUNDLE_ID:-ai.lada.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/lada/lada/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/LADA/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :LADABuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :LADAGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/LADA"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/LADA"
fi
chmod +x "$APP_ROOT/Contents/MacOS/LADA"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/LADA" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/LADA/Resources/LADA.icns" "$APP_ROOT/Contents/Resources/LADA.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/LADA/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying LADAKit resources"
LADAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/LADAKit_LADAKit.bundle"
if [ -d "$LADAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/LADAKit_LADAKit.bundle"
  cp -R "$LADAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/LADAKit_LADAKit.bundle"
else
  echo "WARN: LADAKit resource bundle not found at $LADAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running LADA"
killall -q LADA 2>/dev/null || true
