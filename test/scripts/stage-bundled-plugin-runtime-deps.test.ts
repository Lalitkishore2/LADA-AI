...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = createTempDir("lada-runtime-deps-");
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@lada/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: {
          "@lada/plugin-sdk": "workspace:*",
          lada: "^1.0.0",
          react: "^19.0.0",
        },
        peerDependenciesMeta: {
          "@lada/plugin-sdk": { optional: true },
          lada: { optional: true },
          react: { optional: true },
        },
        devDependencies: {
        ...
        lada: { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".lada-runtime-deps-stamp.json"),
  ...
  it("stages runtime deps from the root node_modules when already installed", () => {
    ...
    expect(fs.existsSync(path.join(pluginDir, ".lada-runtime-deps-stamp.json"))).toBe(true);
  });
