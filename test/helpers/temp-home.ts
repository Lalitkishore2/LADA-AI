...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked LADA_HOME.
  delete process.env.LADA_HOME;
  process.env.LADA_STATE_DIR = path.join(base, ".lada");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "lada-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".lada", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
