...
function installDiscoveryHooks(
  ...
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("lada/plugin-sdk/agent-runtime", () => {
      return {
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
      };
    });
    vi.doMock("lada/plugin-sdk/provider-auth", () => {
      return {
        MINIMAX_OAUTH_MARKER: "minimax-oauth",
        applyAuthProfileConfig: (config: LADAConfig) => config,
    ...
    it("keeps env-managed catalog provider-owned", async () => {
      ...
      await expect(
        ...
        provider: {
          ...
          models: [expect.objectContaining({ id: "lada-sonnet-4-5" })],
        },
      });
    });
  });
