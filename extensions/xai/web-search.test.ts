import { NON_ENV_SECRETREF_MARKER } from "lada/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "lada/plugin-sdk/runtime-env";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of throwing", async () => {
    await withEnv({ XAI_API_KEY: undefined }, async () => {
      ...
      await expect(maybeTool.execute({ query: "LADA" })).resolves.toMatchObject({
