...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **LADA**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.lada-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
lada wiki doctor
lada wiki init
lada wiki ingest ./notes/alpha.md
lada wiki compile
lada wiki lint
lada wiki search "alpha"
lada wiki get entity.alpha --from 1 --lines 80

lada wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

lada wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

lada wiki bridge import
lada wiki unsafe-local import

lada wiki obsidian status
lada wiki obsidian search "alpha"
lada wiki obsidian open syntheses/alpha-summary.md
lada wiki obsidian command workspace:quick-switcher
lada wiki obsidian daily
```
...
- The compiled agent digests in `.lada-wiki/cache/agent-digest.json` and `.lada-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
