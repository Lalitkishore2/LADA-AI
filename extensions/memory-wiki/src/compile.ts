...
const AGENT_DIGEST_PATH = ".lada-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".lada-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- lada:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- lada:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- lada:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- lada:wiki:${group.dir}:index:end -->`,
