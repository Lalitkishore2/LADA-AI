...
import {
  ...
  withTrailingNewline,
} from "lada/plugin-sdk/memory-host-markdown";
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- lada:wiki:index:start -->",
      endMarker: "<!-- lada:wiki:index:end -->",
...
- Use \`.lada-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the LADA memory-wiki plugin.
...
- \`.lada-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".lada-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".lada-wiki", "log.jsonl"), "", createdFiles);
