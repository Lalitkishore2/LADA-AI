...
import type { z } from "lada/plugin-sdk/zod";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "lada-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "lada.plugin.json")) &&
...
function resolveAcpxPluginRootFromLADALayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common LADA layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromLADALayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveLADARoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveLADARoot(pluginRoot);
