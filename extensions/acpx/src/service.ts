...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: LADAPluginServiceContext): Promise<void> {
      if (process.env.LADA_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (LADA_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.LADA_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: LADAPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
