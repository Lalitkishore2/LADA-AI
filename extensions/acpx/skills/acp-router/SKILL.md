---
name: acp-router
description: Route plain-language requests for Pi, LADA, Codex, Cursor, Copilot, LADA ACP, OpenCode, Gemini CLI, Qwen, Kiro, Kimi, iFlow, Factory Droid, Kilocode, or ACP harness work into either LADA ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Pi/LADA/Codex/Cursor/Copilot/LADA/OpenCode/Gemini/Qwen/Kiro/Kimi/iFlow/Droid/Kilocode (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.

## Intent detection

Trigger this skill when the user asks LADA to:

- run something in Pi / LADA / Codex / Cursor / Copilot / LADA / OpenCode / Gemini / Qwen / Kiro / Kimi / iFlow / Droid / Kilocode
...
- After reading, follow `LADA ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. LADA ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no LADA ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@lada/acpx` extension package.
...
- `lada -> lada acp`
- `lada -> npx -y @zed-industries/lada-agent-acp@0.21.0`
...
- adapter command missing (for example `lada-agent-acp` not found):
