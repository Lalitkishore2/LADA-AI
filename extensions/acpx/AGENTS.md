...
The bundled ACPX extension is a thin LADA wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `lada/acpx`, not in this extension.
...
Use this flow when LADA needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `lada/acpx` repo first.
2. In LADA, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild LADA and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for LADA ACP validation.
...
If a change feels like shared ACP runtime behavior instead of LADA-specific glue, move it to `lada/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
