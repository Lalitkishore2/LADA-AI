import type { AnyAgentTool, LADAPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: LADAPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<LADAPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
