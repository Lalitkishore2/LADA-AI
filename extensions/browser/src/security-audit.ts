import type { LADAPluginSecurityAuditContext } from "lada/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "lada/plugin-sdk/secret-input";
import { formatCliCommand } from "lada/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "lada/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: LADAPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("lada security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.LADA_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.LADA_GATEWAY_PASSWORD) ||
