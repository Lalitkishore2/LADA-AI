...
function localLADAProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "lada",
...
function createLocalLADAResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localLADAProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveLADAUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-reset-"));
    const profileDir = path.join(tempRoot, "lada");
    ...
    const ops = createLocalLADAResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveLADAUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-reset-no-own-"));
    const profileDir = path.join(tempRoot, "lada");
    fs.mkdirSync(profileDir, { recursive: true });
