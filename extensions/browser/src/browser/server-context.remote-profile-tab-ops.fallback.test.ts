...
describe("browser remote profile fallback and attachOnly behavior", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    const state = deps.makeState("lada");
    ...
    const launchMock = vi.mocked(deps.chromeModule.launchLADAChrome);
    const ctx = deps.createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("lada").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote lada profiles", async () => {
