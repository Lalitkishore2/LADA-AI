...
export function resolveLADAUserDataDir(profileName = DEFAULT_LADA_BROWSER_PROFILE_NAME) {
...
export function buildLADAChromeLaunchArgs(params: {
...
export async function launchLADAChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveLADAUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_LADA_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildLADAChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateLADAProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 lada browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`lada browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`lada browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 lada browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopLADAChrome(
