...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredLADATmpDir: vi.fn(() => "/tmp/lada"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-lada-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-lada-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredLADATmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredLADATmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredLADATmpDir.mockReturnValue("/tmp/lada");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".lada-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredLADATmpDir.mockReturnValue("/tmp/lada-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "lada-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "lada-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredLADATmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "lada-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "lada-preferred", "downloads")}${path.sep}`),
    );
  });
