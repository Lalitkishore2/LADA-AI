  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "lada",
        profiles: {
          lada: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.LADA_GATEWAY_PORT;
    process.env.LADA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.LADA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.LADA_GATEWAY_PASSWORD;
