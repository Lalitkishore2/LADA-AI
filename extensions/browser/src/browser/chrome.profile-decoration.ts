...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".lada-profile-decorated");
}
 ...
 */
export function decorateLADAProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_LADA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_LADA_BROWSER_COLOR).toUpperCase();
