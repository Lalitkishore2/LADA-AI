...
vi.mock("./chrome.js", () => ({
  resolveLADAUserDataDir: vi.fn(() => "/tmp/lada-test/lada/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "lada-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveLADAUserDataDir).mockReturnValue(userDataDir);
