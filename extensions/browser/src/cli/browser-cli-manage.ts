...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "lada";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "lada" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (lada|existing-session). Default: lada")
    .action(
      async (
        opts: {
          name: string;
