...
export function createLlmTaskTool(api: LADAPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via lada.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredLADATmpDir(), "lada-llm-task-"),
        );
