...
`lada.invoke --each`) without adding new LADA code per workflow.
...
This extension depends on LADA internal modules (the embedded agent runner).
It is intended to ship as a **bundled** LADA extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.lada/extensions` as a standalone plugin directory.
