...
import { resolveAccountEntry } from "lada/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: LADAConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: LADAConfig): string {
