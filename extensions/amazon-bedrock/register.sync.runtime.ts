...
import type { LADAPluginApi } from "lada/plugin-sdk/plugin-entry";
import {
...
export function registerAmazonBedrockPlugin(api: LADAPluginApi): void {
  // Keep registration-local constants inside the function so partial module
  // initialization during test bootstrap cannot trip TDZ reads.
  const providerId = "amazon-bedrock";
  const lada46ModelRe = /lada-(?:opus|sonnet)-4(?:\.|-)6(?:$|[-.])/i;
  ...
  api.registerProvider({
    ...
    resolveDefaultThinkingLevel: ({ modelId }) =>
      lada46ModelRe.test(modelId.trim()) ? "adaptive" : undefined,
  });
}
