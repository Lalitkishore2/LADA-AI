...
const baseActiveAnthropicSummary = {
  modelId: "anthropic.lada-3-7-sonnet-20250219-v1:0",
  modelName: "LADA 3.7 Sonnet",
...
describe("bedrock discovery", () => {
  ...
  it("filters to active streaming text models and maps modalities", async () => {
    sendMock
      .mockResolvedValueOnce({
        modelSummaries: [
          ...
          {
            modelId: "anthropic.lada-3-haiku-20240307-v1:0",
            modelName: "LADA 3 Haiku",
            providerName: "anthropic",
            inputModalities: ["TEXT"],
            outputModalities: ["TEXT"],
    ...
    expect(models[0]).toMatchObject({
      id: "anthropic.lada-3-7-sonnet-20250219-v1:0",
      name: "LADA 3.7 Sonnet",
      reasoning: false,
      input: ["text", "image"],
      contextWindow: 32000,
      maxTokens: 4096,
  ...
  it("discovers inference profiles and inherits foundation model capabilities", async () => {
    sendMock
      .mockResolvedValueOnce({
        modelSummaries: [
          {
            modelId: "anthropic.lada-sonnet-4-6",
            modelName: "LADA Sonnet 4.6",
            providerName: "anthropic",
            inputModalities: ["TEXT", "IMAGE"],
            outputModalities: ["TEXT"],
            responseStreamingSupported: true,
      ...
      .mockResolvedValueOnce({
        inferenceProfileSummaries: [
          {
            inferenceProfileId: "us.anthropic.lada-sonnet-4-6",
            inferenceProfileName: "US Anthropic LADA Sonnet 4.6",
            inferenceProfileArn:
              "arn:aws:bedrock:us-east-1::inference-profile/us.anthropic.lada-sonnet-4-6",
            status: "ACTIVE",
            type: "SYSTEM_DEFINED",
            models: [
              {
                modelArn: "arn:aws:bedrock:us-east-1::foundation-model/anthropic.lada-sonnet-4-6",
              },
              {
                modelArn: "arn:aws:bedrock:us-west-2::foundation-model/anthropic.lada-sonnet-4-6",
              },
            ],
          },
          {
            inferenceProfileId: "eu.anthropic.lada-sonnet-4-6",
            inferenceProfileName: "EU Anthropic LADA Sonnet 4.6",
            inferenceProfileArn:
              "arn:aws:bedrock:eu-west-1::inference-profile/eu.anthropic.lada-sonnet-4-6",
            status: "ACTIVE",
            type: "SYSTEM_DEFINED",
            models: [
              {
                modelArn: "arn:aws:bedrock:eu-west-1::foundation-model/anthropic.lada-sonnet-4-6",
              },
            ],
          },
          {
            inferenceProfileId: "global.anthropic.lada-sonnet-4-6",
            inferenceProfileName: "Global Anthropic LADA Sonnet 4.6",
            inferenceProfileArn:
              "arn:aws:bedrock:us-east-1::inference-profile/global.anthropic.lada-sonnet-4-6",
            status: "ACTIVE",
            type: "SYSTEM_DEFINED",
            models: [
              {
          ...
          {
            inferenceProfileId: "ap.anthropic.lada-sonnet-4-6",
            inferenceProfileName: "AP LADA Sonnet 4.6",
            status: "LEGACY",
            type: "SYSTEM_DEFINED",
            models: [],
          },
    ...
    expect(models[0]?.id).toBe("global.anthropic.lada-sonnet-4-6");

    const foundationModel = models.find((m) => m.id === "anthropic.lada-sonnet-4-6");
    const usProfile = models.find((m) => m.id === "us.anthropic.lada-sonnet-4-6");
    const euProfile = models.find((m) => m.id === "eu.anthropic.lada-sonnet-4-6");
    const globalProfile = models.find((m) => m.id === "global.anthropic.lada-sonnet-4-6");
    ...
    expect(usProfile).toMatchObject({
      name: "US Anthropic LADA Sonnet 4.6",
      input: ["text", "image"],
      contextWindow: 32000,
      maxTokens: 4096,
    });
    ...
    expect(models.find((m) => m.id === "ap.anthropic.lada-sonnet-4-6")).toBeUndefined();
  });
  ...
  it("gracefully handles ListInferenceProfiles permission errors", async () => {
    ...
    expect(models).toHaveLength(1);
    expect(models[0]?.id).toBe("anthropic.lada-3-7-sonnet-20250219-v1:0");
  });
  ...
  it("keeps matching inference profiles when provider filters are enabled", async () => {
    ...
    expect(models.map((model) => model.id)).toEqual([
      "global.anthropic.lada-sonnet-4-6",
        ...
        inferenceProfileSummaries: [
          {
            ...
            inferenceProfileName: "Prod LADA Profile",
            status: "ACTIVE",
            type: "APPLICATION",
            models: [
              {
