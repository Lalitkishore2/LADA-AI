...
import { resolveRequiredHomeDir } from "lada/plugin-sdk/provider-auth";
...
const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "lada-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  env?: NodeJS.ProcessEnv;
  outputDir?: string;
  createBackupArchive?: typeof import("lada/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("lada/plugin-sdk/runtime")).createBackupArchive;
