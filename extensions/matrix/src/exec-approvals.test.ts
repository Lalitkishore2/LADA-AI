...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<LADAConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<LADAConfig["channels"]>["matrix"]>>,
): LADAConfig {
  return {
    channels: {
      matrix: {
