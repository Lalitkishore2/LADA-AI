...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleLADADevices.length > 0) {
      params.logger.warn(
        `matrix: stale LADA devices detected for ${params.auth.userId}: ${deviceHealth.staleLADADevices.map((device) => device.deviceId).join(", ")}. Run 'lada matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
