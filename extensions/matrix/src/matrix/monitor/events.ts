...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another LADA runtime is using the same account. " +
    `Check 'lada matrix verify status --verbose --account ${accountId}' and 'lada matrix devices list --account ${accountId}'.`
  );
}
