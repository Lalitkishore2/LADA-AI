...
import { withFileLock } from "lada/plugin-sdk/infra-runtime";
...
// `lada matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.LADA_STATE_DIR || path.join(process.env.HOME || "/tmp", ".lada");
