import { formatErrorMessage, type PinnedDispatcherPolicy } from "lada/plugin-sdk/infra-runtime";
import { coerceSecretRef } from "lada/plugin-sdk/provider-auth";
import { retryAsync } from "lada/plugin-sdk/retry-runtime";
...
export async function resolveMatrixAuth(params?: {
  ...
  const login = (await retryMatrixAuthRequest("matrix auth login", async () => {
    return (await loginClient.doRequest("POST", "/_matrix/client/v3/login", undefined, {
      ...
      initial_device_display_name: resolved.deviceName ?? "LADA Gateway",
    })) as {
      access_token?: string;
      user_id?: string;
      device_id?: string;
