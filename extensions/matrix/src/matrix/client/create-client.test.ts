...
describe("createMatrixClient", () => {
  const storagePaths = {
    rootDir: "/tmp/lada-matrix-create-client-test",
    storagePath: "/tmp/lada-matrix-create-client-test/storage.json",
    recoveryKeyPath: "/tmp/lada-matrix-create-client-test/recovery.key",
    idbSnapshotPath: "/tmp/lada-matrix-create-client-test/idb.snapshot",
    metaPath: "/tmp/lada-matrix-create-client-test/storage-meta.json",
    accountKey: "default",
  ...
  it("persists storage metadata by default", async () => {
    ...
    expect(MatrixClientMock).toHaveBeenCalledWith("https://matrix.example.org", "tok", {
      ...
      cryptoDatabasePrefix: "lada-matrix-default-token-hash",
