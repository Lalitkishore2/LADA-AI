...
- Extension production code should import from `lada/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `lada.plugin.json` and the package
  `lada` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `lada/plugin-sdk/*`.
