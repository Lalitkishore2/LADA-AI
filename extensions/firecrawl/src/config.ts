...
type WebSearchConfig = NonNullable<LADAConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<LADAConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LADAConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: LADAConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: LADAConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: LADAConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: LADAConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: LADAConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: LADAConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: LADAConfig, override?: number): number {
