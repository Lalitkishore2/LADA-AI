...
import type { ChannelSetupInput } from "lada/plugin-sdk/setup";
...
export const qqbotConfigAdapter = {
  listAccountIds: (cfg: LADAConfig) => listQQBotAccountIds(cfg),
  resolveAccount: (cfg: LADAConfig, accountId?: string | null) =>
    resolveQQBotAccount(cfg, accountId, { allowUnresolvedSecretRef: true }),
  defaultAccountId: (cfg: LADAConfig) => resolveDefaultQQBotAccountId(cfg),
  setAccountEnabled: ({
    cfg,
    accountId,
    enabled,
  ...
  deleteAccount: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) =>
    deleteAccountFromConfigSection({
      cfg,
      sectionKey: "qqbot",
      accountId,
...
export const qqbotSetupAdapterShared = {
  resolveAccountId: ({ cfg, accountId }: { cfg: LADAConfig; accountId?: string | null }) =>
