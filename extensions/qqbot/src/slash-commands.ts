...
import { resolveRuntimeServiceVersion } from "lada/plugin-sdk/cli-runtime";
...
const QQBOT_PLUGIN_GITHUB_URL = "https://github.com/lada/lada/tree/main/extensions/qqbot";
const QQBOT_UPGRADE_GUIDE_URL = "https://q.qq.com/qqbot/lada/upgrade.html";
...
/**
 * /bot-ping — test current network latency between LADA and QQ.
 */
registerCommand({
  name: "bot-ping",
  description: "测试 LADA 与 QQ 之间的网络延迟",
  usage: [
    `/bot-ping`,
    ``,
    `测试当前 LADA 宿主机与 QQ 服务器之间的网络延迟。`,
    `返回网络传输耗时和插件处理耗时。`,
  ].join("\n"),
  handler: (ctx) => {
    const now = Date.now();
...
/**
 * /bot-version — show the LADA framework version.
 */
registerCommand({
  name: "bot-version",
  description: "查看 LADA 框架版本",
  usage: [`/bot-version`, ``, `查看当前 LADA 框架版本。`].join("\n"),
  handler: async () => {
    const frameworkVersion = resolveRuntimeServiceVersion();
    const lines = [`🦞 LADA 版本：${frameworkVersion}`];
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["lada", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (/STATE_DIR$/i.test(key) && /(LADA|CLAWDBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }

  for (const name of [".lada", ".clawdbot", ".moltbot", "lada", "clawdbot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(lada|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["lada", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "lada.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|lada|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
