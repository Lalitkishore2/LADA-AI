...
/**
 * Return a path under `~/.lada/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".lada", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.lada/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under LADA's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".lada", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".lada", "workspace", "qqbot");
