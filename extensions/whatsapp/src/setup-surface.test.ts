...
import { DEFAULT_ACCOUNT_ID, type LADAConfig } from "lada/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: LADAConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  loginWeb: vi.fn(async () => {}),
  pathExists: vi.fn(async () => false),
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/lada-whatsapp-test",
  })),
}));
...
vi.mock("lada/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/setup")>(
    "lada/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: LADAConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/lada-whatsapp-test" });
  });
  ...
  it("uses configured defaultAccount for omitted-account setup status", async () => {
    hoisted.detectWhatsAppLinked.mockImplementation(
      async (_cfg: LADAConfig, accountId: string) => accountId === "work",
    );
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("lada channels login"),
      "WhatsApp",
    );
  });
