...
import type { ChannelSetupWizard } from "lada/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<LADAConfig["channels"]>["whatsapp"]>;
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so LADA can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for LADA" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("lada channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
