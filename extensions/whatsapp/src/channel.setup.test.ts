import { DEFAULT_ACCOUNT_ID } from "lada/plugin-sdk/routing";
  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: LADAConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<LADAConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
