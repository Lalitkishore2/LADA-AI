...
import { VERSION } from "lada/plugin-sdk/cli-runtime";
import { resolveAmbientNodeProxyAgent } from "lada/plugin-sdk/extension-shared";
import { danger, success } from "lada/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "lada/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "lada/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("lada channels login")}`,
              ),
            );
          }
        }
