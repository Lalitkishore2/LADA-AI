...
import { generateSecureUuid } from "lada/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "lada/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "lada/plugin-sdk/runtime-env";
import { getChildLogger } from "lada/plugin-sdk/text-runtime";
import { redactIdentifier } from "lada/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "lada/plugin-sdk/text-runtime";
import {
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: LADAConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const cfg = options.cfg ?? loadConfig();
