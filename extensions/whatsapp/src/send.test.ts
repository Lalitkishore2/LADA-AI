...
import { redactIdentifier } from "lada/plugin-sdk/logging-core";
...
let resetLogger: typeof import("lada/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("lada/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("lada/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `lada-outbound-${crypto.randomUUID()}.log`);
