import { resolveInboundDebounceMs } from "lada/plugin-sdk/channel-inbound";
import { formatCliCommand } from "lada/plugin-sdk/cli-runtime";
import { waitForever } from "lada/plugin-sdk/cli-runtime";
import { hasControlCommand } from "lada/plugin-sdk/command-detection";
import { enqueueSystemEvent } from "lada/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "lada/plugin-sdk/reply-history";
import { resolveAgentRoute } from "lada/plugin-sdk/routing";
import { logVerbose } from "lada/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "lada/plugin-sdk/runtime-env";
import { getChildLogger } from "lada/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "lada/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("lada channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("lada channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
