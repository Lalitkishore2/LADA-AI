...
const CONFIG_KEY = Symbol.for("lada:testConfigMock");
...
vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/lada-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("lada:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("lada:lastSocket")];
  if (typeof getter === "function") {
