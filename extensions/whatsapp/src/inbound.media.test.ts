...
vi.mock("lada/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/media-runtime")>(
    "lada/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `lada-inbound-media-${crypto.randomUUID()}`);
