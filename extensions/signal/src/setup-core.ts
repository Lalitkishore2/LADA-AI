...
import { formatCliCommand, formatDocsLink } from "lada/plugin-sdk/setup-tools";
import {
  normalizeE164,
...
export const signalDmPolicy = {
  ...
  resolveConfigKeys: (cfg: LADAConfig, accountId?: string) =>
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "LADA"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("lada gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
