...
import { runPluginCommandWithTimeout } from "lada/plugin-sdk/run-command";
import type { RuntimeEnv } from "lada/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "lada/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-signal-"));
