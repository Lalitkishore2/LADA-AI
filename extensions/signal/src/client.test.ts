...
vi.mock("lada/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("lada/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/core")>(
    "lada/plugin-sdk/core",
  );
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/text-runtime", () => ({
