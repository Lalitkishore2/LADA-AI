import type { MockFn } from "lada/plugin-sdk/testing";
...
vi.mock("lada/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/lada-sessions.json"),
...
vi.mock("lada/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/conversation-runtime")>(
    "lada/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/security-runtime")>(
    "lada/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("lada/plugin-sdk/reply-runtime"),
      import("lada/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
