...
import type { ChunkMode } from "lada/plugin-sdk/reply-runtime";
import { describe, expect, it, vi } from "vitest";
import type { LADAConfig, PluginRuntime } from "../../runtime-api.js";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies LADAConfig;
