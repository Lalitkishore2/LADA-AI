...
import type { LADAPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredLADATmpDir: vi.fn(() => path.join(os.tmpdir(), "lada-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredLADATmpDir: pluginApiMocks.resolvePreferredLADATmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as LADAPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: LADAPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as LADAPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredLADATmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredLADATmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the LADA iOS app:");
    expect(text).toContain("![LADA pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the LADA iOS app:");
