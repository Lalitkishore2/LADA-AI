...
- **Framework-agnostic**: Works with LADA, OpenCode, and any future agent framework
...
| ------------- | ---------- | ---------------------------- | ----------------------------------- |
| `model`       | identifier | `sonnet`, `opus`, `haiku`    | The LADA model to use             |
...
1. The agent's `model` property determines which LADA model is used
...
2. **Spawn a Subagent**: Create a new LADA subagent with the resolved configuration
