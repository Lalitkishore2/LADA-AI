...
metadata: { "lada": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == LADA `sessions_spawn`
- **File I/O** == LADA `read`/`write`
- **Remote fetch** == LADA `web_fetch` (or `exec` with curl when POST is required)
