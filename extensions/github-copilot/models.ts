...
import { normalizeModelCompat } from "lada/plugin-sdk/provider-model-shared";
...
export function resolveCopilotTransportApi(
  ...
  return (normalizeOptionalLowercaseString(modelId) ?? "").includes("lada")
    ? "anthropic-messages"
    : "openai-responses";
}
...
export function resolveCopilotForwardCompatModel(
  ...
  // by simply adding them to agents.defaults.models in lada.json — no
