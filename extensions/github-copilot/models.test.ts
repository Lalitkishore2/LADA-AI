...
vi.mock("lada/plugin-sdk/json-store", () => ({
  loadJsonFile,
  saveJsonFile,
}));

vi.mock("lada/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/lada-state",
}));

import type { ProviderResolveDynamicModelContext } from "lada/plugin-sdk/core";
...
describe("github-copilot model defaults", () => {
  describe("getDefaultCopilotModelIds", () => {
    it("includes lada-sonnet-4.6", () => {
      expect(getDefaultCopilotModelIds()).toContain("lada-sonnet-4.6");
    });

    it("includes lada-sonnet-4.5", () => {
      expect(getDefaultCopilotModelIds()).toContain("lada-sonnet-4.5");
    });
  ...
  describe("buildCopilotModelDefinition", () => {
    it("builds a valid definition for lada-sonnet-4.6", () => {
      const def = buildCopilotModelDefinition("lada-sonnet-4.6");
      expect(def.id).toBe("lada-sonnet-4.6");
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/lada-state/credentials/github-copilot.token.json";
