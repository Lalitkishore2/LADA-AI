...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: LADAConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/lada-opus-4-5", workspace: "/tmp/lada" } },
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/lada", enabled: true },
    });
...
describe("slack thread.requireExplicitMention", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-slack-explicit-mention-"));
  });
  ...
  it("drops thread reply without explicit mention when requireExplicitMention is true", async () => {
    ...
    const { storePath } = makeTmpStorePath();
    vi.spyOn(
      await import("lada/plugin-sdk/config-runtime"),
      "resolveStorePath",
