import type { SlackSlashCommandConfig } from "lada/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "lada");
  const name = normalizedName || "lada";
  return {
