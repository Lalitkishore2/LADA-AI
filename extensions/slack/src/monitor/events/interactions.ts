...
// Prefix for LADA-generated action IDs to scope our handler
const LADA_ACTION_PREFIX = "lada:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${LADA_ACTION_PREFIX}`);

  // Handle LADA modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
