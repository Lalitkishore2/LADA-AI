...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): LADAConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as LADAConfig;
  ...
  async function resolveReadToken(cfg: LADAConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: LADAConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
