...
import { patchChannelConfigForAccount } from "lada/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type LADAConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "LADA";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for LADA`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "LADA"): string[] {
  return [
