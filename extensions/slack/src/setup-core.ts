import { hasConfiguredSecretInput } from "lada/plugin-sdk/secret-input";
import {
...
import { formatDocsLink } from "lada/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: LADAConfig, accountId: string): LADAConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: LADAConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: LADAConfig; accountId: string }) =>
    ...
    disable: (cfg: LADAConfig) => setSetupChannelEnabled(cfg, channel, false),
