...
import { normalizeChannelId, type ChannelId } from "lada/plugin-sdk/channel-targets";
import type {
  LADAConfig,
  TtsAutoMode,
  TtsConfig,
...
import { formatErrorMessage } from "lada/plugin-sdk/error-runtime";
import { redactSensitiveText } from "lada/plugin-sdk/logging-core";
import { resolveSendableOutboundReplyParts } from "lada/plugin-sdk/reply-payload";
import type { ReplyPayload } from "lada/plugin-sdk/reply-runtime";
import { isVerbose, logVerbose } from "lada/plugin-sdk/runtime-env";
import { resolvePreferredLADATmpDir } from "lada/plugin-sdk/sandbox";
import {
  ...
  resolveUserPath,
  stripMarkdown,
} from "lada/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.LADA_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: LADAConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: LADAConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: LADAConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: LADAConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: LADAConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: LADAConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredLADATmpDir();
