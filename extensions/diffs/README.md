...
Read-only diff viewer plugin for **LADA** agents.
...
Set plugin-wide defaults in `~/.lada/lada.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/lada",
        },
      },
    },
  },
...
Before:
LADA supports plugins.

After:
LADA supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/lada-diffs`).
