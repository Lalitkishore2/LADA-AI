...
describe("bedrock mantle discovery", () => {
  ...
  it("discovers models from Mantle /v1/models endpoint sorted by id", async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        data: [
          ...
          { id: "anthropic.lada-sonnet-4-6", object: "model", owned_by: "anthropic" },
    ...
    expect(models[0]).toMatchObject({
      ...
      name: "anthropic.lada-sonnet-4-6",
      reasoning: false,
      input: ["text"],
    });
  ...
  it("filters out models with empty IDs", async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        data: [
          { id: "anthropic.lada-sonnet-4-6", object: "model" },
          { id: "", object: "model" },
          { id: "  ", object: "model" },
        ],
      }),
    ...
    expect(models[0]?.id).toBe("anthropic.lada-sonnet-4-6");
  });
  ...
  it("returns cached models on subsequent calls within refresh interval", async () => {
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        data: [{ id: "anthropic.lada-sonnet-4-6", object: "model" }],
      }),
    });
  ...
  it("returns stale cache on fetch failure", async () => {
    ...
    expect(stale[0]?.id).toBe("anthropic.lada-sonnet-4-6");
  });
