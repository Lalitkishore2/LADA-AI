...
function resolveSetupAccountId(cfg: LADAConfig): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: LADA_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: LADAConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var LADA_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: LADAConfig): "open" | "allowlist" | "disabled" {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.LADA_TWITCH_ACCESS_TOKEN?.trim();
