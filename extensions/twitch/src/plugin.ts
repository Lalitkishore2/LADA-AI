/**
 * Twitch channel plugin for LADA.
 *
...
} from "lada/plugin-sdk/channel-pairing";
 ...
 * for LADA. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: LADAConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: LADAConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: LADAConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: LADAConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("lada/plugin-sdk/runtime-env").RuntimeEnv;
