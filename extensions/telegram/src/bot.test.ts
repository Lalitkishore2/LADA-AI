...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/lada-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const modelId = "us.anthropic.lada-3-5-sonnet-20240620-v1:0";
    const storePath = `/tmp/lada-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/lada-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: LADAConfig = {
      agents: {
        defaults: {
          model: "lada-opus-4-6",
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "mdl_sel_anthropic/lada-opus-4-6",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 1234, type: "private" },
            date: 1736380800,
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/lada-telegram-model-html-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "LADA" },
        },
      },
      me: { id: 999, username: "lada_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
