...
vi.mock("lada/plugin-sdk/runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/runtime")>(
    "lada/plugin-sdk/runtime",
  );
  return {
    ...actual,
...
describe("telegram doctor", () => {
  ...
  it("formats username repair warnings", () => {
    ...
    expect(warnings[1]).toContain("lada doctor --fix");
  });
});
