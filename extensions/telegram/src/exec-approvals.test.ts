...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<LADAConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<LADAConfig["channels"]>["telegram"]>>,
): LADAConfig {
  return {
    channels: {
      telegram: {
