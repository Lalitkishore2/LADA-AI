...
import {
  ...
  type LADAConfig,
} from "lada/plugin-sdk/account-core";
import type {
...
import { formatSetExplicitDefaultInstruction } from "lada/plugin-sdk/routing";
import { createSubsystemLogger, isTruthyEnvValue } from "lada/plugin-sdk/runtime-env";
...
const debugAccounts = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.LADA_DEBUG_TELEGRAM_ACCOUNTS)) {
...
function listConfiguredAccountIds(cfg: LADAConfig): string[] {
...
export function listTelegramAccountIds(cfg: LADAConfig): string[] {
...
export function resolveDefaultTelegramAccountId(cfg: LADAConfig): string {
...
export function mergeTelegramAccountConfig(
  ...
  // See: https://github.com/lada/lada/issues/30673
...
export function listEnabledTelegramAccounts(cfg: LADAConfig): ResolvedTelegramAccount[] {
