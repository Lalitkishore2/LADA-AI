...
describe("parseModelCallbackData", () => {
  it("parses supported callback variants", () => {
    const cases = [
      ...
      [
        "mdl_sel_anthropic/lada-sonnet-4-5",
        { type: "select", provider: "anthropic", model: "lada-sonnet-4-5" },
      ],
      ["mdl_sel_openai/gpt-4/turbo", { type: "select", provider: "openai", model: "gpt-4/turbo" }],
      [
        "mdl_sel/us.anthropic.lada-3-5-sonnet-20240620-v1:0",
        { type: "select", model: "us.anthropic.lada-3-5-sonnet-20240620-v1:0" },
      ],
      [
        "mdl_sel/anthropic/lada-3-7-sonnet",
        { type: "select", model: "anthropic/lada-3-7-sonnet" },
      ],
      ["  mdl_prov  ", { type: "providers" }],
    ] as const;
  ...
  it("returns explicit provider selections unchanged", () => {
    const result = resolveModelSelection({
      ...
      byProvider: new Map([
        ...
        ["anthropic", new Set(["lada-sonnet-4-5"])],
      ]),
    });
...
describe("buildModelSelectionCallbackData", () => {
  it("uses standard callback when under limit and compact callback when needed", () => {
    ...
    const longModel = "us.anthropic.lada-3-5-sonnet-20240620-v1:0";
...
describe("buildModelsKeyboard", () => {
    ...
    const cases = [
      {
        ...
        firstText: "lada-sonnet-4",
      },
      {
        name: "current model marked",
        currentModel: "anthropic/lada-sonnet-4",
        firstText: "lada-sonnet-4 ✓",
      },
      {
        name: "legacy bare model id fallback still marks current model",
        currentModel: "lada-sonnet-4",
        firstText: "lada-sonnet-4 ✓",
      },
    ] as const;
    for (const testCase of cases) {
      const result = buildModelsKeyboard({
        provider: "anthropic",
        models: ["lada-sonnet-4", "lada-opus-4"],
      ...
      expect(result[0]?.[0]?.callback_data).toBe("mdl_sel_anthropic/lada-sonnet-4");
      expect(result[1]?.[0]?.text).toBe("lada-opus-4");
  ...
  it("uses modelNames for display text when provided", () => {
    const modelNames = new Map([
      ["nexos/a1b2c3d4-e5f6-7890-abcd-ef1234567890", "LADA Sonnet 4"],
      ["nexos/lada-opus-4", "LADA Opus 4"],
    ]);
    const result = buildModelsKeyboard({
      provider: "nexos",
      models: ["a1b2c3d4-e5f6-7890-abcd-ef1234567890", "lada-opus-4"],
      currentPage: 1,
      totalPages: 1,
      modelNames,
    });
    ...
    expect(result[0]?.[0]?.text).toBe("LADA Sonnet 4");
    expect(result[1]?.[0]?.text).toBe("LADA Opus 4");
    ...
    const cases = [
      {
        ...
        provider: "anthropic",
        model: "lada-3-5-sonnet-20241022-with-suffix",
        expected: "lada-3-5-sonnet-20241022-with-suffix",
      },
      {
  ...
  it("uses compact selection callback when provider/model callback exceeds 64 bytes", () => {
    const model = "us.anthropic.lada-3-5-sonnet-20240620-v1:0";
    const result = buildModelsKeyboard({
...
describe("large model lists (OpenRouter-scale)", () => {
  ...
  it("all callback_data stays within 64-byte limit", () => {
    ...
    const models = [
      "anthropic/lada-3-5-sonnet-20241022",
