...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): LADAConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as LADAConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): LADAConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as LADAConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: LADAConfig, accountId = "media") {
    await handleTelegramAction(
