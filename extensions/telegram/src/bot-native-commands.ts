...
import type { ChannelGroupPolicy } from "lada/plugin-sdk/config-runtime";
import type {
  ReplyToMode,
...
import { getRuntimeConfigSnapshot } from "lada/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "lada/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("lada/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("lada/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): LADAConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: LADAConfig): TelegramAccountConfig => {
    try {
