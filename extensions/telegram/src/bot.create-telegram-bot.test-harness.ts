import { resolveDefaultModelForAgent } from "lada/plugin-sdk/agent-runtime";
...
type LoadConfigFn = typeof import("lada/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("lada/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("lada/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("lada/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/lada-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("lada/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("lada/plugin-sdk/web-media", () => ({
  loadWebMedia,
...
function createModelsProviderDataFromConfig(cfg: LADAConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: LADAConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: LADAConfig) => {
