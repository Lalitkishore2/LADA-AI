...
function resolveStoredDreaming(config: LADAConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: LADAConfig = {}) {
  let command: LADAPluginCommandDefinition | undefined;
  let runtimeConfig: LADAConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: LADAConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: LADAPluginCommandDefinition) => {
      command = definition;
    }),
  } as unknown as LADAPluginApi;
