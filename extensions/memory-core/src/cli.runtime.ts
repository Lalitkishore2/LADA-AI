...
import { resolveMemoryRemDreamingConfig } from "lada/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: LADAConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function formatDreamingSummary(cfg: LADAConfig): string {
...
function resolveAgent(cfg: LADAConfig, agent?: string) {
...
function resolveAgentIds(cfg: LADAConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: lada memory status --fix --agent ${agentId}`)}`);
      }
    }
