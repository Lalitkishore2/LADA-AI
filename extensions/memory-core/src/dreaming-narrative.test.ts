...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("lada-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- lada:dreaming:diary:start -->");
    expect(content).toContain("<!-- lada:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- lada:dreaming:diary:start -->");
    const end = content.indexOf("<!-- lada:dreaming:diary:end -->");
