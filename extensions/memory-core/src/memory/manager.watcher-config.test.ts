...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): LADAConfig {
    const defaults: NonNullable<NonNullable<LADAConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: LADAConfig) {
