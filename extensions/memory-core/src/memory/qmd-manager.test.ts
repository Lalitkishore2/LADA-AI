...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("lada.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("lada.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("lada.qmdEmbedQueueTail");
...
vi.mock("lada/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("lada/plugin-sdk/memory-core-host-engine-foundation")
  >("lada/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/file-lock")>(
    "lada/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
} from "lada/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: LADAConfig }) {
