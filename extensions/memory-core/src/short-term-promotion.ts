...
import { formatMemoryDreamingDay } from "lada/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "lada/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "lada-memory-promotion:";
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*lada-memory-promotion:([^\n]+?)\s*-->/gi);
