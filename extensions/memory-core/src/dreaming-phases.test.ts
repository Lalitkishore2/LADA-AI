...
import type { LADAConfig } from "lada/plugin-sdk/memory-core";
import { resolveSessionTranscriptsDirForAgent } from "lada/plugin-sdk/memory-core";
import {
...
const LIGHT_DREAMING_TEST_CONFIG: LADAConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function createHarness(config: LADAConfig, workspaceDir?: string) {
  const logger = {
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn(),
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("lada-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__lada_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __lada_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__lada_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    const workspaceDir = await createDreamingWorkspace();
    vi.stubEnv("LADA_TEST_FAST", "1");
    vi.stubEnv("LADA_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await beforeAgentReply(
      { cleanedBody: "__lada_memory_core_rem_sleep__" },
      { trigger: "heartbeat", workspaceDir },
    );
