...
import { resolveMemoryDreamingConfig } from "lada/plugin-sdk/memory-core-host-status";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: LADAConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: LADAConfig, enabled: boolean): LADAConfig {
...
function formatStatus(cfg: LADAConfig): string {
...
export function registerDreamingCommand(api: LADAPluginApi): void {
  api.registerCommand({
    name: "dreaming",
