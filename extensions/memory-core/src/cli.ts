    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada memory status", "Show index and provider status."],
          [
            "lada memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["lada memory status --deep", "Probe embedding provider readiness."],
          ["lada memory index --force", "Force a full reindex."],
          ['lada memory search "meeting notes"', "Quick search using positional query."],
          [
            'lada memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `lada memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          ["lada memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.lada.ai/cli/memory")}\n`,
    );
