...
import type { LADAPluginApi, LADAPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<LADAPluginApi> = {}): LADAPluginApi {
...
function fakeCtx(overrides: Partial<LADAPluginToolContext> = {}): LADAPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: LADAPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
