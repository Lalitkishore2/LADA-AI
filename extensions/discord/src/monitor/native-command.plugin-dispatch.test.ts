...
import type { NativeCommandSpec } from "lada/plugin-sdk/command-auth";
import type { LADAConfig } from "lada/plugin-sdk/config-runtime";
import { clearPluginCommands, registerPluginCommand } from "lada/plugin-sdk/plugin-runtime";
...
vi.mock("lada/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/plugin-runtime")>(
    "lada/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/reply-runtime")>(
...
function createConfig(): LADAConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: LADAConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: LADAConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: LADAConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(async () => {
    ...
    const actualPluginRuntime = await vi.importActual<
      typeof import("lada/plugin-sdk/plugin-runtime")
    >("lada/plugin-sdk/plugin-runtime");
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("lada/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("lada/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("lada/plugin-sdk/reply-runtime").dispatchReplyWithDispatcher,
    );
  ...
  it("does not bypass configured ACP readiness for Discord /new", async () => {
    ...
    const resolveRouteState = vi.fn(async () =>
      createConfiguredRouteState({
        sessionKey: "agent:lada:acp:binding:discord:default:9373ab192b2317f4",
        agentId: "lada",
      }),
    );
