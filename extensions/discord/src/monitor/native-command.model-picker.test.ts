...
import * as commandRegistryModule from "lada/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "lada/plugin-sdk/command-auth";
import type { ModelsProviderData } from "lada/plugin-sdk/command-auth";
import type { LADAConfig } from "lada/plugin-sdk/config-runtime";
import * as dispatcherModule from "lada/plugin-sdk/reply-dispatch-runtime";
import * as globalsModule from "lada/plugin-sdk/runtime-env";
import * as commandTextModule from "lada/plugin-sdk/text-runtime";
...
describe("Discord model picker interactions", () => {
  ...
  it("clicking Recents button renders recents view", async () => {
    ...
    expect(updateText).toContain("lada-sonnet-4-5");
  });
  ...
  it("clicking recents model button applies model through /model pipeline", async () => {
    ...
    // rs=2 -> first deduped recent (default is anthropic/lada-sonnet-4-5, so openai/gpt-4o remains)
    const submitInteraction = await runSubmitButton({
      context,
      data: {
        cmd: "model",
