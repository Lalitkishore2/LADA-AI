...
import * as agentRuntimeModule from "lada/plugin-sdk/simple-completion-runtime";
...
describe("generateThreadTitle", () => {
  it("calls shared one-shot model prep with aws-sdk allowance", async () => {
    prepareSimpleCompletionModelForAgentMock.mockResolvedValueOnce({
      selection: {
        provider: "openrouter",
        modelId: "anthropic/lada-sonnet-4-5",
        profileId: "work",
        agentDir: "/tmp/lada-agent",
      },
      model: {
        provider: "openrouter",
        id: "anthropic/lada-sonnet-4-5",
      },
      auth: {
    ...
    const cfg = {
      agents: {
        defaults: {
          model: "openrouter/anthropic/lada-sonnet-4-5@work",
        },
      },
    } as LADAConfig;
