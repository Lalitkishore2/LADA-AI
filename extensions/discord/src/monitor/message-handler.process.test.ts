import { DEFAULT_EMOJIS } from "lada/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/lada-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("lada/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("lada/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("lada/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/lada-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/lada-discord-process-test-sessions.json" },
      },
    });
