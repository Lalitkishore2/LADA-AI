...
import type { loadConfig } from "lada/plugin-sdk/config-runtime";
import { clearSessionStoreCacheForTest } from "lada/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("lada/plugin-sdk/conversation-binding-runtime");
...
vi.mock("lada/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("lada/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("lada/plugin-sdk/conversation-binding-runtime")>(
        "lada/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("lada/plugin-sdk/agent-runtime", () => ({
  ...
  resolveDefaultModelForAgent: (params: { cfg: ReturnType<typeof loadConfig> }) => {
    ...
    return {
      provider: "anthropic",
      model: "lada-sonnet-4.5",
    };
  },
}));

vi.mock("lada/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `lada-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("lada/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("lada/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("lada/plugin-sdk/command-auth");
  ...
  function createConfig() {
    return {
      agents: {
        defaults: {
          model: {
            primary: "anthropic/lada-sonnet-4.5",
          },
        },
      },
      session: {
