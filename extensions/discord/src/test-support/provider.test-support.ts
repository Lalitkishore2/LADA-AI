...
import type { RuntimeEnv } from "lada/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: LADAConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): LADAConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("lada/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/acp-runtime")>(
    "lada/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/command-auth")>(
...
vi.mock("lada/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/config-runtime")>(
...
vi.mock("lada/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/runtime-env")>(
    "lada/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("lada/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("lada/plugin-sdk/infra-runtime")>(
