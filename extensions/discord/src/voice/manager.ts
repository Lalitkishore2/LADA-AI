...
import { resolveAgentDir } from "lada/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "lada/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "lada/plugin-sdk/agent-runtime";
import type { LADAConfig } from "lada/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "lada/plugin-sdk/config-runtime";
import { resolveAgentRoute } from "lada/plugin-sdk/routing";
import { logVerbose, shouldLogVerbose } from "lada/plugin-sdk/runtime-env";
import { createSubsystemLogger } from "lada/plugin-sdk/runtime-env";
import type { RuntimeEnv } from "lada/plugin-sdk/runtime-env";
import { parseTtsDirectives } from "lada/plugin-sdk/speech";
import { formatErrorMessage } from "lada/plugin-sdk/ssrf-runtime";
import { resolvePreferredLADATmpDir } from "lada/plugin-sdk/temp-path";
...
function resolveVoiceTtsConfig(params: { cfg: LADAConfig; override?: TtsConfig }): {
  cfg: LADAConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredLADATmpDir(), "discord-voice-"));
