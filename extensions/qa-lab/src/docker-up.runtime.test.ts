...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (args.join(" ").includes("ps --format json lada-qa-gateway")) {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/lada",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/lada`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/lada`,
        ),
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json lada-qa-gateway @/repo/lada`,
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/lada`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/lada`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json lada-qa-gateway @/repo/lada`,
      ]);
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json lada-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q lada-qa-gateway")) {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/lada`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json lada-qa-gateway @/repo/lada`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps -q lada-qa-gateway @/repo/lada`,
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @/repo/lada",
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
