...
function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    HOME: params.homeDir,
    LADA_HOME: params.homeDir,
    LADA_CONFIG_PATH: params.configPath,
    LADA_STATE_DIR: params.stateDir,
    LADA_OAUTH_DIR: path.join(params.stateDir, "credentials"),
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lada-qa-suite-"));
  ...
  const configPath = path.join(tempRoot, "lada.json");
  ...
  const keepTemp = process.env.LADA_QA_KEEP_TEMP === "1";
