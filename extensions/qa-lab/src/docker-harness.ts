...
const QA_LAB_UI_OVERLAY_DIR = "/opt/lada-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        LADA_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      LADA_CONFIG_PATH: /tmp/lada/lada.json
      ...
      LADA_PROFILE: ""
    volumes:
      - ./state:/opt/lada-scaffold:ro
      - ${repoMount}:/opt/lada-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/lada/workspace /tmp/lada/state && cp /opt/lada-scaffold/lada.json /tmp/lada/lada.json && cp -R /opt/lada-scaffold/seed-workspace/. /tmp/lada/workspace/ && ln -snf /opt/lada-repo /tmp/lada/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t lada:qa-local-prebaked --build-arg LADA_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "lada:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "lada.json"),
  ];
