...
} from "lada/plugin-sdk/memory-core";
import { buildAgentSessionKey } from "lada/plugin-sdk/routing";
...
function createQaActionConfig(baseUrl: string): LADAConfig {
  return {
    channels: {
      "qa-channel": {
        enabled: true,
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.LADA_QA_DEBUG === "1") {
...
async function runQaCli(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error(`qa cli timed out: lada ${args.join(" ")}`));
...
async function callPluginToolsMcp(params: {
  ...
  const client = new Client({ name: "lada-qa-suite", version: "0.0.0" }, {});
  try {
        ...
        await runScenario("Threaded follow-up", [
          {
            ...
            run: async () => {
              ...
              state.addInboundMessage({
                ...
                senderId: "alice",
                senderName: "Alice",
                text: "@lada reply in one short sentence inside this thread only. Do not use ACP or any external runtime. Confirm you stayed in-thread.",
                threadId,
              ...
              try {
                ...
                const managed = jobs.find(
                  (job) =>
                    ...
                    job.payload.text === "__lada_memory_core_short_term_promotion_dream__",
                );
                if (!managed?.id) {
          ...
          },
          {
            ...
            run: async () => {
              ...
              const homeDir =
                ...
                env.gateway.runtimeEnv.LADA_HOME ??
        ...
        await runScenario("Memory tools in channel context", [
          {
            ...
            run: async () => {
              ...
              const prompt =
                "@lada Memory tools check: what is the hidden project codename stored only in memory? Use memory tools first.";
              state.addInboundMessage({
                conversation: { id: "qa-room", kind: "channel", title: "QA Room" },
                senderId: "alice",
                senderName: "Alice",
...
export async function runQaSuite(params?: {
  ...
  try {
    ...
    const keepTemp = process.env.LADA_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
    });
    await mock?.stop();
