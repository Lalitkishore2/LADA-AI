  ...
  it("reports doctor-oriented legacy issues and warnings", () => {
    ...
    expect(
      formatVoiceCallLegacyConfigWarnings({
        ...
        doctorFixCommand: "lada doctor --fix",
      }),
    ).toEqual([
      `[voice-call] legacy config keys detected under plugins.entries.voice-call.config; runtime loading will not rewrite them, and support for the legacy shape will be removed in ${VOICE_CALL_LEGACY_CONFIG_REMOVAL_VERSION}. Run "lada doctor --fix".`,
