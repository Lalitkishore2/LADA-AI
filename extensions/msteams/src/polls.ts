...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["ladaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["lada", "pollId"]) ??
    readNestedString(value, ["lada", "poll", "id"]) ??
    readNestedString(value, ["data", "ladaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "lada", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { ladaPollId: pollId, pollId },
        },
      },
    ],
  };
