...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the LADA User-Agent to guarded Graph attachment fetches", async () => {
  ...
  it("adds the LADA User-Agent to Graph shares downloads for reference attachments", async () => {
    vi.mocked(fetchWithSsrFGuard).mockImplementation(async (params: { url: string }) => {
      const url = params.url;
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ LADA\/.+$/);
  });
});
