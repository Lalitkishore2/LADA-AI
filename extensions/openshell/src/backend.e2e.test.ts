...
const LADA_OPENSHELL_E2E = process.env.LADA_E2E_OPENSHELL === "1";
const LADA_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const LADA_OPENSHELL_COMMAND =
  process.env.LADA_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "lada-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && LADA_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: LADA_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(LADA_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-openshell-e2e-"));
      ...
      const gatewayName = `lada-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `lada-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/lada-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: LADA_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("lada-openshell-e2e");
