import type { LADAConfig } from "lada/plugin-sdk/plugin-entry";
import { normalizeProviderId } from "lada/plugin-sdk/provider-model-shared";
...
function resolveAnthropicPrimaryModelRef(raw?: string): string | null {
  ...
  if (aliasKey === "opus") {
    return "anthropic/lada-opus-4-6";
  }
  if (aliasKey === "sonnet") {
    return "anthropic/lada-sonnet-4-6";
  }
  return trimmed;
}
  ...
  return Boolean(
    ...
    (parsed.provider === "anthropic" ||
      (parsed.provider === "amazon-bedrock" &&
        normalizeLowercaseStringOrEmpty(parsed.model).includes("anthropic.lada"))),
  );
}

function usesLADACliModelSelection(config: LADAConfig): boolean {
...
export function applyAnthropicConfigDefaults(params: {
  ...
  if (authMode === "oauth" && usesLADACliModelSelection(params.config)) {
    const nextModels = defaults.models ? { ...defaults.models } : {};
    let modelsMutated = false;
