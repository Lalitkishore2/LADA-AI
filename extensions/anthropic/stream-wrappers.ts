...
import type { ProviderWrapStreamFnContext } from "lada/plugin-sdk/plugin-entry";
import {
  applyAnthropicPayloadPolicyToParams,
  composeProviderStreamWrappers,
  resolveAnthropicPayloadPolicy,
  streamWithPayloadPatch,
} from "lada/plugin-sdk/provider-stream-shared";
import { createSubsystemLogger } from "lada/plugin-sdk/runtime-env";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "lada/plugin-sdk/text-runtime";

const log = createSubsystemLogger("anthropic-stream");

const ANTHROPIC_CONTEXT_1M_BETA = "context-1m-2025-08-07";
const ANTHROPIC_1M_MODEL_PREFIXES = ["lada-opus-4", "lada-sonnet-4"] as const;
...
export function createAnthropicBetaHeadersWrapper(
  ...
  return (model, context, options) => {
    ...
    if (isOauth && requestedContext1m) {
      log.warn(
        `ignoring context1m for Anthropic LADA CLI or legacy token auth on ${model.provider}/${model.id}; falling back to the standard context window because Anthropic rejects context-1m beta with non-API-key auth`,
      );
    }
