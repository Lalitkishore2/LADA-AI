...
function runWrapper(apiKey: string | undefined): Record<string, string> | undefined {
  ...
  wrapper(
    { provider: "anthropic", id: "lada-opus-4-6" } as never,
    {} as never,
    { apiKey } as never,
  );
...
describe("anthropic stream wrappers", () => {
  ...
  it("strips context-1m for LADA CLI or legacy token auth and warns", () => {
  ...
  it("skips service_tier for OAuth token in composed stream chain", () => {
    ...
    wrapped?.(
      { provider: "anthropic", api: "anthropic-messages", id: "lada-sonnet-4-6" } as never,
      {} as never,
