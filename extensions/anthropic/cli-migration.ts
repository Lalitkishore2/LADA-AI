...
type AgentDefaultsModel = NonNullable<NonNullable<LADAConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<LADAConfig["agents"]>["defaults"]>["models"];
type LADACliCredential = NonNullable<ReturnType<typeof readLADACliCredentialsForSetup>>;

function toLADACliModelRef(raw: string): string | null {
  const trimmed = raw.trim();
  if (!normalizeLowercaseStringOrEmpty(trimmed).startsWith("anthropic/")) {
    return null;
  }
  const modelId = trimmed.slice("anthropic/".length).trim();
  if (!normalizeLowercaseStringOrEmpty(modelId).startsWith("lada-")) {
    return null;
  }
  return `lada-cli/${modelId}`;
}
...
function rewriteModelSelection(model: AgentDefaultsModel): {
  ...
  if (typeof model === "string") {
    const converted = toLADACliModelRef(model);
    return converted
  ...
  if (typeof current.primary === "string") {
    const converted = toLADACliModelRef(current.primary);
    if (converted) {
  ...
  if (Array.isArray(currentFallbacks)) {
    const nextFallbacks = currentFallbacks.map((entry) =>
      typeof entry === "string" ? (toLADACliModelRef(entry) ?? entry) : entry,
    );
...
function rewriteModelEntryMap(models: Record<string, unknown> | undefined): {
  ...
  for (const [rawKey, value] of Object.entries(models)) {
    const converted = toLADACliModelRef(rawKey);
    if (!converted) {
      continue;
    }
...
export function hasLADACliAuth(options?: { allowKeychainPrompt?: boolean }): boolean {
  return Boolean(
...
function buildLADACliAuthProfiles(
  credential?: LADACliCredential | null,
...
export function buildAnthropicCliMigrationResult(
  ...
  const nextModels = seedLADACliAllowlist(existingModels);
  const defaultModel = rewrittenModel.primary ?? CLAUDE_CLI_DEFAULT_MODEL_REF;

  return {
    profiles: buildLADACliAuthProfiles(credential),
    configPatch: {
      agents: {
        defaults: {
