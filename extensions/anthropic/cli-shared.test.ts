...
import {
  ...
  normalizeLADABackendConfig,
  normalizeLADAPermissionArgs,
  normalizeLADASettingSourcesArgs,
} from "./cli-shared.js";

describe("normalizeLADAPermissionArgs", () => {
  it("injects bypassPermissions when args omit permission flags", () => {
    expect(
      normalizeLADAPermissionArgs(["-p", "--output-format", "stream-json", "--verbose"]),
    ).toEqual([
      "-p",
      "--output-format",
      "stream-json",
  ...
  it("removes legacy skip-permissions and injects bypassPermissions", () => {
    expect(
      normalizeLADAPermissionArgs(["-p", "--dangerously-skip-permissions", "--verbose"]),
    ).toEqual(["-p", "--verbose", "--permission-mode", "bypassPermissions"]);
  });

  it("keeps explicit permission-mode overrides", () => {
    expect(normalizeLADAPermissionArgs(["-p", "--permission-mode", "acceptEdits"])).toEqual([
      "-p",
      "--permission-mode",
      "acceptEdits",
    ]);
  ...
  it("treats a bare permission-mode flag as malformed and falls back to bypassPermissions", () => {
    expect(
      normalizeLADAPermissionArgs(["-p", "--permission-mode", "--output-format", "stream-json"]),
    ).toEqual(["-p", "--output-format", "stream-json", "--permission-mode", "bypassPermissions"]);
  });
});

describe("normalizeLADASettingSourcesArgs", () => {
  it("injects user-only setting sources when args omit the flag", () => {
    expect(
      normalizeLADASettingSourcesArgs(["-p", "--output-format", "stream-json", "--verbose"]),
  ...
  it("forces explicit project or local setting sources back to user-only", () => {
    expect(normalizeLADASettingSourcesArgs(["-p", "--setting-sources", "project"])).toEqual([
      "-p",
      "--setting-sources",
      "user",
    ]);
    expect(normalizeLADASettingSourcesArgs(["-p", "--setting-sources=local,user"])).toEqual([
      "-p",
...
describe("normalizeLADABackendConfig", () => {
  it("normalizes both args and resumeArgs for custom overrides", () => {
    const normalized = normalizeLADABackendConfig({
      command: "lada",
  ...
  it("marks lada cli as host-managed, restricts setting sources, and clears inherited env overrides", () => {
    const backend = buildAnthropicCliBackend();
