...
vi.mock("lada/plugin-sdk/provider-web-search", () => ({
...
vi.mock("lada/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: lada", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "lada" });
  });

  it("runTavilyExtract sends X-Client-Source: lada", async () => {
