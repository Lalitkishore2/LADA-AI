...
function normalizeDenyList(cfg: LADAPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: LADAPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: LADAPluginApi) {
    ...
    const timerService: LADAPluginService = {
