...
vi.mock("lada/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("lada/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("lada/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("lada/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
