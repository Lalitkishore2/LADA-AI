...
import { createDangerousNameMatchingMutableAllowlistWarningCollector } from "lada/plugin-sdk/channel-policy";
import type { LADAConfig } from "lada/plugin-sdk/config-runtime";
import { isZalouserMutableGroupEntry } from "./security-audit.js";

type ZalouserChannelsConfig = NonNullable<LADAConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: LADAConfig): ChannelDoctorConfigMutation {
...
const ZALOUSER_LEGACY_CONFIG_RULES: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "lada doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "lada doctor --fix".',
