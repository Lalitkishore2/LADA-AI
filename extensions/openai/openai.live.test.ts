...
import type { ResolvedTtsConfig } from "lada/plugin-sdk/agent-runtime";
import type { LADAConfig } from "lada/plugin-sdk/config-runtime";
import { loadConfig } from "lada/plugin-sdk/config-runtime";
import { encodePngRgba, fillPixel } from "lada/plugin-sdk/media-runtime";
...
const LIVE_MODEL_ID = process.env.LADA_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.LADA_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.LADA_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.LADA_LIVE_TEST === "1";
...
function createLiveConfig(): LADAConfig {
