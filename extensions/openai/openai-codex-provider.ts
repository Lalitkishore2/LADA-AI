...
import { buildOauthProviderAuthResult } from "lada/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "lada/plugin-sdk/provider-auth-login";
import {
...
import { buildProviderStreamFamilyHooks } from "lada/plugin-sdk/provider-stream-family";
import { fetchCodexUsage } from "lada/plugin-sdk/provider-usage";
import { readStringValue } from "lada/plugin-sdk/text-runtime";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `lada models auth login --provider openai-codex` or `lada configure`.";
}
