...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-lada-turn-id": turnId,
      "x-lada-turn-attempt": attempt,
    },
    metadata: {
      lada_session_id: sessionHeaders["x-lada-session-id"] ?? "",
      lada_turn_id: turnId,
      lada_turn_attempt: attempt,
      lada_transport: ctx.transport,
    },
  };
}
