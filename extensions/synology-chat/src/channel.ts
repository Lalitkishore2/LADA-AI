/**
 * Synology Chat Channel Plugin for LADA.
 *
...
import { createChatChannelPlugin, type ChannelPlugin } from "lada/plugin-sdk/channel-core";
import { waitUntilAbort } from "lada/plugin-sdk/channel-lifecycle";
import {
...
import { attachChannelToResult } from "lada/plugin-sdk/channel-send-result";
import { createEmptyChannelDirectoryAdapter } from "lada/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "lada pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: LADAConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: LADAConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to LADA",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "LADA: your access has been approved.",
