...
function getChannelConfig(cfg: LADAConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: LADAConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: LADAConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: LADAConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL LADA uses to send replies back to Chat.",
      ],
