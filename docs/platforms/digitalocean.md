---
summary: "LADA on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up LADA on DigitalOcean
  - Looking for cheap VPS hosting for LADA
title: "DigitalOcean (Platform)"
---

# LADA on DigitalOcean

## Goal

Run a persistent LADA Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
lada config set gateway.bind tailnet
lada gateway restart
```
...
- Using API-based models (LADA, GPT) instead of local models
...
- `~/.lada/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u lada-gateway.service --no-pager -n 50
```
