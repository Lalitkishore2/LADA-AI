---
summary: "LADA on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up LADA on Oracle Cloud
  - Looking for low-cost VPS hosting for LADA
  - Want 24/7 LADA on a small server
title: "Oracle Cloud (Platform)"
---

# LADA on Oracle Cloud (OCI)

## Goal

Run a persistent LADA Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for LADA (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname lada
...
sudo tailscale up --ssh --hostname=lada
```

This enables Tailscale SSH, so you can connect via `ssh lada` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@lada` (or use the Tailscale IP).
...
lada config set gateway.bind loopback

# Require auth for the Gateway + Control UI
lada config set gateway.auth.mode token
lada doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
lada config set gateway.tailscale.mode serve
lada config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart lada-gateway.service
```
...
# Check daemon status
systemctl --user status lada-gateway.service
...
```
https://lada.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `lada security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.lada`
- **Security audit:** `lada security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@lada
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=lada --reset
```
...
journalctl --user -u lada-gateway.service -n 50
```
...
- `~/.lada/` — `lada.json`, per-agent `auth-profiles.json`, channel/provider state, and session data
- `~/.lada/workspace/` — workspace (SOUL.md, memory, artifacts)
