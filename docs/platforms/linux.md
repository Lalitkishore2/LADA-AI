...
2. `npm i -g lada@latest`
3. `lada onboard --install-daemon`
...
LADA installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `lada gateway install` and
`lada onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/lada-gateway[-<profile>].service`:

```
[Unit]
Description=LADA Gateway (profile: <profile>, v<version>)
