...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `lada.internal.`), **or**
...
Run `lada devices list` again before approval.

4. Verify connection:

```bash
lada nodes status
lada gateway call node.list --params "{}"
```
...
- `LADA_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine LADA iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight LADA builds.
...
```bash
export LADA_APNS_TEAM_ID="TEAMID"
export LADA_APNS_KEY_ID="KEYID"
export LADA_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.lada/credentials/apns
chmod 700 ~/.lada/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.lada/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.lada/credentials/apns/AuthKey_KEYID.p8
export LADA_APNS_PRIVATE_KEY_PATH="$HOME/.lada/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_lada-gw._tcp` on `local.` and, when configured, the same
...
`lada.internal.`) and Tailscale split DNS.
...
```bash
lada nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__lada__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__lada__/canvas/` and `/__lada__/a2ui/`.
...
```bash
lada nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__lada; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
lada nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `lada devices list` and approve manually.
