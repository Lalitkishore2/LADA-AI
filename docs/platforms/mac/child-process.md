...
service via the external `lada` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.lada.gateway`
  (or `ai.lada.<profile>` when using `--profile`/`LADA_PROFILE`; legacy `com.lada.*` is supported).
