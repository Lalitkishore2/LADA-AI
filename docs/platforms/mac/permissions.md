...
- Same path: run the app from a fixed location (for LADA, `dist/LADA.app`).
...
```bash
sudo tccutil reset Accessibility ai.lada.mac
sudo tccutil reset ScreenCapture ai.lada.mac
...
Workaround: move files into the LADA workspace (`~/.lada/workspace`) if you want to avoid per-folder grants.
