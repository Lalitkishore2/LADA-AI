---
summary: "LADA on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up LADA on a Raspberry Pi
  - Running LADA on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# LADA on Raspberry Pi

## Goal

Run a persistent, always-on LADA Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u lada-gateway.service -f
```

## 9) Access the LADA Dashboard
...
```bash
ssh user@gateway-host 'lada dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/lada-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/lada-compile-cache
mkdir -p /var/tmp/lada-compile-cache
export LADA_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `LADA_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running LADA, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit lada-gateway.service
```

```ini
[Service]
Environment=LADA_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/lada-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep LADA state/cache on SSD-backed storage to avoid SD-card
...
Most LADA features work on ARM64, but some external binaries may need ARM builds:
...
**Don't try to run local LLMs on a Pi** — even small models are too slow. Let LADA/GPT do the heavy lifting.
...
systemctl --user is-enabled lada-gateway.service

# Enable if not
systemctl --user enable lada-gateway.service
...
```bash
# Check logs
journalctl --user -u lada-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/lada  # if using hackable install
npm run build
systemctl --user restart lada-gateway.service
```
