...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `lada hooks`.

There are two kinds of hooks in LADA:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in LADA. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `lada hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "lada": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
1. **Bundled hooks**: shipped with LADA
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.lada/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `lada.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.lada/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
lada hooks enable <hook-name>
```
...
1. Verify the hook is enabled: `lada hooks list`
