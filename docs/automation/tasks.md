...
- `lada tasks list` shows all tasks; `lada tasks audit` surfaces issues.
...
lada tasks list --runtime acp
lada tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
lada tasks show <lookup>

# Cancel a running task (kills the child session)
lada tasks cancel <lookup>

# Change notification policy for a task
lada tasks notify <lookup> state_changes

# Run a health audit
lada tasks audit

# Preview or apply maintenance
lada tasks maintenance
lada tasks maintenance --apply

# Inspect TaskFlow state
lada tasks flow list
lada tasks flow show <lookup>
lada tasks flow cancel <lookup>
```
...
| CLI operations         | `cli`        | `lada agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `lada tasks cancel`                        |
...
When a task reaches a terminal state, LADA notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, LADA also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
lada tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
lada tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
lada tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `lada status` when issues are detected.
...
```bash
lada tasks maintenance [--json]
lada tasks maintenance --apply [--json]
```
...
```bash
lada tasks flow list [--status <status>] [--json]
lada tasks flow show <lookup> [--json]
lada tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `lada tasks list`.

## Status integration (task pressure)

`lada status` includes an at-a-glance task summary:
...
```
$LADA_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `lada tasks` to inspect individual task records and `lada tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.lada/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
