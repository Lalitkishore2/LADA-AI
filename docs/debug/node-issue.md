...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in LADA
...
Running LADA via Node with `tsx` fails at startup with:

```
[lada] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `lada status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch lada.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node lada.mjs status` works on Node 25.
