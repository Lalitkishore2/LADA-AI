...
summary: "Complete reference for every LADA config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.lada/lada.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — LADA uses safe defaults when omitted.
...
```json5
{
  channels: {
    modelByChannel: {
      discord: {
        "123456789012345678": "anthropic/lada-opus-4-6",
      },
      slack: {
      ...
      telegram: {
        ...
        "-1001234567890:topic:99": "anthropic/lada-sonnet-4-6",
      },
    },
  },
}
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.lada/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `lada doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `lada doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["lada-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-lada",
          requireMention: false,
...
- LADA additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `lada plugins install @lada/mattermost`.
...
- `commands.callbackUrl` must resolve to the LADA gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, LADA rejects callbacks with
...
LADA spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `lada channels add` (or channel onboarding) while still on a single-account top-level channel config, LADA promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `lada doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@lada", "lada"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@lada"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `lada.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, LADA auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/lada" } },
}
```
...
```json5
{
  agents: {
    defaults: {
      ...
      model: {
        primary: "anthropic/lada-opus-4-6",
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, LADA tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, LADA falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
Anthropic LADA 4.6 models default to `adaptive` thinking when no explicit thinking level is set.
  ...
  agents: {
    defaults: {
      compaction: {
        ...
        model: "openrouter/anthropic/lada-sonnet-4-6", // optional compaction-only model override
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before LADA aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.lada/sandboxes",
        docker: {
          image: "lada-sandbox:bookworm-slim",
          containerPrefix: "lada-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/lada-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "lada-sandbox-browser:bookworm-slim",
          network: "lada-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that LADA materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.lada/sandboxes`
...
In `remote` mode, host-local edits made outside LADA are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `lada.json`.
noVNC observer access uses VNC auth by default and LADA emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `lada-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from LADA_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `LADA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `LADA_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `LADA_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.lada/workspace",
        agentDir: "~/.lada/agents/main/agent",
        model: "anthropic/lada-opus-4-6", // or { primary, fallbacks }
        ...
        groupChat: { mentionPatterns: ["@lada"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.lada/workspace-home" },
      { id: "work", workspace: "~/.lada/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, LADA resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.lada/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.lada/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.lada/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.lada/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, LADA starts a fresh thread session instead of inheriting parent transcript history.
...
| ----------------- | ---------------------- | --------------------------- |
| `{model}`         | Short model name       | `lada-opus-4-6`           |
| `{modelFull}`     | Full model identifier  | `anthropic/lada-opus-4-6` |
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.lada/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, LADA treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:lada`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.lada/attachments/<uuid>/` with a `.manifest.json`.
...
LADA uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.lada/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `LADA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), LADA forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
- `models.providers.*.models.*.compat.requiresStringContent`: optional compatibility hint for string-only OpenAI-compatible chat endpoints. When `true`, LADA flattens pure text `messages[].content` arrays into plain strings before sending the request.
...
```json5
{
  agents: {
    defaults: {
      model: { primary: "opencode/lada-opus-4-6" },
      models: { "opencode/lada-opus-4-6": { alias: "Opus" } },
    },
  },
}
```

Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `lada onboard --auth-choice opencode-zen` or `lada onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `lada onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `lada onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and LADA now keys that off endpoint
...
Anthropic-compatible, built-in provider. Shortcut: `lada onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `lada onboard --auth-choice synthetic-api-key`.
...
`lada onboard --auth-choice minimax-global-api` or
`lada onboard --auth-choice minimax-cn-api`.
The model catalog now defaults to M2.7 only.
On the Anthropic-compatible streaming path, LADA disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.lada.install`
...
- Loaded from `~/.lada/extensions`, `<workspace>/.lada/extensions`, plus `plugins.load.paths`.
- Discovery accepts native LADA plugins plus compatible Codex bundles and LADA bundles, including manifestless LADA default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native LADA plugin schema when available).
...
- Enabled LADA bundle plugins can also contribute embedded Pi defaults from `settings.json`; LADA applies those as sanitized agent settings, not as raw LADA config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `lada plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      lada: { cdpPort: 18800, color: "#FF4500" },
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want LADA to discover `/json/version`; use WS(S)
...
- Local managed `lada` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or LADA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `LADA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `LADA_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `LADA_APNS_RELAY_BASE_URL` / `LADA_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `LADA_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
LADA_CONFIG_PATH=~/.lada/a.json \
LADA_STATE_DIR=~/.lada-a \
lada gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.lada-dev` + port `19001`), `--profile <name>` (uses `~/.lada-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/lada/tls/server.crt",
      keyPath: "/etc/lada/tls/server.key",
      caPath: "/etc/lada/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.lada/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-lada-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `LADA_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.lada/workspace/canvas",
    liveReload: true,
    // enabled: false, // or LADA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__lada__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__lada__/a2ui/`
...
- Also serves A2UI at `/__lada__/a2ui/`.
...
- Hostname defaults to `lada`. Override with `LADA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.lada/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `lada dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.lada/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `lada.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.lada/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/lada-vault-resolver",
...
- Legacy OAuth imports from `~/.lada/credentials/oauth.json`.
...
- Default log file: `/tmp/lada/lada-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.lada/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$LADA_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one LADA.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `LADA_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `lada doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `lada.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
