...
- `lada gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `lada doctor` reports no blocking config/service issues.
- `lada channels status --probe` shows live per-account transport status and,
...
lada logs --follow
lada models status
lada config get agents.defaults.models
```
...
- LADA model runs fail only on normal agent turns
...
lada infer model run --model <provider/model> --prompt "hi" --json
lada logs --follow
```

Look for:

- direct tiny calls succeed, but LADA runs fail only on larger prompts
...
- direct tiny requests succeed, but LADA agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → LADA transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   LADA's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while LADA agent turns still crash
...
lada status
lada channels status --probe
lada pairing list --channel <channel> [--account <id>]
lada config get channels
lada logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `lada config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `lada devices list` then `lada devices approve <requestId>`.                                                                                                                                                                                            |
...
If `lada devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `lada devices rotate --scope ...` can only request operator scopes that
...
lada logs --follow
lada doctor
lada gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `lada onboard --mode local` / `lada setup` to restamp the expected local-mode config. If you are running LADA via Podman, the default config path is `~/.lada/lada.json`.
...
Use this when `lada gateway probe` reaches something, but still prints a warning block.

```bash
lada gateway probe
lada gateway probe --json
lada gateway probe --ssh user@gateway-host
```
...
lada cron status
lada cron list
lada cron runs --id <jobId> --limit 20
lada system heartbeat last
lada logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so LADA skips the model call.
...
lada approvals get --node <idOrNameOrIp>
lada logs --follow
lada status
```
...
```bash
lada browser status
lada browser start --browser-profile lada
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `lada browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
lada gateway status
lada config get gateway.mode
lada config get gateway.remote.url
lada config get gateway.auth.mode
```
...
```bash
lada config get gateway.bind
lada config get gateway.auth.mode
lada config get gateway.auth.token
lada gateway status
lada logs --follow
```
