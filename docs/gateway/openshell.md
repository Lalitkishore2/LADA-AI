---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for LADA agents"
read_when:
...
OpenShell is a managed sandbox backend for LADA. Instead of running Docker
containers locally, LADA delegates sandbox lifecycle to the `openshell` CLI,
...
- LADA Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, LADA creates an OpenShell
...
- You edit files locally outside LADA and want those changes visible in the
...
- When the sandbox is first created, LADA seeds the remote workspace from
...
- LADA does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside LADA after the initial seed,
the remote sandbox does **not** see those changes. Use
`lada sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"lada"`  | Sandbox source for first-time create                  |
...
1. LADA calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. LADA calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `lada sandbox` commands
