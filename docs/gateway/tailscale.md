...
LADA can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. LADA requires a shared password.
...
- `token` (default when `LADA_GATEWAY_TOKEN` is set)
- `password` (shared secret via `LADA_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. LADA verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
LADA only treats a request as Serve when it arrives from loopback with
...
Prefer `LADA_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
lada gateway --tailscale serve
lada gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want LADA to undo `tailscale serve`
