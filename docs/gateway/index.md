...
lada gateway --port 18789 --verbose
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `LADA_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `LADA_GATEWAY_TOKEN` / `LADA_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

LADA’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `lada`, `lada/default`, and `lada/<agentId>`.
- `lada/default` is the stable alias that always maps to the configured default agent.
- Use `x-lada-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `LADA_GATEWAY_PORT` → `gateway.port` → `18789` |
...
```bash
lada gateway status
lada gateway status --deep   # adds a system-level service scan
...
LaunchAgent labels are `ai.lada.gateway` (default) or `ai.lada.<profile>` (named profile). `lada doctor` audits and repairs service config drift.
...
```bash
lada gateway install
systemctl --user enable --now lada-gateway[-<profile>].service
lada gateway status
```
...
```ini
[Unit]
Description=LADA Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/lada gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `LADA Gateway`
(or `LADA Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, LADA falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now lada-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/lada-gateway[-<profile>].service` and adjust
`ExecStart=` if your `lada` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `LADA_CONFIG_PATH`
- Unique `LADA_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
LADA_CONFIG_PATH=~/.lada/a.json LADA_STATE_DIR=~/.lada-a lada gateway --port 19001
LADA_CONFIG_PATH=~/.lada/b.json LADA_STATE_DIR=~/.lada-b lada gateway --port 19002
```
...
```bash
lada --dev setup
lada --dev gateway --allow-unconfigured
lada --dev status
```
