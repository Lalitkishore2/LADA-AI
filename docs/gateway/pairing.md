...
lada nodes status
lada nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.lada`):

- `~/.lada/nodes/paired.json`
- `~/.lada/nodes/pending.json`

If you override `LADA_STATE_DIR`, the `nodes/` folder moves with it.
