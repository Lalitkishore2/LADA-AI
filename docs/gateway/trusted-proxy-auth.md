...
read_when:
  - Running LADA behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of LADA
...
- You run LADA behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. LADA checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. LADA extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that LADA can verify.
...
- LADA can stay on loopback HTTP behind the proxy.
...
If LADA itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://lada.example.com
    to: http://lada-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
LADA rejects ambiguous configurations where both a `gateway.auth.token` (or `LADA_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-lada-scopes`.

Examples:

- `x-lada-scopes: operator.read`
- `x-lada-scopes: operator.read,operator.write`
- `x-lada-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, LADA honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-lada-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-lada-scopes` explicitly when you want a trusted-proxy request to
...
`lada security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
LADA rejected a loopback-source trusted-proxy request.
...
3. Update LADA config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `lada security audit` and review findings
