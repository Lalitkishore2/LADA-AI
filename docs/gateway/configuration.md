...
read_when:
  - Setting up LADA for the first time
...
LADA reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.lada/lada.json`.

If the file is missing, LADA uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `lada onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    lada onboard       # full onboarding flow
    lada configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    lada config get agents.defaults.workspace
    lada config set agents.defaults.heartbeat.every "2h"
    lada config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.lada/lada.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
LADA only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `lada config schema` prints the same JSON Schema family used by Control UI
...
- Only diagnostic commands work (`lada doctor`, `lada logs`, `lada health`, `lada status`)
- Run `lada doctor` to see exact issues
- Run `lada doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Model refs use `provider/model` format (e.g. `anthropic/lada-opus-4-6`).
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `lada.json`.
    ...
    ```bash
    lada config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `LADA_APNS_RELAY_BASE_URL` and `LADA_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `LADA_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-lada-token`); query-string tokens are rejected.
...
The Gateway watches `~/.lada/lada.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `lada config set` for single keys.
    </Warning>
    ...
    ```bash
    lada gateway call config.get --params '{}'  # capture payload.hash
    lada gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.lada/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    lada gateway call config.patch --params '{
...
LADA reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.lada/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, LADA runs your login shell and imports only the missing keys:
...
Env var equivalent: `LADA_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${LADA_GATEWAY_TOKEN}" } },
