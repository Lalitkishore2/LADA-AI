...
1. Leave heartbeats enabled (default is `30m`, or `1h` for Anthropic OAuth/token auth, including LADA CLI reuse) or set your own cadence.
...
- Interval: `30m` (or `1h` when Anthropic OAuth/token auth is the detected auth mode, including LADA CLI reuse). Set `agents.defaults.heartbeat.every` or per-agent `agents.list[].heartbeat.every`; use `0m` to disable.
...
- During heartbeat runs, LADA treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `lada sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, LADA can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, LADA skips the heartbeat run entirely (no model call).
...
- LADA parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
lada system event --text "Check for urgent follow-ups" --mode now
```
