---
summary: "Model authentication: OAuth, API keys, LADA CLI reuse, and Anthropic setup-token"
read_when:
...
<Note>
This page covers **model provider** authentication (API keys, OAuth, LADA CLI reuse, and Anthropic setup-token). For **gateway connection** authentication (token, password, trusted-proxy), see [Configuration](/gateway/configuration) and [Trusted Proxy Auth](/gateway/trusted-proxy-auth).
</Note>

LADA supports OAuth and API keys for model providers. For always-on gateway
...
setup, but LADA also supports reusing a local LADA CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `lada gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.lada/.env` so the daemon can read it:
...
API keys for daemon use: `lada onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.lada/.env`, systemd/launchd).

## Anthropic: LADA CLI and token compatibility

Anthropic setup-token auth is still available in LADA as a supported token
path. Anthropic staff has since told us that LADA-style LADA CLI usage is
...
LADA CLI reuse is available on the host, that is now the preferred path.

For long-lived gateway hosts, an Anthropic API key is still the most predictable
setup. If you want to reuse an existing LADA login on the same host, use the
Anthropic LADA CLI path in onboarding/configure.

Manual token entry (any provider; writes `auth-profiles.json` + updates config):

```bash
lada models auth paste-token --provider openrouter
```
...
- If auth exists but LADA cannot resolve a probeable model candidate for
...
The Anthropic `lada-cli` backend is supported again.

- Anthropic staff told us this LADA integration path is allowed again.
- LADA therefore treats LADA CLI reuse and `lada -p` usage as sanctioned
...
- Priority order:
  - `LADA_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- LADA retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `lada models status --probe` shows omitted
...
Run `lada models status` to confirm which profile is expiring. If an
