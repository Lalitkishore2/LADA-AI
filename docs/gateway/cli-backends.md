...
LADA can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **LADA tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
lada agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, LADA now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), LADA will
...
2. **Builds a system prompt** using the same LADA prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `lada-cli` backend is supported again. Anthropic staff
told us LADA-style LADA CLI usage is allowed again, so LADA treats
`lada -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>
...
- LADA drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
LADA will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, LADA appends the
...
- For Gemini CLI JSON output, LADA reads reply text from `response` and
...
- `stats.cached` is normalized into LADA `cacheRead`.
- If `stats.input` is missing, LADA derives input tokens from
...
If no MCP servers are enabled, LADA still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct LADA tool calls.** LADA does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. LADA sessions still work
  normally.
