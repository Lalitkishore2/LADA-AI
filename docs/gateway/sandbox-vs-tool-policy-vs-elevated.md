...
LADA has three related (but different) controls:
...
Use the inspector to see what LADA is _actually_ doing:
...
- LADA validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:lada`: all built-in LADA tools (excludes provider plugins)
