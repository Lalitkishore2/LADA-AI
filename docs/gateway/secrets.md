...
LADA supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `LADA_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, LADA runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). LADA validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/LADA QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/lada"],
...
- LADA resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__LADA_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `lada.json` refs)
...
When reload-time activation fails after a healthy state, LADA enters degraded secrets state.
...
- Strict command paths (for example `lada memory` remote-memory paths and `lada qr --remote` when it needs remote shared-secret refs) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `lada status`, `lada status --all`, `lada channels status`, `lada channels resolve`, `lada security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `lada secrets reload`.
...
```bash
lada secrets audit --check
lada secrets configure
lada secrets audit --check
```
...
- plaintext values at rest (`lada.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `lada.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `lada secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `lada.json` plus `auth-profiles.json` for one agent scope
...
- `lada secrets configure --providers-only`
- `lada secrets configure --skip-provider-setup`
- `lada secrets configure --agent <id>`
...
```bash
lada secrets apply --from /tmp/lada-secrets-plan.json
lada secrets apply --from /tmp/lada-secrets-plan.json --allow-exec
lada secrets apply --from /tmp/lada-secrets-plan.json --dry-run
lada secrets apply --from /tmp/lada-secrets-plan.json --dry-run --allow-exec
```
...
LADA intentionally does not write rollback backups containing historical plaintext secret values.
