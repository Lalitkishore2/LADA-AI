...
LADA gives agents tools to work across sessions, inspect status, and
...
After the target responds, LADA can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned LADA
...
the completion origin only identifies a channel LADA can still reuse the
