...
- **Session store** (chat history + routing state) under `~/.lada/agents/<agentId>/sessions`.
...
`~/.lada/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.lada` (or `LADA_STATE_DIR`)
- Workspace: `~/.lada/workspace` (or `~/.lada/workspace-<agentId>`)
- Agent dir: `~/.lada/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.lada/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, LADA runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.lada/workspace` (or `~/.lada/workspace-<profile>` when `LADA_PROFILE` is set).
- State defaults to `~/.lada/agents/main/agent`.
...
```bash
lada agents add coding
lada agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.lada/agents/<agentId>`.
...
```bash
lada channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.lada/workspace-alex" },
      { id: "mia", workspace: "~/.lada/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, LADA upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, LADA falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.lada/workspace-main" },
      { id: "coding", workspace: "~/.lada/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.lada/workspace-main" },
      { id: "alerts", workspace: "~/.lada/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
lada channels login --channel whatsapp --account personal
lada channels login --channel whatsapp --account biz
```

`~/.lada/lada.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.lada/workspace-home",
        agentDir: "~/.lada/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.lada/workspace-work",
        agentDir: "~/.lada/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.lada/credentials/whatsapp/personal
          // authDir: "~/.lada/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.lada/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.lada/workspace-chat",
        model: "anthropic/lada-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.lada/workspace-opus",
