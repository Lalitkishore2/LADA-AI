---
summary: "What the LADA system prompt contains and how it is assembled"
read_when:
...
LADA builds a custom system prompt for every agent run. The prompt is **LADA-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by LADA and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full LADA-owned prompt. The provider runtime can:
...
- **LADA Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to LADA docs (repo or npm package) and when to read them.
...
LADA can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **LADA
...
occurs, LADA can inject a warning block in Project Context; control this with
...
When eligible skills exist, LADA injects a compact **available skills list**
...
local LADA docs directory (either `docs/` in the repo workspace or the bundled npm
...
for LADA behavior, commands, configuration, or architecture, and to run
`lada status` itself when possible (asking the user only when it lacks access).
