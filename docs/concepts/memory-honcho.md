...
[Honcho](https://honcho.dev) adds AI-native memory to LADA. It persists
...
```bash
lada plugins install @honcho-ai/lada-honcho
lada honcho setup
lada gateway --force
```
...
Settings live under `plugins.entries["lada-honcho"].config`:
    ...
    entries: {
      "lada-honcho": {
        config: {
          ...
          workspaceId: "lada", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `lada honcho setup` detects and
...
```bash
lada honcho setup                        # Configure API key and migrate files
lada honcho status                       # Check connection status
lada honcho ask <question>               # Query Honcho about the user
lada honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/lada-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho LADA integration guide](https://docs.honcho.dev/v3/guides/integrations/lada)
- [Memory](/concepts/memory) -- LADA memory overview
