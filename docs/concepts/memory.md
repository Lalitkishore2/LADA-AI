...
summary: "How LADA remembers things across sessions"
read_when:
...
LADA remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.lada/workspace`).
...
<Info>
LADA auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, LADA
...
```bash
lada memory status          # Check index status and provider
lada memory search "query"  # Search from the command line
lada memory index --force   # Rebuild the index
```
