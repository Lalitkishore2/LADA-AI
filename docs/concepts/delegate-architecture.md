---
summary: "Delegate architecture: running LADA as a named agent on behalf of an organization"
...
Goal: run LADA as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an LADA agent that:
...
LADA's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of LADA's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.lada/workspace-delegate",
  tools: {
...
- Cron run history: `~/.lada/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.lada/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through LADA's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
lada agents add delegate
```

This creates:

- Workspace: `~/.lada/workspace-delegate`
- State: `~/.lada/agents/delegate/agent`
- Sessions: `~/.lada/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.lada/workspace" },
      {
        id: "delegate",
        workspace: "~/.lada/workspace-delegate",
        tools: {
...
~/.lada/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.lada/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.lada/workspace-org",
        agentDir: "~/.lada/agents/org-assistant/agent",
...
recall view. LADA redacts credential/token-like text, truncates long
