...
LADA selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models LADA can use (plus aliases).
...
subscription** (OAuth) and **Anthropic** (API key or LADA CLI).
...
- If you omit the provider, LADA resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, LADA
...
lada models list
lada models status
lada models set <provider/model>
lada models set-image <provider/model>

lada models aliases list
lada models aliases add <alias> <provider/model>
lada models aliases remove <alias>

lada models fallbacks list
lada models fallbacks add <provider/model>
lada models fallbacks remove <provider/model>
lada models fallbacks clear

lada models image-fallbacks list
lada models image-fallbacks add <provider/model>
lada models image-fallbacks remove <provider/model>
lada models image-fallbacks clear
```

`lada models` (no subcommand) is a shortcut for `models status`.
...
keys are usually the most predictable; LADA CLI reuse and existing Anthropic
...
`lada models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.lada/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: LADA writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever LADA regenerates `models.json`, including command-driven paths like `lada agent`.
