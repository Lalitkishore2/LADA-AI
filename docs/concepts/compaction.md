---
summary: "How LADA summarizes long conversations to stay within model limits"
read_when:
...
When a conversation approaches that limit, LADA **compacts** older messages
...
When LADA splits history into compaction chunks, it keeps assistant tool
calls paired with their matching `toolResult` entries. If a split point lands
inside a tool block, LADA moves the boundary so the pair stays together and
...
LADA compacts and retries). Typical overflow signatures include
...
<Info>
Before compacting, LADA automatically reminds the agent to save important
...
```json5
{
  agents: {
    defaults: {
      compaction: {
        model: "openrouter/anthropic/lada-sonnet-4-6",
      },
    },
  },
}
