...
read_when:
  - You want to understand how LADA assembles model context
...
A **context engine** controls how LADA builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

LADA ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.lada/lada.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other LADA plugin. Install
...
lada plugins install -l ./my-context-engine
```
...
Every time LADA runs a model prompt, the context engine participates at
...
LADA currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. LADA
...
The built-in `legacy` engine preserves LADA's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. LADA uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. LADA disables Pi's built-in
...
`ownsCompaction: false` does **not** mean LADA automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `lada/plugin-sdk/core` to use
  LADA's built-in compaction behavior.
...
LADA resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to LADA's built-in summarization. Plugin engines can implement
...
- Use `lada doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, LADA
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `lada plugins install -l ./my-engine` to link a
