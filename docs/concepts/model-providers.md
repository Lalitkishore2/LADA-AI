...
- Model refs use `provider/model` (example: `opencode/lada-opus-4-6`).
- If you set `agents.defaults.models`, it becomes the allowlist.
- CLI helpers: `lada onboard`, `lada models list`, `lada models set <provider/model>`.
...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  LADA merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while LADA keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `lada onboard`, `lada models auth`, and headless setup
...
- `normalizeTransport`: provider normalizes transport-family `api` / `baseUrl`
  before generic model assembly; LADA checks the matched provider first,
...
- `normalizeConfig`: provider normalizes `models.providers.<id>` config before
  runtime uses it; LADA checks the matched provider first, then other
...
- `anthropic`: LADA 4.6 forward-compat fallback, auth repair hints, usage
...
- `amazon-bedrock`: provider-owned context-overflow matching and failover
  ...
  the shared `anthropic-by-model` replay family for LADA-only replay-policy
  guards on Anthropic traffic
- `anthropic-vertex`: LADA-only replay-policy guards on Anthropic-message
  traffic
...
- `github-copilot`: onboarding/device login, forward-compat model fallback,
  LADA-thinking transcript hints, runtime token exchange, and usage endpoint
  fetching
...
That covers providers that still fit LADA's normal transports. A provider
...
- Configure multiple keys via:
  - `LADA_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
LADA ships with the pi‑ai catalog. These providers require **no**
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `LADA_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.4`, `openai/gpt-5.4-pro`
- CLI: `lada onboard --auth-choice openai-api-key`
...
- Hidden LADA attribution headers (`originator`, `version`,
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in LADA because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `LADA_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/lada-opus-4-6`
- CLI: `lada onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; LADA maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
- Anthropic note: Anthropic staff told us LADA-style LADA CLI usage is allowed again, so LADA treats LADA CLI reuse and `lada -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic setup-token remains available as a supported LADA token path, but LADA now prefers LADA CLI reuse and `lada -p` when available.
...
- CLI: `lada onboard --auth-choice openai-codex` or `lada models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; LADA maps that to `service_tier=priority`
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like LADA.
...
- Example models: `opencode/lada-opus-4-6`, `opencode-go/kimi-k2.5`
- CLI: `lada onboard --auth-choice opencode-zen` or `lada onboard --auth-choice opencode-go`

```json5
{
  agents: { defaults: { model: { primary: "opencode/lada-opus-4-6" } } },
}
```
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `LADA_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy LADA config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `lada onboard --auth-choice gemini-api-key`
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent`
  ...
  `cachedContents/...` handle; Gemini cache hits surface as LADA `cacheRead`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in LADA is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Install Gemini CLI first:
    - `brew install gemini-cli`
    - or `npm install -g @google/gemini-cli`
  - Enable: `lada plugins enable google`
  - Login: `lada models auth login --provider google-gemini-cli --set-default`
  - Default model: `google-gemini-cli/gemini-3-flash-preview`
  - Note: you do **not** paste a client id or secret into `lada.json`. The CLI login flow stores
  ...
  - Gemini CLI JSON replies are parsed from `response`; usage falls back to
    `stats`, with `stats.cached` normalized into LADA `cacheRead`.
...
- CLI: `lada onboard --auth-choice zai-api-key`
...
- Example model: `vercel-ai-gateway/anthropic/lada-opus-4.6`
- CLI: `lada onboard --auth-choice ai-gateway-api-key`
...
- CLI: `lada onboard --auth-choice kilocode-api-key`
...
- LADA applies OpenRouter's documented app-attribution headers only when
...
- CLI: `lada onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `lada onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- Example model: `moonshot/kimi-k2.5`
- CLI: `lada onboard --auth-choice moonshot-api-key` or `lada onboard --auth-choice moonshot-api-key-cn`
...
- Example model: `volcengine-plan/ark-code-latest`
- CLI: `lada onboard --auth-choice volcengine-api-key`
...
LADA falls back to the unfiltered catalog instead of showing an empty
...
- Example model: `byteplus-plan/ark-code-latest`
- CLI: `lada onboard --auth-choice byteplus-api-key`
...
- CLI: `lada onboard --auth-choice synthetic-api-key`
...
On MiniMax's Anthropic-compatible streaming path, LADA disables thinking by
...
`lada onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), LADA forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- Proxy-style OpenAI-compatible routes also skip native OpenAI-only request
  ...
  OpenAI reasoning-compat payload shaping, and no hidden LADA attribution
  headers.
- If `baseUrl` is empty/omitted, LADA keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
lada onboard --auth-choice opencode-zen
lada models set opencode/lada-opus-4-6
lada models list
```
