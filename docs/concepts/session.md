---
summary: "How LADA manages conversation sessions"
read_when:
...
LADA organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `lada security audit`.
...
LADA automatically bounds session storage over time. By default, it runs
...
Preview with `lada sessions cleanup --dry-run`.

## Inspecting sessions

- `lada status` -- session store path and recent activity.
- `lada sessions --json` -- all sessions (filter with `--active <minutes>`).
