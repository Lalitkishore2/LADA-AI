...
Zalo ships as a bundled plugin in current LADA releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `lada plugins install @lada/zalo`
- Or from a source checkout: `lada plugins install ./path/to/local/zalo-plugin`
...
This page reflects current LADA behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `lada pairing list zalo`
  - `lada pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in LADA.
...
- Example: `lada message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `lada channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `lada logs --follow`
