...
read_when:
  ...
  - Reviewing LADA security posture
title: "Pairing"
---

# Pairing

“Pairing” is LADA’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `lada-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.lada/credentials/`:
...
3. On your phone, open the LADA iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `lada nodes pending|approve|reject|rename`) is a
