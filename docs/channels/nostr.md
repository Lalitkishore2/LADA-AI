...
read_when:
  - You want LADA to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables LADA to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current LADA releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`lada onboard`) and `lada channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
lada plugins install @lada/nostr
```

Use a local checkout (dev workflows):

```bash
lada plugins install --link <path-to-local-nostr-plugin>
```
...
lada channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "lada@example.com",
        lud16: "lada@example.com",
      },
    },
  },
}
