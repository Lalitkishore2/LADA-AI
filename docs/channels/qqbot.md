...
read_when:
  - You want to connect LADA to QQ
...
QQ Bot connects to LADA via the official QQ Bot API (WebSocket gateway). The
...
Current LADA releases bundle QQ Bot, so normal packaged builds do not need
a separate `lada plugins install` step.
...
```bash
lada channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `lada channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single LADA instance:
...
```bash
lada channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
