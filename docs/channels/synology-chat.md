---
summary: "Synology Chat webhook setup and LADA config"
read_when:
  - Setting up Synology Chat with LADA
...
Synology Chat ships as a bundled plugin in current LADA releases, so normal
...
```bash
lada plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `lada onboard` now shows Synology Chat in the same channel setup list as `lada channels add`.
   - Non-interactive setup: `lada channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your LADA gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in LADA.
   - Guided: `lada onboard`
   - Direct: `lada channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- LADA accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `lada pairing list synology-chat`
  - `lada pairing approve synology-chat <CODE>`
...
```bash
lada message send --channel synology-chat --target 123456 --text "Hello from LADA"
lada message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. LADA now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached LADA
- `Rate limit exceeded`:
