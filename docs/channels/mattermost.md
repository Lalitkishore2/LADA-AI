---
summary: "Mattermost bot setup and LADA config"
read_when:
...
Mattermost ships as a bundled plugin in current LADA releases, so normal
...
```bash
lada plugins install @lada/mattermost
```

Local checkout (when running from a git repo):

```bash
lada plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure LADA and start the gateway.
...
Native slash commands are opt-in. When enabled, LADA registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, LADA derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when LADA registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as LADA.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to LADA.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from LADA, not `404`.
...
- Approve via:
  - `lada pairing list mattermost`
  - `lada pairing approve mattermost <CODE>`
...
Use these target formats with `lada message send` or cron/webhooks:
...
LADA resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), LADA sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When LADA sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, LADA derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and LADA run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="lada-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"lada-mattermost-interactions",
...
`lada message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: LADA
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as LADA. Set an
