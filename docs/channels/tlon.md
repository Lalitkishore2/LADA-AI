...
Tlon is a decentralized messenger built on Urbit. LADA connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current LADA releases, so normal packaged
...
```bash
lada plugins install @lada/tlon
```

Local checkout (when running from a git repo):

```bash
lada plugins install ./path/to/local/tlon-plugin
```
...
By default, LADA blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `lada message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, LADA replies in-thread.
