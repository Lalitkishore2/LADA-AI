...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects LADA to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current LADA releases, so no separate plugin install
is required.
...
```bash
lada plugins install @lada/feishu
```
...
If you just installed LADA, run onboarding:
...
2. Configuring app credentials in LADA
...
1. You already ran `lada channels add` for Feishu
2. The gateway is running (`lada gateway status`)
...
```bash
lada pairing approve feishu <CODE>
```
...
2. Run `lada logs --follow` and look for `chat_id`
...
2. Run `lada logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `lada gateway status`  | Show gateway status           |
| `lada gateway install` | Install/start gateway service |
| `lada gateway stop`    | Stop gateway service          |
| `lada gateway restart` | Restart gateway service       |
| `lada logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `lada gateway status`
6. Check logs: `lada logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.lada/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.lada/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
