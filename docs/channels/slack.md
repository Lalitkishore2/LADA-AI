...
```json
{
  "display_information": {
    "name": "LADA",
    "description": "Slack connector for LADA"
  },
  "features": {
    "bot_user": {
      "display_name": "LADA",
    ...
    "slash_commands": [
      {
        "command": "/lada",
        "description": "Send a message to LADA",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `lada pairing approve slack <code>`.
...
- If streaming fails mid-reply, LADA falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while LADA is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are LADA-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, LADA falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `lada channels status --probe --json` shows `botTokenStatus` or
