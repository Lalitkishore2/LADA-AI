...
read_when:
  - Setting up Zalo Personal for LADA
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside LADA.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current LADA releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `lada plugins install @lada/zalouser`
- Or from a source checkout: `lada plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `lada channels login --channel zalouser`
...
```bash
lada directory self --channel zalouser
lada directory peers list --channel zalouser --query "name"
lada directory groups list --channel zalouser --query "work"
```
...
- `lada pairing list zalouser`
- `lada pairing approve zalouser <code>`
...
- On startup, LADA resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, LADA stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in LADA state. Example:
...
- LADA sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, LADA sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `lada channels logout --channel zalouser && lada channels login --channel zalouser`
...
- The channel now runs fully in LADA without external CLI binaries.
