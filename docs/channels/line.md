...
read_when:
  - You want to connect LADA to LINE
...
LINE connects to LADA via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current LADA releases, so normal
...
```bash
lada plugins install @lada/line
```

Local checkout (when running from a git repo):

```bash
lada plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so LADA applies strict pre-auth body limits and timeout before verification.
- LADA processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
lada pairing list line
lada pairing approve line <CODE>
```
