...
read_when:
  - Setting up Twitch chat integration for LADA
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. LADA connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current LADA releases, so normal
...
```bash
lada plugins install @lada/twitch
```

Local checkout (when running from a git repo):

```bash
lada plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `LADA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "lada", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use LADA_TWITCH_ACCESS_TOKEN env var)
...
```bash
LADA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
