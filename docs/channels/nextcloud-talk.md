...
Nextcloud Talk ships as a bundled plugin in current LADA releases, so
...
```bash
lada plugins install @lada/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
lada plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "LADA" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `lada pairing list nextcloud-talk`
  - `lada pairing approve nextcloud-talk <CODE>`
