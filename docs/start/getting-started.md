---
summary: "Get LADA installed and run your first chat in minutes."
read_when:
...
Install LADA, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.lada/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.lada/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run LADA as a service account or want custom paths:

- `LADA_HOME` — home directory for internal path resolution
- `LADA_STATE_DIR` — override the state directory
- `LADA_CONFIG_PATH` — override the config file path
