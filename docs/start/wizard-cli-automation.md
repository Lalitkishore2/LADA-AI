---
summary: "Scripted onboarding and agent setup for the LADA CLI"
read_when:
...
Use `--non-interactive` to automate `lada onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but LADA now prefers LADA CLI reuse when available.
