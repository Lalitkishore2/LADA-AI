...
CLI onboarding is the **recommended** way to set up LADA on macOS,
...
`lada dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`lada configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
1. **Model/Auth** — choose any supported provider/auth flow (API key, OAuth, or provider-specific manual auth), including Custom Provider
   ...
   For Anthropic, interactive onboarding/configure offers **Anthropic LADA CLI** as the preferred local path and **Anthropic API key** as the recommended production path. Anthropic setup-token also remains available as a supported token-auth path.
2. **Workspace** — Location for agent files (default `~/.lada/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `lada doctor` first.
</Note>
...
Use `lada agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.lada/workspace-<agentId>`.
...
- CLI command reference: [`lada onboard`](/cli/onboard)
