---
summary: "Deploy LADA Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run LADA on a Kubernetes cluster
  - You want to test LADA in a Kubernetes environment
title: "Kubernetes"
---

# LADA on Kubernetes

A minimal starting point for running LADA on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

LADA is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/lada 18789:18789 -n lada
...
```bash
kubectl get secret lada-secrets -n lada -o jsonpath='{.data.LADA_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: lada (configurable via LADA_NAMESPACE)
├── Deployment/lada        # Single pod, init container + gateway
├── Service/lada           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/lada-config  # lada.json + AGENTS.md
└── Secret/lada-secrets    # Gateway token + API keys
```
...
Edit `lada.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret lada-secrets -n lada \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/lada -n lada
```

### Custom namespace

```bash
LADA_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/lada/lada:latest # or pin to a specific version from https://github.com/lada/lada/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # lada.json + AGENTS.md
