---
summary: "Host LADA on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up LADA on Oracle Cloud
  - Looking for free VPS hosting for LADA
...
Run a persistent LADA Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@lada`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=lada --reset` to re-authenticate.

**Gateway will not start** -- Run `lada doctor --non-interactive` and check logs with `journalctl --user -u lada-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep LADA up to date
