...
LADA ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/lada`, override with
  `LADA_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
lada update --tag 2026.4.1-beta.1
...
lada update --tag lada@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `lada update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  LADA prompts for confirmation (skip with `--yes`).
...
Preview what `lada update` would do without making changes:

```bash
lada update --dry-run
lada update --channel beta --dry-run
lada update --tag 2026.4.1-beta.1 --dry-run
lada update --dry-run --json
```
...
When you switch channels with `lada update`, LADA also syncs plugin
sources:
