---
summary: "Install LADA — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs LADA, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://lada.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want LADA and Node kept under a local prefix such as
`~/.lada`, without depending on a system-wide Node install:

```bash
curl -fsSL https://lada.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g lada@latest
```
...
Or skip the link and use `pnpm lada ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:lada/lada#main
```
...
```bash
lada --version      # confirm the CLI is available
lada doctor         # check for config issues
lada gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `lada onboard --install-daemon` or `lada gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy LADA on a cloud server or VPS:
...
<CardGroup cols={3}>
  <Card title="Updating" href="/install/updating" icon="refresh-cw">
    Keep LADA up to date.
  </Card>
  <Card title="Migrating" href="/install/migrating" icon="arrow-right">
    Move to a new machine.
  </Card>
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove LADA completely.
  </Card>
</CardGroup>

## Troubleshooting: `lada` not found

If the install succeeded but `lada` is not found in your terminal:
