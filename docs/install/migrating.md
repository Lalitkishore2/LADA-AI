---
summary: "Move (migrate) an LADA install from one machine to another"
read_when:
  - You are moving LADA to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating LADA to a New Machine

This guide moves an LADA gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.lada/` by default) and your **workspace**, you preserve:

- **Config** -- `lada.json` and all gateway settings
...
<Tip>
Run `lada status` on the old machine to confirm your state directory path.
Custom profiles use `~/.lada-<profile>/` or a path set via `LADA_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    lada gateway stop
    cd ~
    tar -czf lada-state.tgz .lada
    ```

    If you use multiple profiles (e.g. `~/.lada-work`), archive each separately.

  </Step>

  <Step title="Install LADA on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.lada/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `LADA_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `lada doctor`.
  </Accordion>

  <Accordion title="Copying only lada.json">
...
- [ ] `lada status` shows the gateway running
