---
summary: "Optional Docker-based setup and onboarding for LADA"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run LADA on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export LADA_IMAGE="ghcr.io/lada/lada:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/lada/lada/pkgs/container/lada).
  ...
  <Step title="Complete onboarding">
    ...
    `lada-gateway` directly. `lada-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm lada-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm lada-cli channels login

    # Telegram
    docker compose run --rm lada-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm lada-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t lada:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node lada-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node lada-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d lada-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `LADA_EXTRA_MOUNTS`
or `LADA_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `lada-cli` shares `lada-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d lada-gateway`, run onboarding
and setup-time config writes through `lada-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `LADA_IMAGE`               | Use a remote image instead of building locally                   |
| `LADA_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `LADA_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `LADA_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `LADA_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `LADA_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `LADA_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec lada-gateway node dist/index.js health --token "$LADA_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `LADA_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `LADA_CONFIG_DIR` to `/home/node/.lada` and
`LADA_WORKSPACE_DIR` to `/home/node/.lada/workspace`, so those paths
survive container replacement.

That mounted config directory is where LADA keeps:

- `lada.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `LADA_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/lada/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/lada/lada/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export LADA_SANDBOX=1
    export LADA_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm lada-cli gateway probe
    docker compose run -T --rm lada-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `lada-cli` uses `network_mode: "service:lada-gateway"` so CLI
    ...
    `no-new-privileges` on `lada-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.lada`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/lada-config /path/to/lada-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export LADA_HOME_VOLUME="lada_home"`
    2. **Bake system deps**: `export LADA_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `LADA_HOME_VOLUME` or `LADA_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/lada/lada/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    LADA runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm lada-cli dashboard --no-open
    docker compose run --rm lada-cli devices list
    docker compose run --rm lada-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm lada-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm lada-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping LADA up to date
