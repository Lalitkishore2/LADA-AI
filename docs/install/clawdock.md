---
summary: "ClawDock shell helpers for Docker-based LADA installs"
read_when:
  - You run LADA with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based LADA installs.
...
| `clawdock-cli <command>`  | Run LADA CLI commands in Docker           |
...
- `~/.lada/.env` for env-backed provider keys and bot tokens
- `~/.lada/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.lada/lada.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `lada.json` quickly. It redacts `.env` values in its printed output.
