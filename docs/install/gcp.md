---
summary: "Run LADA Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want LADA running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# LADA on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent LADA Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "LADA 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the LADA Gateway in Docker
- Persist `~/.lada` + `~/.lada/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.lada` state includes `lada.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-lada-project --name="LADA Gateway"
    gcloud config set project my-lada-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create lada-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh lada-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the LADA repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    LADA_IMAGE=lada:latest
    LADA_GATEWAY_TOKEN=change-me-now
    LADA_GATEWAY_BIND=lan
    LADA_GATEWAY_PORT=18789

    LADA_CONFIG_DIR=/home/$USER/.lada
    LADA_WORKSPACE_DIR=/home/$USER/.lada/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.lada
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `LADA_GATEWAY_TOKEN`.
    ...
    services:
      lada-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - LADA_GATEWAY_BIND=${LADA_GATEWAY_BIND}
          - LADA_GATEWAY_PORT=${LADA_GATEWAY_PORT}
          - LADA_GATEWAY_TOKEN=${LADA_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${LADA_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`LADA_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm lada-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh lada-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop lada-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type lada-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start lada-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create lada-deploy \
     --display-name="LADA Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-lada-project \
     --member="serviceAccount:lada-deploy@my-lada-project.iam.gserviceaccount.com" \
