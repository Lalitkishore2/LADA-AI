...
LADA supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, LADA also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `lada configure --section web`. It stores the key in
`~/.lada/lada.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.lada/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
