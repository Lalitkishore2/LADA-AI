...
read_when:
  - You want to use Groq with LADA
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. LADA connects
...
media-understanding provider, LADA uses Groq's `whisper-large-v3-turbo`
...
Groq's model catalog changes frequently. Run `lada models list | grep groq`
