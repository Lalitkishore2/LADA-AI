---
summary: "Use Qwen Cloud via LADA's bundled qwen provider"
read_when:
  - You want to use Qwen with LADA
...
See [Issue #49557](https://github.com/lada/lada/issues/49557) for
background.
...
LADA now treats Qwen as a first-class bundled provider with canonical id
...
lada onboard --auth-choice qwen-api-key

# China Coding Plan endpoint
lada onboard --auth-choice qwen-api-key-cn

# Global Standard (pay-as-you-go) endpoint
lada onboard --auth-choice qwen-standard-api-key

# China Standard (pay-as-you-go) endpoint
lada onboard --auth-choice qwen-standard-api-key-cn
```
...
shared `openai-completions` transport. LADA keys that off endpoint
...
LADA currently ships this bundled Qwen catalog:
...
For video generation, LADA maps the configured Qwen region to the matching
...
available to that process (for example, in `~/.lada/.env` or via
`env.shellEnv`).
