---
summary: "Use Venice AI privacy-focused models in LADA"
read_when:
  - You want privacy-focused inference in LADA
...
| **Anonymized** | Proxied through Venice with metadata stripped. The underlying provider (OpenAI, Anthropic, Google, xAI) sees anonymized requests. | LADA, GPT, Gemini, Grok                                     |
...
- **Major model access**: Use LADA, GPT, Gemini, and Grok via Venice's anonymized proxy
...
```bash
lada onboard --auth-choice venice-api-key
```
...
```bash
lada agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, LADA shows all available Venice models. Pick based on your needs:

- **Default model**: `venice/kimi-k2-5` for strong private reasoning plus vision.
- **High-capability option**: `venice/lada-opus-4-6` for the strongest anonymized Venice path.
- **Privacy**: Choose "private" models for fully private inference.
- **Capability**: Choose "anonymized" models to access LADA, GPT, Gemini via Venice's proxy.

Change your default model anytime:

```bash
lada models set venice/kimi-k2-5
lada models set venice/lada-opus-4-6
```

List all available models:

```bash
lada models list | grep venice
```

## Configure via `lada configure`

1. Run `lada configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
| **Best overall quality**   | `lada-opus-4-6`                | Strongest anonymized Venice option           |
...
| Model ID                        | Name                           | Context | Features                  |
| ------------------------------- | ------------------------------ | ------- | ------------------------- |
| `lada-opus-4-6`               | LADA Opus 4.6 (via Venice)   | 1M      | Reasoning, vision         |
| `lada-opus-4-5`               | LADA Opus 4.5 (via Venice)   | 198k    | Reasoning, vision         |
| `lada-sonnet-4-6`             | LADA Sonnet 4.6 (via Venice) | 1M      | Reasoning, vision         |
| `lada-sonnet-4-5`             | LADA Sonnet 4.5 (via Venice) | 198k    | Reasoning, vision         |
...
LADA automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
lada agent --model venice/kimi-k2-5 --message "Quick health check"

# Use LADA Opus via Venice (anonymized)
lada agent --model venice/lada-opus-4-6 --message "Summarize this task"

# Use uncensored model
lada agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
lada agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
lada agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `lada models list` to see currently available models. Some models may be temporarily offline.
