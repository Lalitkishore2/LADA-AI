---
summary: "Use Synthetic's Anthropic-compatible API in LADA"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. LADA registers it as the
...
```bash
lada onboard --auth-choice synthetic-api-key
```
...
Note: LADA's Anthropic client appends `/v1` to the base URL, so use
