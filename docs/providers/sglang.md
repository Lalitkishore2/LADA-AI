---
summary: "Run LADA with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run LADA against a local SGLang server
...
LADA can connect to SGLang using the `openai-completions` API.

LADA can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, LADA will query:
