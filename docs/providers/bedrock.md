---
summary: "Use Amazon Bedrock (Converse API) models with LADA"
read_when:
  - You want to use Amazon Bedrock models with LADA
...
LADA can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
LADA can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  LADA will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  LADA only auto-adds the
...
- `enabled` defaults to auto mode. In auto mode, LADA only enables the
...
- For explicit `models.providers["amazon-bedrock"]` entries, LADA can still
...
```json5
{
  models: {
    providers: {
      "amazon-bedrock": {
        ...
        models: [
          {
            id: "us.anthropic.lada-opus-4-6-v1:0",
            name: "LADA Opus 4.6 (Bedrock)",
            reasoning: true,
            input: ["text", "image"],
  ...
  agents: {
    defaults: {
      model: { primary: "amazon-bedrock/us.anthropic.lada-opus-4-6-v1:0" },
    },
  },
}
```
...
When running LADA on an EC2 instance with an IAM role attached, the AWS SDK
can use the instance metadata service (IMDS) for authentication. For Bedrock
model discovery, LADA only auto-enables the implicit provider from AWS env
...
lada config set plugins.entries.amazon-bedrock.config.discovery.enabled true
lada config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
...
LADA discovers **regional and global inference profiles** alongside
...
automatically. This means cross-region LADA profiles work without manual
provider overrides.

Inference profile IDs look like `us.anthropic.lada-opus-4-6-v1:0` (regional)
or `anthropic.lada-opus-4-6-v1:0` (global). If the backing model is already
...
foundation models in `lada models list`.
...
- If you rely on auto mode, set one of the supported AWS auth env markers on the
  ...
  `plugins.entries.amazon-bedrock.config.discovery.enabled: true`.
- LADA surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
