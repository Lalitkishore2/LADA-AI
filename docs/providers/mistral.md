---
summary: "Use Mistral models and Voxtral transcription with LADA"
read_when:
  - You want to use Mistral models in LADA
...
LADA supports Mistral for both text/image model routing (`mistral/...`) and
...
```bash
lada onboard --auth-choice mistral-api-key
# or non-interactive
lada onboard --mistral-api-key "$MISTRAL_API_KEY"
```
...
LADA currently ships this bundled Mistral catalog:
...
LADA maps the session **thinking** level to Mistral’s API:
