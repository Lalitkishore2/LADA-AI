...
```bash
lada onboard --auth-choice moonshot-api-key
# or
lada onboard --auth-choice moonshot-api-key-cn
```

Kimi Coding:

```bash
lada onboard --auth-choice kimi-code-api-key
```
...
LADA also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
- Native Moonshot endpoints (`https://api.moonshot.ai/v1` and
  ...
  shared `openai-completions` transport. LADA now keys that off endpoint
...
LADA also maps runtime `/think` levels for Moonshot:

- `/think off` -> `thinking.type=disabled`
- any non-off thinking level -> `thinking.type=enabled`

When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. LADA normalizes incompatible `tool_choice` values to `auto` for compatibility.
