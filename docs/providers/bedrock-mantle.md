---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with LADA"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with LADA
...
LADA includes a bundled **Amazon Bedrock Mantle** provider that connects to
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, LADA uses it directly. Otherwise,
LADA attempts to generate a Mantle bearer token from the AWS default
...
- LADA can mint the Mantle bearer token for you from AWS SDK-compatible
