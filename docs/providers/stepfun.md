---
summary: "Use StepFun models with LADA"
read_when:
  - You want StepFun models in LADA
...
LADA includes a bundled StepFun provider plugin with two provider ids:
...
```bash
lada onboard --auth-choice stepfun-standard-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
lada onboard --auth-choice stepfun-plan-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
```
...
- The provider is bundled with LADA, so there is no separate plugin install step.
...
- Use `lada models list` and `lada models set <provider/model>` to inspect or switch models.
