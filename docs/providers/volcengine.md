...
read_when:
  - You want to use Volcano Engine or Doubao models with LADA
...
```bash
lada onboard --auth-choice volcengine-api-key
```
...
`lada onboard --auth-choice volcengine-api-key` currently sets
...
loaded yet, LADA falls back to the unfiltered catalog instead of showing an
