---
title: "ComfyUI"
summary: "ComfyUI workflow image, video, and music generation setup in LADA"
read_when:
  - You want to use local ComfyUI workflows with LADA
...
LADA ships a bundled `comfy` plugin for workflow-driven ComfyUI runs.
...
The bundled plugin is workflow-driven, so LADA does not try to map generic
...
LADA treats that legacy shape as the image workflow config.
...
the configured graph. LADA does not pass input videos into Comfy workflows.
