---
title: "Kilo Gateway"
summary: "Use Kilo Gateway's unified API to access many models in LADA"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in LADA
---
...
```bash
lada onboard --auth-choice kilocode-api-key
```
...
LADA treats `kilocode/kilo/auto` as the stable default ref, but does not
publish a source-backed task-to-upstream-model mapping for that route.

## Available models

LADA dynamically discovers available models from the Kilo Gateway at startup. Use
...
kilocode/anthropic/lada-sonnet-4
...
- Model refs are `kilocode/<model-id>` (e.g., `kilocode/anthropic/lada-sonnet-4`).
...
- At startup, LADA tries `GET https://api.kilo.ai/api/gateway/models` and
...
- Gemini-backed Kilo refs stay on the proxy-Gemini path, so LADA keeps
