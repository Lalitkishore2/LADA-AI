---
summary: "Use xAI Grok models in LADA"
read_when:
  - You want to use Grok models in LADA
...
LADA ships a bundled `xai` provider plugin for Grok models.
...
```bash
lada onboard --auth-choice xai-api-key
```
...
LADA now uses the xAI Responses API as the bundled xAI transport. The same
...
LADA now includes these xAI model families out of the box:
...
```bash
lada config set tools.web.search.provider grok
```
...
- Auth is API-key only today. There is no xAI OAuth/device-code flow in LADA yet.
- `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the normal xAI provider path because it requires a different upstream API surface than the standard LADA xAI transport.

## Notes

- LADA applies xAI-specific tool-schema and tool-call compatibility fixes automatically on the shared runner path.
...
- `web_search`, `x_search`, and `code_execution` are exposed as LADA tools. LADA enables the specific xAI built-in it needs inside each tool request instead of attaching all native tools to every chat turn.
