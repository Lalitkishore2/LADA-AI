---
summary: "Run LADA through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run LADA against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact LADA compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. LADA works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated LADA provider plugin.
...
3. Add an explicit LADA provider entry and point your default model at it.
...
If LADA runs fail with an error like:
...
LADA will flatten pure text content parts into plain strings before sending
the request.

## Gemma and tool-schema caveat

Some current `inferrs` + Gemma combinations accept small direct
`/v1/chat/completions` requests but still fail on full LADA agent-runtime
turns.
...
That disables LADA's tool schema surface for the model and can reduce prompt
pressure on stricter local backends.

If tiny direct requests still work but normal LADA agent turns continue to
crash inside `inferrs`, the remaining issue is usually upstream model/server
behavior rather than LADA's transport layer.
...
- Direct tiny `/v1/chat/completions` calls pass, but `lada infer model run`
  fails: try `compat.supportsTools: false`.
- LADA no longer gets schema errors, but `inferrs` still crashes on larger
