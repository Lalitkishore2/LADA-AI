---
title: "Runway"
summary: "Runway video generation setup in LADA"
read_when:
  - You want to use Runway video generation in LADA
...
LADA ships a bundled `runway` provider for hosted video generation.
...
```bash
lada onboard --auth-choice runway-api-key
```

2. Set Runway as the default video provider:

```bash
lada config set agents.defaults.videoGenerationModel.primary "runway/gen4.5"
```
