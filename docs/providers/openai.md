---
summary: "Use OpenAI via API keys or Codex subscription in LADA"
read_when:
  - You want to use OpenAI models in LADA
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like LADA.

## Default interaction style

LADA can add a small OpenAI-specific prompt overlay for both `openai/*` and
...
without replacing the base LADA system prompt. The friendly overlay also
...
- `"off"`: disable the overlay and use the base LADA prompt only.
...
If you want the unmodified base LADA prompt, set the overlay to `"off"`:
...
```bash
lada config set plugins.entries.openai.config.personality off
```

LADA normalizes this setting case-insensitively at runtime, so values like
...
- In LADA, ChatGPT/Codex sign-in is routed through `openai-codex/*`, not `openai/*`

### CLI setup

```bash
lada onboard --auth-choice openai-api-key
# or non-interactive
lada onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. LADA forwards both through the `openai/*` Responses path.
LADA intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

LADA does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in LADA.
...
- Supports `size`
- Current OpenAI-specific caveat: LADA does not forward `aspectRatio` or
...
- Current OpenAI-specific caveat: LADA currently only forwards `size`
...
lada onboard --auth-choice openai-codex

# Or run OAuth directly
lada models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. LADA
...
managed by Codex CLI. On expiry, LADA re-reads the external Codex source
first and, when the provider can refresh it, writes the refreshed credential
back to Codex storage instead of taking ownership in a separate LADA-only
copy.

If your Codex account is entitled to Codex Spark, LADA also supports:

- `openai-codex/gpt-5.3-codex-spark`

LADA treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

LADA also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
LADA treats the Codex model metadata and the runtime context cap as separate
values.
...
LADA uses `pi-ai` for model streaming. For both `openai/*` and
`openai-codex/*`, default transport is `"auto"` (WebSocket-first, then SSE
fallback).

In `"auto"` mode, LADA also retries one early, retryable WebSocket failure
...
After a connect or early-turn WebSocket failure in `"auto"` mode, LADA marks
...
OpenAI Responses), LADA also attaches stable session and turn identity state
...
LADA also normalizes OpenAI usage counters across transport variants before
...
`prompt_tokens` / `completion_tokens`; LADA treats those as the same input
and output counters for `/status`, `/usage`, and session logs. When native
WebSocket traffic omits `total_tokens` (or reports `0`), LADA falls back to
...
For `openai/*` (Responses API), LADA also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. LADA enables it by default for
...
LADA, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
LADA forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, LADA leaves `service_tier` untouched

### OpenAI fast mode

LADA exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, LADA maps it to OpenAI priority processing:
...
LADA treats direct OpenAI, Codex, and Azure OpenAI endpoints differently
...
- hidden LADA attribution headers (`originator`, `version`, and
...
`baseUrl` on `api.openai.com`), LADA now auto-enables OpenAI server-side
