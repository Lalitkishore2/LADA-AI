...
read_when:
  - You want to use Google Gemini models with LADA
...
```bash
lada onboard --auth-choice gemini-api-key
```
...
```bash
lada models auth login --provider google-gemini-cli --set-default
```

Environment variables:

- `LADA_GEMINI_OAUTH_CLIENT_ID`
- `LADA_GEMINI_OAUTH_CLIENT_SECRET`
...
command is installed and on `PATH`. LADA supports both Homebrew installs
...
For direct Gemini API runs (`api: "google-generative-ai"`), LADA now
...
- Gemini cache-hit usage is normalized into LADA `cacheRead` from
