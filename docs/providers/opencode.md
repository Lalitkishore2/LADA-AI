---
summary: "Use OpenCode Zen and Go catalogs with LADA"
read_when:
...
OpenCode exposes two hosted catalogs in LADA:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. LADA keeps the runtime provider ids
...
lada onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
lada onboard --auth-choice opencode-go
lada onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
- Example models: `opencode/lada-opus-4-6`, `opencode/gpt-5.4`, `opencode/gemini-3-pro`
...
- Gemini-backed OpenCode refs stay on the proxy-Gemini path, so LADA keeps
