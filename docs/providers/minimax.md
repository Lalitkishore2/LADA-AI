---
summary: "Use MiniMax models in LADA"
read_when:
  - You want MiniMax models in LADA
...
LADA's MiniMax provider defaults to **MiniMax M2.7**.
...
entries, LADA materializes `MiniMax-M2.7` and
...
```bash
lada onboard --auth-choice minimax-global-oauth
# or
lada onboard --auth-choice minimax-cn-oauth
```
...
See the MiniMax plugin package README in the LADA repo for details.
...
```bash
lada onboard --auth-choice minimax-global-api
# or
...
On the Anthropic-compatible streaming path, LADA now disables MiniMax
...
```json5
{
  ...
  agents: {
    defaults: {
      models: {
        "anthropic/lada-opus-4-6": { alias: "primary" },
...
- On `api: "anthropic-messages"`, LADA injects
...
- LADA normalizes MiniMax coding-plan usage to the same `% left` display
  used by other providers. MiniMax's raw `usage_percent` / `usagePercent`
  fields are remaining quota, not consumed quota, so LADA inverts them.
  Count-based fields win when present. When the API returns `model_remains`,
  LADA prefers the chat-model entry, derives the window label from
...
- Use `lada models list` to confirm the current provider id, then switch with
  `lada models set minimax/MiniMax-M2.7` or
  `lada models set minimax-portal/MiniMax-M2.7`.
...
- Running `lada configure` and selecting a **MiniMax** auth option, or
