...
read_when:
  - You want to use Chutes with LADA
...
OpenAI-compatible API. LADA supports both browser OAuth and direct API-key
...
- Auth:
  - OAuth via `lada onboard --auth-choice chutes`
  - API key via `lada onboard --auth-choice chutes-api-key`
...
```bash
lada onboard --auth-choice chutes
```

LADA launches the browser flow locally, or shows a URL + redirect-paste
flow on remote/headless hosts. OAuth tokens auto-refresh through LADA auth
profiles.
...
```bash
lada onboard --auth-choice chutes-api-key
```
...
When Chutes auth is available, LADA queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, LADA falls
back to a bundled static catalog so onboarding and startup still work.

## Default aliases

LADA also registers three convenience aliases for the bundled Chutes
catalog:
