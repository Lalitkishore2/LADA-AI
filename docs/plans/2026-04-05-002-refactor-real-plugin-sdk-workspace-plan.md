...
| ----------- | ------------------------ | ------------------------------------ | -------------------------------------------- |
| Legacy mode | `lada/plugin-sdk/*`  | all existing non-opted-in extensions | current permissive behavior remains          |
| Opt-in mode | `@lada/plugin-sdk/*` | first-wave extensions only           | package-local `rootDir` + project references |
...
- root `tsconfig.json` maps `lada/plugin-sdk/*` directly to
  `src/plugin-sdk/*.ts`
...
- moves the SDK toward a workspace package named `@lada/plugin-sdk`
...
- R2. Name the new package `@lada/plugin-sdk`.
- R3. Give the new SDK package its own `package.json` and `tsconfig.json`.
- R4. Keep legacy `lada/plugin-sdk/*` imports working for non-opted-in
...
- Root `tsconfig.json` currently maps:
  - `lada/plugin-sdk` -> `src/plugin-sdk/index.ts`
  - `lada/plugin-sdk/*` -> `src/plugin-sdk/*.ts`
...
The initial `@lada/plugin-sdk` package only needs to cover these:
...
- Introduce `@lada/plugin-sdk` as a new workspace package while keeping the
  legacy root `lada/plugin-sdk/*` surface alive during migration.
...
- Keep `@lada/plugin-sdk` private in the first wave.
...
- Whether the first-wave package can point directly at package-local `src/*.ts`
  ...
  still required for the `@lada/plugin-sdk` package.
  This is an implementation-owned TS graph validation question.

- Whether the root `lada` package should proxy first-wave SDK subpaths to
...
- [ ] **Unit 1: Introduce the real `@lada/plugin-sdk` workspace package**
...
- Add a new workspace package named `@lada/plugin-sdk`.
...
- The repo contains a valid `@lada/plugin-sdk` workspace package with a
...
- [ ] **Unit 3: Migrate the first-wave extensions onto `@lada/plugin-sdk`**
...
- Modify: production and test imports under each of the 10 extension roots that
  currently reference `lada/plugin-sdk/*`

**Approach:**

- Add `@lada/plugin-sdk: workspace:*` to the first-wave extension
  `devDependencies`.
- Replace `lada/plugin-sdk/*` imports in those packages with
  `@lada/plugin-sdk/*`.
...
- Package dependency shape used by other `@lada/*` workspace packages
...
- Integration: migrated extensions do not require root `lada/plugin-sdk/*`
  aliases for typechecking.

**Verification:**

- The first-wave extensions build and test against `@lada/plugin-sdk`
...
- Keep root `lada/plugin-sdk/*` as the compatibility surface for legacy
...
- **API surface parity:** first-wave subpaths must remain semantically identical
  through both `lada/plugin-sdk/*` and `@lada/plugin-sdk/*` during the
  transition.
