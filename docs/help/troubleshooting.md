---
summary: "Symptom first troubleshooting hub for LADA"
read_when:
  - LADA is not working and you need the fastest path to a fix
...
- `lada status` → shows configured channels and no obvious auth errors.
- `lada status --all` → full report is present and shareable.
- `lada gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `lada gateway status` → `Runtime: running` and `RPC probe: ok`.
- `lada doctor` → no blocking config/service errors.
- `lada channels status --probe` → reachable gateway returns live per-account
...
- `lada logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in LADA

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `lada infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on LADA agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger LADA prompts crash the
...
## Plugin install fails with missing lada extensions

If install fails with `package.json missing lada.extensions`, the plugin package
is using an old shape that LADA no longer accepts.

Fix in the plugin package:

1. Add `lada.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `lada plugins install <package>` again.
...
```mermaid
flowchart TD
  A[LADA is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `lada gateway status`
    - `RPC probe: ok`
...
- `requests-in-flight` → main lane busy; heartbeat wake was deferred. - `unknown accountId` → heartbeat delivery target account does not exist.
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      lada config get tools.exec.host
      lada config get tools.exec.security
      lada config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `lada` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `lada browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
