...
This guide summarizes a sane workflow for working on the pi integration in LADA.
...
```bash
LADA_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm lada agent --message "Hello" --thinking low`
...
State lives under the LADA state directory. Default is `~/.lada`. If `LADA_STATE_DIR` is set, use that directory instead.

To reset everything:

- `lada.json` for config
