---
summary: "CLI reference for `lada agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
lada agent --to +15555550123 --message "status update" --deliver
lada agent --agent ops --message "Summarize logs"
...
lada agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: LADA persists markers from the active source config snapshot, not from resolved runtime secret values.
