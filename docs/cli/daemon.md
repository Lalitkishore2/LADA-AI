---
summary: "CLI reference for `lada daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `lada daemon ...` in scripts
...
`lada daemon ...` maps to the same service control surface as `lada gateway ...` service commands.
...
Use [`lada gateway`](/cli/gateway) for current docs and examples.
