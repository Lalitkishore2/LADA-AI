---
summary: "CLI reference for `lada cron` (schedule and run background jobs)"
read_when:
...
Tip: run `lada cron --help` for the full command surface.
...
Note: `lada cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `lada cron runs --id <job-id>` to follow the eventual outcome.

Note: `lada cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.lada/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`lada doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
lada cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
lada cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
lada cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
lada cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
lada cron run <job-id>
lada cron run <job-id> --due
lada cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
lada cron edit <job-id> --agent ops
lada cron edit <job-id> --clear-agent
lada cron edit <job-id> --session current
lada cron edit <job-id> --session "session:daily-brief"
```
...
```bash
lada cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
lada cron edit <job-id> --best-effort-deliver
lada cron edit <job-id> --no-best-effort-deliver
lada cron edit <job-id> --no-deliver
```
