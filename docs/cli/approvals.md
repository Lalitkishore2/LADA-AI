---
summary: "CLI reference for `lada approvals` (exec approvals for gateway or node hosts)"
read_when:
...
```bash
lada approvals get
lada approvals get --node <id|name|ip>
lada approvals get --gateway
```

`lada approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
lada approvals set --file ./exec-approvals.json
lada approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
lada approvals set --node <id|name|ip> --file ./exec-approvals.json
lada approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested LADA policy aligned, also set:
...
```bash
lada approvals allowlist add "~/Projects/**/bin/rg"
lada approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
lada approvals allowlist add --agent "*" "/usr/bin/uname"

lada approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `lada nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.lada/exec-approvals.json`.
