---
summary: "CLI reference for `lada qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
lada qr
lada qr --setup-code-only
lada qr --json
lada qr --remote
lada qr --url wss://gateway.example/ws
```
...
- With `--remote`, LADA requires either `gateway.remote.url` or
