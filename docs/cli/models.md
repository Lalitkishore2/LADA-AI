---
summary: "CLI reference for `lada models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
lada models status
lada models list
lada models set <model-or-alias>
lada models scan
```

`lada models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise LADA falls back to matching OAuth/API-key
...
the command uses `LADA_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, LADA resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `LADA_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but LADA could not resolve a probeable
...
```bash
lada models auth add
lada models auth login --provider <id>
lada models auth setup-token --provider <id>
lada models auth paste-token
```
...
`lada plugins list` to see which providers are installed.

Examples:

```bash
lada models auth login --provider openai-codex --set-default
```
