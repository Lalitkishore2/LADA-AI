---
summary: "CLI reference for `lada message` (send + channel actions)"
read_when:
...
- On cache miss, LADA will attempt a live directory lookup when the provider supports it.
...
- `lada message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, LADA does not force a `default` account SecretRef scope
...
```
lada message send --channel discord \
...
```bash
lada message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
lada message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
lada message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
lada message poll --channel msteams \
...
```
lada message react --channel slack \
...
```
lada message react --channel signal \
...
```
lada message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
lada message send --channel telegram --target @mychat \
