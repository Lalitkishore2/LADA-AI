---
summary: "CLI reference for `lada memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
lada memory status
lada memory status --deep
lada memory status --fix
lada memory index --force
lada memory search "meeting notes"
lada memory search --query "deployment" --max-results 20
lada memory promote --limit 10 --min-score 0.75
lada memory promote --apply
lada memory promote --json --min-recall-count 0 --min-unique-queries 0
lada memory promote-explain "router vlan"
lada memory promote-explain "router vlan" --json
lada memory rem-harness
lada memory rem-harness --json
lada memory status --json
lada memory status --deep --index
lada memory status --deep --index --verbose
lada memory status --agent main
lada memory index --agent main --verbose
```
...
```bash
lada memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `lada cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
lada memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
lada memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
