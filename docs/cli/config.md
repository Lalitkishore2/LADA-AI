---
summary: "CLI reference for `lada config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `lada.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `lada configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `lada config` without a subcommand
...
```bash
lada config file
lada config --section model
lada config --section gateway --section daemon
lada config schema
lada config get browser.executablePath
...
lada config set secrets.providers.vaultfile --provider-source file --provider-path /etc/lada/secrets.json --provider-mode json
...
lada config validate
lada config validate --json
```

### `config schema`

Print the generated JSON schema for `lada.json` to stdout as JSON.
...
```bash
lada config schema > lada.schema.json
```
...
lada config get agents.list[0].id
```
...
lada config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
lada config set agents.defaults.heartbeat.every "0m"
lada config set gateway.port 19001 --strict-json
lada config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`lada config set` supports four assignment styles:

1. Value mode: `lada config set <path> <value>`
2. SecretRef builder mode:

```bash
lada config set channels.discord.token \
...
```bash
lada config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/lada-vault \
  --provider-arg read \
...
```bash
lada config set --batch-file ./config-set.batch.json --dry-run
```
...
lada config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/lada/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `lada.json`.
...
- `config file`: Print the active config file path (resolved from `LADA_CONFIG_PATH` or default location).
