---
summary: "CLI reference for `lada devices` (device pairing + token rotation/revocation)"
read_when:
...
```
lada devices remove <deviceId>
lada devices remove <deviceId> --json
```

### `lada devices clear --yes [--pending]`

Clear paired devices in bulk.

```
lada devices clear --yes
lada devices clear --yes --pending
lada devices clear --yes --pending --json
```

### `lada devices approve [requestId] [--latest]`

Approve a pending device pairing request. If `requestId` is omitted, LADA
automatically approves the most recent pending request.

Note: if a device retries pairing with changed auth details (role/scopes/public
key), LADA supersedes the previous pending entry and issues a new
`requestId`. Run `lada devices list` right before approval to use the
current ID.

```
lada devices approve
...
### `lada devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
lada devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `lada devices revoke --device <id> --role <role>`
...
```
lada devices revoke --device <deviceId> --role node
```
...
```bash
lada devices rotate --device <deviceId> --role operator
```
