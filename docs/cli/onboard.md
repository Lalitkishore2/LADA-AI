---
summary: "CLI reference for `lada onboard` (interactive onboarding)"
read_when:
...
```bash
lada onboard
lada onboard --flow quickstart
lada onboard --flow manual
lada onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`LADA_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `lada gateway run`.
...
- Fastest first chat: `lada dashboard` (Control UI, no channel setup).
