---
summary: "LADA Gateway CLI (`lada gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is LADA’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `lada gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.lada/lada.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
- `lada onboard --mode local` and `lada setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
- `--token <token>`: token override (also sets `LADA_GATEWAY_TOKEN` for the process).
...
```bash
lada gateway health --url ws://127.0.0.1:18789
```
...
```bash
lada gateway usage-cost
lada gateway usage-cost --days 7
lada gateway usage-cost --json
```
...
lada gateway status --require-rpc
```
...
```bash
lada gateway call status
lada gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `LADA_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `LADA_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_lada-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `lada.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
lada gateway discover --timeout 4000
lada gateway discover --json | jq '.beacons[].wsUrl'
```
