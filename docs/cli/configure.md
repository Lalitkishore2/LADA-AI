---
summary: "CLI reference for `lada configure` (interactive configuration prompts)"
read_when:
...
Tip: `lada config` without a subcommand opens the same wizard. Use
`lada config get|set|unset` for non-interactive edits.

For web search, `lada configure --section web` lets you choose a provider
...
lada configure --section model --section channels
lada configure --section gateway --section daemon
```
