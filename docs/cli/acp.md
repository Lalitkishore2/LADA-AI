...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an LADA Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`lada acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to LADA channel
conversations instead of hosting an ACP harness session, use
[`lada mcp serve`](/cli/mcp) instead.
...
- LADA acts as an ACP server
- an IDE or ACP client connects to LADA
- LADA forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where LADA runs an
external harness such as Codex or LADA through `acpx`.

Quick rule:

- editor/client wants to talk ACP to LADA: use `lada acp`
- LADA should launch Codex/LADA/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the LADA gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
lada acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
lada acp --url wss://gateway-host:18789 --token-file ~/.lada/gateway.token

# Attach to an existing session key
lada acp --session agent:main:main

# Attach by label (must already exist)
lada acp --session-label "support inbox"

# Reset the session key before the first prompt
lada acp --session agent:main:main --reset-session
```
...
lada acp client --server-args --url wss://gateway-host:18789 --token-file ~/.lada/gateway.token

# Override the server command (default: lada)
lada acp client --server "node" --server-args lada.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run LADA through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an LADA Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `lada acp` over stdio.

Example config (persisted):

```bash
lada config set gateway.remote.url wss://gateway-host:18789
lada config set gateway.remote.token <token>
```
...
```bash
lada acp --session agent:main:main
lada acp --session agent:design:main
lada acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see LADA plugin tools, enable the
...
## Use from `acpx` (Codex, LADA, other ACP clients)

If you want a coding agent such as Codex or LADA to talk to your
LADA bot over ACP, use `acpx` with its built-in `lada` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx lada` at `lada acp`.
3. Target the LADA session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default LADA ACP session
acpx lada exec "Summarize the active LADA session state."

# Persistent named session for follow-up turns
acpx lada sessions ensure --name codex-bridge
acpx lada -s codex-bridge --cwd /path/to/repo \
  "Ask my LADA work agent for recent context relevant to this repo."
```

If you want `acpx lada` to target a specific Gateway and session key every
time, override the `lada` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "lada": {
      "command": "env LADA_HIDE_BANNER=1 LADA_SUPPRESS_NOTES=1 lada acp --url ws://127.0.0.1:18789 --token-file ~/.lada/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local LADA checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env LADA_HIDE_BANNER=1 LADA_SUPPRESS_NOTES=1 node lada.mjs acp ...
```

This is the easiest way to let Codex, LADA, or another ACP-aware client
pull contextual information from an LADA agent without scraping a terminal.
...
In Zed, open the Agent panel and select “LADA ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`LADA_GATEWAY_TOKEN`, `LADA_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`LADA_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `LADA_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `lada acp client` sets `LADA_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `lada`).
