---
summary: "CLI reference for `lada logs` (tail gateway logs via RPC)"
read_when:
...
`lada logs` also accepts the standard Gateway client flags:
...
lada logs
lada logs --follow
lada logs --follow --interval 2000
lada logs --limit 500 --max-bytes 500000
lada logs --json
lada logs --plain
lada logs --no-color
lada logs --limit 500
lada logs --local-time
lada logs --follow --local-time
lada logs --url ws://127.0.0.1:18789 --token "$LADA_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `lada logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
