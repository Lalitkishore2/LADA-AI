---
summary: "LADA CLI reference for `lada` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.lada-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.lada-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `lada update` (source installs only).
...
LADA uses a lobster palette for CLI output.
...
Note: plugins can add additional top-level commands (for example `lada voicecall`).

## Security

- `lada security audit` — audit config + local state for common security foot-guns.
- `lada security audit --deep` — best-effort live Gateway probe.
- `lada security audit --fix` — tighten safe defaults and state/config permissions.
...
- `lada plugins list` — discover plugins (use `--json` for machine output).
- `lada plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `lada plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `lada plugins marketplace list <marketplace>` — list marketplace entries before install.
- `lada plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `lada plugins doctor` — report plugin load errors.
...
- `lada memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `lada memory index` — reindex memory files.
- `lada memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `lada memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `LADA Completion` block into your shell profile and points it at the cached script under the LADA state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.lada/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `lada config` with no
...
- `config set --dry-run`: validate assignments without writing `lada.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `lada.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `lada --update` rewrites to `lada update`.

### `backup`

Create and verify local backup archives for LADA state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `lada health` or `lada status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `lada doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, LADA promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
lada channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
lada channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
lada channels remove --channel discord --account work --delete
lada channels status --probe
lada status --deep
```

### `directory`

Look up self, peer, and group IDs for channels that expose a directory surface. See [`lada directory`](/cli/directory).
...
Tip: use `lada skills search`, `lada skills install`, and `lada skills update` for ClawHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`lada qr`](/cli/qr).
...
- After scanning, approve the request with `lada devices list` / `lada devices approve <requestId>`.

### `clawbot`

Legacy alias namespace. Currently supports `lada clawbot qr`, which maps to [`lada qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `lada plugins install`)
- `hooks update [id]` (deprecated alias for `lada plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `lada hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the LADA-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended LADA + Tailscale DNS config.
...
- `lada message send --target +15555550123 --message "Hi"`
- `lada message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Running `lada agents` with no subcommand is equivalent to `lada agents list`.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, LADA may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose LADA channels over MCP stdio.

#### `mcp serve`

Expose routed LADA channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `lada nodes`
...
LADA can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `lada status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so LADA inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise LADA falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `lada tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named LADA services are treated as first-class and aren't flagged as "extra".
...
Anthropic note: Anthropic staff told us LADA-style LADA CLI usage is
allowed again, so LADA treats LADA CLI reuse and `lada -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but LADA now prefers LADA CLI reuse and `lada -p` when available.

### `models` (root)

`lada models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported LADA token path, but LADA now prefers LADA CLI reuse and `lada -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `LADA_GATEWAY_TOKEN` / `LADA_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `LADA_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`lada browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`lada voicecall`](/cli/voicecall).
...
Search the live LADA docs index.
