---
summary: "CLI reference for `lada secrets` (reload, audit, configure, apply)"
read_when:
...
Use `lada secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
lada secrets reload
lada secrets reload --json
lada secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `lada.json` refs)
...
lada secrets audit
lada secrets audit --check
lada secrets audit --json
lada secrets audit --allow-exec
```
...
```bash
lada secrets configure
lada secrets configure --plan-out /tmp/lada-secrets-plan.json
lada secrets configure --apply --yes
lada secrets configure --providers-only
lada secrets configure --skip-provider-setup
lada secrets configure --agent ops
lada secrets configure --json
```
...
- `configure` targets secret-bearing fields in `lada.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, LADA fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
lada secrets apply --from /tmp/lada-secrets-plan.json --json
```
...
- `lada.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.lada/.env` known secret keys whose values were migrated
