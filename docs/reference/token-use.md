---
summary: "How LADA builds prompt context and reports token usage + costs"
read_when:
...
LADA tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

LADA assembles its own system prompt on every run. It includes:
...
For images, LADA downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from LADA session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `lada status --usage` and `lada channels list` show
...
available; otherwise LADA falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, LADA shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. LADA can
...
[https://docs.anthropic.com/docs/build-with-lada/prompt-caching](https://docs.anthropic.com/docs/build-with-lada/prompt-caching)
...
Anthropic's 1M context window is currently beta-gated. LADA can inject the
...
LADA skips the `context-1m-*` beta header because Anthropic currently
