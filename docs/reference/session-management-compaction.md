...
This document explains how LADA manages sessions end-to-end:
...
LADA is designed around a single **Gateway process** that owns session state.
...
LADA persists sessions in two layers:
...
- Transcripts: `~/.lada/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

LADA resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, LADA reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
lada sessions cleanup --dry-run
lada sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.lada/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
LADA intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When LADA splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, LADA
...
- If a trailing tool-result block would otherwise push the chunk over target,
  LADA preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (LADA consumes the events, but Pi decides when to compact).
...
LADA also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, LADA bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, LADA leaves it alone.
...
LADA supports “silent” turns for background tasks where the user should not see intermediate output.
...
- LADA strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, LADA also suppresses **draft/typing streaming** when a
...
LADA uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but LADA’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `lada status`.
