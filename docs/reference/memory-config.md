...
This page lists every configuration knob for LADA memory search. For
...
`lada.json` unless noted otherwise.
...
When `provider` is not set, LADA selects the first available:
...
If LADA runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, LADA falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.lada/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
LADA prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not LADA config. If you need to
