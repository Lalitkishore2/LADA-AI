...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. LADA normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- Anthropic prompt caching: [https://platform.lada.com/docs/en/build-with-lada/prompt-caching](https://platform.lada.com/docs/en/build-with-lada/prompt-caching)
...
- Anthropic request IDs and errors: [https://platform.lada.com/docs/en/api/errors](https://platform.lada.com/docs/en/api/errors)
...
- With Anthropic API-key auth profiles, LADA seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so LADA can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. LADA does not need to inject block-level cache markers.
- LADA uses `prompt_cache_key` to keep cache routing stable across turns and uses `prompt_cache_retention: "24h"` only when `cacheRetention: "long"` is selected on direct OpenAI hosts.
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). LADA maps that to `cacheRead`.
...
- Anthropic LADA model refs (`amazon-bedrock/*anthropic.lada*`) support explicit `cacheRetention` pass-through.
- Non-Anthropic Bedrock models are forced to `cacheRetention: "none"` at runtime.

### OpenRouter Anthropic models

For `openrouter/anthropic/*` model refs, LADA injects Anthropic
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, LADA
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; LADA maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, LADA automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  LADA manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  LADA maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, LADA derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean LADA is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

LADA splits the system prompt into a **stable prefix** and a **volatile
...
## LADA cache-stability guards

LADA also keeps several cache-sensitive payload shapes deterministic before
...
LADA exposes dedicated cache-trace diagnostics for embedded agent runs.
...
LADA keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
```sh
LADA_LIVE_TEST=1 LADA_LIVE_CACHE_TEST=1 pnpm test:live:cache
```
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.lada/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$LADA_STATE_DIR/logs/cache-trace.jsonl`
...
- `LADA_CACHE_TRACE=1` enables cache tracing.
- `LADA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `LADA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `LADA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `LADA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. LADA's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
