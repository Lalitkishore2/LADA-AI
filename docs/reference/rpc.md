...
LADA integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- LADA owns lifecycle when `channels.signal.autoStart=true`.
...
- LADA spawns `imsg rpc` as a child process (legacy iMessage integration).
