---
summary: "Install and use Codex, LADA, and Cursor bundles as LADA plugins"
read_when:
  - You want to install a Codex, LADA, or Cursor-compatible bundle
  - You need to understand how LADA maps bundle content into native features
...
LADA can install plugins from three external ecosystems: **Codex**, **LADA**,
and **Cursor**. These are called **bundles** — content and metadata packs that
LADA maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native LADA plugins. Native plugins run
...
Many useful plugins are published in Codex, LADA, or Cursor format. Instead
of requiring authors to rewrite them as native LADA plugins, LADA
detects these formats and maps their supported content into the native feature
set. This means you can install a LADA command pack or a Codex skill bundle
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    lada plugins install ./my-bundle

    # Archive
    lada plugins install ./my-bundle.tgz
  ...
  <Step title="Verify detection">
    ...
    Bundles show as `Format: bundle` with a subtype of `codex`, `lada`, or `cursor`.
...
## What LADA maps from bundles

Not every bundle feature runs in LADA today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal LADA skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | LADA, Cursor |
| Hook packs    | LADA-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
| MCP tools     | Bundle MCP config merged into embedded Pi settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | LADA `.lsp.json` and manifest-declared `lspServers` merged into embedded Pi LSP defaults  | LADA         |
| Settings      | LADA `settings.json` imported as embedded Pi defaults                                     | LADA         |

#### Skill content

- bundle skill roots load as normal LADA skill roots
- LADA `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means LADA markdown command files work through the normal LADA skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal LADA hook-pack
...
- LADA merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- LADA exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, LADA uses `sse`
...
LADA registers bundle MCP tools with provider-safe names in the form
...
- LADA `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- LADA sanitizes shell override keys before applying them
...
- enabled LADA bundles can contribute LSP server config
- LADA loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `lada plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but LADA does not run them:

- LADA `agents`, `hooks.json` automation, `outputStyles`
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit LADA best when they use skill roots and LADA-style
    hook-pack directories (`HOOK.md` + `handler.ts`).

  </Accordion>

  <Accordion title="LADA bundles">
    Two detection modes:

    - **Manifest-based:** `.lada-plugin/plugin.json`
    - **Manifestless:** default LADA layout (`skills/`, `commands/`, `agents/`, `hooks/`, `.mcp.json`, `.lsp.json`, `settings.json`)

    LADA-specific behavior:
...
LADA checks for native plugin format first:

1. `lada.plugin.json` or valid `package.json` with `lada.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.lada-plugin/`, or default LADA/Cursor layout) — treated as **bundle**

If a directory contains both, LADA uses the native path. This prevents
...
- LADA does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `lada plugins inspect <id>`. If a capability is listed but marked as
    not wired, that is a product limit — not a broken install.
  </Accordion>

  <Accordion title="LADA command files do not appear">
    Make sure the bundle is enabled and the markdown files are inside a detected
    `commands/` or `skills/` root.
  </Accordion>

  <Accordion title="LADA settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. LADA does
    not treat bundle settings as raw config patches.
  </Accordion>

  <Accordion title="LADA hooks do not execute">
    `hooks/hooks.json` is detect-only. If you need runnable hooks, use the
    LADA hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
