...
read_when:
  - You see the LADA_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the LADA_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external LADA plugin
---

# Plugin SDK Migration

LADA has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`lada/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`lada/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`lada/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `lada/plugin-sdk/slack`, `lada/plugin-sdk/discord`,
`lada/plugin-sdk/signal`, `lada/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`lada/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
- Anthropic keeps LADA-specific stream helpers in its own `api.ts` /
...
<Steps>
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `lada/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "lada/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { resolveControlCommandGate } from "lada/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "lada/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `LADASchema` |
  ...
  | `plugin-sdk/provider-usage` | Provider usage helpers | `fetchLADAUsage`, `fetchGeminiUsage`, `fetchGithubCopilotUsage`, and other provider usage helpers |
...
```bash
LADA_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 lada gateway run
LADA_SUPPRESS_EXTENSION_API_WARNING=1 lada gateway run
```
