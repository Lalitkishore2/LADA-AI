...
read_when:
  - Building or debugging native LADA plugins
...
This page covers the internal architecture of the LADA plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside LADA. Every
native LADA plugin registers against one or more capability types:
...
LADA classifies every loaded plugin into a shape based on its actual
...
Use `lada plugins inspect <id>` to see a plugin's shape and capability
...
When you run `lada doctor` or `lada plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `lada status --all` and `lada plugins doctor`.

## Architecture overview

LADA's plugin system has four layers:

1. **Manifest + discovery**
   LADA finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `lada.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native LADA plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of LADA reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting LADA
...
That split lets LADA validate config, explain missing/disabled plugins, and
...
normal chat actions. LADA keeps one shared `message` tool in core, and
...
LADA treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's LADA-facing
  surfaces
...
So if LADA adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If LADA has shared
...
```ts
import type { LADAPluginDefinition } from "lada/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "lada/plugin-sdk/media-understanding";

const plugin: LADAPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
LADA already treats image/audio/video understanding as one shared
...
`LADAPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   LADA can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that LADA knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `LADAPluginApi` or
  `api.runtime`
...
Native LADA plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because LADA currently treats them
...
name: `@lada/<id>` by default, or an approved typed suffix such as
...
LADA exports capabilities, not implementation convenience.
...
At startup, LADA does roughly this:
...
The manifest is the control-plane source of truth. LADA uses it to:
...
LADA keeps short in-process caches for:
...
- Set `LADA_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `LADA_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `LADA_PLUGIN_DISCOVERY_CACHE_MS` and
  `LADA_PLUGIN_MANIFEST_CACHE_MS`.
...
LADA still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, LADA calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | LADA tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on LADA's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before LADA's static catalog updates; it also uses
...
- GitHub Copilot uses `catalog`, `auth`, `resolveDynamicModel`, and
  ...
  needs provider-owned device login, model fallback behavior, LADA transcript
...
- Anthropic Vertex uses `buildReplayPolicy` through the
  `anthropic-by-model` replay family so LADA-specific replay cleanup stays
  scoped to LADA ids instead of every `anthropic-messages` transport.
- Amazon Bedrock uses `buildReplayPolicy`, `matchesContextOverflowError`,
  ...
  LADA-only `anthropic-by-model` guard.
...
- MiniMax uses `buildReplayPolicy` through the
  ...
  Anthropic-message and OpenAI-compatible semantics; it keeps LADA-only
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as LADA adds those
...
- LADA only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "LADA plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-lada-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-lada-scopes` only when the header is explicitly present
  - if `x-lada-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-lada-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `lada/plugin-sdk` import when
authoring plugins:

- `lada/plugin-sdk/plugin-entry` for plugin registration primitives.
- `lada/plugin-sdk/core` for the generic shared plugin-facing contract.
- `lada/plugin-sdk/config-schema` for the root `lada.json` Zod schema
  export (`LADASchema`).
- Stable channel primitives such as `lada/plugin-sdk/channel-setup`,
  `lada/plugin-sdk/setup-runtime`,
  `lada/plugin-sdk/setup-adapter-runtime`,
  `lada/plugin-sdk/setup-tools`,
  `lada/plugin-sdk/channel-pairing`,
  `lada/plugin-sdk/channel-contract`,
  `lada/plugin-sdk/channel-feedback`,
  `lada/plugin-sdk/channel-inbound`,
  `lada/plugin-sdk/channel-lifecycle`,
  `lada/plugin-sdk/channel-reply-pipeline`,
  `lada/plugin-sdk/command-auth`,
  `lada/plugin-sdk/secret-input`, and
  `lada/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `lada/plugin-sdk/channel-config-helpers`,
  `lada/plugin-sdk/allow-from`,
  `lada/plugin-sdk/channel-config-schema`,
  `lada/plugin-sdk/telegram-command-config`,
  `lada/plugin-sdk/channel-policy`,
  `lada/plugin-sdk/approval-runtime`,
  `lada/plugin-sdk/config-runtime`,
  `lada/plugin-sdk/infra-runtime`,
  `lada/plugin-sdk/agent-runtime`,
  `lada/plugin-sdk/lazy-runtime`,
  `lada/plugin-sdk/reply-history`,
  `lada/plugin-sdk/routing`,
  `lada/plugin-sdk/status-helpers`,
  `lada/plugin-sdk/text-runtime`,
  `lada/plugin-sdk/runtime-store`, and
  `lada/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `lada/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `lada/plugin-sdk/*` subpaths. LADA core/test code may use the repo
...
- Current bundled provider examples:
  - Anthropic uses `api.js` / `contract-api.js` for LADA stream helpers such
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  LADA is not yet serving a runtime snapshot.
...
- Avoid the root `lada/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `lada/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `lada/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `lada/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `lada/plugin-sdk/channel-targets`; channel-specific
...
`lada/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape LADA writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to LADA's
...
- if both `catalog` and `discovery` are registered, LADA uses `catalog`
...
- Read-only command paths such as `lada status`, `lada status --all`,
  `lada channels status`, `lada channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `lada.extensions`:
...
Security guardrail: every `lada.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `lada plugins install` installs plugin dependencies with
...
Optional: `lada.setupEntry` can point at a lightweight setup-only module.
When LADA needs setup surfaces for a disabled channel plugin, or
...
Optional: `lada.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let LADA load the
...
Channel plugins can advertise setup/discovery metadata via `lada.channel` and
install hints via `lada.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@lada/nextcloud-talk",
  "lada": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `lada.channel` fields beyond the minimal example:
...
LADA can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.lada/mpm/plugins.json`
- `~/.lada/mpm/catalog.json`
- `~/.lada/plugins/catalog.json`

Or point `LADA_PLUGIN_CATALOG_PATHS` (or `LADA_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "lada": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "lada/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `LADAPluginApi` and/or `api.runtime` with the smallest useful
...
This is how LADA stays opinionated without becoming hardcoded to one
