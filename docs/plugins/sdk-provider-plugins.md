...
summary: "Step-by-step guide to building a model provider plugin for LADA"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to LADA
...
(LLM) to LADA. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so LADA can detect
    credentials without loading your plugin runtime. `modelSupport` is optional
    and lets LADA auto-load your provider plugin from shorthand model ids
    like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `lada.compat` and `lada.build` fields
  ...
  <Step title="Register the provider">
    ...
    ```typescript index.ts
    import { definePluginEntry } from "lada/plugin-sdk/plugin-entry";
    import { createProviderApiKeyAuthMethod } from "lada/plugin-sdk/provider-auth";
    ...
    `lada onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "lada/plugin-sdk/provider-entry";
    ...
    `lada/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `lada/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "lada/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "lada/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "lada/plugin-sdk/provider-tools";
    ...
    | `anthropic-by-model` | LADA-aware replay policy chosen by `modelId`, so Anthropic-message transports only get LADA-specific thinking-block cleanup when the resolved model is actually a LADA id |
    ...
    | `hybrid-anthropic-openai` | Hybrid policy for providers that mix Anthropic-message and OpenAI-compatible model surfaces in one plugin; optional LADA-only thinking-block dropping stays scoped to the Anthropic side |
    ...
    `lada/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `lada/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@lada/anthropic-provider` exports
    ...
    they also encode LADA OAuth beta handling and `context1m` gating.
    ...
    `lada/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@lada/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@lada/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      LADA calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    LADA classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # lada.providers metadata
├── lada.plugin.json      # Manifest with providerAuthEnvVars
