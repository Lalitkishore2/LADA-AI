...
summary: "Create your first LADA plugin in minutes"
read_when:
  - You want to create a new LADA plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to LADA
---
...
You do not need to add your plugin to the LADA repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`lada plugins install <package-name>`. LADA tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect LADA to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`lada/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json lada.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to LADA",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    lada plugins install clawhub:@myorg/lada-my-plugin
    ```

    LADA also checks ClawHub before npm for bare package specs like
    `@myorg/lada-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, LADA retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `lada/plugin-sdk/error-runtime`
...
Always import from focused `lada/plugin-sdk/<subpath>` paths:
...
- Anthropic: LADA stream wrappers and `service_tier` / beta helpers
...
package-root seam instead of promoting it into `lada/plugin-sdk/*`.

Some generated `lada/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `lada` metadata</Check>
<Check>**lada.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [lada/lada](https://github.com/lada/lada/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official LADA X account [@lada](https://x.com/lada) for release announcements.
