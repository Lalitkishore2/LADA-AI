...
read_when:
  ...
  - You are defining plugin config schemas or package.json lada metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`lada.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `lada` field that tells the plugin system what
...
```json lada-clawhub-package.json
{
  "name": "@myorg/lada-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "lada": {
    "extensions": ["./index.ts"],
...
`lada.channel` is cheap package metadata for channel discovery and setup
...
`lada.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported LADA version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `lada doctor --fix`.
...
When enabled, LADA loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `lada.plugin.json` in the package root.
LADA uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to LADA",
  "configSchema": {
    "type": "object",
...
LADA loads when it only needs setup surfaces (onboarding, config repair,
...
**When LADA uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `lada/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that LADA validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "lada/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `lada onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "lada/plugin-sdk/channel-setup";
...
helpers from `lada/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`lada/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `lada/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "lada/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/lada-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
lada plugins install @myorg/lada-my-plugin
```

LADA tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
lada plugins install clawhub:@myorg/lada-my-plugin   # ClawHub only
```
...
```bash
lada plugins install <package-name>
```

<Info>
  For npm-sourced installs, `lada plugins install` runs
