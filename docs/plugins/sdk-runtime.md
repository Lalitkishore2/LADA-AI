...
```typescript
const model = api.runtime.agent.defaults.model; // e.g. "anthropic/lada-sonnet-4-6"
...
Bind a Task Flow runtime to an existing LADA session key or trusted tool
...
trusted LADA session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "LADA plugin SDK", count: 5 },
});
```
