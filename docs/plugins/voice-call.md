...
read_when:
  - You want to place an outbound voice call from LADA
...
Voice calls for LADA via a plugin. Supports outbound notifications and
...
- Use `lada voicecall ...` or the `voice_call` tool
...
lada plugins install "$PLUGIN_SRC"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `lada doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `lada doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `lada doctor --fix`:
...
```bash
lada voicecall call --to "+15555550123" --message "Hello from LADA"
lada voicecall start --to "+15555550123"   # alias for call
lada voicecall continue --call-id <id> --message "Any questions?"
lada voicecall speak --call-id <id> --message "One moment"
lada voicecall end --call-id <id>
lada voicecall status --call-id <id>
lada voicecall tail
lada voicecall latency                     # summarize turn latency from logs
