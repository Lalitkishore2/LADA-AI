...
summary: "Step-by-step guide to building a messaging channel plugin for LADA"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect LADA to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects LADA to a
...
<Info>
  If you have not built any LADA plugin before, read
...
Channel plugins do not need their own send/edit/react tools. LADA keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `lada/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization and transport hooks. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, `createApproverRestrictedNativeApprovalCapability`, and `createChannelNativeApprovalRuntime` from `lada/plugin-sdk/approval-runtime` so core owns request filtering, routing, dedupe, expiry, and gateway subscription.
...
- `lada/plugin-sdk/approval-auth-runtime`
- `lada/plugin-sdk/approval-client-runtime`
- `lada/plugin-sdk/approval-delivery-runtime`
- `lada/plugin-sdk/approval-native-runtime`
- `lada/plugin-sdk/approval-reply-runtime`

Likewise, prefer `lada/plugin-sdk/setup-runtime`,
`lada/plugin-sdk/setup-adapter-runtime`,
`lada/plugin-sdk/reply-runtime`,
`lada/plugin-sdk/reply-dispatch-runtime`,
`lada/plugin-sdk/reply-reference`, and
`lada/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `lada/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `lada/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `lada/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `lada/plugin-sdk/setup` seam only when you also need the
...
- `lada/plugin-sdk/account-core`,
  `lada/plugin-sdk/account-id`,
  `lada/plugin-sdk/account-resolution`, and
  `lada/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `lada/plugin-sdk/inbound-envelope` and
  `lada/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `lada/plugin-sdk/messaging-targets` for target parsing/matching
- `lada/plugin-sdk/outbound-media` and
  `lada/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `lada/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `lada/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `lada/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `lada/plugin-sdk/channel-inbound` for the shared layer.
...
`lada/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#ladachannel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/lada-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "lada": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect LADA to Acme Chat."
        }
      }
    }
    ```

    ```json lada.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { LADAConfig } from "lada/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "lada/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so LADA
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "lada/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    LADA loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    LADA. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to LADA.
...
├── package.json              # lada.channel metadata
├── lada.plugin.json      # Manifest with config schema
