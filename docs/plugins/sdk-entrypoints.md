...
**Import:** `lada/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `LADAPluginConfigSchema \| () => LADAPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: LADAPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `lada.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- LADA resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `lada/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: LADAPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: LADAPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and LADA
...
LADA loads this instead of the full entry when a channel is disabled,
...
- `lada/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `lada/plugin-sdk/channel-setup` for optional-install setup surfaces
- `lada/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
- use `descriptors` when the registrar owns one or more root commands and you
  want LADA to lazy-load the real CLI module on first invocation
...
LADA classifies loaded plugins by their registration behavior:
...
Use `lada plugins inspect <id>` to see a plugin's shape.
