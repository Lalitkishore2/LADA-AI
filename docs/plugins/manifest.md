...
read_when:
  - You are building an LADA plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (lada.plugin.json)

This page is for the **native LADA plugin manifest** only.
...
- LADA bundle: `.lada-plugin/plugin.json` or the default LADA component
  layout without a manifest
- Cursor bundle: `.cursor-plugin/plugin.json`

LADA auto-detects those bundle layouts too, but they are not validated
against the `lada.plugin.json` schema described here.

For compatible bundles, LADA currently reads bundle metadata plus declared
skill roots, LADA command roots, LADA bundle `settings.json` defaults,
LADA bundle LSP defaults, and supported hook packs when the layout matches
LADA runtime expectations.

Every native LADA plugin **must** ship a `lada.plugin.json` file in the
**plugin root**. LADA uses this manifest to validate configuration
...
`lada.plugin.json` is the metadata LADA reads before it loads your
plugin code.
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that LADA can inspect without loading plugin code.                                                                                                                      |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that LADA can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
LADA reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, LADA falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that LADA can
...
Use `modelSupport` when LADA should infer your provider plugin from
shorthand model ids like `gpt-5.4` or `lada-sonnet-4.6` before plugin runtime
loads.
...
Legacy top-level capability keys are deprecated. Use `lada doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `lada.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `lada` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if LADA must know it before loading plugin code, put it in `lada.plugin.json`
...
`lada` block instead of `lada.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `lada.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `lada.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `lada.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `lada.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `lada.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `lada.install.npmSpec` / `lada.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `lada.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `lada.install.minHostVersion`                                 | Minimum supported LADA host version, using a semver floor like `>=2026.3.22`.                                                            |
| `lada.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `lada.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |

`lada.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`lada.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`lada.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`lada.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
- The manifest is **required for native LADA plugins**, including local filesystem loads.
