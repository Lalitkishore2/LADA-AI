...
read_when:
  ...
  - You want a reference for all registration methods on LADAPluginApi
...
prefer `lada/plugin-sdk/channel-core`; keep `lada/plugin-sdk/core` for
...
`lada/plugin-sdk/slack`, `lada/plugin-sdk/discord`,
`lada/plugin-sdk/signal`, `lada/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `LADASchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `lada.json` Zod schema export (`LADASchema`) |
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-usage` | `fetchLADAUsage` and similar |
...
The `register(api)` callback receives an `LADAPluginApi` object with these
methods:
...
- User config still wins. LADA merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `lada/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `LADAConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `lada/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when LADA is already running. If no runtime
...
subpath yet. Current bundled example: the Anthropic provider keeps its LADA
...
- `@lada/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `lada/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `lada/plugin-sdk/speech`, `.../provider-model-shared`, or another
