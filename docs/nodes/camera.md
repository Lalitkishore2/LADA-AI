...
LADA supports **camera capture** for agent workflows:
...
```bash
lada nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
lada nodes camera snap --node <id> --facing front
lada nodes camera clip --node <id> --duration 3000
lada nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`lada.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `lada` CLI to invoke camera commands on the macOS node.

Examples:

```bash
lada nodes camera list --node <id>            # list camera ids
lada nodes camera snap --node <id>            # prints MEDIA:<path>
lada nodes camera snap --node <id> --max-width 1280
lada nodes camera snap --node <id> --delay-ms 2000
lada nodes camera snap --node <id> --device-id <id>
lada nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
lada nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
lada nodes camera clip --node <id> --device-id <id>
lada nodes camera clip --node <id> --no-audio
```

Notes:

- `lada nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
lada nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
