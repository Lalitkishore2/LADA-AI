...
```bash
lada nodes describe --node <idOrNameOrIp>
lada nodes canvas snapshot --node <idOrNameOrIp>
lada logs --follow
```
...
lada nodes status
lada approvals get --node <idOrNameOrIp>
lada approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`lada approvals get --node ...`), not in the gateway pairing record.
