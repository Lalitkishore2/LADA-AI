...
```bash
lada tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `lada tui --deliver`
...
- Local shell commands receive `LADA_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `LADA_THEME=light` before launching `lada tui`.
- To force the original dark palette instead, set `LADA_THEME=dark`.
...
- Check the Gateway logs: `lada logs --follow`.
- Confirm the agent can run: `lada status` and `lada models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `lada agents list` and your routing config.
