...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/lada`)
...
If the page fails to load, start the Gateway first: `lada gateway`.
...
created. Re-run `lada devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `lada devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.lada/lada.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop lada`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. LADA
...
```bash
lada gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
LADA **blocks** Control UI connections without device identity.
...
```bash
LADA_CONTROL_UI_BASE_PATH=/lada/ pnpm ui:build
```
