...
- On completion, LADA best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.
- For manual spawns, delivery is resilient:
  - LADA tries direct `agent` delivery first with a stable idempotency key.
...
- Completion delivery keeps the resolved requester route:
  ...
  - if the completion origin only provides a channel, LADA fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works
...
- For ACP harness sessions (Codex, LADA, Gemini CLI), use `sessions_spawn` with `runtime: "acp"` and see [ACP Agents](/tools/acp-agents).
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, LADA uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. LADA creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, LADA falls back to that session's requester when available
