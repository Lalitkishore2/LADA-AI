---
summary: "ClawHub guide: public registry, native LADA install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **LADA skills and plugins**.

- Use native `lada` commands to search/install/update skills and install
...
```bash
lada plugins install clawhub:<package>
...
Native `lada` commands install into your active workspace and persist source
...
`lada plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, LADA stops and points you at
`lada skills install <slug>` instead.

## What ClawHub is

- A public registry for LADA skills and plugins.
...
4. Users browse, download, and install skills in LADA.
...
If you want to add new capabilities to your LADA agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `lada skills install <skill-slug>`
3. Start a new LADA session so it picks up the new skill.
...
## How it fits into LADA

Native `lada skills install` installs into the active workspace `skills/`
directory. `lada plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but LADA warns
...
current working directory. If an LADA workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). LADA loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.lada/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches LADA how to perform a
...
Interested in becoming a moderator? Ask in the LADA Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to LADA workspace).
...
Code plugins must include the required LADA metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/lada/skills` and `~/.lada/skills`). This is designed to find older skill installs without extra flags.
