...
LADA can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere LADA can send audio.
...
When `messages.tts.provider` is unset, LADA picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `lada.json`.
...
- If `provider` is **unset**, LADA uses the first configured speech provider in registry auto-select order.
...
`~/.lada/settings/tts.json`, override with `LADA_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, LADA retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so LADA registers
