...
- Levels (aliases): `off | minimal | low | medium | high | xhigh | adaptive`
  ...
  - adaptive → provider-managed adaptive reasoning budget (supported for Anthropic LADA 4.6 model family)
  - `x-high`, `x_high`, `extra-high`, `extra high`, and `extra_high` map to `xhigh`.
  - `highest`, `max` map to `high`.
- Provider notes:
  - Anthropic LADA 4.6 models default to `adaptive` when no explicit thinking level is set.
  ...
  - Moonshot (`moonshot/*`) maps `/think off` to `thinking: { type: "disabled" }` and any non-`off` level to `thinking: { type: "enabled" }`. When thinking is enabled, Moonshot only accepts `tool_choice` `auto|none`; LADA normalizes incompatible values to `auto`.
...
5. Fallback: `adaptive` for Anthropic LADA 4.6 models, `low` for other reasoning-capable models, `off` otherwise.
...
- LADA resolves fast mode in this order:
...
- For `openai-codex/*`, fast mode sends the same `service_tier=priority` flag on Codex Responses. LADA keeps one shared `/fast` toggle across both auth paths.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. LADA still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
...
- The first option is always `Default (<resolved level>)`, where the resolved default comes from the active session model: `adaptive` for LADA 4.6 on Anthropic/Bedrock, `low` for other reasoning-capable models, `off` otherwise.
