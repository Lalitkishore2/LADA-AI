---
summary: "Use ACP runtime sessions for Codex, LADA, Cursor, Gemini CLI, LADA ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let LADA run external coding harnesses (for example Pi, LADA, Codex, Cursor, Copilot, LADA ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask LADA in plain language to "run this in Codex" or "start LADA in a thread", LADA should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or LADA to connect as an external MCP client directly
to existing LADA channel conversations, use [`lada mcp serve`](/cli/mcp)
instead of ACP.
...
| ---------------------------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Run Codex, LADA, Gemini CLI, or another external harness _through_ LADA | This page: ACP agents                 | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls |
| Expose an LADA Gateway session _as_ an ACP server for an editor or client      | [`lada acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to LADA over stdio/WebSocket                                          |
| Reuse a local AI CLI as a text-only fallback model                                 | [CLI Backends](/gateway/cli-backends) | Not ACP. No LADA tools, no ACP controls, no harness runtime                                             |
...
- On startup, LADA probes that binary and self-repairs it if needed.
...
- `/acp spawn codex`: LADA should be ready to bootstrap `acpx`, but the Codex ACP adapter may still need a first-run fetch.
- `/acp spawn lada`: same story for the LADA ACP adapter, plus LADA-side auth on that host.
...
- "Run this as a one-shot LADA ACP session and summarize the result."
...
Use ACP when you want an external harness runtime. Use sub-agents when you want LADA-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | LADA native sub-agent runtime  |
...
For LADA through ACP, the stack is:

1. LADA ACP session control plane
2. bundled `acpx` runtime plugin
3. LADA ACP adapter
4. LADA-side runtime/session machinery

Important distinction:

- ACP LADA is a harness session with ACP controls, session resume, background-task tracking, and optional conversation/thread binding.
...
- LADA keeps owning the channel transport, auth, safety, and delivery.
...
- If you spawn to a different ACP agent and do not pass `--cwd`, LADA inherits the **target agent's** workspace by default, not the requester's.
- If that inherited workspace path is missing (`ENOENT`/`ENOTDIR`), LADA falls back to the backend default cwd instead of silently reusing the wrong tree.
...
- ACP session: the durable Codex/LADA/Gemini runtime state LADA routes to
...
- `/acp spawn codex --thread auto`: LADA may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared LADA binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when LADA needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, LADA returns a clear unsupported message.
...
- LADA binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, LADA returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning LADA agent id.
...
- `agents.list[].runtime.acp.agent` (harness id, for example `codex` or `lada`)
  ...
  agents: {
    list: [
      ...
      },
      {
        ...
        runtime: {
          type: "acp",
          acp: { agent: "lada", backend: "acpx", mode: "persistent" },
        },
      },
    ],
  },
...
- LADA ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, LADA inherits the target agent workspace from agent config.
...
- If `agentId` is omitted, LADA uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, LADA may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new LADA session you are creating, so `mode: "session"` still requires `thread: true`.
- The target agent must support `session/load` (Codex and LADA do).
...
ACP sessions currently run on the host runtime, not inside the LADA sandbox.
...
If no target resolves, LADA returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, LADA returns a clear unsupported-control error.
...
| `/acp model`         | Set runtime model override.                               | `/acp model anthropic/lada-opus-4-6`                        |
...
When LADA uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal LADA `agentId` path).
...
```bash
lada plugins install acpx
lada config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
lada plugins install ./path/to/local/acpx-plugin
```
...
- Relative paths resolve from LADA workspace directory.
...
- LADA startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install LADA globally with `npm install -g lada`, the acpx
...
normally and reports the missing dependency through `lada acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose LADA plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or LADA to call installed
LADA plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
lada config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `lada-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled LADA
  plugins.
...
These ACPX harness permissions are separate from LADA exec approvals and separate from CLI-backend vendor bypass flags such as LADA CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
lada config set plugins.entries.acpx.config.permissionMode approve-all
lada config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** LADA currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
