...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `lada sandbox explain`.
...
Legacy `agent.*` configs are migrated by `lada doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=lada-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${LADA_STATE_DIR:-$HOME/.lada}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
