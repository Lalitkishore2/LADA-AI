...
read_when:
  ...
  - Debugging why lada is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (LADA-managed)"
---

# Browser (lada-managed)

LADA can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `lada` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **lada** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`lada`, `work`, `remote`, ...).
...
```bash
lada browser --browser-profile lada status
...
If `lada browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`lada browser`),
...
If `lada browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "lada"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.lada/lada.json`.
...
- If you override the Gateway port (`gateway.port` or `LADA_GATEWAY_PORT`),
...
- Default profile is `lada` (LADA-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `lada` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
LADA uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
lada config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, LADA will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `lada browser stop` stops the browser process that
  LADA launched
- attach-only and remote CDP profiles: `lada browser stop` closes the active
  ...
  though no browser process was launched by LADA
...
LADA preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, LADA can
...
- If you set `nodeHost.browserProxy.allowProfiles`, LADA treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. LADA can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let LADA
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). LADA supports both:

- **HTTP(S) endpoints** — LADA calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — LADA connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-lada-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, LADA
  auto-generates `gateway.auth.token` on startup and persists it to config.
- LADA does **not** auto-generate that token when `gateway.auth.mode` is
...
LADA supports multiple named profiles (routing configs). Profiles can be:

- **lada-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `lada` profile is auto-created if missing.
...
LADA can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when LADA attaches.
...
```bash
lada browser --browser-profile user start
lada browser --browser-profile user status
lada browser --browser-profile user tabs
lada browser --browser-profile user snapshot --format ai
```
...
- `lada doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `lada` profile because it can
  act inside your signed-in browser session.
- LADA does not launch the browser for this driver; it attaches to an
  existing session only.
- LADA uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, LADA passes it through to target that explicit
...
When launching locally, LADA picks the first available:
...
- `x-lada-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `lada` browser when a per-tab CDP
...
```bash
docker compose run --rm lada-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`LADA_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `lada browser snapshot --interactive --compact --depth 6`
- `lada browser snapshot --efficient`
- `lada browser snapshot --labels`
- `lada browser snapshot --selector "#main" --interactive`
- `lada browser snapshot --frame "iframe#main" --interactive`
- `lada browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `lada browser stop` is still the
...
- `lada browser requests --filter api --clear`
- `lada browser pdf`
- `lada browser responsebody "**/api" --max-chars 5000`
...
- `lada browser storage local set theme dark`
- `lada browser storage session clear`
- `lada browser set offline on`
- `lada browser set headers --headers-json '{"X-Debug":"1"}'`
- `lada browser set credentials user pass`
- `lada browser set credentials --clear`
- `lada browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `lada browser set geo --clear`
- `lada browser set media dark`
- `lada browser set timezone America/New_York`
- `lada browser set locale en-US`
...
- Download and trace output paths are constrained to LADA temp roots:
  - traces: `/tmp/lada` (fallback: `${os.tmpdir()}/lada`)
  - downloads: `/tmp/lada/downloads` (fallback: `${os.tmpdir()}/lada/downloads`)
- Upload paths are constrained to an LADA temp uploads root:
  - uploads: `/tmp/lada/uploads` (fallback: `${os.tmpdir()}/lada/uploads`)
...
LADA supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `lada browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `lada browser click 12`, `lada browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `lada browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `lada browser click e12`, `lada browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `lada browser wait --url "**/dash"`
- Wait for load state:
  - `lada browser wait --load networkidle`
- Wait for a JS predicate:
  - `lada browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `lada browser wait "#main"`
...
1. `lada browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `lada browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `lada browser trace start`
   - reproduce the issue
   - `lada browser trace stop` (prints `TRACE:<path>`)
...
```bash
lada browser status --json
lada browser snapshot --interactive --json
lada browser requests --filter api --json
lada browser cookies --json
```
...
- The lada browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `lada browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (lada, chrome, or remote CDP).
