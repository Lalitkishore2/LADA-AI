...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.lada/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so LADA picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    lada agent --message "give me a greeting"
    ```
...
| `metadata.lada.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.lada.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.lada.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `lada agent --message "..."` to test before sharing
...
| `~/.lada/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with LADA) | Low        | Global                |
