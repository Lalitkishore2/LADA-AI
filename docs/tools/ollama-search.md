...
LADA supports **Ollama Web Search** as a bundled `web_search` provider.
...
- an Ollama host that is reachable from LADA
- `ollama signin`
...
If no explicit Ollama base URL is set, LADA uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, LADA reuses
...
- If the Ollama host is auth-protected, LADA reuses the normal Ollama
  provider API key when present.
- LADA warns during setup if Ollama is unreachable or not signed in, but
