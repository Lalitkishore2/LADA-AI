...
LADA supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), LADA reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.lada/.env`.

If you omit `baseUrl`, LADA defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, LADA defaults to `kimi-k2.5`.
