---
summary: "Install, configure, and manage LADA plugins"
read_when:
  - Installing or configuring plugins
  - Understanding plugin discovery and load rules
  - Working with Codex/LADA-compatible plugin bundles
title: "Plugins"
...
Plugins extend LADA with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with LADA), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    lada plugins install @lada/voice-call

    # From a local directory or archive
    lada plugins install ./my-plugin
    lada plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@lada/voice-call
...
`lada doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`lada.install.allowInvalidConfigRecovery`.

## Plugin types

LADA recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `lada.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/LADA/Cursor-compatible layout; mapped to LADA features | `.codex-plugin/`, `.lada-plugin/`, `.cursor-plugin/` |

Both show up under `lada plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@lada/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@lada/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@lada/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@lada/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@lada/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@lada/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with LADA)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `lada browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `lada gateway` path), that
...
LADA scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.lada/<plugin-root>/*.ts` and `\<workspace\>/.lada/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.lada/<plugin-root>/*.ts` and `~/.lada/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with LADA. Many are enabled by default (model providers, speech).
...
```bash
lada plugins list                       # compact inventory
lada plugins list --enabled            # only loaded plugins
lada plugins list --verbose            # per-plugin detail lines
lada plugins list --json               # machine-readable inventory
lada plugins inspect <id>              # deep detail
lada plugins inspect <id> --json       # machine-readable
lada plugins inspect --all             # fleet-wide table
lada plugins info <id>                 # inspect alias
lada plugins doctor                    # diagnostics

lada plugins install <package>         # install (ClawHub first, then npm)
lada plugins install clawhub:<pkg>     # install from ClawHub only
lada plugins install <spec> --force    # overwrite existing install
lada plugins install <path>            # install from local path
lada plugins install -l <path>         # link (no copy) for dev
lada plugins install <plugin> --marketplace <source>
lada plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
lada plugins install <spec> --pin      # record exact resolved npm spec
lada plugins install <spec> --dangerously-force-unsafe-install
lada plugins update <id>             # update one plugin
lada plugins update <id> --dangerously-force-unsafe-install
lada plugins update --all            # update all
lada plugins uninstall <id>          # remove config/install records
...
Bundled plugins ship with LADA. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `lada plugins enable <id>`.
...
override instead, while `lada skills install` remains the separate ClawHub
skill download/install flow.

Compatible bundles participate in the same plugin list/inspect/enable/disable
flow. Current runtime support includes bundle skills, LADA command-skills,
LADA `settings.json` defaults, LADA `.lsp.json` and manifest-declared
`lspServers` defaults, Cursor command-skills, and compatible Codex hook
directories.

`lada plugins inspect <id>` also reports detected bundle capabilities plus
supported or unsupported MCP and LSP server entries for bundle-backed plugins.

Marketplace sources can be a LADA known-marketplace name from
`~/.lada/plugins/known_marketplaces.json`, a local marketplace root or
...
See [`lada plugins` CLI reference](/cli/plugins) for full details.
...
LADA loads the entry object and calls `register(api)` during plugin
...
- [Plugin Bundles](/plugins/bundles) — Codex/LADA/Cursor bundle compatibility
