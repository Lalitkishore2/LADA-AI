...
`ask: "always"` in `~/.lada/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `lada approvals get`, `lada approvals get --gateway`, or
`lada approvals get --node <id|name|ip>` to inspect the requested policy,
...
- **gateway host** → `lada` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, LADA also tries to bind
...
- requested exec policy in LADA config (`tools.exec.*`)
- host-local approvals policy in `~/.lada/exec-approvals.json`
...
- In YOLO mode, LADA does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
lada config set tools.exec.host gateway
lada config set tools.exec.security full
lada config set tools.exec.ask off
...
```bash
lada approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
When `tools.exec.strictInlineEval=true`, LADA treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.lada/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `lada approvals allowlist ...`).
- `lada security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `lada doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, LADA still rejects the `env` builtin in safe-bin
...
`~/.lada/exec-approvals.json` directly.

CLI: `lada approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, LADA sends a followup `agent` turn to the same session.
...
topics, LADA preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, LADA prevents the agent from reusing
