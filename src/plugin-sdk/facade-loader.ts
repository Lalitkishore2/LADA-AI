...
let facadeLoaderJitiFactory:
  ...
  | undefined;
let cachedLADAPackageRoot: string | undefined;
...
function getLADAPackageRoot() {
  if (cachedLADAPackageRoot) {
    return cachedLADAPackageRoot;
  }
  cachedLADAPackageRoot =
    resolveLoaderPackageRoot({
...
function resolveSourceFirstPublicSurfacePath(params: {
  ...
  const sourceRoot =
    params.bundledPluginsDir ?? path.resolve(getLADAPackageRoot(), "extensions");
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      params.location.boundaryRoot === getLADAPackageRoot()
        ? "LADA package root"
        : (() => {
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedLADAPackageRoot = undefined;
}
