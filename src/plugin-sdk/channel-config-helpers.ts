...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`lada pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`lada pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends LADAConfig = LADAConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends LADAConfig = LADAConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends LADAConfig = LADAConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: LADAConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends LADAConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends LADAConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends LADAConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
