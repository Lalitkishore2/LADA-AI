...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getMusicGenerationProvider: vi.fn<
      (providerId: string, config?: LADAConfig) => MusicGenerationProvider | undefined
    >(() => undefined),
    ...
    listMusicGenerationProviders: vi.fn<(config?: LADAConfig) => MusicGenerationProvider[]>(
      () => [],
    ),
...
describe("music-generation runtime", () => {
  ...
  it("lists runtime music-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeMusicGenerationProviders({ config: {} as LADAConfig })).toEqual(
      providers,
    );
    expect(mocks.listMusicGenerationProviders).toHaveBeenCalledWith({} as LADAConfig);
  });
