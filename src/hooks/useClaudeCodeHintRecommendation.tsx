...
/**
 * Surfaces plugin-install prompts driven by `<lada-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/lada-code-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/ladaCodeHints.js';
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useLADACodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
