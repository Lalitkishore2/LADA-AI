...
This directory contains hooks that ship with LADA. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.lada/workspace`)
...
homepage: https://docs.lada.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.lada/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.lada/lada.json`:
...
2. Restart gateway: `pkill -9 -f 'lada.*gateway' && pnpm lada gateway`
3. Enable the hook: `lada hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.lada.ai/automation/hooks
