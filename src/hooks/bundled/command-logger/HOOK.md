...
homepage: https://docs.lada.ai/automation/hooks#command-logger
metadata:
  {
    "lada":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.lada/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.lada/logs/commands.log ~/.lada/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/lada`:
   ```
   /home/username/.lada/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
