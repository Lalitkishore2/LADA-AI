...
import { getRateLimitWarning, getUsingOverageText } from 'src/services/ladaAiLimits.js';
import { useLADAAiLimits } from 'src/services/ladaAiLimitsHook.js';
import { getSubscriptionType } from 'src/utils/auth.js';
import { hasLADAAiBillingAccess } from 'src/utils/billing.js';
...
export function useRateLimitWarningNotification(model) {
  ...
  let t0;
  if ($[0] !== ladaAiLimits || $[1] !== model) {
    t0 = getRateLimitWarning(ladaAiLimits, model);
    $[0] = ladaAiLimits;
    $[1] = model;
    $[2] = t0;
  } else {
  ...
  let t1;
  if ($[3] !== ladaAiLimits) {
    t1 = getUsingOverageText(ladaAiLimits);
    $[3] = ladaAiLimits;
    $[4] = t1;
  } else {
    t1 = $[4];
  ...
  let t4;
  let t5;
  if ($[7] !== addNotification || $[8] !== ladaAiLimits.isUsingOverage || $[9] !== hasShownOverageNotification || $[10] !== usingOverageText) {
    t4 = () => {
      if (getIsRemoteMode()) {
        return;
      }
      if (ladaAiLimits.isUsingOverage && !hasShownOverageNotification && (!isTeamOrEnterprise || hasBillingAccess)) {
        addNotification({
          key: "limit-reached",
        ...
        if (!ladaAiLimits.isUsingOverage && hasShownOverageNotification) {
          setHasShownOverageNotification(false);
        }
      }
    };
    t5 = [ladaAiLimits.isUsingOverage, usingOverageText, hasShownOverageNotification, addNotification, hasBillingAccess, isTeamOrEnterprise];
    $[7] = addNotification;
    $[8] = ladaAiLimits.isUsingOverage;
