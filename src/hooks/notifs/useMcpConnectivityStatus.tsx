...
import { hasLADAAiMcpEverConnected } from '../../services/mcp/ladaai.js';
...
export function useMcpConnectivityStatus(t0) {
  ...
  if ($[0] !== addNotification || $[1] !== mcpClients) {
    t2 = () => {
      ...
      const failedLADAAiClients = mcpClients.filter(_temp2);
      const needsAuthLocalServers = mcpClients.filter(_temp3);
      const needsAuthLADAAiServers = mcpClients.filter(_temp4);
      if (failedLocalClients.length === 0 && failedLADAAiClients.length === 0 && needsAuthLocalServers.length === 0 && needsAuthLADAAiServers.length === 0) {
        return;
      }
      ...
      if (failedLADAAiClients.length > 0) {
        addNotification({
          key: "mcp-ladaai-failed",
          jsx: <><Text color="error">{failedLADAAiClients.length} lada.ai{" "}{failedLADAAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
      ...
      if (needsAuthLADAAiServers.length > 0) {
        addNotification({
          key: "mcp-ladaai-needs-auth",
          jsx: <><Text color="warning">{needsAuthLADAAiServers.length} lada.ai{" "}{needsAuthLADAAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
          priority: "medium"
        });
      }
    };
...
function _temp4(client_2) {
  return client_2.type === "needs-auth" && client_2.config.type === "ladaai-proxy" && hasLADAAiMcpEverConnected(client_2.name);
}
function _temp3(client_1) {
  return client_1.type === "needs-auth" && client_1.config.type !== "ladaai-proxy";
}
function _temp2(client_0) {
  return client_0.type === "failed" && client_0.config.type === "ladaai-proxy" && hasLADAAiMcpEverConnected(client_0.name);
}
function _temp(client) {
  return client.type === "failed" && client.config.type !== "sse-ide" && client.config.type !== "ws-ide" && client.config.type !== "ladaai-proxy";
}
