...
async function _temp2() {
  ...
  const subscriptionType = await getExistingLADASubscription();
  ...
  return {
    ...
    jsx: <Text color="suggestion">Use your existing LADA {subscriptionType} plan with LADA<Text color="text" dimColor={true}>{" "}· /login to activate</Text></Text>,
    priority: "low"
  };
}
...
async function getExistingLADASubscription(): Promise<'Max' | 'Pro' | null> {
  ...
  if (profile.account.has_lada_max) {
    return 'Max';
  }
  if (profile.account.has_lada_pro) {
    return 'Pro';
  }
  return null;
}
