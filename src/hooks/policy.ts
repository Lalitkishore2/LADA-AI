import type { LADAConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "lada-bundled": {
    ...
    canOverride: ["lada-bundled"],
    canBeOverriddenBy: ["lada-managed", "lada-plugin"],
  },
  "lada-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["lada-bundled", "lada-plugin"],
    canBeOverriddenBy: ["lada-managed"],
  },
  "lada-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["lada-bundled", "lada-managed", "lada-plugin"],
    canBeOverriddenBy: ["lada-managed"],
  },
  "lada-workspace": {
    precedence: 40,
    ...
    canOverride: ["lada-workspace"],
    canBeOverriddenBy: ["lada-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "lada-plugin") {
    return { enabled: true };
  }
