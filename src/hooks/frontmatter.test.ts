...
`;
    ...
    expect(parsed.lada.emoji).toBe("💾");
    expect(parsed.lada.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.lada.emoji).toBe("📝");
    expect(parsed.lada.events).toEqual(["command"]);
    expect(parsed.lada.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.lada.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"lada": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"lada": {"events": ["test"]}}');
  });
...
describe("resolveLADAMetadata", () => {
  it("extracts lada metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveLADAMetadata(frontmatter);
  ...
  it("returns undefined when lada key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        lada: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with LADA" },
            { id: "npm", kind: "npm", package: "@lada/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@lada/hook");
  });
    ...
    "lada":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with LADA" }],
      },
  }
---
...
`;
    ...
    const lada = resolveLADAMetadata(frontmatter);
    expect(lada).toBeDefined();
    expect(lada?.emoji).toBe("💾");
    expect(lada?.events).toEqual(["command:new", "command:reset"]);
    expect(lada?.requires?.config).toEqual(["workspace.dir"]);
    expect(lada?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const lada = resolveLADAMetadata(frontmatter);
    expect(lada?.emoji).toBe("disk");
    expect(lada?.events).toEqual(["command:new"]);
  });
});
