...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\LADA\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/lada/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/lada/lada.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/lada/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/lada/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the lada executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\lada.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/lada-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/lada-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\lada-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/lada/custom.js", "gateway"])).toBe(false);
