...
vi.mock("./lada-root.js", async () => {
  const actual = await vi.importActual<typeof import("./lada-root.js")>("./lada-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "lada-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["LADA_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.LADA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveLADAPackageRoot } = await import("./lada-root.js"));
    ...
    vi.mocked(resolveLADAPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveLADAPackageRoot).mockResolvedValue("/opt/lada");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/lada/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
