...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open LADA to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "lada.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.LADA_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.LADA_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set LADA_APNS_TEAM_ID and LADA_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.LADA_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.LADA_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.LADA_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set LADA_APNS_PRIVATE_KEY_P8 or LADA_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading LADA_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
