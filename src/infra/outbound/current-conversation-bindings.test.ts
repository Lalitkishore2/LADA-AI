...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.LADA_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-current-bindings-"));
    process.env.LADA_STATE_DIR = testStateDir;
