import { mapAllowFromEntries } from "lada/plugin-sdk/channel-config-helpers";
...
function buildWebChatDeliveryError(): Error {
  return new Error(
    `Delivering to WebChat is not supported via \`${formatCliCommand("lada agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
  );
}
