...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: LADAConfig | null = null;
  ...
  get(key: string, cfg: LADAConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: LADAConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: LADAConfig): void {
