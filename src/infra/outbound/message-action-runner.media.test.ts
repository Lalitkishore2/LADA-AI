...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred LADA tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideLADA = path.join(os.tmpdir(), "outside-lada", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
