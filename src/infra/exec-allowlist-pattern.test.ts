...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevLADAHome = process.env.LADA_HOME;
    const prevHome = process.env.HOME;
    process.env.LADA_HOME = "/srv/lada-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/lada-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevLADAHome === undefined) {
        delete process.env.LADA_HOME;
      } else {
        process.env.LADA_HOME = prevLADAHome;
      }
