...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "lada.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (LADA)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "lada.internal", expected: "lada.internal." },
    { value: "lada.internal.", expected: "lada.internal." },
    { value: "  lada.internal  ", expected: "lada.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { LADA_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { LADA_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("lada.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "lada.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/lada",
    });

    expectZoneRecords(txt, [
      `$ORIGIN lada.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_lada-gw._tcp IN PTR studio-london._lada-gw._tcp`,
      `studio-london._lada-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (LADA)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/lada`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN lada.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._lada-gw._tcp IN TXT`,
        `displayName=Mac Studio (LADA)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("lada.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("lada.internal."),
