...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.LADA_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for LADA. You can keep running the web gateway without it: \`${formatCliCommand("lada gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
