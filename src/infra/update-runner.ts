...
const DEFAULT_PACKAGE_NAME = "lada";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["lada", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lada-update-preflight-"));
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "lada.mjs");
      ...
      const doctorStep = await runStep(
        step("lada doctor", doctorArgv, gitRoot, { LADA_UPDATE_IN_PROGRESS: "1" }),
      );
