...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers LADA_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for LADA_HOME",
      env: { LADA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when LADA_HOME is tilde-only and no fallback home exists",
      env: { LADA_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores LADA_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          LADA_HOME: "/srv/lada-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { LADA_HOME: "/srv/lada-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/lada-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/lada-home " },
      expected: "/srv/lada-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/lada-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of LADA_HOME", () => {
    expect(
