...
export function triggerLADARestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.LADA_SYSTEMD_UNIT,
      process.env.LADA_PROFILE,
    );
  ...
  const label =
    process.env.LADA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LADA_PROFILE);
