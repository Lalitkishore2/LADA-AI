...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: LADAConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: LADAConfig) {
...
function resolveHeartbeatAgents(cfg: LADAConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: LADAConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: LADAConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: LADAConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
