...
function expectSingleAnthropicProvider(summary: UsageSummary) {
  ...
  const lada = summary.providers[0];
  expect(lada?.provider).toBe("anthropic");
  return lada;
}
...
describe("provider usage loading", () => {
  it("loads usage snapshots with injected auth", async () => {
    ...
    const lada = summary.providers.find((p) => p.provider === "anthropic");
    const minimax = summary.providers.find((p) => p.provider === "minimax");
    const zai = summary.providers.find((p) => p.provider === "zai");
    expect(lada?.windows[0]?.label).toBe("5h");
  ...
  it("discovers LADA usage from token auth profiles", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        const lada = expectSingleAnthropicProvider(summary);
        expect(lada?.windows[0]?.label).toBe("5h");
        expect(mockFetch).toHaveBeenCalled();
      },
      {
        env: {
          LADA_STATE_DIR: (home) => path.join(home, ".lada"),
        },
        prefix: "lada-provider-usage-",
      },
    );
  });

  it("falls back to lada.ai web usage when OAuth scope is missing", async () => {
    await withEnvAsync({ CLAUDE_AI_SESSION_KEY: "sk-ant-web-1" }, async () => {
      const mockFetch = createProviderUsageFetch(async (url) => {
        ...
        if (url.includes("lada.ai/api/organizations/org-1/usage")) {
          return makeResponse(200, {
            five_hour: { utilization: 20, resets_at: "2026-01-07T01:00:00Z" },
        ...
        if (url.includes("lada.ai/api/organizations")) {
      ...
      const lada = expectSingleAnthropicProvider(summary);
      expect(lada?.windows.some((w) => w.label === "5h")).toBe(true);
      expect(lada?.windows.some((w) => w.label === "Week")).toBe(true);
    });
  });
});
