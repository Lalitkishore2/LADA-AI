...
export const POSIX_LADA_TMP_DIR = "/tmp/lada";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredLADATmpDirOptions = {
...
export function resolvePreferredLADATmpDir(
  options: ResolvePreferredLADATmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "lada" : `lada-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[lada] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback LADA temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback LADA temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_LADA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_LADA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_LADA_TMP_DIR)) {
      return POSIX_LADA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_LADA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_LADA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_LADA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_LADA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_LADA_TMP_DIR;
