...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __ladaDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__ladaDiagnosticEventsState) {
    globalStore.__ladaDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__ladaDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: LADAConfig): boolean {
