...
const MATRIX_REPO_INSTALL_COMMAND = `lada plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/lada-matrix-missing",
        installPath: "/tmp/lada-matrix-missing",
      },
    });
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/lada-matrix-missing",
      'Reinstall with "lada plugins install @lada/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/lada-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<lada plugins install @lada/matrix>" when you want to return to the standard Matrix plugin.',
