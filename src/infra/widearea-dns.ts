...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.LADA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*lada-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "lada";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "lada");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "lada-gw");
  ...
  records.push(`_lada-gw._tcp IN PTR ${instanceLabel}._lada-gw._tcp`);
  records.push(`${instanceLabel}._lada-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._lada-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; lada-content-hash: ${contentHash}\n${contentBody}`;
}
