...
function candidateBinDirs(opts: EnsureLADAPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current LADA process is on.
  try {
  ...
  // Bundled macOS app: `lada` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "lada");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.LADA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "lada"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `lada`
...
/**
 * Best-effort PATH bootstrap so skills that require the `lada` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureLADACliOnPath(opts: EnsureLADAPathOpts = {}) {
  if (isTruthyEnvValue(process.env.LADA_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.LADA_PATH_BOOTSTRAPPED = "1";
