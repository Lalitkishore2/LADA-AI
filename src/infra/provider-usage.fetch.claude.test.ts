...
import { fetchLADAUsage } from "./provider-usage.fetch.lada.js";
...
function expectMissingScopeError(result: Awaited<ReturnType<typeof fetchLADAUsage>>) {
...
async function expectMissingScopeWithoutFallback(mockFetch: ScopeFallbackFetch) {
  ...
  // real LADA session variables from other suites.
  ...
  const result = await fetchLADAUsage("token", 5000, mockFetch);
...
describe("fetchLADAUsage", () => {
  ...
  it("falls back to lada web usage when oauth scope is missing", async () => {
