...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LADA_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "LADA";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(LADA\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.LADA_MDNS_HOSTNAME?.trim() || "lada";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
