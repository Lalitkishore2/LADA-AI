...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "lada-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".lada");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".lada", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      LADA_STATE_DIR: path.join(home, ".lada"),
      ...env,
    };
