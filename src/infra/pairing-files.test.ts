...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/lada-state", "devices")).toEqual({
      dir: path.join("/tmp/lada-state", "devices"),
      pendingPath: path.join("/tmp/lada-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/lada-state", "devices", "paired.json"),
    });
  });
