...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__lada_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "lada-root");
...
function setPackageRoot(root: string, name = "lada") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./lada-root.js").resolveLADAPackageRootSync,
  asyncResolver: typeof import("./lada-root.js").resolveLADAPackageRoot,
  opts: Parameters<typeof import("./lada-root.js").resolveLADAPackageRootSync>[0],
...
vi.mock("./lada-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveLADAPackageRoot", () => {
  let resolveLADAPackageRoot: typeof import("./lada-root.js").resolveLADAPackageRoot;
  let resolveLADAPackageRootSync: typeof import("./lada-root.js").resolveLADAPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveLADAPackageRoot, resolveLADAPackageRootSync } =
      await import("./lada-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "lada");
        const pkgRoot = path.join(project, "node_modules", "lada");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "lada");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "lada.mjs")));
    ...
    {
      name: "falls through from a non-lada moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-lada");
    ...
    {
      name: "returns null for non-lada package roots",
      setup: () => {
        const pkgRoot = fx("not-lada");
        setPackageRoot(pkgRoot, "not-lada");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "lada.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "lada");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveLADAPackageRootSync,
      resolveLADAPackageRoot,
      opts,
      expected,
    );
  });
