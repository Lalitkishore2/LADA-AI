...
import { accumulateUsage, updateUsage } from 'src/services/api/lada.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // a memory directory and needs LADA to know how to use it (which
    ...
    for await (const message of query({
      ...
      if (
        ...
        // If the SDK subprocess restarts before then (lada-desktop kills
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. lada.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see lada.ts message_delta
...
/**
 * Sends a single prompt to the LADA API and returns the response.
 * Assumes that lada is being used non-interactively -- will not
