...
async function runReplyToCurrentCase(home: string, text: string) {
  ...
  const res = await getReplyFromConfig(
    ...
    {},
    makeWhatsAppDirectiveConfig(home, { model: "anthropic/lada-opus-4-6" }),
  );
...
async function expectThinkStatusForReasoningModel(params: {
  ...
  const res = await getReplyFromConfig(
    ...
    {},
    makeWhatsAppDirectiveConfig(params.home, { model: "anthropic/lada-opus-4-6" }),
  );
...
describe("directive behavior", () => {
  ...
  it("renders model list and status variants across catalog/config combinations", async () => {
    await withTempHome(async (home) => {
      ...
      expect(unavailableCatalogText).toContain("Current: anthropic/lada-opus-4-6");
      ...
      expect(allowlistedStatusText).toContain("anthropic/lada-opus-4-6");
      expect(allowlistedStatusText).toContain("openai/gpt-4.1-mini");
      expect(allowlistedStatusText).not.toContain("lada-sonnet-4-1");
      expect(allowlistedStatusText).toContain("auth:");

      loadModelCatalogMock.mockResolvedValue([
        { id: "lada-opus-4-6", name: "Opus 4.5", provider: "anthropic" },
  ...
  it("ignores inline /model and /think directives while still running agent content", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.model).toBe("lada-opus-4-6");
      runEmbeddedPiAgentMock.mockClear();
      ...
      const inlineThinkRes = await getReplyFromConfig(
        ...
        {},
        makeWhatsAppDirectiveConfig(home, { model: { primary: "anthropic/lada-opus-4-6" } }),
      );
