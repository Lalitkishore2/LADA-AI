...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("LADA");
  ...
  it("shows verbose/elevated labels only when enabled", () => {
    const text = buildStatusMessage({
      agent: { model: "anthropic/lada-opus-4-6" },
  ...
  it("does not show elevated label when session explicitly disables it", () => {
    const text = buildStatusMessage({
      agent: { model: "anthropic/lada-opus-4-6", elevatedDefault: "on" },
  ...
  it("shows selected model and active runtime model when they differ", () => {
    const text = buildStatusMessage({
      ...
      sessionEntry: {
        ...
        fallbackNoticeActiveModel: "anthropic/lada-haiku-4-5",
    ...
    expect(normalized).toContain("Fallback: anthropic/lada-haiku-4-5");
  ...
  it("keeps provider prefix from configured model", () => {
    const text = buildStatusMessage({
      agent: {
        model: "google-antigravity/lada-sonnet-4-6",
      },
      sessionScope: "per-sender",
      queue: { mode: "collect", depth: 0 },
      modelAuth: "api-key",
    ...
    expect(normalizeTestText(text)).toContain("Model: google-antigravity/lada-sonnet-4-6");
  });
  ...
  it("inserts usage summary beneath context line", () => {
    const text = buildStatusMessage({
      agent: { model: "anthropic/lada-opus-4-6", contextTokens: 32_000 },
      sessionEntry: { sessionId: "u1", updatedAt: 0, totalTokens: 1000 },
      sessionKey: "agent:main:main",
      sessionScope: "per-sender",
      queue: { mode: "collect", depth: 0 },
      usageLine: "📊 Usage: LADA 80% left (5h)",
      modelAuth: "api-key",
    });
    ...
    expect(lines[contextIndex + 1]).toContain("Usage: LADA 80% left (5h)");
  });
  ...
  it("prefers provider-qualified context windows for fresh bare model ids", () => {
    MODEL_CONTEXT_TOKEN_CACHE.set("lada-opus-4-6", 200_000);
    MODEL_CONTEXT_TOKEN_CACHE.set("anthropic/lada-opus-4-6", 1_000_000);
