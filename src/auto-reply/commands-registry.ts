...
export function isCommandEnabled(cfg: LADAConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: LADAConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: LADAConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: LADAConfig) {
