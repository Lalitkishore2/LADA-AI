...
type HomeEnvSnapshot = {
  ...
  LADA_STATE_DIR: string | undefined;
  LADA_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    LADA_AGENT_DIR: process.env.LADA_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".lada", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.LADA_STATE_DIR = path.join(home, ".lada");
    process.env.LADA_AGENT_DIR = path.join(home, ".lada", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".lada", "agent");
