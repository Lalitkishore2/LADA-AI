...
function baseConfig(): LADAConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
function resolveModelSelectionForCommand(params: {
  ...
  return resolveModelSelectionFromDirective({
    ...
    cfg: { commands: { text: true } } as unknown as LADAConfig,
    agentDir: TEST_AGENT_DIR,
...
async function persistModelDirectiveForTest(params: {
  ...
  const persisted = await persistInlineDirectives({
    ...
    model: params.model ?? "lada-opus-4-6",
    initialModelLabel:
      params.initialModelLabel ??
      `${params.provider ?? "anthropic"}/${params.model ?? "lada-opus-4-6"}`,
...
describe("/model chat UX", () => {
  ...
  it("auto-applies closest match for typos", () => {
    ...
    const cfg = { commands: { text: true } } as unknown as LADAConfig;
    ...
    const resolved = resolveModelSelectionFromDirective({
      ...
      defaultModel: "lada-opus-4-6",
      aliasIndex: baseAliasIndex(),
      allowedModelKeys: new Set(["anthropic/lada-opus-4-6"]),
      allowedModelCatalog: [{ provider: "anthropic", id: "lada-opus-4-6" }],
      provider: "anthropic",
    });
  ...
  it("rejects numeric /model selections with a guided error", () => {
    const resolved = resolveModelSelectionForCommand({
      command: "/model 99",
      allowedModelKeys: new Set(["anthropic/lada-opus-4-6", "openai/gpt-4o"]),
      allowedModelCatalog: [],
    });
  ...
  it("treats explicit default /model selection as resettable default", () => {
    const resolved = resolveModelSelectionForCommand({
      command: "/model anthropic/lada-opus-4-6",
      allowedModelKeys: new Set(["anthropic/lada-opus-4-6", "openai/gpt-4o"]),
      allowedModelCatalog: [],
    });
  ...
  it("keeps openrouter provider/model split for exact selections", () => {
    const resolved = resolveModelSelectionForCommand({
      command: "/model openrouter/anthropic/lada-opus-4-6",
      allowedModelKeys: new Set(["openrouter/anthropic/lada-opus-4-6"]),
      allowedModelCatalog: [],
    });
  ...
  it("keeps @YYYYMMDD as part of the model when the stored numeric profile is for another provider", () => {
    ...
    const resolved = resolveModelSelectionForCommand({
      command: `/model custom/vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}`,
      allowedModelKeys: new Set([`custom/vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}`]),
      allowedModelCatalog: [],
    });
    ...
    expect(resolved.modelSelection).toEqual({
      provider: "custom",
      model: `vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}`,
      isDefault: false,
    });
  ...
  it("persists explicit auth profiles after @YYYYMMDD version suffixes in mixed-content messages", async () => {
    const { sessionEntry } = await persistModelDirectiveForTest({
      command: `/model custom/vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}@work hello`,
      profiles: {
        work: {
          type: "api_key",
          provider: "custom",
      ...
      allowedModelKeys: [`custom/vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}`],
    });

    expect(sessionEntry.providerOverride).toBe("custom");
    expect(sessionEntry.modelOverride).toBe(`vertex-ai_lada-haiku-4-5@${OPENAI_DATE_PROFILE_ID}`);
...
describe("handleDirectiveOnly model persist behavior (fixes #1435)", () => {
  const allowedModelKeys = new Set(["anthropic/lada-opus-4-6", "openai/gpt-4o"]);
  const allowedModelCatalog = [
    { provider: "anthropic", id: "lada-opus-4-6", name: "LADA Opus 4.5" },
  ...
  function createHandleParams(overrides: Partial<HandleParams>): HandleParams {
    ...
    return {
      ...
      provider: "anthropic",
      model: "lada-opus-4-6",
      initialModelLabel: "anthropic/lada-opus-4-6",
