...
import { scheduleGatewaySigusr1Restart, triggerLADARestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting LADA in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerLADARestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting LADA via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
