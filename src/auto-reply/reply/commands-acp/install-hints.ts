...
export function resolveAcpInstallCommandHint(cfg: LADAConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (workspaceLocalPath && existsSync(workspaceLocalPath)) {
      return `lada plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^lada plugins install /u, "");
    ...
    return "lada plugins install acpx";
  }
