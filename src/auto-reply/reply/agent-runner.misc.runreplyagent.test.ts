...
vi.mock("../../cron/store.js", () => {
  return {
    ...
    resolveCronStorePath: (storePath?: string) => storePath ?? "/tmp/lada-cron-store.json",
  };
});
  ...
  function createRun(params?: {
    ...
    const followupRun = {
      ...
      run: {
        ...
        config:
          provider === "lada-cli"
            ? { agents: { defaults: { cliBackends: { "lada-cli": {} } } } }
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("surfaces the restart notice when gateway shutdown aborts preflight compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-preflight-restart-"));
  ...
  it("updates totalTokens after auto-compaction using lastCallUsage", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-compact-tokens-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-compact-meta-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("accumulates compactions across fallback attempts without double-counting a single attempt", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-compact-fallback-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
    ...
    runWithModelFallbackMock.mockImplementationOnce(async ({ run }: RunWithModelFallbackParams) => {
      ...
      return {
        ...
        provider: "openai",
        model: "gpt-5.4",
        attempts: [{ provider: "anthropic", model: "lada", error: "attempt failed" }],
      };
    });
  ...
  it("does not count failed compaction end events from earlier fallback attempts", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-compact-fallback-failed-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-usage-last-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("does not enqueue legacy post-compaction audit warnings", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "lada-no-audit-warning-"));
...
describe("runReplyAgent lada-cli routing", () => {
  function createRun() {
    const typing = createMockTypingController();
    const sessionCtx = {
      Provider: "webchat",
    ...
    const followupRun = {
      ...
      run: {
        ...
        sessionFile: "/tmp/session.jsonl",
        workspaceDir: "/tmp",
        config: { agents: { defaults: { cliBackends: { "lada-cli": {} } } } },
        skillsSnapshot: {},
        provider: "lada-cli",
        model: "opus-4.5",
        thinkLevel: "low",
    ...
    return runReplyAgent({
      ...
      typing,
      sessionCtx,
      defaultModel: "lada-cli/opus-4.5",
      resolvedVerboseLevel: "off",
      isNewSession: false,
      blockStreamingEnabled: false,
      resolvedBlockStreamingBreak: "message_end",
  ...
  it("uses the CLI runner for lada-cli provider", async () => {
...
describe("runReplyAgent messaging tool suppression", () => {
  ...
  it("persists usage fields even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "lada-session-store-")),
      "sessions.json",
    );
    const sessionKey = "main";
    ...
    expect(store[sessionKey]?.model).toBe("lada-opus-4-6");
  });
