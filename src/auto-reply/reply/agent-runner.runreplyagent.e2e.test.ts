...
function createMinimalRun(params?: {
  ...
  const followupRun = {
    ...
    run: {
      ...
      skipProviderRuntimeHints: process.env.LADA_TEST_FAST === "1",
      ...params?.runOverrides,
    },
  ...
  it("updates fallback reason summary while fallback stays active", async () => {
    ...
    for (const testCase of cases) {
      const sessionEntry: SessionEntry = {
        sessionId: "session",
        updatedAt: Date.now(),
        fallbackNoticeSelectedModel: "anthropic/lada",
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "lada-memory-flush-"));
  });
