...
describe("runAgentTurnWithFallback", () => {
  beforeEach(() => {
    ...
    state.runWithModelFallbackMock.mockImplementation(async (params: FallbackRunnerParams) => ({
      result: await params.run("anthropic", "lada"),
      provider: "anthropic",
      model: "lada",
      attempts: [],
    }));
  ...
  it("does not show a rate-limit countdown for mixed-cause fallback exhaustion", async () => {
    state.runWithModelFallbackMock.mockRejectedValueOnce(
      Object.assign(
        new Error(
          "All models failed (2): anthropic/lada: 429 (rate_limit) | openai/gpt-5.4: 402 (billing)",
        ),
        {
          name: "FallbackSummaryError",
          attempts: [
            { provider: "anthropic", model: "lada", error: "429", reason: "rate_limit" },
  ...
  it("keeps raw generic errors on internal control surfaces", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("Logs: lada logs --follow");
    }
  });
    ...
    state.runWithModelFallbackMock.mockImplementation(
      async (params: { run: (provider: string, model: string) => Promise<unknown> }) => ({
        result: await params.run(
          ...
          fallbackInvocation === 0 ? "lada" : "gpt-5.4",
        ),
        provider: fallbackInvocation === 0 ? "anthropic" : "openai",
        model: fallbackInvocation++ === 0 ? "lada" : "gpt-5.4",
        attempts: [],
      }),
    );
  ...
  it("drops authProfileId when fallback switches providers", async () => {
    ...
    followupRun.run.model = "lada-opus";
    followupRun.run.authProfileId = "anthropic:lada";
  ...
  it("keeps same-provider auth profile when fallback only changes model", async () => {
    ...
    const entry = {
      sessionId: "session",
      updatedAt: 1,
      authProfileOverride: "anthropic:lada",
