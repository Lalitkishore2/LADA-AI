...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "lada-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop lada!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@lada_bot", { botUsername: "lada_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@lada_bot", { botUsername: "lada_bot" })).toBe(true);
