...
const createLADAToolsMock = vi.fn();
...
async function loadFreshInlineActionsModuleForTest() {
  ...
  vi.doMock("../../agents/lada-tools.runtime.js", () => ({
    createLADATools: (...args: unknown[]) => createLADAToolsMock(...args),
  }));
...
describe("handleInlineActions", () => {
  ...
  it("rewrites LADA bundle markdown commands into a native agent prompt", async () => {
    const typing = createTypingController();
    handleCommandsMock.mockResolvedValue({ shouldContinue: false, reply: { text: "done" } });
    const ctx = buildTestCtx({
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createLADAToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
