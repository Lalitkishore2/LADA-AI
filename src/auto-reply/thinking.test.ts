...
describe("listThinkingLevels", () => {
  ...
  it("always includes adaptive", () => {
    ...
    expect(listThinkingLevels("anthropic", "lada-opus-4-6")).toContain("adaptive");
  });
});
...
describe("resolveThinkingDefaultForModel", () => {
  ...
  it("uses provider-advertised adaptive defaults", () => {
    providerRuntimeMocks.resolveProviderDefaultThinkingLevel.mockImplementation(
      ({ provider, context }) =>
        provider === "anthropic" && context.modelId === "lada-opus-4-6" ? "adaptive" : undefined,
    );

    expect(
      resolveThinkingDefaultForModel({ provider: "anthropic", model: "lada-opus-4-6" }),
    ).toBe("adaptive");
  });
  ...
  it("uses provider-advertised adaptive defaults for Bedrock aliases", () => {
    providerRuntimeMocks.resolveProviderDefaultThinkingLevel.mockImplementation(
      ({ provider, context }) =>
        provider === "amazon-bedrock" && context.modelId === "lada-sonnet-4-6"
          ? "adaptive"
          : undefined,
    );

    expect(
      resolveThinkingDefaultForModel({ provider: "aws-bedrock", model: "lada-sonnet-4-6" }),
    ).toBe("adaptive");
  });
