...
const piEmbeddedMocks = getSharedMocks("lada.trigger-handling.pi-embedded-mocks", () => ({
...
const providerUsageMocks = vi.hoisted(() => ({
  ...
  formatUsageSummaryLine: vi.fn().mockReturnValue("📊 Usage: LADA 80% left"),
  formatUsageWindowSummary: vi.fn().mockReturnValue("LADA 80% left"),
...
const modelCatalogMocks = getSharedMocks("lada.trigger-handling.model-catalog-mocks", () => ({
  loadModelCatalog: vi.fn().mockResolvedValue([
    ...
    {
      provider: "openrouter",
      id: "anthropic/lada-opus-4-6",
      name: "LADA Opus 4.5 (OpenRouter)",
      contextWindow: 200000,
    },
...
const modelFallbackMocks = getSharedMocks("lada.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("lada.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    ladaHome: process.env.LADA_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("LADA_HOME", snapshot.ladaHome);
  restoreKey("LADA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.LADA_STATE_DIR = join(home, ".lada");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "lada-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".lada", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): LADAConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/lada-opus-4-6" },
        workspace: join(home, "lada"),
