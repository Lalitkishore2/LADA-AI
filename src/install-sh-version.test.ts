...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = makeTempDir(tempRoots, "lada-install-sh-");
  const cliPath = path.join(root, "lada");
...
resolve_lada_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_LADA_BIN: cliPath,
        LADA_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("LADA 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("LADA dev's build");

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("LADA dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = makeTempDir(tempRoots, "lada-install-stdin-");
