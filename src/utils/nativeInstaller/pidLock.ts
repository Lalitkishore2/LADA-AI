 ...
 *
 * This module provides PID-based locking for running LADA versions.
...
/**
 * Validate that a running process is actually a LADA process
 * This helps mitigate PID reuse issues
 */
function isLADAProcess(pid: number, expectedExecPath: string): boolean {
  if (!isProcessRunning(pid)) {
    return false
  }

  // If the PID matches our current process, we know it's valid
  // This handles test environments where the command might not contain 'lada'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'lada' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('lada') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a LADA process?
  // This helps with PID reuse scenarios
  if (!isLADAProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be LADA - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a LADA process (to handle PID reuse scenarios)
