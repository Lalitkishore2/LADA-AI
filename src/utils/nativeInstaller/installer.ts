...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'lada.exe' : 'lada'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'lada', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'lada', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'lada', 'locks'),
...
async function isPossibleLADABinary(filePath: string): Promise<boolean> {
  try {
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('lada-cli-native-'),
    )
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/lada to the version binary
  ...
  if (!(await isPossibleLADABinary(executablePath))) {
...
async function versionIsAvailable(version: string): Promise<boolean> {
  ...
  return isPossibleLADABinary(installPath)
}
...
async function updateLatest(
  ...
  if (
    ...
    (await isPossibleLADABinary(executablePath))
  ) {
...
export async function checkInstall(
  ...
  // Check if lada executable exists and is valid.
  ...
  // isPossibleLADABinary stats the path internally, so we don't pre-check
  // with access() — that would be a TOCTOU between access and the stat.
  if (isWindows) {
    // On Windows it's a copied executable, not a symlink
    if (!(await isPossibleLADABinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but lada command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleLADABinary(absoluteTarget))) {
        messages.push({
          message: `LADA symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but lada command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleLADABinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid LADA binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^lada\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the lada symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed lada symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove lada symlink: ${error}`))
  }
}

/**
 * Clean up old lada aliases from shell configuration files
...
export async function cleanupShellAliases(): Promise<SetupMessage[]> {
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      const { filtered, hadAlias } = filterLADAAliases(lines)

      if (hadAlias) {
        await writeFileLines(configFile, filtered)
        messages.push({
          message: `Removed lada alias from ${configFile}. Run: unalias lada`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up lada alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'lada.cmd')
      const binPs1 = join(globalPrefix, 'lada.ps1')
      const binExe = join(globalPrefix, 'lada')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'lada')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @anthropic-ai/lada-code
  const codePackageResult = await attemptNpmUninstall(
    '@anthropic-ai/lada-code',
  )
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@anthropic-ai/lada-code') {
  ...
  // Check for local installation at ~/.lada/local
  const localInstallDir = join(homedir(), '.lada', 'local')
