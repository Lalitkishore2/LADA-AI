...
/**
 * Detects if a file path is a session-related file under ~/.lada.
 * Returns the type of session file or null if not a session file.
 */
export function detectSessionFileType(
  filePath: string,
): 'session_memory' | 'session_transcript' | null {
  const configDir = getLADAConfigHomeDir()
...
/**
 * Check if a file is a LADA-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: CLAUDE.md, CLAUDE.local.md, .lada/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  const configDirCmp = toComparable(getLADAConfigHomeDir())
...
// Excludes CLAUDE.md, CLAUDE.local.md, .lada/rules/ (user-managed).
