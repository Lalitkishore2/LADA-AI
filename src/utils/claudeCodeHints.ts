 ...
 * LADA hints protocol.
 *
 * CLIs and SDKs running under LADA can emit a self-closing
 * `<lada-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/lada-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type LADACodeHintType = 'plugin'

export type LADACodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: LADACodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<lada-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractLADACodeHints(
  output: string,
  command: string,
): { hints: LADACodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<lada-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: LADACodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[ladaCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[ladaCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[ladaCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as LADACodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: LADACodeHint | null = null
...
export function setPendingHint(hint: LADACodeHint): void {
...
export function getPendingHintSnapshot(): LADACodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetLADACodeHintStore(): void {
  pendingHint = null
