 ...
 * =====================
 * This module manages an isolated tmux socket for LADA's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, LADA could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started LADA from within tmux.
 *
 * HOW IT WORKS:
 * 1. LADA creates its own tmux socket: `lada-<PID>` (e.g., `lada-12345`)
 ...
 *    (set in Shell.ts via getLADATmuxEnv())
 *
 * This means ANY tmux command run through LADA - whether via the Tmux tool
 * directly or via Bash - will operate on LADA's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getLADATmuxEnv() returns a value that overrides the
...
const CLAUDE_SOCKET_PREFIX = 'lada'
...
/**
 * Gets the socket name for LADA's isolated tmux session.
 * Format: lada-<PID>
 */
export function getLADASocketName(): string {
  if (!socketName) {
 ...
 */
export function getLADASocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setLADASocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for LADA's isolated socket.
 *
 ...
 * initializing the socket. getLADATmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Any LADA instance spawned on this socket will inherit this env var,
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setLADASocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
