 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches LADA
...
/**
 * Launch LADA in the detected terminal emulator.
 *
 ...
 *
 * For pure-argv paths: ladaPath, --prefill, query, cwd travel as distinct
 * argv elements end-to-end. No sh -c. No shellQuote(). The terminal does
 * chdir(cwd) and execvp(lada, argv). Spaces/quotes/metacharacters in
...
export async function launchInTerminal(
  ...
  const ladaArgs = ['--deep-link-origin']
  if (action.repo) {
    ladaArgs.push('--deep-link-repo', action.repo)
    if (action.lastFetchMs !== undefined) {
      ladaArgs.push('--deep-link-last-fetch', String(action.lastFetchMs))
    }
  }
  if (action.query) {
    ladaArgs.push('--prefill', action.query)
  }

  switch (process.platform) {
    case 'darwin':
      return launchMacosTerminal(terminal, ladaPath, ladaArgs, action.cwd)
    case 'linux':
      return launchLinuxTerminal(terminal, ladaPath, ladaArgs, action.cwd)
    case 'win32':
      return launchWindowsTerminal(terminal, ladaPath, ladaArgs, action.cwd)
    default:
      return false
  }
}
...
async function launchMacosTerminal(
  ...
  switch (terminal.command) {
    ...
    case 'iTerm': {
      const shCmd = buildShellCommand(ladaPath, ladaArgs, cwd)
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after lada exits.
    ...
    case 'PowerShell': {
      ...
      args.push(
        '-NoExit',
        '-Command',
        `${cdCmd}& ${psQuote(ladaPath)} ${ladaArgs.map(psQuote).join(' ')}`,
      )
      break
    }
    ...
    default: {
      ...
      args.push(
        '/k',
        `${cdCmd}${cmdQuote(ladaPath)} ${ladaArgs.map(a => cmdQuote(a)).join(' ')}`,
      )
      break
    }
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[ladaPath, ...ladaArgs].map(shellQuote).join(' ')}`
}
 ...
 * *child process* (lada.exe) uses CommandLineToArgvW, where a trailing
