 ...
 * (e.g. anthropics/lada-code, anthropic-experimental/sandbox-runtime).
...
const INTERNAL_MODEL_REPOS = [
  'github.com:anthropics/lada-cli-internal',
  'github.com/anthropics/lada-cli-internal',
  ...
  'github.com/anthropics/trellis',
  'github.com:anthropics/lada-for-hiring',
  'github.com/anthropics/lada-for-hiring',
...
export function sanitizeModelName(shortName: string): string {
  ...
  if (shortName.includes('opus-4-6')) return 'lada-opus-4-6'
  if (shortName.includes('opus-4-5')) return 'lada-opus-4-5'
  if (shortName.includes('opus-4-1')) return 'lada-opus-4-1'
  if (shortName.includes('opus-4')) return 'lada-opus-4'
  if (shortName.includes('sonnet-4-6')) return 'lada-sonnet-4-6'
  if (shortName.includes('sonnet-4-5')) return 'lada-sonnet-4-5'
  if (shortName.includes('sonnet-4')) return 'lada-sonnet-4'
  if (shortName.includes('sonnet-3-7')) return 'lada-sonnet-3-7'
  if (shortName.includes('haiku-4-5')) return 'lada-haiku-4-5'
  if (shortName.includes('haiku-3-5')) return 'lada-haiku-3-5'
  // Unknown models get a generic name
  return 'lada'
}

/**
 * Attribution state for tracking LADA's contributions to files.
 */
...
/**
 * Summary of LADA's contribution for a commit.
 */
...
export type AttributionData = {
  ...
  surfaceBreakdown: Record<string, { ladaChars: number; percent: number }>
 ...
 * Format: "surface/model" (e.g., "cli/lada-sonnet")
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate LADA's character contribution
    let ladaContribution: number
    ...
    if (oldContent === '' || newContent === '') {
      ...
      ladaContribution = Math.max(oldChangedLen, newChangedLen)
    }

    // Get current file state if it exists
    const existingState = existingFileStates.get(normalizedPath)
    const existingContribution = existingState?.ladaContribution ?? 0

    return {
      contentHash: computeContentHash(newContent),
      ladaContribution: existingContribution + ladaContribution,
      mtime,
    }
  } catch (error) {
...
/**
 * Track a file modification by LADA.
...
export function trackFileModification(
  ...
  logForDebugging(
    `Attribution: Tracked ${newFileState.ladaContribution} chars for ${normalizedPath}`,
  )
...
/**
 * Track a file creation by LADA (e.g., via bash command).
 * Used when LADA creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by LADA (e.g., via bash rm command).
 * Used when LADA deletes a file through a non-tracked mechanism.
 */
export function trackFileDeletion(
  ...
  const newFileState: FileAttributionState = {
    ...
    ladaContribution: existingContribution + deletedChars,
    mtime: Date.now(),
  }
  ...
  logForDebugging(
    `Attribution: Tracked deletion of ${normalizedPath} (${deletedChars} chars removed, total contribution: ${newFileState.ladaContribution})`,
  )
...
export async function calculateCommitAttribution(
    ...
    for (const [path, fileState] of fileStates) {
      ...
      if (existing) {
        mergedFileStates.set(path, {
          ...fileState,
          ladaContribution:
            existing.ladaContribution + fileState.ladaContribution,
        })
      } else {
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // LADA deleted this file (tracked deletion)
          ladaChars = fileState.ladaContribution
          humanChars = 0
        } else {
      ...
      ladaChars = Math.max(0, ladaChars)
      humanChars = Math.max(0, humanChars)

      const total = ladaChars + humanChars
      const percent = total > 0 ? Math.round((ladaChars / total) * 100) : 0
  ...
  for (const result of fileResults) {
    ...
    files[result.file] = {
      ladaChars: result.ladaChars,
      humanChars: result.humanChars,
      percent: result.percent,
      surface: result.surface,
    }

    totalLADAChars += result.ladaChars
    totalHumanChars += result.humanChars

    surfaceCounts[result.surface] =
      (surfaceCounts[result.surface] ?? 0) + result.ladaChars
  }

  const totalChars = totalLADAChars + totalHumanChars
  const ladaPercent =
    totalChars > 0 ? Math.round((totalLADAChars / totalChars) * 100) : 0

  // Calculate surface breakdown (percentage of total content per surface)
  const surfaceBreakdown: Record<
    string,
    { ladaChars: number; percent: number }
  > = {}
  for (const [surface, chars] of Object.entries(surfaceCounts)) {
    ...
    surfaceBreakdown[surface] = { ladaChars: chars, percent }
  }
