...
/**
 * Builds a deep link URL for LADA Desktop to resume a CLI session.
 * Format: lada://resume?session={sessionId}&cwd={cwd}
 * In dev mode: lada-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'lada-dev' : 'lada'
...
/**
 * Check if LADA Desktop app is installed.
 * On macOS, checks for /Applications/LADA.app.
 * On Linux, checks if xdg-open can handle lada:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for LADA.app in /Applications
    return pathExists('/Applications/LADA.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for lada://
...
/**
 * Detect the installed LADA Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/LADA.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'AnthropicLADA')
    try {
...
/**
 * Build and open a deep link to resume the current session in LADA Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'LADA Desktop is not installed. Install it from https://lada.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open LADA Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
