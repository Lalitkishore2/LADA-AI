/**
 * MDM (Mobile Device Management) profile enforcement for LADA managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.anthropic.ladacode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\LADACode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\LADACode` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/lada-code/managed-settings.json instead)
 *
