...
// LADA configuration directory names
...
export type LADAConfigDirectory = (typeof CLAUDE_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/anthropics/lada-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.lada/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.lada/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .lada directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.lada/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .lada/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  subdir: LADAConfigDirectory,
  cwd: string,
): string[] {
  ...
  while (true) {
    ...
    const ladaSubdir = join(current, '.lada', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getLADAConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.lada', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .lada/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .lada/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .lada/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/lada-code#29599, #28182, #26992).
    //
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.lada', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainLADASubdir = join(canonicalRoot, '.lada', subdir)
        if (!projectDirs.includes(mainLADASubdir)) {
          projectDirs.push(mainLADASubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.lada is
  ...
  (subdir: LADAConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * @param dir Directory (eg. "~/.lada/commands")
 * @returns Array of parsed markdown files with metadata
 */
