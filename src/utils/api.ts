...
export async function toolToAPISchema(
  ...
  if (!base) {
    ...
    // with LADA 4.5 reject this field with 400. See GH#32742, PR #21729.
    if (
  ...
  // github.com/anthropics/lada-code/issues/20031
 ...
 * (see https://console.statsig.com/4aF3Ewatb6xPVpCwxb5nA3/dynamic_configs/lada_cli_system_prompt_prefixes)
 */
...
export async function logContextMetrics(
  ...
  const ladaMdSize = userContext.ladaMd?.length ?? 0

  // Calculate total context size
  const totalContextSize = gitStatusSize + ladaMdSize
...
export function normalizeToolInput<T extends Tool>(
  ...
  switch (tool.name) {
    ...
    case BashTool.name: {
      ...
      // Logging for commands that are only echoing a string. This is to help us understand how often  LADA talks via bash
    ...
    case FileEditTool.name: {
      ...
      // This is a workaround for tokens lada can't see
