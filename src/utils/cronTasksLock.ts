// Scheduler lease lock for .lada/scheduled_tasks.json.
//
// When multiple LADA sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.lada', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .lada/ doesn't exist yet — create it and retry once. In steady
