/**
 * Marketplace manager for LADA plugins
 *
...
export function getDeclaredMarketplaces(): Record<string, DeclaredMarketplace> {
  ...
  // An explicit extraKnownMarketplaces entry for lada-plugins-official
 ...
 *
 * Reads the configuration file at ~/.lada/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.lada/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.lada/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a LADA bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.lada/plugins/known_marketplaces.json,
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'LADA-Code-Plugin-Manager',
  }
 ...
 * - GitHub: Clones repo and looks for .lada-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.lada-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .lada-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .lada-plugin/)
      ...
      case 'directory': {
        // For directories, look for .lada-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.lada-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.lada/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: lada plugin disable <plugin>@${name}`,
    )
  }
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .lada-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.lada-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `LADA version. Run 'lada marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `lada plugin marketplace update` (no name arg).
  ...
  try {
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: lada plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      try {
        ...
        const reason =
          name === 'lada-code-plugins'
            ? `We've deprecated "lada-code-plugins" in favor of "lada-plugins-official".`
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: lada plugin marketplace remove "${name}"`,
        )
      }
