 ...
 * `<lada-code-hint />` tag to stderr (detected by the Bash/PowerShell tools).
 *
 * State persists in GlobalConfig.ladaCodeHints — a show-once record per
...
/**
 * Hard cap on `ladaCodeHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.ladaCodeHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      ladaCodeHints: {
        ...current.ladaCodeHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.ladaCodeHints?.disabled) return current
    return {
      ...current,
      ladaCodeHints: { ...current.ladaCodeHints, disabled: true },
    }
  })
}
