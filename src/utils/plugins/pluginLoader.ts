 ...
 *
 * This module is responsible for discovering, loading, and validating LADA plugins
 ...
 * Format: ~/.lada/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.lada/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.lada-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".lada-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".lada-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.lada-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".lada-plugin/name")
 ...
 * main.tsx's getLADACodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .lada-plugin/ directory (for export)
