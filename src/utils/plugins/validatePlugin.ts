 ...
 * one into the other. Surfaced as warnings by `lada plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .lada-plugin directory
  if (dirName === '.lada-plugin') {
 ...
 * '..' a user added to "climb out of .lada-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .lada-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .lada-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .lada-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `lada plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — LADA ` +
  ...
  if (result.success) {
    ...
    // but the LADA.ai marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. LADA accepts ` +
          `it, but the LADA.ai marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.lada-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.lada-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `lada plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and LADA ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .lada-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.lada-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .lada-plugin/marketplace.json or .lada-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
