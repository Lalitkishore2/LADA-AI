 ...
 *
 * The enabled/disabled state remains in .lada/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.lada/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.lada/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.lada/plugins/cache/{marketplace}/{plugin}/{version}
 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where LADA
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.lada-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .lada-plugin dir is present
          if (dirEntries.includes('.lada-plugin')) {
