 ...
 * 1. Caching a plugin to ~/.lada/plugins/cache/
  ...
  if (cacheResult.path !== versionedPath) {
    ...
    if (isSubdirectory) {
      ...
      const tempPath = join(
        ...
        `.lada-plugin-temp-${Date.now()}-${randomBytes(4).toString('hex')}`,
      )
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    // non-official) because dbt external_lada_code_plugin_installs.sql
