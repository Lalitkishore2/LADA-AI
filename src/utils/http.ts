...
import { getLADACodeUserAgent } from './userAgent.js'
import { getWorkload } from './workloadContext.js'

// WARNING: We rely on `lada-cli` in the user agent for log filtering.
...
export function getUserAgent(): string {
  ...
  return `lada-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.CLAUDE_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}
...
export function getMCPUserAgent(): string {
  ...
  return `lada-code/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `LADA-User` is
// Anthropic's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the lada-code suffix lets them distinguish
// local CLI traffic from lada.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `LADA-User (${getLADACodeUserAgent()}; +https://support.anthropic.com/)`
}
...
export async function withOAuth401Retry<T>(
  ...
  try {
    ...
    const failedAccessToken = getLADAAIOAuthTokens()?.accessToken
