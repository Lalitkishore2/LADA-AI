...
function getLocalInstallDir(): string {
  return join(getLADAConfigHomeDir(), 'local')
}
export function getLocalLADAPath(): string {
  return join(getLocalInstallDir(), 'lada')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.lada/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'lada-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'lada')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/lada" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update LADA CLI package in the local directory
 ...
 */
export async function installOrUpdateLADAPackage(
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install LADA CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'lada'))
    return true
  } catch {
    return false
  }
