 ...
 * user-controlled files (~/.lada/settings.json and ~/.lada.json),
 ...
 *
 * We read from global config (~/.lada.json) and user settings
 * (~/.lada/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.lada/settings.json),
