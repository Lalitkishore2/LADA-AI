/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with LADA CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and LADA CLI's
...
import { getLADATempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve LADA-specific path patterns for sandbox-runtime.
 *
 * LADA uses special path prefixes in permission rules:
...
/**
 * Convert LADA settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and LADA temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getLADATempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where LADA started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.lada', 'settings.json'))
    denyWrite.push(resolve(cwd, '.lada', 'settings.local.json'))
  }

  // Block writes to .lada/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .lada/commands and
  // .lada/agents but not .lada/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full LADA capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.lada', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.lada', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when LADA's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/lada-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * LADA's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/lada-code#29316.
 */
 ...
 * This is a LADA CLI-specific function that updates local settings.
 */
...
/**
 * LADA CLI sandbox manager - wraps sandbox-runtime with LADA-specific features
 */
