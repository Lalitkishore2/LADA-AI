 ...
 * Used for managing lada aliases and PATH entries
 */
...
import { getLocalLADAPath } from './localInstaller.js'

export const CLAUDE_ALIAS_REGEX = /^\s*alias\s+lada\s*=/
...
/**
 * Filter out installer-created lada aliases from an array of lines
 * Only removes aliases pointing to $HOME/.lada/local/lada
...
/**
 * Check if a lada alias exists in any shell config file
 * Returns the alias target if found, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findLADAAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (CLAUDE_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+lada=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a lada alias exists and points to a valid executable
 * Returns the alias target if valid, null otherwise
 * @param options Optional overrides for testing (env, homedir)
 */
export async function findValidLADAAlias(
  options?: ShellConfigOptions,
): Promise<string | null> {
  const aliasTarget = await findLADAAlias(options)
