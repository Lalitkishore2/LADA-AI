...
export const getGlobalLADAFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getLADAConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getLADAConfigHomeDir(), '.config.json')
  }

  const filename = `.lada${fileSuffixForOauthConfig()}.json`
