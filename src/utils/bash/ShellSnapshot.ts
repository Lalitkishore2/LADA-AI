...
/**
 * Generates LADA specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getLADACodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in LADA's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and LADA content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const ladaCodeContent = await getLADACodeSnapshotContent()
 ...
 *
 * This function is a critical part of LADA CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows LADA CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with LADA defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getLADAConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - LADA home: ${getLADAConfigHomeDir()}`)
