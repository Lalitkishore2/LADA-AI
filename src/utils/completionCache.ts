...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const ladaDir = join(home, '.lada')

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(ladaDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(ladaDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(ladaDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: lada completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const ladaBin = process.argv[1] || 'lada'
  const result = await execFileNoThrow(ladaBin, [
    'completion',
    shell.shellFlag,
    '--output',
    shell.cacheFile,
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: lada completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('lada completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.lada/.
 * Called after `lada update` so completions stay in sync with the new binary.
 */
