...
import { getAdditionalDirectoriesForLADAMd } from '../../bootstrap/state.js'
import {
...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directory (~/.lada/skills)
  ...
  // User commands directory (~/.lada/commands)
  ...
  // Project skills directory (.lada/skills)
  ...
  // Project commands directory (.lada/commands)
  ...
  for (const dir of getAdditionalDirectoriesForLADAMd()) {
    const additionalSkillsPath = platformPath.join(dir, '.lada', 'skills')
    try {
