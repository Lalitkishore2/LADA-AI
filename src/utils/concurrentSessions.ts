...
function getSessionsDir(): string {
  return join(getLADAConfigHomeDir(), 'sessions')
}

/**
 * Kind override from env. Set by the spawner (`lada --bg`, daemon
...
/**
 * True when this REPL is running inside a `lada --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `lada ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `lada ps` sparkline
...
/**
 * Push live activity state for `lada ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/lada-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.lada/sessions/ is shared with Windows-native LADA (symlink
