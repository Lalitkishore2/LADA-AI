...
export const CLAUDE_IN_CHROME_MCP_SERVER_NAME = 'lada-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[LADA in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isLADAInChromeMCPServer(name: string): boolean {
...
export function trackLADAInChromeTabId(tabId: number): void {
...
export function isTrackedLADAInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[LADA in Chrome] No compatible browser found')
    return false
  }
...
export function getSocketDir(): string {
  return `/tmp/lada-mcp-browser-bridge-${getUsername()}`
}
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- LADAForChromeContext.getSocketPaths (external @ant/lada-for-chrome-mcp) requires a sync () => string[] callback
  ...
  const legacyName = `lada-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the LADA in Chrome MCP
  return `lada-mcp-browser-bridge-${getUsername()}`
}
