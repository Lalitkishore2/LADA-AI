...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/lada'))
      return join(homedir(), '.local/bin/lada')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.lada', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@anthropic-ai/lada-code']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/lada
    // Linux / macOS have prefix/bin/lada and prefix/lib/node_modules
    // Windows has prefix/lada and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'lada')
      : join(npmPrefix, 'bin', 'lada')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/lada
      ...
      // If no bin/lada exists, check for orphaned packages (no bin/lada symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'lada')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'lada')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: lada install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run lada install to update configuration',
      })
    }
  }
  ...
  const existingAlias = await findLADAAlias()
  const validAlias = await findValidLADAAlias()

  // Check if running local installation but it's not in PATH
  if (type === 'npm-local') {
    // Check if lada is already accessible via PATH
    const whichResult = await which('lada')
    const ladaInPath = !!whichResult

    // Only show warning if lada is NOT in PATH AND no valid alias exists
    if (!ladaInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias lada="~/.lada/local/lada"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias lada="~/.lada/local/lada"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/lada-code'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@anthropic-ai/lada-code'
        ) {
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `lada install` for native installation',
      })
    }
  }
