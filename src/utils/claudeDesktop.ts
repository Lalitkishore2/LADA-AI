...
export async function getLADADesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - LADA Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/LADA/lada_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for LADA Desktop config in each user directory
  ...
  throw new Error(
    'Could not find LADA Desktop config file in Windows. Make sure LADA Desktop is installed on Windows.',
  )
}

export async function readLADADesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - LADA Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getLADADesktopConfigPath()
