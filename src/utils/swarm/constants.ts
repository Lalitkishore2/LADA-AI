...
export const SWARM_SESSION_NAME = 'lada-swarm'
export const SWARM_VIEW_WINDOW_NAME = 'swarm-view'
export const TMUX_COMMAND = 'tmux'
export const HIDDEN_SESSION_NAME = 'lada-hidden'
 ...
 * Includes PID to ensure multiple LADA instances don't conflict.
 */
export function getSwarmSocketName(): string {
  return `lada-swarm-${process.pid}`
}

/**
 * Environment variable to override the command used to spawn teammate instances.
 * If not set, defaults to process.execPath (the current LADA binary).
