...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.lada/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.lada/settings.json)'
    case 'projectSettings':
      return 'Project settings (.lada/settings.json)'
    case 'localSettings':
      return 'Local settings (.lada/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.lada/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.lada/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by LADA)'
    default:
