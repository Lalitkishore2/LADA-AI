 ...
 *
 * The parent lada process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // Anthropic auth — lada re-reads these per-request, subprocesses don't need them
  'ANTHROPIC_API_KEY',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before lada spawns;
  ...
  // lada-code-action-specific duplicates — action JS consumes these during
  // prepare, before spawning lada. ALL_INPUTS contains anthropic_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on CLAUDE_CODE_SUBPROCESS_ENV_SCRUB. lada-code-action sets this
