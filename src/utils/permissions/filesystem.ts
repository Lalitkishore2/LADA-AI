...
import { getLADAConfigHomeDir } from '../envUtils.js'
import {
  getFsImplementation,
...
/**
 * If filePath is inside a .lada/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .lada/ (settings.json, hooks/, etc.).
 */
export function getLADASkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.lada', 'skills')),
      prefix: '/.lada/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.lada', 'skills')),
      prefix: '~/.lada/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.lada/skills/*/**' which matches ALL skills. Return null
...
export function isLADASettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.lada/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.lada${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.lada${sep}settings.local.json`)
  ) {
    // Include .lada/settings.json even for other projects
    return true
  }
...
// Always ask when LADA tries to edit its own config files
function isLADAConfigFilePath(filePath: string): boolean {
  if (isLADASettingsPath(filePath)) {
    return true
  }

  // Check if file is within .lada/commands or .lada/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.lada', 'commands')
  const agentsDir = join(getOriginalCwd(), '.lada', 'agents')
  const skillsDir = join(getOriginalCwd(), '.lada', 'skills')
 ...
 * Path format: ~/.lada/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for LADA to write temporary files.
...
/**
 * Returns the user-specific LADA temp directory name.
 * On Unix: 'lada-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'lada' (tmpdir() is already per-user)
 */
export function getLADATempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'lada'
  }
...
/**
 * Returns the LADA temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/lada-{uid}/ (resolved to /private/tmp/lada-{uid}/ on macOS)
 * - On Windows: {tmpdir}/lada/ (e.g., C:\Users\{user}\AppData\Local\Temp\lada\)
 * This is a per-user temporary directory used by LADA for all temp files.
 *
 ...
 * resolution, paths like /tmp/lada-{uid}/... wouldn't match /private/tmp/lada-{uid}/...
 */
...
export const getLADATempDir = memoize(function getLADATempDir(): string {
  const baseTmpDir =
  ...
  return join(resolvedBaseTmpDir, getLADATempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getLADATempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/lada-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getLADATempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/lada-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/lada-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .lada/worktrees/ is a structural path (where LADA stores
      // git worktrees), not a user-created dangerous directory. Skip the .lada
      // segment when it's followed by 'worktrees'. Any nested .lada directories
      ...
      if (dir === '.lada') {
        ...
        if (
          ...
          break // Skip this .lada, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .lada , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .lada , .bashrc..., settings.json.
 ...
 * - LADA config files (.lada/settings.json, .lada/commands/, .lada/agents/)
 * - MCP CLI state files (managed internally by LADA)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `LADA requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for LADA config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isLADAConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `LADA requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `LADA requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .lada/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `LADA requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `LADA requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `LADA requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `LADA requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .lada is a dangerous directory
  ...
  // 1.6. Check for .lada/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .lada/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .lada/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.lada) rule in userSettings (e.g. from sandbox
  ...
  // "allow LADA to edit its own settings for this session" option actually works.
  const ladaFolderAllowRule = matchingRuleForInput(
    path,
    {
      ...toolPermissionContext,
      alwaysAllowRules: {
  ...
  if (ladaFolderAllowRule) {
    // Check if this rule is scoped under .lada/ (project or global).
    // Accepts both the broad patterns ('/.lada/**', '~/.lada/**') and
    // narrowed ones like '/.lada/skills/my-skill/**' so users can grant
    ...
    // '/.lada/../**' from leaking this bypass outside .lada/.
    const ruleContent = ladaFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      ...
      return {
        ...
        decisionReason: {
          type: 'rule',
          rule: ladaFolderAllowRule,
        },
      }
    }
  }

  // 1.7. Check comprehensive safety validations (Windows patterns, LADA config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .lada/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.lada/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getLADASkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.lada/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      const jobsRoot = join(getLADAConfigHomeDir(), 'jobs')
      ...
      // the case where ~/.lada is a symlink (e.g. to /data/lada-config).
  ...
  // ~/.lada/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .lada/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs LADA to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .lada/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .lada/ only (not ~/.lada/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.lada', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/lada/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.lada/tasks/) for swarm task coordination
  const tasksDir = join(getLADAConfigHomeDir(), 'tasks') + sep
  if (
  ...
  // Teams directory (~/.lada/teams/) for swarm coordination
  const teamsReadDir = join(getLADAConfigHomeDir(), 'teams') + sep
  if (
