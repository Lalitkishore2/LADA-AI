 ...
 * validation purposes, so commands like `echo foo > /tmp/lada/x.txt` don't
 * prompt for permission when /tmp/lada/ is already in the sandbox allowlist.
 *
 * Respects the deny-within-allow list: paths in denyWithinAllow (like
 * .lada/settings.json) are still blocked even if their parent is in allowOnly.
 */
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, LADA config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
