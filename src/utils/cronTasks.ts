// Scheduled prompts, stored in <project>/.lada/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.lada', 'scheduled_tasks.json')
...
/**
 * Read and parse .lada/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .lada/scheduled_tasks.json with the given tasks. Creates .lada/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.lada'), { recursive: true })
 ...
 * written to .lada/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while LADA
