...
/**
 * Convert a PersistedStatsCache to LADACodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all LADA sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateLADACodeStats(): Promise<LADACodeStats> {
 ...
 */
export async function aggregateLADACodeStatsForRange(
  range: StatsDateRange,
): Promise<LADACodeStats> {
  if (range === 'all') {
    return aggregateLADACodeStats()
  }
  ...
  return processedStatsToLADACodeStats(stats)
}

/**
 * Convert ProcessedStats to LADACodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToLADACodeStats(
...
function getEmptyStats(): LADACodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
