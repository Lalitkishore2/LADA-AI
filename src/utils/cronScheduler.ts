// Non-React scheduler core for .lada/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .lada/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while LADA
  ...
  function check() {
    ...
    // exists to stop two LADA sessions in the same cwd from double-firing
  ...
  async function enable() {
    ...
    // dies. Prevents double-firing when multiple LADAs share a cwd.
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while LADA was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .lada/scheduled_tasks.json.\n\n` +
