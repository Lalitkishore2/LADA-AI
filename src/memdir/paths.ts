 ...
 *   2. ~/.lada (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.CLAUDE_CODE_REMOTE_MEMORY_DIR) {
    return process.env.CLAUDE_CODE_REMOTE_MEMORY_DIR
  }
  return getLADAConfigHomeDir()
}
 ...
 *
 * SECURITY: projectSettings (.lada/settings.json committed to the repo) is
 ...
 * same repo share one auto-memory directory (anthropics/lada-code#24382).
 */
