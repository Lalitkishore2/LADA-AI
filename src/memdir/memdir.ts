 ...
 *
 * Shared by buildMemoryPrompt and ladamd getMemoryFiles (previously
 ...
 * Shipped because LADA was burning turns on `ls`/`mkdir -p` before writing.
 ...
 * (~/.lada/projects/<slug>/memory/) is created in one call with no
 ...
 * Used by agent memory (which has no getLADAMds() equivalent).
 */
 ...
 * files + MEMORY.md. MEMORY.md is still loaded into context (via ladamd.ts)
