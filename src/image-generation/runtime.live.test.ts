...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.LADA_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.LADA_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.LADA_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.LADA_LIVE_IMAGE_GENERATION_MODELS);
...
function withPluginsEnabled(cfg: LADAConfig): LADAConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
