...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getImageGenerationProvider: vi.fn<
      (providerId: string, config?: LADAConfig) => ImageGenerationProvider | undefined
    >(() => undefined),
    ...
    listImageGenerationProviders: vi.fn<(config?: LADAConfig) => ImageGenerationProvider[]>(
      () => [],
    ),
...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as LADAConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as LADAConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateImage({ cfg: {} as LADAConfig, prompt: "draw a cat" });
