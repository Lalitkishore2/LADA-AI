...
let readLADACliCredentialsCached: typeof import("./cli-credentials.js").readLADACliCredentialsCached;
...
let writeLADACliKeychainCredentials: typeof import("./cli-credentials.js").writeLADACliKeychainCredentials;
let writeLADACliCredentials: typeof import("./cli-credentials.js").writeLADACliCredentials;
...
function mockExistingLADAKeychainItem() {
...
async function readCachedLADACliCredentials(allowKeychainPrompt: boolean) {
...
function mockLADACliCredentialRead() {
...
describe("cli credentials", () => {
  beforeAll(async () => {
    ({
      readLADACliCredentialsCached,
      readCodexCliCredentialsCached,
      resetCliCredentialCachesForTest,
      writeLADACliKeychainCredentials,
      writeLADACliCredentials,
  ...
  it("updates the LADA keychain item in place", async () => {
    mockExistingLADAKeychainItem();

    const ok = writeLADACliKeychainCredentials(
      {
        access: "new-access",
        refresh: "new-refresh",
  ...
  it("falls back to the file store when the keychain update fails", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-"));
    const credPath = path.join(tempDir, ".lada", ".credentials.json");
    ...
    const ok = writeLADACliCredentials(
      {
        access: "new-access",
        refresh: "new-refresh",
    ...
    expect(updated.ladaAiOauth?.accessToken).toBe("new-access");
    expect(updated.ladaAiOauth?.refreshToken).toBe("new-refresh");
    expect(updated.ladaAiOauth?.expiresAt).toBeTypeOf("number");
  });

  it.each([
    {
      name: "caches LADA CLI credentials within the TTL window",
    ...
    {
      name: "refreshes LADA CLI credentials after the TTL window",
    ...
    async ({ allowKeychainPromptSecondRead, advanceMs, expectedCalls, expectSameObject }) => {
      mockLADACliCredentialRead();
      vi.setSystemTime(new Date("2025-01-01T00:00:00Z"));

      const first = await readCachedLADACliCredentials(true);
      if (advanceMs > 0) {
        vi.advanceTimersByTime(advanceMs);
      }
      const second = await readCachedLADACliCredentials(allowKeychainPromptSecondRead);
  ...
  it("reads Codex credentials from keychain when available", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-codex-"));
  ...
  it("invalidates cached Codex credentials when auth.json changes within the TTL window", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-codex-cache-"));
    process.env.CODEX_HOME = tempHome;
    const authPath = path.join(tempHome, "auth.json");
  ...
  it("updates existing Codex auth.json in place", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-codex-write-"));
    process.env.CODEX_HOME = tempHome;
    try {
      fs.mkdirSync(tempHome, { recursive: true, mode: 0o700 });
      const authPath = path.join(tempHome, "auth.json");
  ...
  it("prefers the existing Codex keychain entry over auth.json on darwin writes", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-codex-keychain-write-"));
    process.env.CODEX_HOME = tempHome;
    try {
