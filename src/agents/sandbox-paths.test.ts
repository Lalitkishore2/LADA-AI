...
async function withOutsideHardlinkInLADATmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside lada tmp root",
      media: path.join(os.tmpdir(), "outside-lada", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked LADA tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked LADA tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLADATmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked LADA tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLADATmp(
