...
describe("anthropic-vertex implicit provider", () => {
  ...
  it("accepts ADC credentials when the file includes a project_id", async () => {
    const agentDir = mkdtempSync(join(tmpdir(), "lada-test-"));
    const adcDir = mkdtempSync(join(tmpdir(), "lada-adc-"));
    ...
    try {
      ...
      expect(providers?.["anthropic-vertex"]?.models).toMatchObject([
        { id: "lada-opus-4-6", maxTokens: 128000, contextWindow: 1_000_000 },
        { id: "lada-sonnet-4-6", maxTokens: 128000, contextWindow: 1_000_000 },
      ]);
    } finally {
