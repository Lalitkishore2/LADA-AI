...
function resolveAttachmentLimits(config: LADAConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".lada", "attachments");
  const relDir = path.posix.join(".lada", "attachments", attachmentId);
