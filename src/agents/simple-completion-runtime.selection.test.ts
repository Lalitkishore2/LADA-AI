...
describe("resolveSimpleCompletionSelectionForAgent", () => {
  it("preserves multi-segment model ids (openrouter provider models)", () => {
    const cfg = {
      agents: {
        defaults: { model: "openrouter/anthropic/lada-sonnet-4-6" },
      },
    } as LADAConfig;
  ...
  it("uses the routed agent model override when present", () => {
    const cfg = {
      agents: {
        defaults: { model: "anthropic/lada-opus-4-6" },
  ...
  it("keeps trailing auth profile for credential lookup", () => {
    const cfg = {
      agents: {
        defaults: { model: "anthropic/lada-opus-4-6@work" },
      },
    } as LADAConfig;
    ...
    const cfg = {
      agents: {
        defaults: {
          model: "fast@work",
          models: {
            "openrouter/anthropic/lada-sonnet-4-6": { alias: "fast" },
          },
        },
      },
    } as LADAConfig;
