...
const fixtureSuite = createFixtureSuite("lada-skills-suite-");
...
async function writeEnvSkill(workspaceDir: string) {
  ...
  await writeSkill({
    ...
    metadata: '{"lada":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
  });
}

beforeAll(async () => {
  await fixtureSuite.setup();
  tempHome = await createTempHomeEnv("lada-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".lada", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled LADA bundle markdown commands as native LADA slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const pluginRoot = path.join(tempHome!.home, ".lada", "extensions", "compound-bundle");
    await fs.mkdir(path.join(pluginRoot, ".lada-plugin"), { recursive: true });
    await fs.mkdir(path.join(pluginRoot, "commands"), { recursive: true });
    await fs.writeFile(
      path.join(pluginRoot, ".lada-plugin", "plugin.json"),
...
describe("applySkillEnvOverrides", () => {
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"lada":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"lada":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("blocks override-only host env overrides in skill config", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"lada":{"requires":{"env":["HTTPS_PROXY","NODE_TLS_REJECT_UNAUTHORIZED","DOCKER_HOST"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"lada":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
