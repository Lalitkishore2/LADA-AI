...
describe("handleAgentEnd", () => {
  ...
  it("attaches raw provider error metadata and includes model/provider in console output", async () => {
    ...
    expect(warn.mock.calls[0]?.[1]).toMatchObject({
      ...
      consoleMessage:
        'embedded run agent end: runId=run-1 isError=true model=lada-test provider=anthropic error=The AI service is temporarily overloaded. Please try again in a moment. rawError={"type":"error","error":{"type":"overloaded_error","message":"Overloaded"}}',
    });
  });
  ...
  it("sanitizes model and provider before writing consoleMessage", async () => {
    ...
    expect(meta).toMatchObject({
      consoleMessage:
        "embedded run agent end: runId=run-1 isError=true model=lada sonnet 4 provider=anthropic]8;;https://evil.test error=LLM request failed: connection refused by the provider endpoint. rawError=connection refused",
    });
