...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses LADA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      LADA_HOME: "/srv/lada-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/lada-home"), ".lada", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".lada", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("lada-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("lada-workspace-");
    await fs.mkdir(path.join(tempDir, ".lada"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-workspace-hardlink-"));
    try {
