...
function mockContextDeps(params: {
  ...
  const ensureLADAModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureLADAModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveLADAAgentDir: () => "/tmp/lada-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("returns configured model context window on first lookup", async () => {
    mockContextModuleDeps(() => ({
      models: {
        providers: {
          openrouter: {
            models: [{ id: "openrouter/lada-sonnet", contextWindow: 321_000 }],
          },
        },
      },
    }));
    ...
    expect(lookupContextTokens("openrouter/lada-sonnet")).toBe(321_000);
  });
  ...
  it("returns sync config overrides for read-only callers", async () => {
    ...
    const { lookupContextTokens } = await importContextModule();
    expect(lookupContextTokens("openrouter/lada-sonnet", { allowAsyncLoad: false })).toBe(
      321_000,
    );
  });
  ...
  it("only warms eagerly for real lada startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        ...
        {
          argv: ["node", "lada", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "lada", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureLADAModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await importContextModule();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureLADAModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
  ...
  it("retries config loading after backoff when an initial load fails", async () => {
    ...
    const loadConfigMock = vi
      ...
      .mockImplementation(() => ({
        models: {
          providers: {
            openrouter: {
              models: [{ id: "openrouter/lada-sonnet", contextWindow: 654_321 }],
            },
          },
        },
      }));
    ...
    try {
      const { lookupContextTokens } = await importContextModule();
      expect(lookupContextTokens("openrouter/lada-sonnet")).toBeUndefined();
      expect(loadConfigMock).toHaveBeenCalledTimes(1);
      expect(lookupContextTokens("openrouter/lada-sonnet")).toBeUndefined();
      expect(loadConfigMock).toHaveBeenCalledTimes(1);
      await vi.advanceTimersByTimeAsync(1_000);
      expect(lookupContextTokens("openrouter/lada-sonnet")).toBe(654_321);
  ...
  it("resolveContextTokensForModel honors configured overrides when provider keys use mixed case", async () => {
    mockDiscoveryDeps([{ id: "openrouter/anthropic/lada-sonnet-4-5", contextWindow: 1_048_576 }]);

    const cfg = createContextOverrideConfig(" OpenRouter ", "anthropic/lada-sonnet-4-5", 200_000);
    const resolveContextTokensForModel = await importResolveContextTokensForModel();

    const result = resolveContextTokensForModel({
      cfg: cfg as never,
      provider: "openrouter",
      model: "anthropic/lada-sonnet-4-5",
    });
    expect(result).toBe(200_000);
  });
  ...
  it("resolveContextTokensForModel prefers exact provider key over alias-normalized match", async () => {
    ...
    const cfg = {
      models: {
        providers: {
          "amazon-bedrock": { models: [{ id: "lada-alias-test", contextWindow: 32_000 }] },
          bedrock: { models: [{ id: "lada-alias-test", contextWindow: 128_000 }] },
        },
      },
    };
