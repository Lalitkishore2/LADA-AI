...
export function isCliProvider(provider: string, cfg?: LADAConfig): boolean {
...
function resolveAllowedFallbacks(params: { cfg: LADAConfig; agentId?: string }): string[] {
  if (params.agentId) {
...
export function buildConfiguredModelCatalog(params: { cfg: LADAConfig }): ModelCatalogEntry[] {
...
export function resolveThinkingDefault(params: {
  ...
  if (
    ...
    (normalizedModel.startsWith("lada-opus-4-6") ||
      normalizedModel.startsWith("lada-sonnet-4-6"))
  ) {
    return "adaptive";
  }
