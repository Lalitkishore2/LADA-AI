...
function isLikelyLADACliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "lada" ||
    entryBasename === "lada.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real LADA CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureLADAModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyLADACliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): LADAConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureLADAModelsJson(cfg);
    } catch {
