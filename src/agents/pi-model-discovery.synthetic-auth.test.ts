...
async function withAgentDir(run: (agentDir: string) => Promise<void>): Promise<void> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-pi-synthetic-auth-"));
  try {
    await run(agentDir);
  } finally {
...
describe("pi model discovery synthetic auth", () => {
  ...
  it("mirrors plugin-owned synthetic cli auth into pi auth storage", async () => {
    await withAgentDir(async (agentDir) => {
      ...
      expect(authStorage.hasAuth("lada-cli")).toBe(true);
      await expect(authStorage.getApiKey("lada-cli")).resolves.toBe("lada-cli-access-token");
    });
  });
});
