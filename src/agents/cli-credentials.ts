...
const CLAUDE_CLI_CREDENTIALS_RELATIVE_PATH = ".lada/.credentials.json";
const CODEX_CLI_AUTH_FILENAME = "auth.json";
const MINIMAX_CLI_CREDENTIALS_RELATIVE_PATH = ".minimax/oauth_creds.json";

const CLAUDE_CLI_KEYCHAIN_SERVICE = "LADA-credentials";
const CLAUDE_CLI_KEYCHAIN_ACCOUNT = "LADA";
...
let ladaCliCache: CachedValue<LADACliCredential> | null = null;
...
type LADACliFileOptions = {
  homeDir?: string;
};

type LADACliWriteOptions = LADACliFileOptions & {
  platform?: NodeJS.Platform;
  writeKeychain?: (credentials: OAuthCredentials) => boolean;
  writeFile?: (credentials: OAuthCredentials, options?: LADACliFileOptions) => boolean;
};
...
function resolveLADACliCredentialsPath(homeDir?: string) {
  const baseDir = homeDir ?? resolveUserPath("~");
  return path.join(baseDir, CLAUDE_CLI_CREDENTIALS_RELATIVE_PATH);
}

function parseLADACliOauthCredential(ladaOauth: unknown): LADACliCredential | null {
  if (!ladaOauth || typeof ladaOauth !== "object") {
    return null;
  }
  const accessToken = (ladaOauth as Record<string, unknown>).accessToken;
  const refreshToken = (ladaOauth as Record<string, unknown>).refreshToken;
  const expiresAt = (ladaOauth as Record<string, unknown>).expiresAt;
...
function readLADACliKeychainCredentials(
  execSyncImpl: ExecSyncFn = execSync,
): LADACliCredential | null {
  try {
    ...
    return parseLADACliOauthCredential(data?.ladaAiOauth);
  } catch {
    return null;
  }
}

export function readLADACliCredentials(options?: {
  ...
  if (platform === "darwin" && options?.allowKeychainPrompt !== false) {
    const keychainCreds = readLADACliKeychainCredentials(options?.execSync);
    if (keychainCreds) {
      log.info("read anthropic credentials from lada cli keychain", {
  ...
  const credPath = resolveLADACliCredentialsPath(options?.homeDir);
  const raw = loadJsonFile(credPath);
  if (!raw || typeof raw !== "object") {
    return null;
  }
...
export function readLADACliCredentialsCached(options?: {
  allowKeychainPrompt?: boolean;
  ttlMs?: number;
  platform?: NodeJS.Platform;
  homeDir?: string;
  ...
  return readCachedCliCredential({
    ...
    cache: ladaCliCache,
    cacheKey: resolveLADACliCredentialsPath(options?.homeDir),
    read: () =>
...
export function writeLADACliKeychainCredentials(
  ...
  try {
    ...
    const existingOauth = existingData?.ladaAiOauth;
    if (!existingOauth || typeof existingOauth !== "object") {
      return false;
    }

    existingData.ladaAiOauth = {
      ...existingOauth,
    ...
    log.info("wrote refreshed credentials to lada cli keychain", {
      expires: new Date(newCredentials.expires).toISOString(),
    });
    return true;
  } catch (error) {
    log.warn("failed to write credentials to lada cli keychain", {
...
export function writeLADACliFileCredentials(
  newCredentials: OAuthCredentials,
  options?: LADACliFileOptions,
): boolean {
  const credPath = resolveLADACliCredentialsPath(options?.homeDir);
  ...
  try {
    ...
    const data = raw as Record<string, unknown>;
    const existingOauth = data.ladaAiOauth as Record<string, unknown> | undefined;
    if (!existingOauth || typeof existingOauth !== "object") {
      return false;
    }
    ...
    log.info("wrote refreshed credentials to lada cli file", {
      expires: new Date(newCredentials.expires).toISOString(),
    });
    return true;
  } catch (error) {
    log.warn("failed to write credentials to lada cli file", {
      error: formatErrorMessage(error),
    });
    return false;
  }
...
export function writeLADACliCredentials(
  newCredentials: OAuthCredentials,
  options?: LADACliWriteOptions,
): boolean {
  const platform = options?.platform ?? process.platform;
  const writeKeychain = options?.writeKeychain ?? writeLADACliKeychainCredentials;
  const writeFile =
    options?.writeFile ??
    ((credentials, fileOptions) => writeLADACliFileCredentials(credentials, fileOptions));
