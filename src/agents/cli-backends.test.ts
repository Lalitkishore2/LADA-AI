...
let normalizeLADABackendConfig: typeof import("./cli-backends.js").normalizeLADABackendConfig;
...
beforeEach(async () => {
  ...
  ({ normalizeLADABackendConfig, resolveCliBackendConfig, resolveCliBackendLiveTest } =
  ...
  registry.cliBackends = [
    createBackendEntry({
      ...
      id: "lada-cli",
      bundleMcp: true,
      bundleMcpMode: "lada-config-file",
      config: {
        command: "lada",
        args: [
          "stream-json",
      ...
      normalizeConfig: normalizeLADABackendConfig,
    }),
    createBackendEntry({
      pluginId: "openai",
      id: "codex-cli",
...
describe("resolveCliBackendLiveTest", () => {
  it("returns plugin-owned live smoke metadata for lada", () => {
    expect(resolveCliBackendLiveTest("lada-cli")).toEqual({
      defaultModelRef: "lada-cli/lada-sonnet-4-6",
      defaultImageProbe: true,
      defaultMcpProbe: true,
      dockerNpmPackage: "@anthropic-ai/lada-code",
      dockerBinaryName: "lada",
    });
  });
...
describe("resolveCliBackendConfig lada-cli defaults", () => {
  it("uses non-interactive permission-mode defaults for fresh and resume args", () => {
    const resolved = resolveCliBackendConfig("lada-cli");

    expect(resolved).not.toBeNull();
    expect(resolved?.bundleMcp).toBe(true);
    expect(resolved?.bundleMcpMode).toBe("lada-config-file");
  ...
  it("retains default lada safety args when only command is overridden", () => {
    const cfg = {
      agents: {
        defaults: {
          cliBackends: {
            "lada-cli": {
              command: "/usr/local/bin/lada",
            },
          },
        },
      },
    ...
    const resolved = resolveCliBackendConfig("lada-cli", cfg);

    expect(resolved).not.toBeNull();
    expect(resolved?.config.command).toBe("/usr/local/bin/lada");
    expect(resolved?.config.args).toContain("--setting-sources");
    expect(resolved?.config.args).toContain("user");
    expect(resolved?.config.args).toContain("--permission-mode");
    expect(resolved?.config.args).toContain("bypassPermissions");
  ...
  it("keeps hardened clearEnv defaults when custom lada env overrides are merged", () => {
    const cfg = {
      agents: {
        defaults: {
          cliBackends: {
  ...
  it("normalizes override-only lada-cli config when the plugin registry is absent", () => {
    const registry = createEmptyPluginRegistry();
    setActivePluginRegistry(registry);
