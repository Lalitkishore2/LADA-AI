...
// https://github.com/lada/lada/issues/33785
...
describe("failover-error", () => {
  it("infers failover reason from HTTP status", () => {
    ...
    // Anthropic LADA Max plan surfaces rate limits as HTTP 402 (#30484)
    expect(
      resolveFailoverReasonFromError({
        status: 402,
  ...
  it("coerces failover-worthy errors into FailoverError with metadata", () => {
    ...
    expect(err?.model).toBe("lada-opus-4-6");
  });
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: lada/lada#53849 — OpenRouter returns 403 with "Key limit exceeded"
