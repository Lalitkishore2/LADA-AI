...
describe("resolveCliAuthEpoch", () => {
  ...
  it("returns undefined when no local or auth-profile credentials exist", async () => {
    setCliAuthEpochTestDeps({
      readLADACliCredentialsCached: () => null,
    ...
    await expect(resolveCliAuthEpoch({ provider: "lada-cli" })).resolves.toBeUndefined();
    await expect(
      resolveCliAuthEpoch({
  ...
  it("changes when lada cli credentials change", async () => {
    let access = "access-a";
    setCliAuthEpochTestDeps({
    ...
    const first = await resolveCliAuthEpoch({ provider: "lada-cli" });
    access = "access-b";
    const second = await resolveCliAuthEpoch({ provider: "lada-cli" });
