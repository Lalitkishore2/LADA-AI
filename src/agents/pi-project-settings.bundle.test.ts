...
async function createWorkspaceBundle(params: {
  ...
  const pluginId = params.pluginId ?? "lada-bundle";
  const pluginRoot = path.join(params.workspaceDir, ".lada", "extensions", pluginId);
...
describe("loadEnabledBundlePiSettingsSnapshot", () => {
  it("loads sanitized settings from enabled bundle plugins", async () => {
    const workspaceDir = await tempDirs.make("lada-workspace-");
