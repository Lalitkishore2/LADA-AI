...
describe("loadModelCatalog", () => {
  beforeEach(async () => {
    ...
    vi.doMock("./models-config.js", () => ({
      ensureLADAModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
    }));
    vi.doMock("./agent-paths.js", () => ({
      resolveLADAAgentDir: () => "/tmp/lada",
    }));
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as LADAConfig });
