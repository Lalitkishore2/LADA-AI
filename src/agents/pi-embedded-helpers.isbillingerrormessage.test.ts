...
describe("isBillingErrorMessage", () => {
  it.each([
    {
      ...
      samples: [
        ...
        "You're out of extra usage. Add more at lada.ai/settings/usage and keep going.",
  ...
  it("classifies Anthropic extra-usage exhaustion variants as billing", () => {
    const samples = [
      "You're out of extra usage. Add more at lada.ai/settings/usage and keep going.",
      "Extra usage is required for long context requests.",
      '{"type":"error","error":{"type":"invalid_request_error","message":"You\'re out of extra usage. Add more at lada.ai/settings/usage and keep going."}}',
...
describe("classifyFailoverReason", () => {
  ...
  it("classifies internal and compatibility error messages", () => {
    ...
    expect(
      classifyFailoverReason(
        'No API key found for provider "openai". Auth store: /tmp/lada-agent-abc/auth-profiles.json (agentDir: /tmp/lada-agent-abc).',
      ),
    ).toBe("auth");
