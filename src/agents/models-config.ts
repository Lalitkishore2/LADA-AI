...
import { planLADAModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: LADAConfig;
  sourceConfigForSecrets: LADAConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: LADAConfig): {
  config: LADAConfig;
  sourceConfigForSecrets: LADAConfig;
} {
...
export async function ensureLADAModelsJson(
  config?: LADAConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveLADAAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planLADAModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
