...
vi.mock("./model-selection.js", () => ({
  buildAllowedModelSet: () => ({
    allowedKeys: new Set<string>(["anthropic/lada", "openai/lada", "openai/gpt-5.4"]),
    allowedCatalog: [],
    allowAny: false,
  }),
  ...
  resolveConfiguredModelRef: () => ({ provider: "anthropic", model: "lada" }),
  resolveDefaultModelForAgent: () => ({ provider: "anthropic", model: "lada" }),
...
describe("agentCommand – LiveSessionModelSwitchError retry", () => {
  ...
  it("does not flip hasSessionModelOverride on auth-only switch with same model", async () => {
    ...
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("anthropic", "lada"));
  ...
  it("flips hasSessionModelOverride on provider-only switch with same model", async () => {
    ...
    state.runAgentAttemptMock.mockResolvedValue(makeSuccessResult("openai", "lada"));
