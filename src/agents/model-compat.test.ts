...
describe("isModernModelRef", () => {
  it("uses provider runtime hooks before fallback heuristics", () => {
    ...
    expect(isModernModelRef({ provider: "openrouter", id: "lada-opus-4-6" })).toBe(false);
  });
  ...
  it("includes plugin-advertised modern models", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      ...
      ["gpt-5.4", "gpt-5.4-pro", "gpt-5.4-mini", "gpt-5.4-nano"].includes(context.modelId)
        ...
        : provider === "openai-codex" && ["gpt-5.4", "gpt-5.4-mini"].includes(context.modelId)
          ? true
          : provider === "opencode" && ["lada-opus-4-6", "gemini-3-pro"].includes(context.modelId)
            ? true
    ...
    expect(isModernModelRef({ provider: "opencode", id: "lada-opus-4-6" })).toBe(true);
...
describe("isHighSignalLiveModelRef", () => {
  it("keeps modern higher-signal LADA families", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      provider === "anthropic" && ["lada-sonnet-4-6", "lada-opus-4-6"].includes(context.modelId)
        ? true
        : undefined,
    );

    expect(isHighSignalLiveModelRef({ provider: "anthropic", id: "lada-sonnet-4-6" })).toBe(true);
    expect(isHighSignalLiveModelRef({ provider: "anthropic", id: "lada-opus-4-6" })).toBe(true);
  });

  it("drops low-signal or old LADA variants even when provider marks them modern", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockReturnValue(true);

    expect(isHighSignalLiveModelRef({ provider: "anthropic", id: "lada-opus-4-5" })).toBe(false);
    expect(
      isHighSignalLiveModelRef({ provider: "anthropic", id: "lada-haiku-4-5-20251001" }),
    ).toBe(false);
    expect(
      isHighSignalLiveModelRef({ provider: "opencode", id: "lada-3-5-haiku-20241022" }),
    ).toBe(false);
  });
});
...
describe("selectHighSignalLiveItems", () => {
  it("prefers curated Google replacements before fallback provider spread", () => {
    const items = [
      { provider: "anthropic", id: "lada-opus-4-6" },
