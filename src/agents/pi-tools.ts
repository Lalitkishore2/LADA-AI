...
function resolveExecConfig(params: { cfg?: LADAConfig; agentId?: string }) {
...
export function createLADACodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createLADAReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
  ...
  // pi-ai's Anthropic OAuth transport remaps tool names to LADA-style names
