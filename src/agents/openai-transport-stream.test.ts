...
describe("openai transport stream", () => {
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("lada-openai-responses-transport");
    expect(prepared).toMatchObject({
      api: "lada-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("lada-anthropic-messages-transport");
    expect(prepared).toMatchObject({
      api: "lada-anthropic-messages-transport",
      provider: "anthropic",
      id: "lada-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("prepares a Google simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "lada-google-generative-ai-transport",
    );
    expect(prepared).toMatchObject({
      api: "lada-google-generative-ai-transport",
      provider: "google",
      id: "gemini-3.1-pro-preview",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("keeps github-copilot LADA models on the shared Anthropic transport seam", () => {
    const model = attachModelProviderRequestTransport(
      {
        id: "lada-sonnet-4.6",
        name: "LADA Sonnet 4.6",
    ...
    void streamFn!(
      {
        id: "anthropic/lada-sonnet-4",
        name: "LADA Sonnet 4",
        api: "lada-openai-completions-transport",
        provider: "openrouter",
        baseUrl: "https://proxy.example.com/v1",
        reasoning: true,
        input: ["text"],
        ...
        contextWindow: 200000,
        maxTokens: 8192,
      } as Model<"lada-openai-completions-transport">,
      {
        systemPrompt: "system",
        messages: [],
        tools: [],
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        lada_session_id: "session-123",
        lada_turn_id: "turn-123",
        lada_turn_attempt: "1",
        lada_transport: "stream",
      },
