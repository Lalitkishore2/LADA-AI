...
const defaultLADAToolsDeps: LADAToolsDeps = {
  callGateway,
};

let openClawToolsDeps: LADAToolsDeps = defaultLADAToolsDeps;
...
function isExperimentalPlanToolEnabled(config?: LADAConfig): boolean {
  return config?.tools?.experimental?.planTool === true;
}

export function createLADATools(
  options?: {
  ...
  const wrappedPluginTools = resolveLADAPluginToolsForOptions({
    options,
    resolvedConfig,
...
export const __testing = {
  setDepsForTest(overrides?: Partial<LADAToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultLADAToolsDeps,
          ...overrides,
        }
      : defaultLADAToolsDeps;
  },
