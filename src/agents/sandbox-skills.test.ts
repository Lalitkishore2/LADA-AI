...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "lada-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.LADA_BUNDLED_SKILLS_DIR = bundledDir;
