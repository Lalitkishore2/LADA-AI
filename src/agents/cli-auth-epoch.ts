...
import {
  ...
  type LADACliCredential,
  type CodexCliCredential,
} from "./cli-credentials.js";

type CliAuthEpochDeps = {
  readLADACliCredentialsCached: typeof readLADACliCredentialsCached;
...
function encodeLADACredential(credential: LADACliCredential): string {
...
function getLocalCliCredentialFingerprint(provider: string): string | undefined {
  switch (provider) {
    case "lada-cli": {
      const credential = cliAuthEpochDeps.readLADACliCredentialsCached({
        ttlMs: 5000,
        allowKeychainPrompt: false,
      });
      return credential ? hashCliAuthEpochPart(encodeLADACredential(credential)) : undefined;
    }
    case "codex-cli": {
