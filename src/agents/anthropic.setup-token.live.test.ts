...
import { resolveLADAAgentDir } from "./agent-paths.js";
import {
...
import { ensureLADAModelsJson } from "./models-config.js";
...
const SETUP_TOKEN_MODEL = process.env.LADA_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "LADA_LIVE_SETUP_TOKEN did not look like a setup-token. Use LADA_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set LADA_LIVE_SETUP_TOKEN_VALUE or LADA_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
...
describe("pickModel", () => {
  it("resolves sonnet-4.6 aliases to lada-sonnet-4-6", () => {
    const model = pickModel(
      [buildTestModel("lada-opus-4-6"), buildTestModel("lada-sonnet-4-6")],
      "sonnet-4.6",
    );
    expect(model?.id).toBe("lada-sonnet-4-6");
  });

  it("resolves opus-4.6 aliases to lada-opus-4-6", () => {
    const model = pickModel(
      [buildTestModel("lada-sonnet-4-6"), buildTestModel("lada-opus-4-6")],
      "opus-4.6",
    );
    expect(model?.id).toBe("lada-opus-4-6");
  });
});
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureLADAModelsJson(cfg, tokenSource.agentDir);
