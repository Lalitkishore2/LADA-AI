...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.lada/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "lada doctor" and "lada approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
