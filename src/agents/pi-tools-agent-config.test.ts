...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-pi-tools-"));
    ...
    try {
      ...
      const tools = createLADACodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: LADAConfig) {
    return createLADACodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<LADAConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<LADAConfig["agents"]>["list"]>[number]["tools"];
  }): LADAConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should apply provider-specific tool policy", () => {
    ...
    const tools = createLADACodingTools({
      ...
      modelId: "lada-opus-4-6-thinking",
    });
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createLADACodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createLADACodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createLADACodingTools({
      config: cfg,
    ...
    const defaultTools = createLADACodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createLADACodingTools({
      config: cfg,
    ...
    const bobTools = createLADACodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createLADACodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createLADACodingTools({
      config: cfg,
