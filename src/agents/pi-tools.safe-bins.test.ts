...
const bundledPluginsDirSnapshot = captureEnv(["LADA_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.LADA_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
const { createLADACodingTools } = await import("./pi-tools.js");
...
describe("createLADACodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lada-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lada-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lada-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "lada-safe-bins-redirect-",
        safeBins: ["head"],
