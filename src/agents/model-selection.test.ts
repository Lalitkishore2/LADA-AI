...
const BUNDLED_ALLOWLIST_CATALOG = [
  { provider: "anthropic", id: "lada-sonnet-4-6", name: "LADA Sonnet 4.5" },
...
function resolveAnthropicOpusThinking(cfg: LADAConfig) {
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<LADAConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<LADAConfig>) {
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("lada-cli", {} as LADAConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as LADAConfig)).toBe(false);
    });
  });
  ...
  describe("parseModelRef", () => {
    ...
    it.each([
      {
        ...
        variants: ["anthropic/lada-3-5-sonnet"],
        defaultProvider: "openai",
        expected: { provider: "anthropic", model: "lada-3-5-sonnet" },
      },
      {
        name: "uses the default provider when omitted",
        variants: ["lada-3-5-sonnet"],
      ...
      },
      {
        ...
        defaultProvider: "anthropic",
        expected: { provider: "anthropic", model: "lada-opus-4-6" },
      },
      {
        name: "normalizes anthropic sonnet aliases",
        variants: ["anthropic/sonnet-4.6", "sonnet-4.6"],
        defaultProvider: "anthropic",
        expected: { provider: "anthropic", model: "lada-sonnet-4-6" },
      },
      {
        name: "keeps dated anthropic model ids unchanged",
        variants: ["anthropic/lada-sonnet-4-20250514", "lada-sonnet-4-20250514"],
        defaultProvider: "anthropic",
        expected: { provider: "anthropic", model: "lada-sonnet-4-20250514" },
      },
      {
      ...
      },
      {
        ...
        variants: ["openrouter/anthropic/lada-sonnet-4-6"],
        defaultProvider: "openai",
        expected: { provider: "openrouter", model: "anthropic/lada-sonnet-4-6" },
      },
      {
      ...
      {
        name: "normalizes Vercel LADA shorthand to anthropic-prefixed model ids",
        variants: ["vercel-ai-gateway/lada-opus-4.6"],
        defaultProvider: "openai",
        expected: { provider: "vercel-ai-gateway", model: "anthropic/lada-opus-4.6" },
      },
      {
      ...
      },
      {
        ...
        variants: ["vercel-ai-gateway/anthropic/lada-opus-4.6"],
        defaultProvider: "openai",
        expected: { provider: "vercel-ai-gateway", model: "anthropic/lada-opus-4.6" },
      },
      {
        name: "passes through non-LADA Vercel model ids unchanged",
    ...
    it("round-trips normalized refs through modelKey", () => {
      ...
      expect(parsed).toEqual({ provider: "anthropic", model: "lada-opus-4-6" });
    ...
    it("preserves explicit runtime provider for vendor-prefixed model ids", () => {
      expect(
        resolvePersistedModelRef({
          ...
          runtimeModel: "anthropic/lada-haiku-4.5",
        }),
      ).toEqual({
        provider: "openrouter",
        model: "anthropic/lada-haiku-4.5",
      });
    });
    ...
    it("preserves explicit wrapper providers for vendor-prefixed override models", () => {
      expect(
        resolvePersistedSelectedModelRef({
          ...
          overrideModel: "anthropic/lada-haiku-4.5",
        }),
      ).toEqual({
        provider: "openrouter",
        model: "anthropic/lada-haiku-4.5",
  ...
  describe("inferUniqueProviderFromConfiguredModels", () => {
    ...
    it("returns undefined for provider-prefixed model ids", () => {
      ...
      expect(
        inferUniqueProviderFromConfiguredModels({
          cfg,
          model: "anthropic/lada-sonnet-4-6",
        }),
      ).toBeUndefined();
    });
    ...
    it("infers provider for slash-containing model id when allowlist match is unique", () => {
      const cfg = {
        agents: {
          defaults: {
            models: {
              "vercel-ai-gateway/anthropic/lada-sonnet-4-6": {},
            },
          },
        },
      } as unknown as LADAConfig;
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<LADAConfig> = {
        agents: {
          defaults: {
            models: {
              "anthropic/lada-3-5-sonnet": { alias: "fast" },
      ...
      expect(index.byAlias.get("fast")?.ref).toEqual({
        provider: "anthropic",
        model: "lada-3-5-sonnet",
      });
      expect(index.byAlias.get("smart")?.ref).toEqual({ provider: "openai", model: "gpt-4o" });
      expect(index.byKey.get(modelKey("anthropic", "lada-3-5-sonnet"))).toEqual(["fast"]);
    });
  });
  ...
  describe("buildAllowedModelSet", () => {
    it("keeps explicitly allowlisted models even when missing from bundled catalog", () => {
      ...
      expect(result.allowedKeys.has("anthropic/lada-sonnet-4-6")).toBe(true);
    ...
    it("includes fallback models in allowed set", () => {
      const cfg = createAgentFallbackConfig({
        fallbacks: ["anthropic/lada-sonnet-4-6", "google/gemini-3-pro"],
      });
    ...
    it("prefers per-agent fallback overrides when agentId is provided", () => {
      const cfg = createAgentFallbackConfig({
        ...
        agentFallbacks: ["anthropic/lada-sonnet-4-6"],
      });
  ...
  describe("resolveAllowedModelRef", () => {
    it("accepts explicit allowlist refs absent from bundled catalog", () => {
      const result = resolveAllowedModelRef({
        cfg: EXPLICIT_ALLOWLIST_CONFIG,
        catalog: BUNDLED_ALLOWLIST_CATALOG,
        raw: "anthropic/lada-sonnet-4-6",
        defaultProvider: "openai",
        defaultModel: "gpt-5.4",
      });

      expect(result).toEqual({
        key: "anthropic/lada-sonnet-4-6",
        ref: { provider: "anthropic", model: "lada-sonnet-4-6" },
      });
    });
  ...
  describe("resolveConfiguredModelRef", () => {
    it("should infer the unique provider from configured models for bare defaults", () => {
      ...
      expect(result).toEqual({ provider: "anthropic", model: "lada-opus-4-6" });
    });
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", () => {
      ...
      try {
        ...
        expect(result).toEqual({ provider: "google", model: "lada-3-5-sonnet" });
        expect(warnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Falling back to "google/lada-3-5-sonnet"'),
        );
      } finally {
    ...
    it("sanitizes control characters in providerless-model warnings", () => {
      ...
      try {
        const cfg: Partial<LADAConfig> = {
          agents: {
            defaults: {
              model: { primary: "\u001B[31mlada-3-5-sonnet\nspoof" },
            },
          },
        };
        ...
        expect(result).toEqual({
          provider: "google",
          model: "\u001B[31mlada-3-5-sonnet\nspoof",
        });
        const warning = warnSpy.mock.calls[0]?.[0] as string;
        expect(warning).toContain('Falling back to "google/lada-3-5-sonnet"');
    ...
    it("falls back to low when no provider thinking hook is active", () => {
      ...
      expect(
        resolveThinkingDefault({
          ...
          model: "us.anthropic.lada-sonnet-4-6-v1:0",
          catalog: [
            {
              provider: "amazon-bedrock",
              id: "us.anthropic.lada-sonnet-4-6-v1:0",
              name: "LADA Sonnet 4.6",
              reasoning: true,
            },
          ],
        }),
...
describe("resolveSubagentConfiguredModelSelection", () => {
  it("prefers the agent primary model over agents.defaults.subagents.model", () => {
    const cfg = {
      agents: {
        defaults: {
          model: { primary: "anthropic/lada-sonnet-4-6" },
