...
const LADA_ATTRIBUTION_PRODUCT = "LADA";
const LADA_ATTRIBUTION_ORIGINATOR = "lada";
...
function formatLADAUserAgent(version: string): string {
  return `${LADA_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: LADA_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: LADA_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatLADAUserAgent(identity.version),
    },
  };
}
