...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    "routes enabled bundle MCP config into the lada-cli backend and executes the tool",
    { timeout: E2E_TIMEOUT_MS },
    async () => {
      const { runCliAgent } = await import("./cli-runner.js");
      const envSnapshot = captureEnv(["HOME"]);
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-cli-bundle-mcp-"));
      ...
      const fakeLADAPath = path.join(binDir, "fake-lada.mjs");
      const pluginRoot = path.join(tempHome, ".lada", "extensions", "bundle-probe");
      await fs.mkdir(workspaceDir, { recursive: true });
      await writeBundleProbeMcpServer(serverScriptPath);
      await writeFakeLADACli(fakeLADAPath);
      await writeLADABundle({ pluginRoot, serverScriptPath });
