...
const VideoGenerateToolSchema = Type.Object({
  ...
  filename: Type.Optional(
    Type.String({
      description:
        "Optional output filename hint. LADA preserves the basename and saves under its managed media directory.",
    }),
  ),
  size: Type.Optional(
    Type.String({
  ...
  durationSeconds: Type.Optional(
    Type.Number({
      description:
        "Optional target duration in seconds. LADA may round this to the nearest provider-supported duration.",
      minimum: 1,
    }),
  ),
  audio: Type.Optional(
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: LADAConfig = options?.config ?? loadConfig();
  ...
  return {
    ...
    description:
      "Generate videos using configured providers. Generated videos are saved under LADA-managed media storage and delivered automatically as attachments. Duration requests may be rounded to the nearest provider-supported value.",
