...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-pdf-"));
  try {
...
const ANTHROPIC_PDF_MODEL = "anthropic/lada-opus-4-6";
...
function withPdfModel(primary: string): LADAConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureLADAModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
