...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    lada: process.env.LADA_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.lada === undefined) {
      delete process.env.LADA_GATEWAY_TOKEN;
    } else {
      process.env.LADA_GATEWAY_TOKEN = envSnapshot.lada;
    }
  });
  ...
  it("uses LADA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.LADA_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
