...
function toLADACodeName(name: string): string {
  return CLAUDE_CODE_TOOL_LOOKUP.get(name.toLowerCase()) ?? name;
}

function fromLADACodeName(name: string, tools: Context["tools"] | undefined): string {
      ...
      for (const block of msg.content) {
        ...
        if (block.type === "toolCall") {
          blocks.push({
            type: "tool_use",
            id: block.id,
            name: isOAuthToken ? toLADACodeName(block.name) : block.name,
...
function convertAnthropicTools(tools: Context["tools"], isOAuthToken: boolean) {
  ...
  return tools.map((tool) => ({
    name: isOAuthToken ? toLADACodeName(tool.name) : tool.name,
        ...
        defaultHeaders: mergeTransportHeaders(
          {
            accept: "application/json",
            "anthropic-dangerous-direct-browser-access": "true",
            "anthropic-beta": `lada-code-20250219,oauth-2025-04-20,${betaFeatures.join(",")}`,
            "user-agent": `lada-cli/${CLAUDE_CODE_VERSION}`,
            "x-app": "cli",
          },
          model.headers,
          options?.headers,
...
function buildAnthropicParams(
  ...
  if (isOAuthToken) {
    params.system = [
      {
        type: "text",
        text: "You are LADA, Anthropic's official CLI for LADA.",
      },
              ...
              const block: TransportContentBlock = {
                ...
                name:
                  typeof contentBlock.name === "string"
                    ? isOAuthToken
                      ? fromLADACodeName(contentBlock.name, context.tools)
                      : contentBlock.name
                    : "",
                arguments:
