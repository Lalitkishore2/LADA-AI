...
function expectFallbackUsed(
  ...
  expect(run).toHaveBeenCalledWith("anthropic", "lada-haiku-3-5");
    ...
    agents: {
      defaults: {
        model: {
          ...
          fallbacks: ["anthropic/lada-haiku-3-5", "google/gemini-2-flash"],
        },
      },
    },
...
describe("runWithModelFallback – probe logic", () => {
  ...
  it("logs primary metadata on probe success and failure fallback decisions", async () => {
    ...
    setLoggerOverride({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `lada-model-fallback-probe-${Date.now()}.log`),
    });
    ...
    expect(fallbackRun).toHaveBeenNthCalledWith(2, "anthropic", "lada-haiku-3-5");
    ...
    expect(decisionPayloads).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          nextCandidateModel: "lada-haiku-3-5",
        }),
        expect.objectContaining({
          event: "model_fallback_decision",
          decision: "candidate_succeeded",
          candidateProvider: "anthropic",
          candidateModel: "lada-haiku-3-5",
          isPrimary: false,
      ...
      agents: {
        defaults: {
          model: {
            ...
            fallbacks: ["anthropic/lada-haiku-3-5", "deepseek/deepseek-chat"],
          },
        },
      },
    } as Partial<LADAConfig>);
