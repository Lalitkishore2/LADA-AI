...
import { markLADAExecEnv } from "../../infra/lada-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "lada.sandbox=1");
  args.push("--label", `lada.sessionKey=${params.scopeKey}`);
  args.push("--label", `lada.createdAtMs=${createdAtMs}`);
  args.push("--label", `lada.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `lada.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markLADAExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "lada.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`lada sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`lada sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("lada sandbox recreate --all");
}
