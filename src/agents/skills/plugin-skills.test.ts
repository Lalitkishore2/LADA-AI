    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "lada.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "lada.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "lada.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("lada-");
  const acpxRoot = await tempDirs.make("lada-acpx-plugin-");
  const helperRoot = await tempDirs.make("lada-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("lada-");
  const pluginRoot = await tempDirs.make("lada-plugin-");
  const outsideDir = await tempDirs.make("lada-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves LADA bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("lada-");
    const pluginRoot = await tempDirs.make("lada-lada-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("lada-");
    const pluginRoot = await tempDirs.make("lada-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
