...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["LADA_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns LADA_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-bundled-override-"));
    process.env.LADA_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "lada-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "lada" }));
