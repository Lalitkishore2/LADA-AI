...
setCliRunnerPrepareTestDeps({
  ...
  resolveLADADocsPath: async () => null,
});
...
export async function setupLADACliRunnerTestModule() {
  const runCliAgent = await setupCliRunnerTestModule();
  return (params: Parameters<typeof import("./lada-cli-runner.js").runLADACliAgent>[0]) =>
    runCliAgent({
      ...params,
      provider: params.provider ?? "lada-cli",
    });
}
