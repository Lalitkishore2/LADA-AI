...
function setConfigOverride(next: LADAConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/lada/lada/issues/31298
    previousFastTestEnv = process.env.LADA_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.LADA_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // LADA_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("LADA runtime context (internal):");
