...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousLADAAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousLADAAgentDir = process.env.LADA_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousLADAAgentDir === undefined) {
      delete process.env.LADA_AGENT_DIR;
    } else {
      process.env.LADA_AGENT_DIR = previousLADAAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "LADA_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.LADA_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.LADA_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: LADAConfig = {
  models: {
    providers: {
      "custom-proxy": {
