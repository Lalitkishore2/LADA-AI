...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## LADA CLI Quick Reference");
    expect(prompt).toContain("lada gateway restart");
  ...
  it("guides harness requests to ACP thread-bound spawns", () => {
    ...
    expect(prompt).toContain(
      'For requests like "do this in codex/lada/cursor/gemini" or similar ACP harnesses, treat it as ACP harness intent',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).not.toContain(
      'For requests like "do this in codex/lada/gemini", treat it as ACP harness intent',
    );
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("LADA docs: /tmp/lada/docs");
    expect(prompt).toContain(
      "For LADA behavior, commands, config, or architecture: consult local docs first.",
    );
  });
  ...
  it("includes model alias guidance when aliases are provided", () => {
    ...
    expect(prompt).toContain("- Opus: anthropic/lada-opus-4-6");
  });

  it("adds LADABot self-update guidance when gateway tool is available", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/lada",
      toolNames: ["gateway", "exec"],
    });

    expect(prompt).toContain("## LADA Self-Update");
  ...
  it("builds runtime line with agent and channel details", () => {
    const line = buildRuntimeLine(
      {
        ...
        node: "v20",
        model: "anthropic/lada",
        defaultModel: "anthropic/lada-opus-4-6",
      },
      "telegram",
      ["inlineButtons"],
      "low",
    ...
    expect(line).toContain("model=anthropic/lada");
    expect(line).toContain("default_model=anthropic/lada-opus-4-6");
  ...
  it("keeps semantically equivalent structured prompt inputs byte-stable", () => {
    const clean = buildAgentSystemPrompt({
      ...
      modelAliasLines: ["- Sonnet: anthropic/lada-sonnet-4-5"],
    });
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/lada. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("sessions_spawn");
    expect(prompt).toContain('runtime: "acp"');
    expect(prompt).toContain("For ACP harness sessions (codex/ladacode/gemini)");
    ...
    expect(prompt).toContain("Do not use `exec` (`lada ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for LADA subagents");
  ...
  it("omits ACP spawning guidance when ACP is disabled", () => {
    ...
    expect(prompt).not.toContain("For ACP harness sessions (codex/ladacode/gemini)");
