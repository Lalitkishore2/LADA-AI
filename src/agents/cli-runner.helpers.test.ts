...
import { resolvePreferredLADATmpDir } from "../infra/tmp-lada-dir.js";
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-write-images-"),
    );
    ...
    try {
      ...
      expect(first.paths[0]).toContain(`${resolvePreferredLADATmpDir()}/lada-cli-images/`);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv[imageArgIndex + 1]).toContain("lada-cli-images");
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-prompt-image-generic-"),
    );
    const sourceImage = path.join(tempDir, "lada-image.png");
    await fs.writeFile(
      sourceImage,
      Buffer.from(
        "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/woAAn8B9FD5fHAAAAAASUVORK5CYII=",
    ...
    try {
      ...
      expect(promptWithImages).toContain("lada-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".lada-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredLADATmpDir(), "lada-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("lada-cli-images");
...
describe("resolveCliRunQueueKey", () => {
  it("scopes LADA CLI serialization to the workspace for fresh runs", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "lada-cli",
        serialize: true,
    ...
    ).toBe("lada-cli:workspace:/tmp/project-a");
  });

  it("scopes LADA CLI serialization to the resumed CLI session id", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "lada-cli",
        serialize: true,
        runId: "run-2",
        workspaceDir: "/tmp/project-a",
        cliSessionId: "lada-session-123",
      }),
    ).toBe("lada-cli:session:lada-session-123");
  });

  it("keeps non-LADA backends on the provider lane when serialized", () => {
    expect(
      resolveCliRunQueueKey({
        backendId: "codex-cli",
        serialize: true,
