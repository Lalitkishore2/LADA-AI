...
const planLADAModelsJsonMock = vi.fn();
...
beforeEach(async () => {
  ...
  vi.doMock("./models-config.plan.js", () => ({
    planLADAModelsJson: (...args: unknown[]) => planLADAModelsJsonMock(...args),
  }));
  ({ ensureLADAModelsJson } = await import("./models-config.js"));
});
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureLADAModelsJson(first), ensureLADAModelsJson(second)]);
      } finally {
