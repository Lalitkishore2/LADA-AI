...
type ModelsConfig = NonNullable<LADAConfig["models"]>;
...
function resolveExplicitBaseUrlProviders(
  providers: LADAConfig["models"] | undefined,
): ReadonlySet<string> {
  return new Set(
...
export async function planLADAModelsJsonWithDeps(
  params: {
    cfg: LADAConfig;
...
export async function planLADAModelsJson(
  params: Parameters<typeof planLADAModelsJsonWithDeps>[0],
): Promise<ModelsJsonPlan> {
  return planLADAModelsJsonWithDeps(params);
}
