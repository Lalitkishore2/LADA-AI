...
function injectLADAMcpConfigArgs(args: string[] | undefined, mcpConfigPath: string): string[] {
  const next: string[] = [];
...
async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-gemini-mcp-"));
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-cli-mcp-"));
  const mcpConfigPath = path.join(tempDir, "mcp.json");
  await fs.writeFile(mcpConfigPath, serializedConfig, "utf-8");
  return {
    backend: {
      ...params.backend,
      args: injectLADAMcpConfigArgs(params.backend.args, mcpConfigPath),
      resumeArgs: injectLADAMcpConfigArgs(
        params.backend.resumeArgs ?? params.backend.args ?? [],
        mcpConfigPath,
      ),
    },
