...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await tempHarness.createTempDir("lada-cli-bundle-mcp-empty-");

    const prepared = await prepareCliBundleMcpConfig({
      enabled: true,
      mode: "lada-config-file",
      backend: {
        command: "node",
        args: ["./fake-lada.mjs"],
      },
  ...
  it("injects a merged --mcp-config overlay for bundle-MCP-enabled backends", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("lada-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("lada-cli-bundle-mcp-workspace-");
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await tempHarness.createTempDir("lada-cli-bundle-mcp-workspace-root-");
    const pluginRoot = path.join(workspaceDir, ".lada", "extensions", "workspace-probe");
    const serverPath = path.join(pluginRoot, "servers", "probe.mjs");
    await fs.mkdir(path.dirname(serverPath), { recursive: true });
    await fs.writeFile(serverPath, "export {};\n", "utf-8");
    await writeLADABundleManifest({
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    try {
      ...
      expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "lada"]);
      expect(raw.mcpServers?.lada?.url).toBe("http://127.0.0.1:23119/mcp");
      expect(raw.mcpServers?.lada?.headers?.Authorization).toBe("Bearer ${LADA_MCP_TOKEN}");
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await tempHarness.createTempDir("lada-cli-bundle-mcp-env-");
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: {},
      env: {
        LADA_MCP_TOKEN: "loopback-token-123",
        LADA_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/lada-bundle-mcp-disabled",
    });
  ...
  it("injects codex MCP config overrides with env-backed loopback headers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/lada-bundle-mcp-codex",
      additionalConfig: {
        mcpServers: {
          lada: {
            type: "http",
    ...
    expect(prepared.backend.args).toEqual([
      ...
      "--json",
      "-c",
      'mcp_servers={ lada = { url = "http://127.0.0.1:23119/mcp", bearer_token_env_var = "LADA_MCP_TOKEN", env_http_headers = { x-session-key = "LADA_MCP_SESSION_KEY" } } }',
    ]);
  ...
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/lada-bundle-mcp-gemini",
      additionalConfig: {
        mcpServers: {
          lada: {
            type: "http",
    ...
    expect(prepared.env?.LADA_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["lada"]);
    expect(raw.mcpServers?.lada?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.lada?.headers?.Authorization).toBe("Bearer loopback-token-123");
