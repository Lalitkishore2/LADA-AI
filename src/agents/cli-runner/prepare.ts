...
const prepareDeps = {
  ...
  getActiveMcpLoopbackRuntime,
  createMcpLoopbackServerConfig,
  resolveLADADocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveLADADocsPath>[0],
  ) => (await import("../docs-path.js")).resolveLADADocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          LADA_MCP_TOKEN: mcpLoopbackRuntime.token,
          LADA_MCP_AGENT_ID: sessionAgentId ?? "",
          LADA_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          LADA_MCP_SESSION_KEY: params.sessionKey ?? "",
          LADA_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const docsPath = await prepareDeps.resolveLADADocsPath({
    workspaceDir,
