...
import { KeyedAsyncQueue } from "lada/plugin-sdk/keyed-async-queue";
...
function isLADACliProvider(providerId: string): boolean {
  return normalizeOptionalLowercaseString(providerId) === "lada-cli";
}
...
export function resolveCliRunQueueKey(params: {
  ...
  if (isLADACliProvider(params.backendId)) {
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredLADATmpDir(), "lada-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".lada-cli-images");
  }
  return path.join(resolvePreferredLADATmpDir(), "lada-cli-images");
}
...
export async function writeCliImages(params: {
  ...
  // Keep content-addressed image paths stable across LADA CLI runs so prompt
