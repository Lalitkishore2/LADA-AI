...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.LADA_CACHE_TRACE,
      file: process.env.LADA_CACHE_TRACE_FILE,
      messages: process.env.LADA_CACHE_TRACE_MESSAGES,
      prompt: process.env.LADA_CACHE_TRACE_PROMPT,
      system: process.env.LADA_CACHE_TRACE_SYSTEM,
    };
    process.env.LADA_CACHE_TRACE = "1";
    process.env.LADA_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.LADA_CACHE_TRACE_MESSAGES = "0";
    process.env.LADA_CACHE_TRACE_PROMPT = "0";
    process.env.LADA_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      const restore = (
        key:
          | "LADA_CACHE_TRACE"
          | "LADA_CACHE_TRACE_FILE"
          | "LADA_CACHE_TRACE_MESSAGES"
          | "LADA_CACHE_TRACE_PROMPT"
          | "LADA_CACHE_TRACE_SYSTEM",
      ...
      restore("LADA_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("LADA_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("LADA_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("LADA_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("LADA_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "LADA_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "LADA_LIVE_ANTHROPIC_CACHE_MODEL",
        preferredModelIds: ["lada-sonnet-4-6", "lada-sonnet-4-6", "lada-haiku-3-5"],
      });
