...
describe("cli-session helpers", () => {
  it("persists binding metadata alongside legacy session ids", () => {
    ...
    setCliSessionBinding(entry, "lada-cli", {
    ...
    expect(entry.cliSessionIds?.["lada-cli"]).toBe("cli-session-1");
    expect(entry.ladaCliSessionId).toBe("cli-session-1");
    expect(getCliSessionBinding(entry, "lada-cli")).toEqual({
      sessionId: "cli-session-1",
      authProfileId: "anthropic:work",
      authEpoch: "auth-epoch",
      extraSystemPromptHash: "prompt-hash",
  ...
  it("keeps legacy bindings reusable until richer metadata is persisted", () => {
    const entry: SessionEntry = {
      sessionId: "lada-session",
      updatedAt: Date.now(),
      cliSessionIds: { "lada-cli": "legacy-session" },
      ladaCliSessionId: "legacy-session",
    };

    expect(resolveCliSessionReuse({ binding: getCliSessionBinding(entry, "lada-cli") })).toEqual({
  ...
  it("invalidates legacy bindings when auth, prompt, or MCP state changes", () => {
    ...
    const binding = getCliSessionBinding(entry, "lada-cli");
  ...
  it("clears provider-scoped and global CLI session state", () => {
    ...
    setCliSessionBinding(entry, "lada-cli", { sessionId: "lada-session" });
    ...
    expect(getCliSessionBinding(entry, "lada-cli")?.sessionId).toBe("lada-session");

    clearAllCliSessions(entry);
    expect(entry.cliSessionBindings).toBeUndefined();
    expect(entry.cliSessionIds).toBeUndefined();
    expect(entry.ladaCliSessionId).toBeUndefined();
  });
