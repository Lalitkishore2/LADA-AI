...
describe("applyExtraParamsToAgent", () => {
  ...
  it("does not rewrite anthropic tool schema for non-kimi endpoints", () => {
    ...
    applyExtraParamsToAgent(agent, undefined, "anthropic", "lada-sonnet-4-6", undefined, "low");
  ...
  it("keeps Anthropic Bedrock models eligible for provider-side caching", () => {
    ...
    applyExtraParamsToAgent(agent, undefined, "amazon-bedrock", "us.anthropic.lada-sonnet-4-5");

    const model = {
      api: "openai-completions",
      provider: "amazon-bedrock",
      id: "us.anthropic.lada-sonnet-4-5",
    } as Model<"openai-completions">;
    const context: Context = { messages: [] };
  ...
  it("passes through explicit cacheRetention for Anthropic Bedrock models", () => {
    ...
    const cfg = {
      agents: {
        defaults: {
          models: {
            "amazon-bedrock/us.anthropic.lada-opus-4-6-v1": {
              params: {
                cacheRetention: "long",
              },
            },
    ...
    applyExtraParamsToAgent(agent, cfg, "amazon-bedrock", "us.anthropic.lada-opus-4-6-v1");

    const model = {
      api: "openai-completions",
      provider: "amazon-bedrock",
      id: "us.anthropic.lada-opus-4-6-v1",
    } as Model<"openai-completions">;
    const context: Context = { messages: [] };
  ...
  it("adds Anthropic 1M beta header when context1m is enabled for Opus/Sonnet", () => {
    const { calls, agent } = createOptionsCaptureAgent();
    const cfg = buildAnthropicModelConfig("anthropic/lada-opus-4-6", { context1m: true });

    applyExtraParamsToAgent(agent, cfg, "anthropic", "lada-opus-4-6");
  ...
  it("does not add Anthropic 1M beta header when context1m is not enabled", () => {
    const cfg = buildAnthropicModelConfig("anthropic/lada-opus-4-6", {
      temperature: 0.2,
    });
    const headers = runAnthropicHeaderCase({
      cfg,
      modelId: "lada-opus-4-6",
  ...
  it("skips context1m beta for OAuth tokens but preserves OAuth-required betas", () => {
    ...
    applyExtraParamsToAgent(agent, cfg, "anthropic", "lada-sonnet-4-6");
  ...
  it("ignores context1m for non-Opus/Sonnet Anthropic models", () => {
    const cfg = buildAnthropicModelConfig("anthropic/lada-haiku-3-5", { context1m: true });
    const headers = runAnthropicHeaderCase({
      cfg,
      modelId: "lada-haiku-3-5",
      options: { headers: { "X-Custom": "1" } },
    });
    expect(headers).toEqual({ "X-Custom": "1" });
  });
