...
function createConfig(overrides?: Record<string, unknown>): LADAConfig {
  return {
  ...
  it("prefers per-agent subagent model over defaults", () => {
    const cfg = createConfig({
      agents: {
        ...
        list: [{ id: "research", subagents: { model: "opencode/lada" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      subagents: { model: "opencode/lada" },
    };
    const plan = resolveSubagentModelAndThinkingPlan({
      cfg,
      targetAgentId: "research",
    ...
    expect(plan).toMatchObject({
      status: "ok",
      resolvedModel: "opencode/lada",
      initialSessionPatch: { model: "opencode/lada" },
    });
  });
  ...
  it("prefers target agent primary model over global default", () => {
    const cfg = createConfig({
      agents: {
        ...
        list: [{ id: "research", model: { primary: "opencode/lada" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      model: { primary: "opencode/lada" },
    };
    const plan = resolveSubagentModelAndThinkingPlan({
      cfg,
      targetAgentId: "research",
