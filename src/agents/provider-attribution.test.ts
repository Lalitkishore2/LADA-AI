...
describe("provider attribution", () => {
  it("resolves the canonical LADA product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in LADA runtime wrapper.",
      product: "LADA",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://lada.ai",
        "X-OpenRouter-Title": "LADA",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { LADA_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "lada",
        version: "2026.3.22",
        "User-Agent": "lada/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { LADA_VERSION: "2026.3.22" })).toEqual({
      originator: "lada",
      version: "2026.3.22",
      "User-Agent": "lada/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { LADA_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ LADA_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
