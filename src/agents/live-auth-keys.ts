...
const GOOGLE_LIVE_SINGLE_KEY = "LADA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "LADA_LIVE_ANTHROPIC_KEY",
    listVar: "LADA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "LADA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `LADA_LIVE_${base}_KEY`;
