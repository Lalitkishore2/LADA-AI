...
function isHighSignalLADAModelId(id: string): boolean {
  const normalized = id.replace(/[_.]/g, "-");
  if (!/\blada\b/i.test(normalized)) {
    return true;
  }
  if (/\bhaiku\b/i.test(normalized)) {
    return false;
  }
  if (/\blada-3(?:[-.]5|[-.]7)\b/i.test(normalized)) {
    return false;
  }
  const versionMatch = normalized.match(/\blada-[a-z0-9-]*?-(\d+)(?:-(\d+))?(?:\b|[-])/i);
  if (!versionMatch) {
    return false;
  }
...
export function isHighSignalLiveModelRef(ref: ModelRef): boolean {
  ...
  return isHighSignalLADAModelId(id);
}
