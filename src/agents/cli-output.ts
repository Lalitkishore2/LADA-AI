...
function isLADACliProvider(providerId: string): boolean {
  return normalizeLowercaseStringOrEmpty(providerId) === "lada-cli";
}
...
function parseLADACliJsonlResult(params: {
  providerId: string;
  parsed: Record<string, unknown>;
  sessionId?: string;
  usage?: CliUsage;
}): CliOutput | null {
  if (!isLADACliProvider(params.providerId)) {
    return null;
  }
  if (
    ...
    // LADA may finish with an empty result after tool-only work. Keep the
...
function parseLADACliStreamingDelta(params: {
  providerId: string;
  parsed: Record<string, unknown>;
  textSoFar: string;
  sessionId?: string;
...
export function createCliJsonlStreamingParser(params: {
  ...
  const handleParsedRecord = (parsed: Record<string, unknown>) => {
    ...
    const delta = parseLADACliStreamingDelta({
...
export function parseCliJsonl(
  ...
  for (const line of lines) {
    for (const parsed of parseJsonRecordCandidates(line)) {
      ...
      const ladaResult = parseLADACliJsonlResult({
        providerId,
        parsed,
        sessionId,
        usage,
