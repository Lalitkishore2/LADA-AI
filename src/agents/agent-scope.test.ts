...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: LADAConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
            fallbacks: ["anthropic/lada-sonnet-4-6"],
          },
        },
        list: [{ id: "main" }],
      },
    ...
    const cfgNoDefaults: LADAConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    expect(resolveAgentModelPrimary(cfg, "linus")).toBe("anthropic/lada-sonnet-4-6");
    expect(resolveAgentExplicitModelPrimary(cfg, "linus")).toBe("anthropic/lada-sonnet-4-6");
    expect(resolveAgentEffectiveModelPrimary(cfg, "linus")).toBe("anthropic/lada-sonnet-4-6");
    expect(resolveAgentModelFallbacksOverride(cfg, "linus")).toEqual(["openai/gpt-5.4"]);

    // If fallbacks isn't present, we don't override the global fallbacks.
    const cfgNoOverride: LADAConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: LADAConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: LADAConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: LADAConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/lada");
  });

  it("uses LADA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "lada-home");
    vi.stubEnv("LADA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as LADAConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".lada", "workspace"));
  });

  it("uses LADA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "lada-home");
    vi.stubEnv("LADA_HOME", home);
    // Clear state dir so it falls back to LADA_HOME
    vi.stubEnv("LADA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as LADAConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".lada", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/lada-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/lada-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "lada-agent-scope-"));
