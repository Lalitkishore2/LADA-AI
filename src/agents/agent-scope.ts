...
type AgentEntry = NonNullable<NonNullable<LADAConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: LADAConfig): AgentEntry[] {
...
export function listAgentIds(cfg: LADAConfig): string[] {
...
export function resolveDefaultAgentId(cfg: LADAConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: LADAConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: LADAConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: LADAConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
