...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "lada-auth-merge-"));
    const previousAgentDir = process.env.LADA_AGENT_DIR;
    ...
    try {
      ...
      process.env.LADA_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.LADA_AGENT_DIR = previousAgentDir;
      }
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("lada-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("lada-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "lada-oauth-migrate-"));
    ...
    try {
      ...
      process.env.LADA_STATE_DIR = root;
      process.env.LADA_AGENT_DIR = agentDir;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.LADA_STATE_DIR = previousStateDir;
      }
      if (previousAgentDir === undefined) {
        delete process.env.LADA_AGENT_DIR;
      } else {
  ...
  it("exposes Codex CLI auth without persisting copied tokens into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "lada-codex-external-sync-"));
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "lada-secrets-runtime-"));
    const previousStateDir = process.env.LADA_STATE_DIR;
    try {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("lada-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "lada-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
