...
import { setupLADACliRunnerTestModule, supervisorSpawnMock } from "./cli-runner.test-support.js";
...
let runLADACliAgent: typeof import("./lada-cli-runner.js").runLADACliAgent;

async function loadFreshLADACliRunnerModuleForTest() {
  runLADACliAgent = await setupLADACliRunnerTestModule();
}
...
describe("runLADACliAgent", () => {
  beforeEach(async () => {
    await loadFreshLADACliRunnerModuleForTest();
  ...
  it("starts a new session with --session-id when none is provided", async () => {
    ...
    expect(spawnInput.argv).toContain("lada");
    expect(spawnInput.argv).toContain("--session-id");
    expect(spawnInput.input).toBe("hi");
  });

  it("starts fresh when only a legacy lada session id is provided", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
    ...
    await runLADACliAgent({
      ...
      timeoutMs: 1_000,
      runId: "run-2",
      ladaSessionId: "c9d7b831-1c31-4d22-80b9-1e50ca207d4b",
    });
  ...
  it("serializes concurrent lada-cli runs in the same workspace", async () => {
    ...
    const firstRun = runLADACliAgent({
      sessionId: "s1",
      sessionFile: "/tmp/session.jsonl",
      workspaceDir: "/tmp",
      prompt: "first",
    ...
    const secondRun = runLADACliAgent({
      sessionId: "s2",
      sessionFile: "/tmp/session.jsonl",
      workspaceDir: "/tmp",
      prompt: "second",
  ...
  it("allows concurrent lada-cli runs across different workspaces", async () => {
    const firstDeferred = createDeferred<ReturnType<typeof successExit>>();
    const secondDeferred = createDeferred<ReturnType<typeof successExit>>();
