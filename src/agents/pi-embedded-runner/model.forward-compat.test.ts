...
function createRuntimeHooks() {
  return createProviderRuntimeTestMock({
    handledDynamicProviders: ["anthropic", "lada-cli", "zai", "openai-codex"],
  });
}
...
function runLADACliSonnetForwardCompatFallback() {
  expectResolvedForwardCompatFallbackWithRegistryResult({
    result: resolveModelWithRegistry({
      provider: "lada-cli",
      modelId: "lada-sonnet-4-6",
...
describe("resolveModel forward-compat tail", () => {
  it(
    "builds an anthropic forward-compat fallback for lada-opus-4-6",
    runAnthropicOpusForwardCompatFallback,
  );

  it(
    "builds an anthropic forward-compat fallback for lada-sonnet-4-6",
    runAnthropicSonnetForwardCompatFallback,
  );

  it(
    "preserves the lada-cli provider for anthropic forward-compat fallback models",
    runLADACliSonnetForwardCompatFallback,
  );
