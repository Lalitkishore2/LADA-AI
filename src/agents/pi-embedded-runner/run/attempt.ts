...
import { resolveLADADocsPath } from "../../docs-path.js";
...
import { createLADACodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveLADAAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createLADACodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveLADADocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // Anthropic LADA endpoints can reject replayed `thinking` blocks
      ...
      try {
        ...
        // double-compaction. See: https://github.com/lada/lada/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("lada:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/lada/lada/issues/8643
