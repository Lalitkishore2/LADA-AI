  ...
  switch (params.provider) {
    ...
    case "github-copilot": {
      ...
      return {
        ...
        name: modelId,
        provider: "github-copilot",
        api: lower.includes("lada") ? "anthropic-messages" : "openai-responses",
    ...
    case "anthropic":
    case "lada-cli": {
      if (lower !== "lada-opus-4-6" && lower !== "lada-sonnet-4-6") {
        return undefined;
      }
      const template = findTemplate(
        params,
        "anthropic",
        lower === "lada-opus-4-6" ? ["lada-opus-4-6"] : ["lada-sonnet-4-6"],
      );
      return cloneTemplate(
        template,
        modelId,
    ...
    case "google-antigravity": {
      if (lower !== "lada-opus-4-6-thinking") {
        return undefined;
      }
      return cloneTemplate(
        undefined,
      ...
      switch (params.provider) {
        case "ollama":
          return (
            ...
            'Set OLLAMA_API_KEY="ollama-local" (any value works) or run "lada configure". ' +
            "See: https://docs.lada.ai/providers/ollama"
          );
        case "vllm":
          return (
            "vLLM requires authentication to be registered as a provider. " +
            'Set VLLM_API_KEY (any value works) or run "lada configure". ' +
            "See: https://docs.lada.ai/providers/vllm"
          );
        default:
          return undefined;
      }
