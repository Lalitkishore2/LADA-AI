...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["LADA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-subagent-"));
  ...
  it("uses isolated temp state when LADA_STATE_DIR is unset in tests", async () => {
    delete process.env.LADA_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "lada-test-state"));
  });
});
