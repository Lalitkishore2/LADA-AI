...
export async function writeLADABundle(params: {
  pluginRoot: string;
  serverScriptPath: string;
}): Promise<void> {
  await fs.mkdir(path.join(params.pluginRoot, ".lada-plugin"), { recursive: true });
  await fs.writeFile(
    path.join(params.pluginRoot, ".lada-plugin", "plugin.json"),
...
export async function writeFakeLADACli(filePath: string): Promise<void> {
  await writeExecutable(
    filePath,
    `#!/usr/bin/env node
import fs from "node:fs/promises";
...
const client = new Client({ name: "fake-lada", version: "1.0.0" });
