...
function makeFallbacksOnlyCfg(): LADAConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): LADAConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-auth-"));
...
async function expectFallsBackToHaiku(params: {
  ...
  expect(run.mock.calls[1]?.[1]).toBe("lada-haiku-3-5");
}
...
// https://github.com/lada/lada/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal LADA compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
      ...
      agents: {
        defaults: {
          model: {
            ...
            fallbacks: ["anthropic/lada-haiku-3-5", "openrouter/deepseek-chat"],
          },
        },
      },
    });
  ...
  it("keeps configured fallback chain when current model is a configured fallback", async () => {
    ...
    const run = vi.fn().mockImplementation(async (provider: string, model: string) => {
      if (provider === "anthropic" && model === "lada-haiku-3-5") {
  ...
  it("falls back on unknown model errors", async () => {
    ...
    const run = vi
      .fn()
      .mockRejectedValueOnce(new Error("Unknown model: anthropic/lada-opus-4-6"))
      .mockResolvedValueOnce("ok");
  ...
  it("does not append configured primary when fallbacksOverride is set", async () => {
    ...
    await expect(
      runWithModelFallback({
        ...
        provider: "anthropic",
        model: "lada-opus-4-5",
        fallbacksOverride: ["anthropic/lada-haiku-3-5"],
        run,
      }),
  ...
  it("refreshes cooldown expiry from persisted auth state before fallback summary", async () => {
    ...
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "anthropic/lada-opus-4-5",
            fallbacks: ["openai/gpt-5.2"],
          },
        },
      },
    ...
    await withTempAuthStore(store, async (tempDir) => {
      const run = vi.fn().mockImplementation(async (provider: string, model: string) => {
        if (provider === "anthropic" && model === "lada-opus-4-5") {
  ...
  it("uses fallbacksOverride instead of agents.defaults.model.fallbacks", async () => {
    ...
    expect(calls).toEqual([
      { provider: "anthropic", model: "lada-opus-4-5" },
  ...
  it("treats an empty fallbacksOverride as disabling global fallbacks", async () => {
    ...
    expect(calls).toEqual([{ provider: "anthropic", model: "lada-opus-4-5" }]);
  });
  ...
  it("keeps explicit fallbacks reachable when models allowlist is present", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
            primary: "anthropic/lada-sonnet-4",
            fallbacks: ["openai/gpt-4o", "ollama/llama-3"],
          },
          models: {
            "anthropic/lada-sonnet-4": {},
          },
        },
      },
    });
  ...
  describe("fallback behavior with session model overrides", () => {
    it("allows fallbacks when session model differs from config within same provider", async () => {
        ...
        agents: {
          defaults: {
            model: {
              ...
              fallbacks: ["anthropic/lada-sonnet-4-5", "google/gemini-2.5-flash"],
            },
          },
        },
      });
      ...
      const result = await runWithModelFallback({
        cfg,
        provider: "anthropic",
        model: "lada-sonnet-4-20250514", // Different from config primary
        run,
      });

      expect(result.result).toBe("fallback success");
      expect(run).toHaveBeenCalledTimes(2);
      expect(run).toHaveBeenNthCalledWith(1, "anthropic", "lada-sonnet-4-20250514");
      expect(run).toHaveBeenNthCalledWith(2, "anthropic", "lada-sonnet-4-5"); // Fallback tried
    });
    ...
    it("allows fallbacks with model version differences within same provider", async () => {
      ...
      const result = await runWithModelFallback({
        cfg,
        provider: "anthropic",
        model: "lada-opus-4-5", // Version difference from config
        run,
      });
    ...
    it("still skips fallbacks when using different provider than config", async () => {
      ...
      expect(run).toHaveBeenNthCalledWith(2, "anthropic", "lada-opus-4-6"); // Config primary as final fallback
    });
    ...
    it("uses fallbacks when session model exactly matches config primary", async () => {
      ...
      const result = await runWithModelFallback({
        cfg,
        provider: "anthropic",
        model: "lada-opus-4-6", // Exact match
        run,
      });
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
    ...
    it("attempts same-provider fallbacks during rate limit cooldown", async () => {
      ...
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
              primary: "anthropic/lada-opus-4-6",
              fallbacks: ["anthropic/lada-sonnet-4-5", "groq/llama-3.3-70b-versatile"],
            },
          },
        },
      });
      ...
      expect(run).toHaveBeenNthCalledWith(1, "anthropic", "lada-sonnet-4-5", {
    ...
    it("does not consume transient probe slot when first same-provider probe fails with model_not_found", async () => {
      ...
      const run = vi
        .fn()
        .mockRejectedValueOnce(new Error("Model not found: anthropic/lada-sonnet-4-5"))
      ...
      expect(run).toHaveBeenNthCalledWith(2, "anthropic", "lada-haiku-3-5", {
        allowTransientCooldownProbe: true,
      });
    });
  });
