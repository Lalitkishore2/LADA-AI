...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; LADA handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `LADA docs: ${docsPath}`,
    "Mirror: https://docs.lada.ai",
    "Source: https://github.com/lada/lada",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For LADA behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `lada status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant operating inside LADA.";
  }

  const lines = [
    "You are a personal assistant operating inside LADA.",
    "",
    "## Tooling",
    ...
    ...(acpHarnessSpawnAllowed
      ? [
          'For requests like "do this in codex/lada/cursor/gemini" or similar ACP harnesses, treat it as ACP harness intent and call `sessions_spawn` with `runtime: "acp"`.',
    ...
    ...safetySection,
    "## LADA CLI Quick Reference",
    "LADA is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- lada gateway status",
    "- lada gateway start",
    "- lada gateway stop",
    "- lada gateway restart",
    "If unsure, ask the user to run `lada help` (or `lada gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## LADA Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, LADA pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by LADA and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'LADA treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
