...
const CLAUDE_CLI_BACKEND_ID = "lada-cli";
...
export function getCliSessionBinding(
  ...
  if (normalized === CLAUDE_CLI_BACKEND_ID) {
    const legacy = normalizeOptionalString(entry.ladaCliSessionId);
    if (legacy) {
...
export function setCliSessionBinding(
  ...
  if (normalized === CLAUDE_CLI_BACKEND_ID) {
    entry.ladaCliSessionId = trimmed;
  }
}
