...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".lada", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledPluginsDir = process.env.LADA_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.LADA_DISABLE_BUNDLED_PLUGINS;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-test-"));
  ...
  afterEach(() => {
    ...
    if (previousBundledPluginsDir === undefined) {
      ...
      process.env.LADA_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousDisableBundledPlugins === undefined) {
      delete process.env.LADA_DISABLE_BUNDLED_PLUGINS;
    } else {
      process.env.LADA_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-test-bin-"));
  ...
  it("auto-continues the same Discord session after approval resolves without a second user turn", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-exec-followup-discord-"));
  ...
  it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-exec-followup-sidefx-"));
    const markerPath = path.join(tempDir, "marker.txt");
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".lada", "skills", "gog");
