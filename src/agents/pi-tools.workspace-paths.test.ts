...
import "./test-helpers/fast-lada-tools.js";
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("lada-ws-", async (workspaceDir) => {
      await withTempDir("lada-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createLADACodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "lada" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("lada-ws-", async (workspaceDir) => {
      await withTempDir("lada-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("lada-ws-", async (workspaceDir) => {
      const cfg: LADAConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createLADACodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-lada.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("lada-sandbox-", async (sandboxDir) => {
      await withTempDir("lada-workspace-", async (workspaceDir) => {
        ...
        const tools = createLADACodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
