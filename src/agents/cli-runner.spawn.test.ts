...
function buildPreparedCliRunContext(params: {
  ...
  return {
    ...
    backendResolved: {
      id: params.provider,
      config: backend,
      bundleMcp: params.provider === "lada-cli",
      pluginId: params.provider === "lada-cli" ? "anthropic" : "openai",
    },
    preparedBackend: {
      backend,
      env: {},
...
describe("runCliAgent spawn path", () => {
  ...
  it("pipes LADA prompts over stdin instead of argv", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
  ...
  it("streams LADA text deltas from stream-json stdout", async () => {
  ...
  it("clears lada-cli provider-routing, auth, and telemetry env while keeping host-managed hardening", async () => {
  ...
  it("loads workspace bootstrap files into the LADA CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "lada-cli-bootstrap-context-"),
    );
