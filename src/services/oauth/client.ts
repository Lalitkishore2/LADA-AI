// OAuth client for handling authentication flows with LADA services
...
/**
 * Check if the user has LADA.ai authentication scope
 * @private Only call this if you're OAuth / auth related code!
 */
export function shouldUseLADAAIAuth(scopes: string[] | undefined): boolean {
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithLADAAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithLADAAi
    ? getOauthConfig().CLAUDE_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show LADA Max upsell
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full LADA AI set. The
  ...
  try {
    ...
    const existing = getLADAAIOAuthTokens()
