 ...
 *   GET  /api/lada_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/lada_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/lada_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
import { getLADACodeUserAgent } from '../../utils/userAgent.js'
...
// GB-tunable per-org (lada_code_team_memory_limits), so any compile-time
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/lada_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
  ...
  if (oauthTokens?.accessToken) {
    return {
      headers: {
        ...
        'User-Agent': getLADACodeUserAgent(),
      },
    }
  }
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // lada_code_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/ladamd.js')
