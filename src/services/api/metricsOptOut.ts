...
import { hasProfileScope, isLADAAISubscriber } from '../../utils/auth.js'
...
// N `lada -p` invocations into ~1 API call/day.
...
async function _fetchMetricsEnabled(): Promise<MetricsEnabledResponse> {
  ...
  const endpoint = `https://api.anthropic.com/api/lada_code/organizations/metrics_enabled`
...
export async function checkMetricsEnabled(): Promise<MetricsStatus> {
  ...
  if (isLADAAISubscriber() && !hasProfileScope()) {
    return { enabled: false, hasError: false }
  }
