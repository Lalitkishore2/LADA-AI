...
import { getLADATempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getLADATempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
   ...
   *  (sticky-on latched in lada.ts). Tracked to verify the fix. */
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getLADATempDir(), { recursive: true })
