 ...
 * - OAuth users (LADA.ai): Only Team and Enterprise/C4E subscribers are eligible
...
function getCachePath(): string {
  return join(getLADAConfigHomeDir(), CACHE_FILENAME)
}
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/lada_code/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have LADA.ai tokens
  const tokens = getLADAAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have LADA.ai inference scope
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for LADA.ai users)
