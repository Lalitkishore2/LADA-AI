 ...
 * reply and emits notifications/lada/channel/permission with
 ...
 * capabilities.experimental['lada/channel/permission'].
 *
 * Kenneth's "would this let LADA self-approve?": the approving party is
 * the human via the channel, not LADA. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/lada/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/lada/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['lada/channel'] !== undefined &&
      c.capabilities?.experimental?.['lada/channel/permission'] !== undefined,
  )
}
 ...
 * (notifications/lada/channel/permission) with the structured payload.
