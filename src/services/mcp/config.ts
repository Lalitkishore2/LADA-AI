...
import { isLADAInChromeMCPServer } from '../../utils/ladaInChrome/common.js'
import {
...
import { fetchLADAAIMcpConfigsIfEligible } from './ladaai.js'
...
/**
 * CCR proxy URL path markers. In remote sessions, lada.ai connectors arrive
...
/**
 * Filter lada.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `lada mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `lada.ai <DisplayName>` so they never key-collide with
 ...
 * `mcp__lada_ai_Slack__*` both hitting mcp.slack.com, ~600 chars/turn wasted).
 *
 ...
 */
export function dedupLADAAiMcpServers(
  ladaAiServers: Record<string, ScopedMcpServerConfig>,
  manualServers: Record<string, ScopedMcpServerConfig>,
): {
  servers: Record<string, ScopedMcpServerConfig>
  suppressed: Array<{ name: string; duplicateOf: string }>
  ...
  const servers: Record<string, ScopedMcpServerConfig> = {}
  const suppressed: Array<{ name: string; duplicateOf: string }> = []
  for (const [name, config] of Object.entries(ladaAiServers)) {
    const sig = getMcpServerSignature(config)
    const manualDup = sig !== null ? manualSigs.get(sig) : undefined
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing lada.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
 ...
 * in getLADACodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  // Block reserved server name "lada-in-chrome"
  if (isLADAInChromeMCPServer(name)) {
  ...
  switch (scope) {
    ...
    case 'ladaai':
      throw new Error('Cannot add MCP server to scope: ladaai')
  }
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getLADACodeMcpConfigs().
...
/**
 * Get LADA MCP configurations (excludes lada.ai servers from the
 ...
 * lada.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns LADA server configurations with appropriate scopes
 */
export async function getLADACodeMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including lada.ai servers.
 * This may be slow due to network calls - use getLADACodeMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load lada.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getLADACodeMcpConfigs()
  }

  // Kick off the lada.ai fetch before getLADACodeMcpConfigs so it overlaps
  // with loadAllPluginsCacheOnly() inside. Memoized — the awaited call below is a cache hit.
  const ladaaiPromise = fetchLADAAIMcpConfigsIfEligible()
  const { servers: ladaCodeServers, errors } = await getLADACodeMcpConfigs(
    {},
    ladaaiPromise,
  )
  const { allowed: ladaaiMcpServers } = filterMcpServersByPolicy(
    await ladaaiPromise,
  )

  // Suppress lada.ai connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `lada.ai Slack`) so the merge below
  // won't catch this — need content-based dedup by URL signature.
  const { servers: dedupedLADAAi } = dedupLADAAiMcpServers(
    ladaaiMcpServers,
    ladaCodeServers,
  )

  // Merge with lada.ai having lowest precedence
  const servers = Object.assign({}, dedupedLADAAi, ladaCodeServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.lada.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // what the best way to do this is. In the meantime, we are limiting this to lada-vscode for now to
  ...
  return Object.values(configs).every(
    c => c.type === 'sdk' && c.name === 'lada-vscode',
  )
}
