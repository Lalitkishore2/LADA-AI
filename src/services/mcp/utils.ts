...
import { getGlobalLADAFile } from '../../utils/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalLADAFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalLADAFile()} [project: ${getCwd()}]`
    case 'dynamic':
...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .lada/settings.json).
  ...
  // In non-interactive mode (SDK, lada -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: lada.ai servers have normalized names starting with "lada_ai_"
  // but aren't in getMcpConfigByName (they're fetched async separately)
  if (!serverConfig && mcpInfo.serverName.startsWith('lada_ai_')) {
    return 'ladaai'
  }
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, ladaai-proxy, sse-ide, ws-ide)
