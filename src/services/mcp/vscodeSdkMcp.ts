 ...
 * notify VSCode when files are edited or written by LADA.
 */
...
export function setupVscodeSdkMcp(sdkClients: MCPServerConnection[]): void {
  const client = sdkClients.find(client => client.name === 'lada-vscode')
  ...
  if (client && client.type === 'connected') {
    ...
    const gates: Record<string, boolean | string> = {
      ...
      // In-band OAuth via lada_authenticate (vs. extension-native PKCE).
