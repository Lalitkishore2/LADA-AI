 ...
 * Used by LADAAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like LADA) should authenticate by including
 ...
 * `lada mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'lada mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of LADAAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (LADAAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
  ...
  try {
    ...
    const provider = new LADAAuthProvider(
      serverName,
      serverConfig,
      redirectUri,
      true,
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to LADA.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/lada-code/issues/28258.
 */
export function wrapFetchWithStepUpDetection(
  baseFetch: FetchLike,
  provider: LADAAuthProvider,
): FetchLike {
...
export class LADAAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `LADA (${this.serverName})`,
  ...
  async refreshAuthorization(
    ...
    const lockfilePath = join(ladaDir, `mcp-refresh-${sanitizedKey}.lock`)
