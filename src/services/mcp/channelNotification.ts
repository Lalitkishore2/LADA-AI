 ...
 *   - sends `notifications/lada/channel` notifications for inbound — this file
 *
 ...
 * Requires lada.ai OAuth auth — API key users are blocked until
 ...
 * this declare `capabilities.experimental['lada/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/lada/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/lada/channel/permission_request'
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['lada/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/lada/channel).
  if (!capabilities?.experimental?.['lada/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare lada/channel capability',
    }
  }
  ...
  if (!getLADAAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires lada.ai authentication (run /login)',
    }
  }
