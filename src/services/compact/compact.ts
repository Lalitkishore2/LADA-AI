...
// Skills can be large (verify=18.7KB, lada-api=20.1KB). Previously re-injected
  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in lada.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of lada.md files
  // TODO: Refactor to use isMemoryFilePath() from ladamd.ts for consistency
  // and to also match child directory memory files (.lada/rules/*.md, etc.)
  try {
