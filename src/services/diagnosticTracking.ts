...
import { LADAError } from '../utils/errors.js'
import { normalizePathForComparison, pathsEqual } from '../utils/file.js'
import { getConnectedIdeClient } from '../utils/ide.js'
import { jsonParse } from '../utils/slowOperations.js'

class DiagnosticsTrackingError extends LADAError {}
...
export class DiagnosticTrackingService {
  ...
  // Map<normalizedPath, lastLADAFsRightDiagnostics>
  ...
  /**
   * Get new diagnostics from file://, _lada_fs_right, and _lada_fs_ URIs that aren't in the baseline.
  ...
  async getNewDiagnostics(): Promise<DiagnosticFile[]> {
    ...
    const diagnosticsForLADAFsRightUrisWithBaselinesMap = new Map<
      string,
      DiagnosticFile
    >()
    allDiagnosticFiles
      .filter(file => this.baseline.has(this.normalizeFileUri(file.uri)))
      .filter(file => file.uri.startsWith('_lada_fs_right:'))
      .forEach(file => {
        diagnosticsForLADAFsRightUrisWithBaselinesMap.set(
    ...
    for (const file of diagnosticsForFileUrisWithBaselines) {
      ...
      // Get the _lada_fs_right file if it exists
      const ladaFsRightFile =
        diagnosticsForLADAFsRightUrisWithBaselinesMap.get(normalizedPath)
      ...
      if (ladaFsRightFile) {
        ...
        if (
          ...
          !this.areDiagnosticArraysEqual(
            previousRightDiagnostics,
            ladaFsRightFile.diagnostics,
          )
        ) {
          fileToUse = ladaFsRightFile
        }
