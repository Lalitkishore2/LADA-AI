...
describe("plugin activation boundary", () => {
  ...
  let browserHelpersPromise:
    | Promise<{
        ...
        DEFAULT_LADA_BROWSER_COLOR: typeof import("./plugin-sdk/browser-config.js").DEFAULT_LADA_BROWSER_COLOR;
        DEFAULT_LADA_BROWSER_PROFILE_NAME: typeof import("./plugin-sdk/browser-config.js").DEFAULT_LADA_BROWSER_PROFILE_NAME;
  ...
  function importBrowserHelpers() {
    browserHelpersPromise ??= Promise.all([
      ...
      DEFAULT_LADA_BROWSER_COLOR: config.DEFAULT_LADA_BROWSER_COLOR,
      DEFAULT_LADA_BROWSER_PROFILE_NAME: config.DEFAULT_LADA_BROWSER_PROFILE_NAME,
  ...
  it("keeps browser helper imports cold and loads only narrow browser helper surfaces on use", async () => {
    ...
    expect(browser.DEFAULT_LADA_BROWSER_COLOR).toBe("#FF4500");
    expect(browser.DEFAULT_LADA_BROWSER_PROFILE_NAME).toBe("lada");
    ...
    expect(browser.resolveProfile(resolved, "lada")).toEqual(
