...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when LADA_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".lada", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when LADA_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", LADA_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".lada-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers LADA_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        LADA_PROFILE: "rescue",
        LADA_STATE_DIR: "C:\\State\\lada",
      },
      expected: path.join("C:\\State\\lada", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", LADA_PROFILE: "default" },
      expected: path.join("/home/test", ".lada", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\lada",
          environment: {
            NODE_ENV: "production",
            LADA_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\lada\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\lada\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\LADA Share\\node.exe" "\\\\fileserver\\LADA Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\LADA Share\\node.exe",
            "\\\\fileserver\\LADA Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from LADA_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ LADA_STATE_DIR: path.join(tmpDir, "custom-state") }),
