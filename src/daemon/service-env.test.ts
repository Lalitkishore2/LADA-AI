...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.LADA_GATEWAY_PORT).toBe("18789");
    expect(env.LADA_GATEWAY_TOKEN).toBeUndefined();
    expect(env.LADA_SERVICE_MARKER).toBe("lada");
    expect(env.LADA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.LADA_SERVICE_VERSION).toBe("string");
    expect(env.LADA_SYSTEMD_UNIT).toBe("lada-gateway.service");
    expect(env.LADA_WINDOWS_TASK_NAME).toBe("LADA Gateway");
    if (process.platform === "darwin") {
      expect(env.LADA_LAUNCHD_LABEL).toBe("ai.lada.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", LADA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.LADA_SYSTEMD_UNIT).toBe("lada-gateway-work.service");
    expect(env.LADA_WINDOWS_TASK_NAME).toBe("LADA Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.LADA_LAUNCHD_LABEL).toBe("ai.lada.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through LADA_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", LADA_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.LADA_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits LADA_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        LADA_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".lada"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", LADA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".lada-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", LADA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".lada"));
  });

  it("uses LADA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", LADA_STATE_DIR: "/var/lib/lada" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/lada"));
  });

  it("expands ~ in LADA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", LADA_STATE_DIR: "~/lada-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/lada-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { LADA_STATE_DIR: "C:\\State\\lada" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\lada");
  });
});
