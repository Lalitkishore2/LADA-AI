...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds LADA_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `lada gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service LADA_GATEWAY_TOKEN does not match gateway.auth.token in lada.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.LADA_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.LADA_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `lada gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
