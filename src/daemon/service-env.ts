...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.LADA_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    LADA_PROFILE: profile,
    LADA_GATEWAY_PORT: String(port),
    LADA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    LADA_SYSTEMD_UNIT: systemdUnit,
    LADA_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    LADA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    LADA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    LADA_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.LADA_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    LADA_GATEWAY_TOKEN: gatewayToken,
    LADA_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    LADA_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    LADA_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    LADA_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    LADA_LOG_PREFIX: "node",
    LADA_SERVICE_MARKER: NODE_SERVICE_MARKER,
    LADA_SERVICE_KIND: NODE_SERVICE_KIND,
    LADA_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    LADA_STATE_DIR: sharedEnv.stateDir,
    LADA_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.LADA_STATE_DIR;
  const configPath = env.LADA_CONFIG_PATH;
