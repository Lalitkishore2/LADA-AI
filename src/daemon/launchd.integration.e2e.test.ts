...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `lada-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      LADA_LAUNCHD_LABEL: `ai.lada.launchd-int-${testId}`,
      LADA_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
