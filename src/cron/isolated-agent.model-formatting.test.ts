...
const DEFAULT_MODEL = "lada-opus-4-6";
...
function parseModelRef(raw: string): { provider: string; model: string } | { error: string } {
  ...
  const model = provider === "anthropic" && modelRaw === "opus-4.5" ? "lada-opus-4-6" : modelRaw;
      ...
      await expectSelectedModel(
        {
          payload: {
            kind: "agentTurn",
            message: DEFAULT_MESSAGE,
            model: "bedrock/lada-sonnet-4-6",
          },
        },
        { provider: "amazon-bedrock", model: "lada-sonnet-4-6" },
      );
    });
  });
  ...
  describe("model precedence isolation", () => {
    ...
    it("job payload model wins over conflicting session override", async () => {
      await expectSelectedModel(
        ...
        { provider: "anthropic", model: "lada-sonnet-4-6" },
      );
    });
