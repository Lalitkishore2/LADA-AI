...
describe("applyJobPatch", () => {
  ...
  it("persists agentTurn payload.fallbacks updates when editing existing jobs", () => {
    ...
    applyJobPatch(job, {
      payload: {
        kind: "agentTurn",
        message: "do it",
        fallbacks: ["anthropic/lada-haiku-3-5", "openai/gpt-5"],
      },
    });

    expect(job.payload.kind).toBe("agentTurn");
    if (job.payload.kind === "agentTurn") {
      expect(job.payload.fallbacks).toEqual(["anthropic/lada-haiku-3-5", "openai/gpt-5"]);
    }
  });
  ...
  it("carries payload.fallbacks when replacing payload kind via patch", () => {
    ...
    expect(payload.kind).toBe("agentTurn");
    if (payload.kind === "agentTurn") {
      expect(payload.fallbacks).toEqual(["anthropic/lada-haiku-3-5", "openai/gpt-5"]);
    }
  });
