...
describe("runCronIsolatedAgentTurn — skill filter", () => {
  ...
  function mockCliFallbackInvocation() {
    runWithModelFallbackMock.mockImplementationOnce(
      async (params: { run: (provider: string, model: string) => Promise<unknown> }) => {
        const result = await params.run("lada-cli", "lada-opus-4-6");
        return { result, provider: "lada-cli", model: "lada-opus-4-6", attempts: [] };
      },
    );
  }
  ...
  describe("model fallbacks", () => {
    ...
    it("preserves defaults when agent overrides primary as string", async () => {
      await expectPrimaryOverridePreservesDefaults("anthropic/lada-sonnet-4-6");
    });

    it("preserves defaults when agent overrides primary in object form", async () => {
      await expectPrimaryOverridePreservesDefaults({ primary: "anthropic/lada-sonnet-4-6" });
    });
    ...
    it("applies payload.model override when model is allowed", async () => {
      ...
      const result = await runCronIsolatedAgentTurn(
        makeSkillParams({
          job: makeSkillJob({
            payload: { kind: "agentTurn", message: "test", model: "anthropic/lada-sonnet-4-6" },
          }),
        }),
      );
      ...
      expect(runParams.model).toBe("lada-sonnet-4-6");
    });

    it("falls back to agent defaults when payload.model is not allowed", async () => {
      resolveAllowedModelRefMock.mockReturnValueOnce({
        error: "model not allowed: anthropic/lada-sonnet-4-6",
      });
      ...
      expect(logWarnMock).toHaveBeenCalledWith(
        "cron: payload.model 'anthropic/lada-sonnet-4-6' not allowed, falling back to agent defaults",
      );
      expectDefaultModelCall({
    ...
    it("does not pass stored cliSessionId on fresh isolated runs (isNewSession=true)", async () => {
      ...
      resolveCronSessionMock.mockReturnValue({
        ...
        sessionEntry: {
          ...
          cliSessionIds: { "lada-cli": "prev-cli-session-abc" },
        },
        systemSent: false,
        isNewSession: true,
      });
    ...
    it("reuses stored cliSessionId on continuation runs (isNewSession=false)", async () => {
      ...
      resolveCronSessionMock.mockReturnValue({
        ...
        sessionEntry: {
          ...
          systemSent: false,
          skillsSnapshot: undefined,
          cliSessionIds: { "lada-cli": "existing-cli-session-def" },
        },
        systemSent: false,
        isNewSession: false,
      });
