      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: LADAConfig }) =>
...
function makeCfg(overrides?: Partial<LADAConfig>): LADAConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeTelegramBoundCfg(accountId = "account-b"): LADAConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: LADAConfig) {
