...
describe("cron service timer regressions", () => {
  ...
  it("#24355: one-shot job retries then succeeds", async () => {
    ...
    const overloadedResult = await runRetryScenario({
      ...
      deleteAfterRun: false,
      firstError:
        "All models failed (2): anthropic/lada-3-5-sonnet: LLM error overloaded_error: overloaded (overloaded); openai/gpt-5.4: LLM error overloaded_error: overloaded (overloaded)",
    });
  ...
  it("respects abort signals while retrying one-shot main-session wake-now heartbeat runs", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/lada-cron-abort-test/jobs.json",
      log: noopLogger,
      nowMs: () => Date.now(),
      enqueueSystemEvent,
      requestHeartbeatNow,
  ...
  it("finishes recurring wake-now main jobs quickly when the main lane is busy (#58833)", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/lada-cron-busy-main-test/jobs.json",
      log: noopLogger,
      nowMs,
      enqueueSystemEvent,
      requestHeartbeatNow,
