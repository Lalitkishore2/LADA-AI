...
const { makeStorePath } = createCronStoreHarness({ prefix: "lada-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses LADA_HOME for tilde expansion", () => {
    vi.stubEnv("LADA_HOME", "/srv/lada-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/lada-home", "cron", "jobs.json"));
  });
});
