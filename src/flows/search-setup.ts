...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<LADAConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<LADAConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: LADAConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: LADAConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: LADAConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: LADAConfig, result: LADAConfig): LADAConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: LADAConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as LADAConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with lada configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "LADA will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.lada.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — LADA will store a reference instead of the API key.",
