...
export type PromptDefaultModelResult = { model?: string; config?: LADAConfig };
...
function resolveConfiguredModelRaw(cfg: LADAConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: LADAConfig): string[] {
...
export function applyModelAllowlist(cfg: LADAConfig, models: string[]): LADAConfig {
