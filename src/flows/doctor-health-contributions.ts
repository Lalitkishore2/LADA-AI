...
import { noteLADACliHealth } from "../commands/doctor-lada-cli.js";
...
export function resolveDoctorMode(cfg: LADAConfig): DoctorFlowMode {
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("lada configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("lada config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("lada setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("lada config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("lada config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runLADACliHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  noteLADACliHealth(ctx.cfg);
}
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("lada doctor --fix")}" to apply changes.`);
  }
}
