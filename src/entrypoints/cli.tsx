...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (LADA)`);
    return;
  }
  ...
  if (process.argv[2] === '--lada-in-chrome-mcp') {
    profileCheckpoint('cli_lada_in_chrome_mcp_path');
    const {
      runLADAInChromeMcpServer
    } = await import('../utils/ladaInChrome/mcpServer.js');
    await runLADAInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/ladaInChrome/chromeNativeHost.js');
  ...
  // Fast-path for `lada remote-control` (also accepts legacy `lada remote` / `lada sync` / `lada bridge`):
  ...
  // Fast-path for `lada daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `lada ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.lada/sessions/ registry. Flag
  ...
  // Fast-path for `lada environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `lada self-hosted-runner`: headless self-hosted-runner
