...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('LADA decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls LADA's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
...
export const McpLADAAIProxyServerConfigSchema = lazySchema(() =>
  z.object({
    type: z.literal('ladaai-proxy'),
    url: z.string(),
    id: z.string(),
  }),
)

// Broader config type for status responses (includes ladaai-proxy which is output-only)
export const McpServerStatusConfigSchema = lazySchema(() =>
  z.union([
    McpServerConfigForProcessTransportSchema(),
    McpLADAAIProxyServerConfigSchema(),
  ]),
)
  ...
  z
    .object({
      ...
      scope: z
        ...
        .optional()
        .describe(
          'Configuration scope (e.g., project, user, local, ladaai, managed)',
        ),
      tools: z
        .array(
          z.object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['lada/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (LADA decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      model: z
        ...
        .optional()
        .describe(
          "Model alias (e.g. 'sonnet', 'opus', 'haiku') or full model ID (e.g. 'lada-opus-4-5'). If omitted or 'inherit', uses the main model",
        ),
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.lada/agent-memory/<agentType>/, 'project' - .lada/agent-memory/<agentType>/, 'local' - .lada/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.lada/settings.json). " +
        "'project' - Project settings (.lada/settings.json). " +
        "'local' - Local settings (.lada/settings.local.json).",
    ),
)
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for lada.ai subscription users.'),
)
