/**
 * Main entrypoint for LADA Agent SDK types.
 *
...
/**
 * A scheduled task from `<dir>/.lada/scheduled_tasks.json`.
 * @internal
 */
...
/**
 * Watch `<dir>/.lada/scheduled_tasks.json` and yield events as tasks fire.
 *
...
/**
 * A user message typed on lada.ai, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a lada.ai remote-control bridge connection from a daemon process.
 *
 ...
 * lada.ai keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on lada.ai) into `query()`'s input
