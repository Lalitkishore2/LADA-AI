...
export function resolveConfigDir(
  ...
  const configPath = env.LADA_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".lada");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.LADA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$LADA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via LADA_STATE_DIR.
