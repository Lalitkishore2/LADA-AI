...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "lada-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "lada", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "lada-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "lada", version: "2.3.4" });
  ...
  it("ignores non-lada package and blank build-info versions", async () => {
    await withTempDir({ prefix: "lada-version-" }, async (root) => {
  ...
  it("prefers LADA_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        LADA_VERSION: "9.9.9",
        LADA_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale LADA_VERSION for compatibility checks", () => {
    const previous = process.env.LADA_VERSION;
    const previousService = process.env.LADA_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.LADA_VERSION = "2026.3.25";
      process.env.LADA_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.LADA_VERSION = previous;
      process.env.LADA_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        LADA_VERSION: "2026.3.99",
        LADA_SERVICE_VERSION: "2026.3.98",
