...
/**
 * Integration test: builds a LADA bundle plugin fixture on disk
 ...
 */
describe("LADA bundle plugin inspect integration", () => {
  let rootDir: string;
  ...
  function setupLADAInspectFixture() {
    ...
    writeFixtureEntries({
      ".lada-plugin/plugin.json": {
        name: "Test LADA Plugin",
        description: "Integration test fixture for LADA bundle inspection",
        version: "1.0.0",
        skills: ["skill-packs"],
  ...
  function expectLoadedLADAManifest() {
    const result = loadBundleManifest({ rootDir, bundleFormat: "lada" });
    expect(result.ok).toBe(true);
    if (!result.ok) {
      throw new Error("expected LADA bundle manifest to load");
    }
    return result.manifest;
  }

  function expectLADAManifestField(params: {
    field: "skills" | "hooks" | "settingsFiles" | "capabilities";
    includes: readonly string[];
  }) {
    const manifest = expectLoadedLADAManifest();
  ...
  function inspectLADABundleRuntimeSupport(kind: "mcp" | "lsp"): {
    supportedServerNames: string[];
    unsupportedServerNames: string[];
    diagnostics: unknown[];
  ...
  beforeAll(() => {
    rootDir = makeTrackedTempDir("lada-lada-bundle", tempDirs);
    setupLADAInspectFixture();
  });
  ...
  it("loads the full LADA bundle manifest with all capabilities", () => {
    const m = expectLoadedLADAManifest();
  ...
  it.each([
    ...
    expectLADAManifestField({ field, includes });
  });
  ...
  it.each([
    ...
    expectBundleRuntimeSupport({
      actual: inspectLADABundleRuntimeSupport(kind),
