...
const loadLADAPluginsMock = vi.fn();
...
vi.mock("./loader.js", () => ({
  loadLADAPlugins: (...args: unknown[]) => loadLADAPluginsMock(...args),
}));
...
function setPluginLoadResult(overrides: Partial<ReturnType<typeof createPluginLoadResult>>) {
  ...
  loadLADAPluginsMock.mockReturnValue(result);
...
describe("plugin status reports", () => {
  ...
  it("treats a CLI-command-only plugin as a plain capability", () => {
    ...
    expect(inspect.capabilities).toEqual([{ kind: "cli-backend", ids: ["lada-cli"] }]);
  });
  ...
  it.each([
    {
      ...
      plugin: createPluginRecord({
        ...
        source: "/tmp/lada-bundle/.lada-plugin/plugin.json",
        format: "bundle",
        bundleFormat: "lada",
        bundleCapabilities: ["skills", "commands", "agents", "settings"],
        rootDir: "/tmp/lada-bundle",
      }),
      expectedId: "lada-bundle",
