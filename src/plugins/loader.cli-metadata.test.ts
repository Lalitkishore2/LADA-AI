...
import { loadLADAPluginCliRegistry, loadLADAPlugins } from "./loader.js";
import {
...
describe("plugin loader CLI metadata", () => {
  ...
  register(api) {
    ...
    const registry = await loadLADAPluginCliRegistry({
      env: { ...process.env, LADA_STATE_DIR: stateDir },
      logger: {
        info: () => {},
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@lada/cli-metadata-channel",
          lada: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      ...
      `const { defineChannelPluginEntry } = require("lada/plugin-sdk/core");
...
module.exports = {
  ...
  it("skips bundled channel full entries that do not provide a dedicated cli-metadata entry", async () => {
    ...
    process.env.LADA_BUNDLED_PLUGINS_DIR = bundledRoot;

    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify(
        {
          name: "@lada/bundled-skip-channel",
          lada: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
  ...
  it("skips bundled non-channel full entries that do not provide a dedicated cli-metadata entry", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@lada/bundled-skip-provider",
          lada: { extensions: ["./index.cjs"] },
        },
        null,
        2,
...
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@lada/full-cli-metadata-channel",
          lada: { extensions: ["./index.cjs"] },
        },
        null,
        2,
