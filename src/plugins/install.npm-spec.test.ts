...
const suiteTempRootTracker = createSuiteTempRootTracker("lada-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@lada/voice-call@0.0.1",
              name: "@lada/voice-call",
              version: "0.0.1",
              filename: packedName,
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@lada/voice-call@0.0.2-beta.1",
      name: "@lada/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@lada/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@lada/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@lada/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
