...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("lada-marketplace-download-"),
    )
...
async function writeMarketplaceManifest(rootDir: string, manifest: unknown): Promise<string> {
  const manifestPath = path.join(rootDir, ".lada-plugin", "marketplace.json");
...
async function writeRemoteMarketplaceFixture(params: {
  ...
  await fs.mkdir(path.join(params.repoDir, ".lada-plugin"), { recursive: true });
  if (params.pluginDir) {
  ...
  await fs.writeFile(
    path.join(params.repoDir, ".lada-plugin", "marketplace.json"),
...
function expectMarketplaceManifestListing(
  ...
  expect(result.sourceLabel.replaceAll("\\", "/")).toContain(".lada-plugin/marketplace.json");
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("lada-marketplace-test-", async (rootDir) => {
  ...
  it("resolves relative plugin paths against the marketplace root", async () => {
    await withTempDir("lada-marketplace-test-", async (rootDir) => {
      ...
      expectLocalMarketplaceInstallResult({
        result,
        pluginDir,
        marketplaceSource: path.join(rootDir, ".lada-plugin", "marketplace.json"),
      });
    });
  });
  ...
  it("resolves LADA-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("lada-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "lada-home");
      await fs.mkdir(path.join(homeDir, ".lada", "plugins"), { recursive: true });
      await fs.mkdir(openClawHome, { recursive: true });
      await fs.writeFile(
        path.join(homeDir, ".lada", "plugins", "known_marketplaces.json"),
        JSON.stringify({
          "lada-plugins-official": {
            source: {
              source: "github",
              repo: "anthropics/lada-plugins-official",
            },
            installLocation: path.join(homeDir, ".lada", "plugins", "marketplaces", "official"),
          },
        }),
      );

      const shortcut = await withEnvAsync(
        { HOME: homeDir, LADA_HOME: openClawHome },
        async () => await resolveMarketplaceInstallShortcut("superpowers@lada-plugins-official"),
      );

      expect(shortcut).toEqual({
        ok: true,
        plugin: "superpowers",
        marketplaceName: "lada-plugins-official",
        marketplaceSource: "lada-plugins-official",
      });
    });
  });
