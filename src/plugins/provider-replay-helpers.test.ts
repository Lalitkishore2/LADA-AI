...
describe("provider replay helpers", () => {
  ...
  it("derives lada-only anthropic replay policy from the model id", () => {
    // Sonnet 4.6 preserves thinking blocks (no drop)
    expect(buildAnthropicReplayPolicyForModel("lada-sonnet-4-6")).toMatchObject({
      sanitizeToolCallIds: true,
      toolCallIdMode: "strict",
      validateAnthropicTurns: true,
    });
    expect(buildAnthropicReplayPolicyForModel("lada-sonnet-4-6")).not.toHaveProperty(
      "dropThinkingBlocks",
    );
    // Legacy models still drop thinking blocks
    expect(buildAnthropicReplayPolicyForModel("lada-3-7-sonnet-20250219")).toMatchObject({
      dropThinkingBlocks: true,
    });
  ...
  it("preserves thinking blocks for LADA Opus 4.5+ and Sonnet 4.5+ models", () => {
    // These models should NOT drop thinking blocks
    for (const modelId of [
      "lada-opus-4-5-20251101",
      "lada-opus-4-6",
      "lada-sonnet-4-5-20250929",
      "lada-sonnet-4-6",
      "lada-haiku-4-5-20251001",
    ]) {
    ...
    for (const modelId of ["lada-3-7-sonnet-20250219", "lada-3-5-sonnet-20240620"]) {
      const policy = buildAnthropicReplayPolicyForModel(modelId);
  ...
  it("builds native Anthropic replay policy with selective tool-call id preservation", () => {
    ...
    const policy46 = buildNativeAnthropicReplayPolicyForModel("lada-sonnet-4-6");
    ...
    expect(buildNativeAnthropicReplayPolicyForModel("lada-3-7-sonnet-20250219")).toMatchObject({
      dropThinkingBlocks: true,
    });
  });
      ...
      buildHybridAnthropicOrOpenAIReplayPolicy(
        {
          provider: "minimax",
          modelApi: "anthropic-messages",
          modelId: "lada-3-7-sonnet-20250219",
        } as never,
        { anthropicModelDropThinkingBlocks: true },
      ),
    ).toMatchObject({
  ...
  it("builds passthrough Gemini signature sanitization only when needed", () => {
    ...
    expect(
      buildPassthroughGeminiSanitizingReplayPolicy("anthropic/lada-sonnet-4-6"),
