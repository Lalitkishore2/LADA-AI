...
function createEnabledBundleConfig(pluginIds: string[]): LADAConfig {
  return {
    plugins: {
...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled LADA bundle MCP config and absolutizes relative args", async () => {
  ...
  it("resolves inline LADA MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "lada-bundle-inline-placeholder",
      async ({ homeDir, workspaceDir }) => {
        const pluginRoot = await writeLADABundleManifest({
          homeDir,
          pluginId: "inline-lada",
          manifest: {
            name: "inline-lada",
            mcpServers: {
              inlineProbe: {
        ...
        const loaded = loadEnabledBundleMcpConfig({
          workspaceDir,
          cfg: createEnabledBundleConfig(["inline-lada"]),
        });
