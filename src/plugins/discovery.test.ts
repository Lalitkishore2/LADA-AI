...
import { clearPluginDiscoveryCache, discoverLADAPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("lada-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    LADA_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverLADAPlugins>[0],
) {
  return discoverLADAPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverLADAPlugins>[0]) {
  return discoverLADAPlugins(params);
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      lada: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "lada.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverLADAPlugins>>,
...
describe("discoverLADAPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".lada", "extensions");
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@lada/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@lada/stray-workspace-plugin",
    });

    const result = discoverLADAPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".lada", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    ...
    const { candidates, diagnostics } = discoverLADAPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-lada");
    ...
    writePluginPackageManifest({
      ...
      packageName: "@opik/opik-lada",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-lada" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "lada"),
      packageName: "lada",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-lada"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".lada", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "lada");
    ...
    const { candidates } = discoverLADAPlugins({
      workspaceDir,
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@lada/ollama-provider",
          pluginId: "ollama",
          entryPath: "src/index.ts",
        });
        return {};
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@lada/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@lada/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@lada/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    {
      name: "auto-detects manifestless LADA bundles from the default layout",
      idHint: "lada-bundle",
      bundleFormat: "lada",
      setup: (stateDir: string) => {
        const bundleDir = path.join(stateDir, "extensions", "lada-bundle");
  ...
  it.each([
    {
      ...
      bundleMarker: ".lada-plugin/plugin.json",
      setup: (stateDir: string) => {
        const pluginDir = path.join(stateDir, "extensions", "legacy-with-bad-bundle");
        mkdirSafe(path.dirname(path.join(pluginDir, ".lada-plugin", "plugin.json")));
        fs.writeFileSync(path.join(pluginDir, "index.ts"), "export default {}", "utf-8");
        fs.writeFileSync(path.join(pluginDir, ".lada-plugin", "plugin.json"), "{", "utf-8");
        return {};
      },
    },
    {
    ...
    expect(legacy?.format).toBe("lada");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@lada/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const result = discoverLADAPlugins({
        env: {
          ...process.env,
          LADA_DISABLE_BUNDLED_PLUGINS: undefined,
          LADA_STATE_DIR: stateDir,
          LADA_BUNDLED_PLUGINS_DIR: bundledDir,
        },
      });
