...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "lada", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".lada", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".lada", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "lada-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".lada", "extensions"),
      },
    });
  });
});
