...
import type { LADAPluginDefinition, LADAPluginModule } from "./types.js";
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    ["lada/plugin-sdk"]: _ignoredLegacyRootAlias,
    ["@lada/plugin-sdk"]: _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as LADAPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti(safeSource)(safeSource) as LADAPluginModule;
    } catch (error) {
