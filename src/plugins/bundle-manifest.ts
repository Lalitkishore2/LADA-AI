...
export const CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH = ".lada-plugin/plugin.json";
...
export type BundlePluginManifest = {
  ...
  // Only include hook roots that LADA can execute via HOOK.md + handler files.
  hooks: string[];
...
function resolveLADAComponentPaths(
...
function resolveLADASkillsRootDirs(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "skills", rootDir, ["skills"]);
}

function resolveLADACommandRootDirs(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "commands", rootDir, ["commands"]);
}

function resolveLADASkillDirs(raw: Record<string, unknown>, rootDir: string): string[] {
  return mergeBundlePathLists(
    resolveLADASkillsRootDirs(raw, rootDir),
    resolveLADACommandRootDirs(raw, rootDir),
    resolveLADAAgentDirs(raw, rootDir),
    resolveLADAOutputStylePaths(raw, rootDir),
  );
}

function resolveLADAAgentDirs(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "agents", rootDir, ["agents"]);
}

function resolveLADAHookPaths(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "hooks", rootDir, ["hooks/hooks.json"]);
}

function resolveLADAMcpPaths(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "mcpServers", rootDir, [".mcp.json"]);
}

function resolveLADALspPaths(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "lspServers", rootDir, [".lsp.json"]);
}

function resolveLADAOutputStylePaths(raw: Record<string, unknown>, rootDir: string): string[] {
  return resolveLADAComponentPaths(raw, "outputStyles", rootDir, ["output-styles"]);
}

function resolveLADASettingsFiles(_raw: Record<string, unknown>, rootDir: string): string[] {
  return fs.existsSync(path.join(rootDir, "settings.json")) ? ["settings.json"] : [];
}

function hasLADAHookCapability(raw: Record<string, unknown>, rootDir: string): boolean {
  return hasInlineCapabilityValue(raw.hooks) || resolveLADAHookPaths(raw, rootDir).length > 0;
}
...
function buildLADACapabilities(raw: Record<string, unknown>, rootDir: string): string[] {
  const capabilities: string[] = [];
  if (resolveLADASkillDirs(raw, rootDir).length > 0) {
    capabilities.push("skills");
  }
  if (resolveLADACommandRootDirs(raw, rootDir).length > 0) {
    capabilities.push("commands");
  }
  if (resolveLADAAgentDirs(raw, rootDir).length > 0) {
    capabilities.push("agents");
  }
  if (hasLADAHookCapability(raw, rootDir)) {
    capabilities.push("hooks");
  }
  if (hasInlineCapabilityValue(raw.mcpServers) || resolveLADAMcpPaths(raw, rootDir).length > 0) {
    capabilities.push("mcpServers");
  }
  if (hasInlineCapabilityValue(raw.lspServers) || resolveLADALspPaths(raw, rootDir).length > 0) {
    capabilities.push("lspServers");
  }
  if (
    hasInlineCapabilityValue(raw.outputStyles) ||
    resolveLADAOutputStylePaths(raw, rootDir).length > 0
  ) {
    capabilities.push("outputStyles");
  }
  if (resolveLADASettingsFiles(raw, rootDir).length > 0) {
...
export function loadBundleManifest(params: {
  ...
  const loaded = loadBundleManifestFile({
    ...
    rejectHardlinks,
    allowMissing: params.bundleFormat === "lada",
  });
  if (!loaded.ok) {
    return loaded;
  }
  ...
  return {
    ...
    manifest: {
      ...
      description,
      version,
      skills: resolveLADASkillDirs(raw, params.rootDir),
      settingsFiles: resolveLADASettingsFiles(raw, params.rootDir),
      hooks: resolveLADAHookPaths(raw, params.rootDir),
      bundleFormat: "lada",
      capabilities: buildLADACapabilities(raw, params.rootDir),
    },
    manifestPath: loaded.manifestPath,
  };
}
...
export function detectBundleManifestFormat(rootDir: string): PluginBundleFormat | null {
  ...
  if (manifestlessLADAMarkers.some((candidate) => fs.existsSync(candidate))) {
    return "lada";
  }
  return null;
}
