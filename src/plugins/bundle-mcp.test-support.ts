...
export function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".lada", "extensions", pluginId);
}

export async function writeLADABundleManifest(params: {
  homeDir: string;
  pluginId: string;
...
export async function withBundleHomeEnv<T>(
  ...
  const env = captureEnv(["HOME", "USERPROFILE", "LADA_HOME", "LADA_STATE_DIR"]);
  try {
