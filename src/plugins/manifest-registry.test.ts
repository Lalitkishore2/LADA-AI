...
import type { LADAPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("lada-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "lada.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  packageManifest?: LADAPackageManifest;
  packageDir?: string;
...
function hermeticEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    LADA_BUNDLED_PLUGINS_DIR: undefined,
    LADA_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
    LADA_VERSION: undefined,
    VITEST: "true",
    ...overrides,
  };
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "lada.plugin.json");
  const linkedManifest = path.join(rootDir, "lada.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "lada.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { LADA_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires LADA >=2026.3.22, but this host is 2026.3.21",
      expectWarn: false,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | lada.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { LADA_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it.each([
    ...
    {
      name: "loads LADA bundle manifests with command roots and settings files",
      idHint: "lada-sample",
      bundleFormat: "lada" as const,
      setup: (bundleDir: string) => {
        setupBundleFixture({
          bundleDir,
          dirs: [".lada-plugin", "skill-packs/starter", "commands-pack"],
          textFiles: {
            "settings.json": '{"hideThinkingBlock":true}',
          },
          manifestRelativePath: ".lada-plugin/plugin.json",
          manifest: {
            name: "LADA Sample",
    ...
    {
      name: "loads manifestless LADA bundles into the registry",
      idHint: "manifestless-lada",
      bundleFormat: "lada" as const,
      setup: (bundleDir: string) => {
        setupBundleFixture({
          bundleDir,
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeA,
        LADA_HOME: undefined,
        LADA_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        LADA_HOME: undefined,
        LADA_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
