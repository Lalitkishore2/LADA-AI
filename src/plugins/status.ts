...
import { normalizeLADAVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeLADAVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeLADAVersionBase(plugin.version) ??
    plugin.version
  );
}
