...
import type { LADAPluginService, LADAPluginServiceContext } from "./types.js";
...
function createRegistry(services: LADAPluginService[]) {
...
function expectServiceLogger(ctx: LADAPluginServiceContext) {
...
function expectServiceContexts(
  contexts: LADAPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: LADAPluginServiceContext[] = [];
