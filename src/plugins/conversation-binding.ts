...
const APPROVALS_PATH = "~/.lada/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "lada-app-server:thread:",
  "lada-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("lada.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("lada.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to LADA instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
