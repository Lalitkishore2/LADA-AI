 ...
 */
export type LADAPluginConfigSchema = {
...
export type LADAPluginToolContext = {
  config?: LADAConfig;
  /** Active runtime-resolved config snapshot when one is available. */
  runtimeConfig?: LADAConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
  sessionKey?: string;
...
export type LADAPluginToolFactory = (
  ctx: LADAPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type LADAPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type LADAPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<LADAConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). LADA uses this to expose
 ...
 *
 * Runs after LADA resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common LADA auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and LADA wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before LADA builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. LADA no
...
/**
 * Provider-owned stream wrapper hook after LADA applies its generic
 ...
 *
 * Return `true` or `false` to override LADA's built-in provider cache TTL
 ...
 *
 * Runs only after LADA exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that LADA should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after LADA loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, LADA falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * LADA calls this only from async model resolution paths. After it
   ...
   *
   * Runs after LADA performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before LADA registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic LADA wrappers.
   *
   ...
   *
   * Called after LADA resolves the raw configured credential but before the
   ...
   * hint than LADA's generic auth-store guidance.
   */
   ...
   * LADA's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when LADA should surface a provider-specific hint for
   ...
   * merging. LADA deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Use this to keep model-family defaults (for example LADA 4.6 =>
   ...
   * without replacing the full LADA-owned system prompt.
   */
   ...
   *
   * LADA uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `lada doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `lada doctor` should offer.
   *
   ...
   *
   * LADA calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep LADA's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * LADA falls back to that stored profile.
   */
...
export type WebSearchProviderPlugin = {
   ...
   * when LADA has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: LADAConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: LADAConfig, value: unknown) => void;
  applySelectionConfig?: (config: LADAConfig) => LADAConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => LADAConfig | Promise<LADAConfig>;
...
export type LADAPluginGatewayMethod = {
  method: string;
 ...
 */
export type LADAPluginCommandDefinition = {
...
export type LADAPluginHttpRouteAuth = "gateway" | "plugin";
export type LADAPluginHttpRouteMatch = "exact" | "prefix";

export type LADAPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type LADAPluginHttpRouteParams = {
  path: string;
  handler: LADAPluginHttpRouteHandler;
  auth: LADAPluginHttpRouteAuth;
  match?: LADAPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type LADAPluginCliContext = {
  program: Command;
  config: LADAConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type LADAPluginCliRegistrar = (ctx: LADAPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want LADA to keep a plugin command lazy-loaded while still
 ...
 */
export type LADAPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type LADAPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type LADAPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type LADAPluginSecurityAuditContext = {
  config: LADAConfig;
  sourceConfig: LADAConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type LADAPluginSecurityAuditCollector = (
  ctx: LADAPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type LADAPluginServiceContext = {
  config: LADAConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type LADAPluginService = {
  id: string;
  start: (ctx: LADAPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: LADAPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  /** Provider id used in model refs, for example `lada-cli/opus`. */
  id: string;
  ...
  /**
   * Whether LADA should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume LADA's bundle
   ...
   * - LADA: `--strict-mcp-config --mcp-config`
   * - Codex: `-c mcp_servers=...`
...
export type LADAPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type LADAPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: LADAPluginReloadRegistration;
  nodeHostCommands?: LADAPluginNodeHostCommand[];
  securityAuditCollectors?: LADAPluginSecurityAuditCollector[];
  register?: (api: LADAPluginApi) => void | Promise<void>;
  activate?: (api: LADAPluginApi) => void | Promise<void>;
};

export type LADAPluginModule =
  | LADAPluginDefinition
  | ((api: LADAPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: LADAConfig) =>
  | {
      config: LADAConfig;
      changes: string[];
    }
...
export type LADAPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | LADAPluginToolFactory,
    opts?: LADAPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: LADAPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: LADAPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: LADAPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: LADAPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, LADA can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: LADAPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: LADAPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: LADAPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: LADAPluginSecurityAuditCollector) => void;
  registerService: (service: LADAPluginService) => void;
   ...
   */
  registerCommand: (command: LADAPluginCommandDefinition) => void;
...
export type PluginFormat = "lada" | "bundle";

export type PluginBundleFormat = "codex" | "lada" | "cursor";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * LADA does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id LADA will install under. */
  pluginId: string;
...
export type PluginHookBeforeInstallContext = {
  ...
  /** Normalized origin of the install target (e.g. "lada-bundled", "plugin-package"). */
  origin?: string;
};
