...
import { discoverLADAPlugins } from "./discovery.js";
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadLADAPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadLADAPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: LADAPluginDefinition;
  register?: LADAPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as LADAPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as LADAPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.LADA_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverLADAPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverLADAPlugins>["candidates"][number];
  right: ReturnType<typeof discoverLADAPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadLADAPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadLADAPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    const discovery = discoverLADAPlugins({
      workspaceDir: options.workspaceDir,
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        const unsupportedCapabilities = (record.bundleCapabilities ?? []).filter(
          (capability) =>
            ...
            !(
              ...
              (record.bundleFormat === "lada" || record.bundleFormat === "cursor")
            ) &&
            !(
              capability === "hooks" &&
              (record.bundleFormat === "codex" || record.bundleFormat === "lada")
            ),
        );
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into LADA yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: LADAPluginModule | null = null;
      try {
        ...
        mod = getJiti(safeSource)(safeImportSource) as LADAPluginModule;
      } catch (err) {
        recordPluginError({
          logger,
          registry,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'lada plugins list' for details.`,
        );
      }
    }
...
export async function loadLADAPluginCliRegistry(
  options: PluginLoadOptions = {},
