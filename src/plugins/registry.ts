...
import type {
  ...
  LADAPluginApi,
  LADAPluginChannelRegistration,
  LADAPluginCliCommandDescriptor,
  LADAPluginCliRegistrar,
  LADAPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  LADAPluginHttpRouteAuth,
  LADAPluginHttpRouteMatch,
  LADAPluginHttpRouteHandler,
  LADAPluginHttpRouteParams,
  LADAPluginHookOptions,
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: LADAPluginCliRegistrar;
  commands: string[];
...
export type PluginReloadRegistration = {
  pluginId: string;
  pluginName?: string;
  registration: LADAPluginReloadRegistration;
  source: string;
  rootDir?: string;
};
...
export type PluginNodeHostCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: LADAPluginNodeHostCommand;
  source: string;
  rootDir?: string;
};
...
export type PluginSecurityAuditCollectorRegistration = {
  pluginId: string;
  pluginName?: string;
  collector: LADAPluginSecurityAuditCollector;
  source: string;
  rootDir?: string;
};
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: LADAPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: LADAPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: LADAPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: LADAPluginHookOptions | undefined,
    config: LADAPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: LADAPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: LADAPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as LADAPluginChannelRegistration).plugin === "object"
        ? (registration as LADAPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: LADAPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerReload = (record: PluginRecord, registration: LADAPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: LADAPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: LADAPluginNodeHostCommand,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: LADAPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: LADAPluginCommandDefinition) => {
