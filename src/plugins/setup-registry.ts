...
function collectConfiguredPluginEntryIds(config: LADAConfig): string[] {
...
function resolveRegister(mod: LADAPluginModule): {
...
export function resolvePluginSetupRegistry(params?: {
  ...
  for (const record of manifestRegistry.plugins) {
    ...
    let mod: LADAPluginModule;
    try {
      mod = getJiti(setupSource)(setupSource) as LADAPluginModule;
    } catch {
      continue;
    }

    const resolved = resolveRegister((mod as { default?: LADAPluginModule }).default ?? mod);
