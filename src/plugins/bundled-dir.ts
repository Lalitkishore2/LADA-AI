...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "lada-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.LADA_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "lada.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.LADA_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveLADAPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveLADAPackageRootSync({ argv1: process.argv[1] }),
      resolveLADAPackageRootSync({ cwd: process.cwd() }),
      resolveLADAPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
