...
const PLUGIN_SDK_SUBPATH_PATTERN = /lada\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packLADAToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = makeTrackedTempDir("lada-packed-root-package-json", tempDirs);
  await tar.x({
    file: archivePath,
    cwd: extractDir,
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references lada/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("mirrors matrix runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = matrixPackageJson.lada?.releaseChecks?.rootDependencyMirrorAllowlist;
  ...
  it("mirrors Bedrock runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = bedrockPackageJson.lada?.releaseChecks?.rootDependencyMirrorAllowlist;
  ...
  it("keeps matrix crypto WASM in the packed artifact manifest", async () => {
    const tempRoot = makeTrackedTempDir("lada-matrix-wasm-pack", tempDirs);
    const packDir = join(tempRoot, "pack");
    mkdirSync(packDir, { recursive: true });

    const archivePath = packLADAToTempDir(packDir);
    ...
    expect(packedPackageJson.dependencies?.["@lada/plugin-package-contract"]).toBeUndefined();
  });
