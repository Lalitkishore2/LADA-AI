...
function createInstalledRuntimePluginDir(
  ...
  const bundledDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `lada-runtime-contract-bundled-${pluginId}-`),
  );
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `lada-runtime-contract-state-${pluginId}-`),
  );
  ...
  fs.writeFileSync(
    path.join(pluginRoot, "lada.plugin.json"),
    JSON.stringify({
      id: pluginId,
      channels: [pluginId],
