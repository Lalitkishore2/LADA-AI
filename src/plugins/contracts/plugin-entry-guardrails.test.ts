...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"lada\/plugin-sdk\/core"/;
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          lada?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.lada?.extensions) ? pkg.lada.extensions : [];
        if (
          extensions.some(
