...
const MARKETPLACE_MANIFEST_CANDIDATES = [
  path.join(".lada-plugin", "marketplace.json"),
  "marketplace.json",
] as const;
...
async function readLADAKnownMarketplaces(): Promise<Record<string, KnownMarketplaceRecord>> {
...
function deriveMarketplaceRootFromManifestPath(manifestPath: string): string {
  ...
  return path.basename(manifestDir) === ".lada-plugin" ? path.dirname(manifestDir) : manifestDir;
}
...
> {
  ...
  const rootDir = path.basename(resolved) === ".lada-plugin" ? path.dirname(resolved) : resolved;
...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-marketplace-"));
...
async function loadMarketplace(params: {
  ...
  const knownMarketplaces = await readLADAKnownMarketplaces();
...
> {
  ...
  try {
    ...
    try {
      ...
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-marketplace-download-"));
...
export async function resolveMarketplaceInstallShortcut(
  ...
  if (!normalizedSource.ok) {
    return {
      ok: false,
      error: `known LADA marketplace "${marketplaceName}" has an invalid source: ${normalizedSource.error}`,
    };
  }
