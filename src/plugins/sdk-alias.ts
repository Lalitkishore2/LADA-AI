...
import { resolveLADAPackageRootSync } from "../infra/lada-root.js";
...
function hasTrustedLADARootIndicator(params: {
  packageRoot: string;
  ...
  const hasLADABin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("lada")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.lada === "string");
  const hasLADAEntrypoint = fs.existsSync(path.join(params.packageRoot, "lada.mjs"));
  return hasCliEntryExport || hasLADABin || hasLADAEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedLADARootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedLADARootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveLADAPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedLADARootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveLADAPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedLADARootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["lada/plugin-sdk", "@lada/plugin-sdk"] as const;
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "lada/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
