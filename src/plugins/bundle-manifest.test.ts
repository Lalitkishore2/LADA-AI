...
function makeTempDir() {
  return makeTrackedTempDir("lada-bundle-manifest", tempDirs);
}

const mkdirSafe = mkdirSafeDir;

function expectLoadedManifest(rootDir: string, bundleFormat: "codex" | "lada" | "cursor") {
...
function expectLADAHookResolution(params: {
  rootDir: string;
  expectedHooks: readonly string[];
  hasHooksCapability: boolean;
}) {
  const manifest = expectLoadedManifest(params.rootDir, "lada");
...
describe("bundle manifest parsing", () => {
  it.each([
    ...
    {
      name: "detects and loads LADA bundle manifests from the component layout",
    ...
    {
      name: "detects manifestless LADA bundles from the default layout",
      bundleFormat: "lada" as const,
      setup: (rootDir: string) => {
        setupBundleFixture({
          rootDir,
  ...
  hooks: "hooks",
    ...
    {
      name: "accepts JSON5 LADA bundle manifests",
      bundleFormat: "lada" as const,
      manifestRelativePath: CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH,
      json5Manifest: `{
  name: "LADA JSON5 Bundle",
  commands: "commands-pack",
  hooks: "hooks-pack",
  outputStyles: "styles",
}`,
      dirs: [".lada-plugin", "commands-pack", "hooks-pack", "styles"],
      expected: {
        id: "lada-json5-bundle",
        name: "LADA JSON5 Bundle",
        bundleFormat: "lada",
  ...
  it.each([
    ...
    {
      name: "rejects JSON5 LADA bundle manifests that parse to non-objects",
      bundleFormat: "lada" as const,
      manifestRelativePath: CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH,
    },
    {
  ...
  it.each([
    {
      name: "resolves LADA bundle hooks from default and declared paths",
      setupKind: "default-hooks",
      expectedHooks: ["hooks/hooks.json"],
      hasHooksCapability: true,
    },
    {
      name: "resolves LADA bundle hooks from manifest-declared paths only",
      setupKind: "custom-hooks",
      expectedHooks: ["custom-hooks"],
      hasHooksCapability: true,
    },
    {
      name: "returns empty hooks for LADA bundles with no hooks directory",
      setupKind: "no-hooks",
      expectedHooks: [],
    ...
    const rootDir = makeTempDir();
    setupLADAHookFixture(rootDir, setupKind);
    expectLADAHookResolution({
      rootDir,
      expectedHooks,
      hasHooksCapability,
    });
  ...
  it("does not misclassify native index plugins as manifestless LADA bundles", () => {
    const rootDir = makeTempDir();
    setupBundleFixture({
      rootDir,
      dirs: ["commands"],
