...
const suiteTempRootTracker = createSuiteTempRootTracker("lada-plugin-install-path");
...
describe("installPluginFromPath", () => {
  ...
  it("installs LADA bundles from an archive path", async () => {
    const { pluginDir, extensionsDir } = setupBundleInstallFixture({
      bundleFormat: "lada",
      name: "LADA Sample",
    });
    const archivePath = path.join(suiteTempRootTracker.makeTempDir(), "lada-bundle.tgz");
    ...
    expect(result.pluginId).toBe("lada-sample");
    expect(fs.existsSync(path.join(result.targetDir, ".lada-plugin", "plugin.json"))).toBe(true);
  });
