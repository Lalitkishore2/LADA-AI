...
export type LADABundleCommandSpec = {
  pluginId: string;
  rawName: string;
  description: string;
...
function readLADABundleManifest(rootDir: string): Record<string, unknown> {
...
function resolveLADACommandRootDirs(rootDir: string): string[] {
  const raw = readLADABundleManifest(rootDir);
...
function loadBundleCommandsFromRoot(params: {
  pluginId: string;
  commandRoot: string;
}): LADABundleCommandSpec[] {
  const entries: LADABundleCommandSpec[] = [];
...
export function loadEnabledLADABundleCommands(params: {
  workspaceDir: string;
  cfg?: LADAConfig;
}): LADABundleCommandSpec[] {
  ...
  const commands: LADABundleCommandSpec[] = [];

  for (const record of registry.plugins) {
    if (
      record.format !== "bundle" ||
      record.bundleFormat !== "lada" ||
    ...
    for (const relativeRoot of resolveLADACommandRootDirs(record.rootDir)) {
