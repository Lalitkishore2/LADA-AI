...
export type PluginCliCommandGroupEntry = {
  pluginId: string;
  placeholders: readonly LADAPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: LADAPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadLADAPluginCliRegistry(
...
export async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  const runtimeRegistry = loadLADAPlugins(
  ...
  try {
    const metadataRegistry = await loadLADAPluginCliRegistry(
      buildPluginCliLoaderParams(params.context, params.loaderOptions),
    );
    return {
      ...params.context,
