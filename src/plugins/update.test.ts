...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-lada",
    targetDir: params?.targetDir ?? "/tmp/opik-lada",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "lada-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/lada-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@lada/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["lada-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/lada-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "lada-codex-app-server",
        spec: "lada-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/lada-codex-app-server",
      }),
      pluginIds: ["lada-codex-app-server"],
      specOverrides: {
        "lada-codex-app-server": "lada-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "lada-codex-app-server",
        targetDir: "/tmp/lada-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "lada-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @lada/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@lada/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @lada/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "lada-codex-app-server@beta",
        resolvedName: "lada-codex-app-server",
        resolvedSpec: "lada-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "lada-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "lada-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "lada-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "lada-codex-app-server",
      }),
      specOverrides: {
        "lada-codex-app-server": "lada-codex-app-server@beta",
      },
      expectedSpec: "lada-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "lada-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "lada-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@lada/voice-call",
      targetDir: "/tmp/lada-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@lada/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@lada/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@lada/voice-call");
    expect(result.config.plugins?.entries?.["@lada/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@lada/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@lada/voice-call",
      installPath: "/tmp/lada-voice-call",
      version: "0.0.2",
    });
  ...
  it("checks marketplace installs during dry-run updates", async () => {
    installPluginFromMarketplaceMock.mockResolvedValue({
      ok: true,
      pluginId: "lada-bundle",
      targetDir: "/tmp/lada-bundle",
      version: "1.2.0",
      extensions: ["index.ts"],
      marketplaceSource: "vincentkoc/lada-marketplace",
      marketplacePlugin: "lada-bundle",
    });

    const result = await updateNpmInstalledPlugins({
      config: createMarketplaceInstallConfig({
        pluginId: "lada-bundle",
        installPath: "/tmp/lada-bundle",
        marketplaceSource: "vincentkoc/lada-marketplace",
        marketplacePlugin: "lada-bundle",
      }),
      pluginIds: ["lada-bundle"],
      dryRun: true,
    });

    expect(installPluginFromMarketplaceMock).toHaveBeenCalledWith(
      expect.objectContaining({
        marketplace: "vincentkoc/lada-marketplace",
        plugin: "lada-bundle",
        expectedPluginId: "lada-bundle",
        dryRun: true,
      }),
    );
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "1.2.0",
        message: "Would update lada-bundle: unknown -> 1.2.0.",
      },
    ]);
  });
  ...
  it("updates marketplace installs and preserves source metadata", async () => {
    installPluginFromMarketplaceMock.mockResolvedValue({
      ...
      version: "1.3.0",
      extensions: ["index.ts"],
      marketplaceName: "Vincent's LADA Plugins",
      marketplaceSource: "vincentkoc/lada-marketplace",
      marketplacePlugin: "lada-bundle",
    });
    ...
    expect(result.config.plugins?.installs?.["lada-bundle"]).toMatchObject({
      source: "marketplace",
      installPath: "/tmp/lada-bundle",
      version: "1.3.0",
      marketplaceName: "Vincent's LADA Plugins",
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { LADA_HOME: "/srv/lada-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/lada-home";
    mockBundledSources(
      createBundledSource({
