...
type LoadLADAPlugins = typeof import("./loader.js").loadLADAPlugins;
...
const loadLADAPluginsMock = vi.fn<LoadLADAPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/lada.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadLADAPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: LADAConfig = {
    plugins: {},
  };
  const autoEnabledConfig: LADAConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadLADAPlugins: (...args: Parameters<LoadLADAPlugins>) =>
        loadLADAPluginsMock(...args),
  ...
  it("maps cli backend ids to owning plugin ids via manifests", () => {
    ...
    expectOwningPluginIds("lada-cli", ["anthropic"]);
