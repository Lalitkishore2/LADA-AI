...
describe("bundle plugins", () => {
  ...
  it.each([
    {
      name: "treats LADA command roots and settings as supported bundle surfaces",
      pluginId: "lada-skills",
      expectedFormat: "lada",
    ...
    },
    {
      ...
      build: (bundleRoot: string) => {
        mkdirSafe(path.join(bundleRoot, ".lada-plugin"));
        fs.writeFileSync(
  ...
  it("warns when bundle MCP only declares unsupported non-stdio transports", () => {
    ...
    const plugin = registry.plugins.find((entry) => entry.id === "lada-mcp-url");
    expect(plugin?.status).toBe("loaded");
    expect(plugin?.bundleCapabilities).toEqual(expect.arrayContaining(["mcpServers"]));
    expect(
      registry.diagnostics.some(
        (diag) =>
          diag.pluginId === "lada-mcp-url" &&
