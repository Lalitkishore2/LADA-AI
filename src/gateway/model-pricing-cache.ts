...
function canonicalizeOpenRouterLookupId(id: string): string {
  ...
  if (provider === "anthropic") {
    model = model
      .replace(/^lada-(\d+)\.(\d+)-/u, "lada-$1-$2-")
      .replace(/^lada-([a-z]+)-(\d+)\.(\d+)$/u, "lada-$1-$2-$3");
  }
  model =
...
export function collectConfiguredModelPricingRefs(config: LADAConfig): ModelRef[] {
...
function scheduleRefresh(params: { config: LADAConfig; fetchImpl: typeof fetch }): void {
  clearRefreshTimer();
