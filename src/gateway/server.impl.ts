...
import { ensureLADACliOnPath } from "../infra/path-env.js";
...
function pruneSkippedStartupSecretSurfaces(config: LADAConfig): LADAConfig {
  const skipChannels =
    isTruthyEnvValue(process.env.LADA_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.LADA_SKIP_PROVIDERS);
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("lada doctor --fix")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.LADA_TEST_MINIMAL_GATEWAY === "1";
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `lada config set gateway.auth.mode token` and `lada config set gateway.auth.token <token>`.",
      );
    }
  }
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachLADATranscriptMeta(update.message, {
