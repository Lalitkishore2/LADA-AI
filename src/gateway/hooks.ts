...
export function resolveHooksConfig(cfg: LADAConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: LADAConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-lada-token"] === "string"
      ? req.headers["x-lada-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-lada-idempotency-key"]) ||
