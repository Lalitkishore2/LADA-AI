...
export type LiveAgentFamily = "lada" | "codex" | "gemini";
...
export function normalizeLiveAgentFamily(raw: string): LiveAgentFamily {
  ...
  if (normalized === "lada" || normalized === "lada-cli") {
    return "lada";
  }
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the LADA MCP tool named cron. " +
      `Call it with JSON arguments ${params.argsJson}. ` +
      "Do the actual tool call; I will verify externally with the LADA cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (family === "lada") {
    return (
      "Return only a tool call for the LADA MCP tool `cron`. " +
      `Use these exact JSON arguments: ${params.argsJson}. ` +
      "No prose. I will verify externally with the LADA cron CLI."
    );
  }
  return (
    "Use the LADA MCP tool named cron. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "No prose before the tool call. I will verify externally with the LADA cron CLI."
  );
}

export async function runLADACliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  delete childEnv.VITEST;
  delete childEnv.VITEST_MODE;
  delete childEnv.VITEST_POOL_ID;
  delete childEnv.VITEST_WORKER_ID;
  const { stdout, stderr } = await execFileAsync(process.execPath, ["lada.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!trimmed) {
    throw new Error(
      [
        `lada ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `lada ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runLADACliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
