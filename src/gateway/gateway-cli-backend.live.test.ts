...
import { clearRuntimeConfigSnapshot, type LADAConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.LADA_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.LADA_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.LADA_LIVE_CLI_BACKEND_DEBUG);
const describeLive = LIVE && CLI_LIVE ? describe : describe.skip;

const DEFAULT_PROVIDER = "lada-cli";
const DEFAULT_MODEL =
  resolveCliBackendLiveTest(DEFAULT_PROVIDER)?.defaultModelRef ?? "lada-cli/lada-sonnet-4-6";
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "LADA_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.LADA_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.LADA_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      const parsed = parseModelRef(rawModel, "lada-cli");
      if (!parsed) {
        throw new Error(
          `LADA_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
        );
      }
      ...
      const cliCommand = process.env.LADA_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `LADA_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }

      const baseCliArgs =
        parseJsonStringArray(
          "LADA_LIVE_CLI_BACKEND_ARGS",
          process.env.LADA_LIVE_CLI_BACKEND_ARGS,
        ) ?? providerDefaults?.args;
      if (!baseCliArgs || baseCliArgs.length === 0) {
        throw new Error(`LADA_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
      }

      const cliClearEnv =
        parseJsonStringArray(
          "LADA_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.LADA_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.LADA_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.LADA_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "LADA_LIVE_CLI_BACKEND_IMAGE_MODE requires LADA_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-live-cli-"));
      const stateDir = path.join(tempDir, "state");
      await fs.mkdir(stateDir, { recursive: true });
      process.env.LADA_STATE_DIR = stateDir;
      const bundleMcp = backendResolved?.bundleMcp === true;
      const bootstrapWorkspace =
        backendResolved?.bundleMcpMode === "lada-config-file"
          ? await createBootstrapWorkspace(tempDir)
          : null;
      const disableMcpConfig = process.env.LADA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      if (
        ...
        const mcpConfigPath = path.join(tempDir, "lada-mcp.json");
        await fs.writeFile(mcpConfigPath, `${JSON.stringify({ mcpServers: {} }, null, 2)}\n`);
        cliArgs = withLADAMcpConfigOverrides(baseCliArgs, mcpConfigPath);
      }

      const cfg: LADAConfig = {};
      const cfgWithCliBackends = cfg as LADAConfig & {
        agents?: {
          defaults?: {
        ...
        if (modelSwitchTarget) {
          ...
          const switchPayload = await client.request(
            "agent",
            {
              ...
              idempotencyKey: `idem-${randomUUID()}`,
              message:
                "We just switched from LADA Sonnet to LADA Opus in the same session. " +
