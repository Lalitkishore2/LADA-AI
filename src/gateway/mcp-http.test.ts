  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-lada-account-id": "work",
        "x-lada-message-channel": "telegram",
      },
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.lada?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.lada?.headers?.Authorization).toBe(
      "Bearer ${LADA_MCP_TOKEN}",
    );
    expect(config.mcpServers?.lada?.headers?.["x-lada-message-channel"]).toBe(
      "${LADA_MCP_MESSAGE_CHANNEL}",
    );
  });
});
