...
const { createLADATools } = await import("../agents/lada-tools.js");
...
type SessionSendTool = ReturnType<typeof createLADATools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createLADATools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["LADA_GATEWAY_PORT", "LADA_GATEWAY_TOKEN"]);
  ...
  process.env.LADA_GATEWAY_PORT = String(gatewayPort);
  process.env.LADA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("LADA_CONFIG_PATH missing in gateway test environment");
      }
