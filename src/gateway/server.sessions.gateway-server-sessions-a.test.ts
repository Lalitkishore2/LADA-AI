...
beforeAll(async () => {
  ...
  sharedSessionStoreDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-"));
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.list surfaces transcript usage and model fallbacks from the transcript", async () => {
    ...
    testState.agentConfig = {
      models: {
        "anthropic/lada-sonnet-4-6": { params: { context1m: true } },
      },
    };
    await fs.writeFile(
    ...
    expect(child?.model).toBe("lada-sonnet-4-6");
  ...
  test("sessions.reset preserves spawned session ownership metadata", async () => {
    ...
    expect(reset.payload?.entry.modelOverride).toBe("lada-opus-4-1");
    ...
    expect(reset.payload?.entry.ladaCliSessionId).toBe("cli-session-123");
    ...
    expect(store["agent:main:subagent:child"]?.modelOverride).toBe("lada-opus-4-1");
    ...
    expect(store["agent:main:subagent:child"]?.ladaCliSessionId).toBe("cli-session-123");
  ...
  test("session:patch hook fires with correct context", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-patch-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("session:patch hook does not fire for webchat clients", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-webchat-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("session:patch hook only fires after successful patch", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-success-hook-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("control-ui client can delete sessions even in webchat mode", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-control-ui-delete-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
