...
const loadLADAPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadLADAPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadLADAPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadLADAPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", async () => {
    ...
    loadLADAPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadLADAPlugins).not.toHaveBeenCalled();
        ...
        entries: {
          "voice-call": {
            subagent: {
              ...
              allowedModels: ["anthropic/lada-haiku-4-5"],
            },
          },
        },
      },
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.lada.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadLADAPlugins.mockReturnValue(pluginRegistry);
