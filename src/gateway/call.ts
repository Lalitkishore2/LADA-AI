...
function loadGatewayConfig(): LADAConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: LADAConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.LADA_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as LADAConfig) : loadGatewayConfig());
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__LADA_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
