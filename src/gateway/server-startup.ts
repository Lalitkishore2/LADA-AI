...
import type { loadLADAPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureLADAModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadLADAPlugins>;
  ...
  // Tests can opt out via LADA_SKIP_CHANNELS (or legacy LADA_SKIP_PROVIDERS).
  const skipChannels =
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (LADA_SKIP_CHANNELS=1 or LADA_SKIP_PROVIDERS=1)",
    );
  }
