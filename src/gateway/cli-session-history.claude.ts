...
import { attachLADATranscriptMeta } from "./session-utils.fs.js";

export const CLAUDE_CLI_PROVIDER = "lada-cli";
const CLAUDE_PROJECTS_RELATIVE_DIR = path.join(".lada", "projects");

type LADACliProjectEntry = {
  type?: unknown;
  timestamp?: unknown;
  uuid?: unknown;
  isSidechain?: unknown;
...
type LADACliMessage = NonNullable<LADACliProjectEntry["message"]>;
type LADACliUsage = LADACliMessage["usage"];
...
function resolveLADAProjectsDir(homeDir?: string): string {
  return path.join(resolveHistoryHomeDir(homeDir), CLAUDE_PROJECTS_RELATIVE_DIR);
}

export function resolveLADACliBindingSessionId(
  ...
  const legacyLADASessionId = entry?.ladaCliSessionId?.trim();
  return legacyLADASessionId || undefined;
}
...
function resolveLADACliUsage(raw: LADACliUsage) {
...
function normalizeLADACliContent(
...
function coalesceLADACliToolMessages(messages: TranscriptLikeMessage[]): TranscriptLikeMessage[] {
...
function parseLADACliHistoryEntry(
  entry: LADACliProjectEntry,
  cliSessionId: string,
  toolNameRegistry: ToolNameRegistry,
  ...
  const content =
    typeof entry.message.content === "string" || Array.isArray(entry.message.content)
      ? normalizeLADACliContent(entry.message.content, toolNameRegistry)
      : undefined;
  ...
  if (type === "user") {
    return attachLADATranscriptMeta(
      {
        role: "user",
        content,
  ...
  return attachLADATranscriptMeta(
    {
      ...
      ...(typeof entry.message.stop_reason === "string" && entry.message.stop_reason.trim()
        ...
        : {}),
      ...(resolveLADACliUsage(entry.message.usage)
        ? { usage: resolveLADACliUsage(entry.message.usage) }
        : {}),
      ...(timestamp !== undefined ? { timestamp } : {}),
    },
    baseMeta,
...
export function resolveLADACliSessionFilePath(params: {
  cliSessionId: string;
  homeDir?: string;
}): string | undefined {
  const projectsDir = resolveLADAProjectsDir(params.homeDir);
...
export function readLADACliSessionMessages(params: {
  cliSessionId: string;
  homeDir?: string;
}): TranscriptLikeMessage[] {
  const filePath = resolveLADACliSessionFilePath(params);
  if (!filePath) {
    return [];
  }
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    try {
      const parsed = JSON.parse(line) as LADACliProjectEntry;
      const message = parseLADACliHistoryEntry(parsed, params.cliSessionId, toolNameRegistry);
      if (message) {
  ...
  return coalesceLADACliToolMessages(messages);
}
