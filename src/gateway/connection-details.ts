...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: LADAConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.LADA_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.LADA_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set LADA_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: lada doctor --fix",
