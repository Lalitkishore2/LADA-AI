...
function cfg(input: Partial<LADAConfig>): LADAConfig {
  return input as LADAConfig;
}
...
const DEFAULT_ENV = {
  LADA_GATEWAY_TOKEN: "env-token",
  LADA_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when LADA env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when LADA env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when LADA env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
