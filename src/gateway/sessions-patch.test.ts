...
async function applySubagentModelPatch(cfg: LADAConfig) {
  return expectPatchOk(
    await runPatch({
      ...
      loadGatewayModelCatalog: async () => [
        { provider: "anthropic", id: "lada-sonnet-4-6", name: "sonnet" },
    ...
    agents: {
      defaults: {
        ...
        subagents: params.defaultsSubagentModel
          ...
          : undefined,
        models: {
          "anthropic/lada-sonnet-4-6": { alias: "default" },
        },
      },
      list: [
        {
...
function createAllowlistedAnthropicModelCfg(): LADAConfig {
  return {
    agents: {
      defaults: {
...
describe("gateway sessions patch", () => {
  ...
  test("clears auth overrides when model patch changes", async () => {
    ...
    const entry = expectPatchOk(
      await runPatch({
        store,
        patch: { key: MAIN_SESSION_KEY, model: "anthropic/lada-sonnet-4-6" },
        loadGatewayModelCatalog: async () => [
          { provider: "anthropic", id: "lada-sonnet-4-6", name: "lada-sonnet-4-6" },
        ],
      }),
    );
    expect(entry.providerOverride).toBe("anthropic");
    expect(entry.modelOverride).toBe("lada-sonnet-4-6");
  ...
  test("allows target agent subagents.model for subagent session even when missing from global allowlist", async () => {
    const cfg = makeKimiSubagentCfg({
      agentPrimaryModel: "anthropic/lada-sonnet-4-6",
