...
async function withGatewayChatHarness(
  ...
  const createSessionDir = async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-gw-"));
...
describe("gateway server chat", () => {
  test("chat.history backfills lada-cli sessions from LADA project files", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      const ladaProjectsDir = path.join(homeDir, ".lada", "projects", "workspace");
      await fs.mkdir(ladaProjectsDir, { recursive: true });
      await fs.writeFile(
        path.join(ladaProjectsDir, `${cliSessionId}.jsonl`),
        [
          JSON.stringify({
