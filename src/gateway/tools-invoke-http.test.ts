...
let lastCreateLADAToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/lada-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateLADAToolsContext?.agentTo,
          agentThreadId: lastCreateLADAToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createLADATools: (ctx: Record<string, unknown>) => {
      lastCreateLADAToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateLADAToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-lada-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-lada-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateLADAToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateLADAToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateLADAToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateLADAToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-lada-message-to": "channel:24514",
        "x-lada-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-lada-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });
