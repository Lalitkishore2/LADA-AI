...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".lada", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "lada");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".lada");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "lada.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.LADA_STATE_DIR = path.join(tempHome, ".lada");
      delete process.env.LADA_CONFIG_PATH;
      process.env.LADA_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-wizard-home-"));
      process.env.HOME = tempHome;
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-minimal-gateway-home-"));
