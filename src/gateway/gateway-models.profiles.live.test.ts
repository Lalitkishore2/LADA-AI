...
import { resolveLADAAgentDir } from "../agents/agent-paths.js";
...
import { ensureLADAModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, LADAConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.LADA_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.LADA_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.LADA_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.LADA_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.LADA_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.LADA_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["LADA_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.LADA_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.LADA_LIVE_MAX_MODELS, 0));
}
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.LADA_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.LADA_LIVE_MODEL_TIMEOUT_MS,
...
describe("shouldSkipToolNonceProbeMissForLiveModel", () => {
  it.each([
    { modelKey: "anthropic/lada-opus-4-6", expected: true },
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<LADAConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.LADA_CONFIG_PATH,
    token: process.env.LADA_GATEWAY_TOKEN,
    skipChannels: process.env.LADA_SKIP_CHANNELS,
    skipGmail: process.env.LADA_SKIP_GMAIL_WATCHER,
    skipCron: process.env.LADA_SKIP_CRON,
    skipCanvas: process.env.LADA_SKIP_CANVAS_HOST,
    disableBonjour: process.env.LADA_DISABLE_BONJOUR,
    logLevel: process.env.LADA_LOG_LEVEL,
    agentDir: process.env.LADA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.LADA_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.LADA_DISABLE_BONJOUR = "1";
    process.env.LADA_LOG_LEVEL = "silent";
  }
  ...
  const hostAgentDir = resolveLADAAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-live-state-"));
  process.env.LADA_STATE_DIR = tempStateDir;
  ...
  process.env.LADA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.lada-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveLADAAgentDir();
  const sanitizedCfg: LADAConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-live-"));
  const tempConfigPath = path.join(tempDir, "lada.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.LADA_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "LADA live tool probe (local, safe): " +
    ...
    process.env.LADA_CONFIG_PATH = previous.configPath;
    process.env.LADA_GATEWAY_TOKEN = previous.token;
    process.env.LADA_SKIP_CHANNELS = previous.skipChannels;
    process.env.LADA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.LADA_SKIP_CRON = previous.skipCron;
    process.env.LADA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.LADA_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.LADA_LOG_LEVEL = previous.logLevel;
    process.env.LADA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.LADA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureLADAModelsJson(cfg);
        ...
        const rawModels = process.env.LADA_LIVE_GATEWAY_MODELS?.trim();
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via LADA_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const authStorage = discoverAuthStorage(agentDir);
    const modelRegistry = discoverModels(authStorage, agentDir);
    const anthropic = modelRegistry.find("anthropic", "lada-opus-4-6") as Model<Api> | null;
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.lada-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
    ...
    try {
      ...
      const toolText = await requestGatewayAgentText({
        ...
        modelKey: "anthropic/lada-opus-4-6",
        message:
