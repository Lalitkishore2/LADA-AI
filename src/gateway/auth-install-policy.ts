...
type GatewayInstallAuthMode = NonNullable<NonNullable<LADAConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: LADAConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.LADA_GATEWAY_PASSWORD?.trim() ||
