...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([LADA_MODEL_ID, LADA_DEFAULT_MODEL_ID]);
  ids.add(`lada/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`lada/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== LADA_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
