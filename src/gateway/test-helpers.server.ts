...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.LADA_CONFIG_PATH) {
    configPaths.add(process.env.LADA_CONFIG_PATH);
  }
  if (process.env.LADA_STATE_DIR) {
    configPaths.add(path.join(process.env.LADA_STATE_DIR, "lada.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "lada-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.LADA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "lada-test-no-bundled-extensions")
    : "lada-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.LADA_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".lada-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".lada-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".lada-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>lada-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__ladaTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__ladaTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["LADA_GATEWAY_TOKEN"]);
  const prev = process.env.LADA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.LADA_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.LADA_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
