...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.LADA_LIVE_ACP_BIND);
const describeLive = LIVE && ACP_BIND_LIVE ? describe : describe.skip;

const CONNECT_TIMEOUT_MS = 90_000;
const LIVE_TIMEOUT_MS = 240_000;
type LiveAcpAgent = "lada" | "codex" | "gemini";
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.LADA_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.LADA_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.LADA_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lada-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "lada.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.LADA_GATEWAY_PORT = previous.port;
        }
