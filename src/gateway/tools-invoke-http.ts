...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-lada-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-lada-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-lada-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-lada-thread-id"));
