...
export function resolveGatewayReloadSettings(cfg: LADAConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: LADAConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: LADAConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: LADAConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: LADAConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: LADAConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: LADAConfig) => {
