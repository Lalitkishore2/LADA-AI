...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("LADA_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.LADA_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.LADA_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliModelSwitchProbeTarget(
  ...
  if (normalizedProvider !== "lada-cli") {
    return undefined;
  }
  if (normalizedModelRef !== "lada-cli/lada-sonnet-4-6") {
    return undefined;
  }
  return "lada-cli/lada-opus-4-6";
}

export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.LADA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function withLADAMcpConfigOverrides(args: string[], mcpConfigPath: string): string[] {
  const next = [...args];
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.LADA_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.LADA_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.LADA_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.LADA_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("LADA_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("LADA_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("LADA_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("LADA_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("LADA_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("LADA_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("LADA_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("LADA_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("LADA_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("LADA_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("LADA_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
...
export async function verifyCliBackendImageProbe(params: {
  ...
  const imageProbe = await params.client.request(
    "agent",
    {
      ...
      // Route all providers through the same attachment pipeline. LADA CLI
