...
describe("model-pricing-cache", () => {
  ...
  it("collects configured model refs across defaults, aliases, overrides, and media tools", () => {
    const config = {
      agents: {
        defaults: {
          model: { primary: "gpt", fallbacks: ["anthropic/lada-sonnet-4-6"] },
        ...
        list: [
          {
            id: "router",
            model: { primary: "openrouter/anthropic/lada-opus-4-6" },
            subagents: { model: { primary: "openrouter/auto" } },
            heartbeat: { model: "anthropic/lada-opus-4-6" },
          },
        ],
      },
      channels: {
      ...
      hooks: {
        gmail: { model: "anthropic/lada-opus-4-6" },
        mappings: [{ model: "zai/glm-5" }],
      },
      tools: {
        subagents: { model: { primary: "anthropic/lada-haiku-4-5" } },
        media: {
    ...
    expect(refs).toEqual(
      expect.arrayContaining([
        ...
        "xai/grok-4",
        "openrouter/anthropic/lada-opus-4-6",
        "openrouter/auto",
        "zai/glm-5",
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as LADAConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("loads openrouter pricing and maps provider aliases, wrappers, and anthropic dotted ids", async () => {
    ...
    expect(
      getCachedGatewayModelPricing({ provider: "anthropic", model: "lada-opus-4-6" }),
    ).toEqual({
      input: 5,
      output: 25,
      cacheRead: 0.5,
