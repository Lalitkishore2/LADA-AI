...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("lada doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with LADA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with LADA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.LADA_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://lada-gateway.ai:18789",
      onConnectError,
    });
