...
export function attachLADATranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__lada && typeof record.__lada === "object" && !Array.isArray(record.__lada)
      ? (record.__lada as Record<string, unknown>)
      : {};
  return {
    ...record,
    __lada: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachLADATranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "lada" && model === "delivery-mirror";
