    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "lada", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-lada-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "lada", messages: [{ role: "user", content: "hi" }] },
          {
            "x-lada-agent-id": "beta",
            "x-lada-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-lada-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const res = await postChatCompletions(port, {
          ...
          messages: [
            ...
            { role: "assistant", content: "I am LADA." },
        ...
        expectMessageContext(message, {
          history: ["User: Hello, who are you?", "Assistant: I am LADA."],
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from LADA.");
      }
