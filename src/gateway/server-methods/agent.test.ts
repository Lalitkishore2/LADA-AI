...
const ORIGINAL_STATE_DIR = process.env.LADA_STATE_DIR;
...
describe("gateway agent handler", () => {
  afterEach(() => {
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.LADA_STATE_DIR = ORIGINAL_STATE_DIR;
    }
  ...
  it("preserves cliSessionIds from existing session entry", async () => {
    const existingCliSessionIds = { "lada-cli": "abc-123-def" };
    const existingLADACliSessionId = "abc-123-def";

    mockMainSessionEntry({
      cliSessionIds: existingCliSessionIds,
      ladaCliSessionId: existingLADACliSessionId,
    });

    const capturedEntry = await runMainAgentAndCaptureEntry("test-idem");
    expect(capturedEntry).toBeDefined();
    expect(capturedEntry?.cliSessionIds).toEqual(existingCliSessionIds);
    expect(capturedEntry?.ladaCliSessionId).toBe(existingLADACliSessionId);
  });
  ...
  it("does not create task rows for inter-session completion wakes", async () => {
    ...
    await invokeAgent(
      {
        message: [
          "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_LADA_INTERNAL_CONTEXT>>>",
  ...
  it("tracks async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "lada-gateway-agent-task-" }, async (root) => {
  ...
  it("handles missing cliSessionIds gracefully", async () => {
    ...
    expect(capturedEntry?.ladaCliSessionId).toBeUndefined();
  });
