...
import { type LADAConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: LADAConfig): string[] {
...
export function listAgentsForGateway(cfg: LADAConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: LADAConfig): string {
...
function resolveSessionStoreAgentId(cfg: LADAConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: LADAConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: LADAConfig): GatewaySessionsDefaults {
...
export async function resolveGatewayModelSupportsImages(params: {
  ...
  try {
    ...
    if (modelEntry) {
      ...
      if (
        normalizedProvider === "lada-cli" &&
        normalizedCandidates.some(
          (candidate) =>
            candidate === "opus" ||
            candidate === "sonnet" ||
            candidate === "haiku" ||
            candidate.startsWith("lada-"),
        )
      ) {
        return true;
      }
