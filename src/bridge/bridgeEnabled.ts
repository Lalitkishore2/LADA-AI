 ...
 *
 * Remote Control requires a lada.ai subscription (the bridge auths to CCR
 * with the lada.ai OAuth token). isLADAAISubscriber() excludes
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isLADAAISubscriber()) {
      return 'Remote Control requires a lada.ai subscription. Run `lada auth login` to sign in with your lada.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `lada setup-token` or CLAUDE_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `lada auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `lada auth login` to refresh your account information.'
    }
...
// program, before enableConfigs() runs. isLADAAISubscriber() → getGlobalConfig()
...
function isLADAAISubscriber(): boolean {
  try {
    return authModule.isLADAAISubscriber()
  } catch {
    return false
  }
}
 ...
 * lada.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of LADA (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`lada update\` to update.`
    }
  }
  return null
}
