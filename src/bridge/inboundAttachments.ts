 ...
 * (oauth-authed, same store), write to ~/.lada/uploads/{sessionId}/, and
 * return @path refs to prepend. LADA's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches LADA, just without @path.
 */
...
function uploadsDir(): string {
  return join(getLADAConfigHomeDir(), 'uploads', getSessionId())
}
