...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with lada.ai subscriptions. Please use `/login` to sign in with your lada.ai account.'

/** Full error printed when `lada remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  lada_code_args?: Record<string, string> | null
...
/**
 * How `lada remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so lada.ai can filter
 ...
 */
export type BridgeWorkerType = 'lada_code' | 'lada_code_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `lada remote-control
