 ...
 *
 * Used by both `lada remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getLADAAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getLADAAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getLADAAIOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `lada remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on lada.ai/code.
 *
