        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // Advertise session capacity so lada.ai/code can show
              ...
              // worker_type lets lada.ai filter environments by origin
    ...
    case 403:
      throw new BridgeFatalError(
        isExpiredErrorType(errorType)
          ? 'Remote Control session has expired. Please restart with `lada remote-control` or /remote-control.'
