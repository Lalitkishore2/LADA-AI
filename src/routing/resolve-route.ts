...
function listAgents(cfg: LADAConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: LADAConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<LADAConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: LADAConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: LADAConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: LADAConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<LADAConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  LADAConfig,
  {
    bindingsRef: LADAConfig["bindings"];
    agentsRef: LADAConfig["agents"];
    sessionRef: LADAConfig["session"];
...
function resolveRouteCacheForConfig(cfg: LADAConfig): Map<string, ResolvedAgentRoute> {
