...
import { getExternalLADAMdIncludes, getMemoryFiles, shouldShowLADAMdExternalIncludesWarning } from './utils/ladamd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], ladaInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    // Check for lada.md includes that need approval
    if (await shouldShowLADAMdExternalIncludesWarning()) {
      const externalIncludes = getExternalLADAMdIncludes(await getMemoryFiles(true));
      const {
        LADAMdExternalIncludesDialog
      } = await import('./components/LADAMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <LADAMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
  ...
  // processes but ignored by LADA itself (see auth.ts).
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      if (!isChannelsEnabled() || !getLADAAIOAuthTokens()?.accessToken) {
  ...
  // Show Chrome onboarding for first-time LADA in Chrome users
  if (ladaInChrome && !getGlobalConfig().hasCompletedLADAInChromeOnboarding) {
    const {
      LADAInChromeOnboarding
    } = await import('./components/LADAInChromeOnboarding.js');
    await showSetupDialog(root, done => <LADAInChromeOnboarding onDone={done} />);
  }
