...
/**
 * Returns a lada config directory path for a given source.
 */
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.lada', dir)
    case 'userSettings':
      return join(getLADAConfigHomeDir(), dir)
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getLADAConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.lada', 'skills')
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.lada', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.lada/skills from
