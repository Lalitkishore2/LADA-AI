...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "LADA_ENV_TEST_A";
    const keyB = "LADA_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "LADA_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "LADA_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "LADA_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "LADA_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "lada-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousLADAHome = process.env.LADA_HOME;
    const previousStateDir = process.env.LADA_STATE_DIR;
    const previousBundledDir = process.env.LADA_BUNDLED_PLUGINS_DIR;
    process.env.LADA_HOME = "/srv/lada-home";
    process.env.LADA_STATE_DIR = "/srv/lada-state";
    process.env.LADA_BUNDLED_PLUGINS_DIR = "/srv/lada-bundled";
    ...
    try {
      ...
      expect(env.LADA_HOME).toBeUndefined();
      expect(env.LADA_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.LADA_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("LADA_HOME", previousLADAHome);
      restoreEnvKey("LADA_STATE_DIR", previousStateDir);
      restoreEnvKey("LADA_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { LADA_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processLADAHome: process.env.LADA_HOME,
          processBundledDir: process.env.LADA_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.LADA_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processLADAHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.LADA_HOME).toBe("/srv/lada-home");
    } finally {
      restoreEnvKey("LADA_HOME", previousLADAHome);
    }
  });
