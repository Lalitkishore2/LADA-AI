...
describe("doctor config flow", () => {
  ...
  it("previews Matrix legacy sync-store migration in read-only mode", async () => {
    ...
    try {
      await withTempHome(async (home) => {
        const stateDir = path.join(home, ".lada");
        await fs.mkdir(path.join(stateDir, "matrix"), { recursive: true });
        await fs.writeFile(
          path.join(stateDir, "lada.json"),
          JSON.stringify({
            channels: {
              matrix: {
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "lada-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "lada-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/lada-matrix-missing"),
      ),
    ).toBe(true);
  });
  ...
  it("migrates top-level heartbeat into agents.defaults.heartbeat on repair", async () => {
    const result = await runDoctorConfigWithInput({
      ...
      config: {
        heartbeat: {
          model: "anthropic/lada-3-5-haiku-20241022",
          every: "30m",
        },
      },
      run: loadAndMaybeMigrateDoctorConfig,
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            String(message).includes('Run "lada doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockRestore();
