...
function createPerplexityConfig(apiKey: string, enabled?: boolean): LADAConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: LADAConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): LADAConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: LADAConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: LADAConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
