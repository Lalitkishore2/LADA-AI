...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("lada security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("lada security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.lada.ai/faq",
    "Troubleshooting: https://docs.lada.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("lada status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("lada logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("lada node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("lada status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("lada gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`lada devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("lada devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("lada devices list")}`),
  ];
}
