...
import { LADASchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: LADAConfig): {
  config: LADAConfig;
  removed: string[];
} {
  const parsed = LADASchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: LADAConfig): void {
