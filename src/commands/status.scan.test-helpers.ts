...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/lada-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = LADAConfig>(
  overrides: T = {} as T,
): LADAConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): LADAConfig {
