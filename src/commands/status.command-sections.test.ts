...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:lada security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:lada security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:lada status --all",
      "  Need to debug live? cmd:lada logs --follow",
      "  Fix reachability first: cmd:lada gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:lada devices approve req-123)",
      "muted(Fallback: cmd:lada devices approve --latest)",
      "muted(Inspect: cmd:lada devices list)",
    ]);
  });
