...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under LADA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("lada-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-oauth-sync-"));
    ...
    process.env.LADA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-oauth-nosync-"));
    process.env.LADA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside LADA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-oauth-external-"));
    process.env.LADA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* LADA_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to LADA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("lada-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
