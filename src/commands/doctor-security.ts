...
import type { LADAConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: LADAConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: LADAConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "LADA default (full)";
  const defaultRequestedAskSource = "LADA default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("lada approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: LADAConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: LADAConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("lada security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.lada/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("lada approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.lada.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("lada configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("lada config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("lada doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("lada config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('lada config set session.dmScope "per-channel-peer"') +
