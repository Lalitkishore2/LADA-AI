...
function asLegacyConfig(value: unknown): LADAConfig {
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeEach(() => {
    ...
    vi.doUnmock("lada/plugin-sdk/text-runtime");
    vi.resetModules();
    previousOauthDir = process.env.LADA_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-oauth-"));
    process.env.LADA_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.LADA_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-wa-auth-"));
    try {
