...
import { CLAUDE_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/ladaInChrome/common.js';
import { isChromeExtensionInstalled } from '../../utils/ladaInChrome/setup.js';
import { getGlobalConfig, saveGlobalConfig } from '../../utils/config.js';
import { env } from '../../utils/env.js';
import { isRunningOnHomespace } from '../../utils/envUtils.js';
const CHROME_EXTENSION_URL = 'https://lada.ai/chrome';
...
type Props = {
  ...
  isLADAAISubscriber: boolean;
  isWSL: boolean;
};
function LADAInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              ladaInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isLADAAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>LADA in Chrome works with the Chrome extension to let you control your browser directly from LADA. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">LADA in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t8;
  if ($[23] !== isLADAAISubscriber) {
    t8 = true && !isLADAAISubscriber && <Text color="error">LADA in Chrome requires a lada.ai subscription.</Text>;
    $[23] = isLADAAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>lada --chrome</Text><Text dimColor={true}> or </Text><Text>lada --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites LADA can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text dimColor={true}>Learn more: https://code.lada.com/docs/en/chrome</Text>;
    $[33] = t10;
  } else {
    t10 = $[33];
  }
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="LADA in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <LADAInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.ladaInChromeDefaultEnabled} isLADAAISubscriber={isSubscriber} isWSL={isWSL} />;
};
