...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("lada doctor --fix")} or ${formatCliCommand("lada memory status --fix")}`
    : `Verify: ${formatCliCommand("lada memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: LADAConfig): Promise<void> {
  try {
 ...
 * Runs as part of `lada doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("lada config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("lada config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("lada memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`lada config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("lada configure --section model")}`,
        `- To disable: ${formatCliCommand("lada config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("lada memory status --deep")}`,
      ].join("\n"),
      "Memory search",
