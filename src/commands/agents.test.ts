...
describe("agents helpers", () => {
  it("buildAgentSummaries includes default + configured agents", () => {
    ...
    expect(main?.model).toBe("anthropic/lada");
  ...
  it("applyAgentConfig merges updates", () => {
    const cfg: LADAConfig = {
      agents: {
        list: [{ id: "work", workspace: "/old-ws", model: "anthropic/lada" }],
      },
    };
    ...
    expect(work?.model).toBe("anthropic/lada");
  });
