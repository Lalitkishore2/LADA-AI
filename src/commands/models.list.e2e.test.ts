...
const ensureLADAModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveLADAAgentDir = vi.fn().mockReturnValue("/tmp/lada-agent");
...
beforeEach(() => {
  ...
  ensureLADAModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("models list does not treat availability-unavailable code as discovery fallback", async () => {
    configureGoogleAntigravityModel("lada-opus-4-6-thinking");
  ...
  it("loadModelRegistry throws when model discovery is unavailable", async () => {
    ...
    modelRegistryState.available = [
      makeGoogleAntigravityTemplate("lada-opus-4-6-thinking", "LADA Opus 4.5 Thinking"),
    ];
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureLADAModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureLADAModelsJson).toHaveBeenCalled();
    expect(ensureLADAModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
  ...
  it("toModelRow does not crash without cfg/authStore when availability is undefined", async () => {
    const row = toModelRow({
      model: makeGoogleAntigravityTemplate(
        "lada-opus-4-6-thinking",
        "LADA Opus 4.6 Thinking",
      ) as never,
      key: "google-antigravity/lada-opus-4-6-thinking",
      tags: [],
