...
vi.mock("../agents/workspace.js", () => ({
  ...
  resolveDefaultAgentWorkspaceDir: () => "/tmp/lada-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "lada-agent-cli-" });
}
...
describe("agentCommand CLI provider handling", () => {
  it("rejects explicit CLI overrides that are outside the models allowlist", async () => {
    vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(
      (provider) => provider.trim().toLowerCase() === "lada-cli",
    );
    try {
      await withTempHome(async (home) => {
        ...
        ).rejects.toThrow('Model override "lada-cli/opus" is not allowed for agent "main".');
      });
    } finally {
      vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(() => false);
    }
    ...
    try {
      await withTempHome(async (home) => {
        ...
        vi.mocked(loadModelCatalog).mockResolvedValueOnce([
          ...
          { id: "opus", name: "Opus", provider: "lada-cli" },
        ]);
  ...
  it("clears stale LADA CLI legacy session IDs before retrying after session expiration", async () => {
    vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(
      (provider) => provider.trim().toLowerCase() === "lada-cli",
    );
    try {
      await withTempHome(async (home) => {
        ...
        writeSessionStoreSeed(store, {
          [sessionKey]: {
            ...
            providerOverride: "lada-cli",
            modelOverride: "opus",
            cliSessionIds: { "lada-cli": "stale-cli-session" },
            ladaCliSessionId: "stale-legacy-session",
          },
        });

        mockConfig(home, store, {
          model: { primary: "lada-cli/opus", fallbacks: [] },
          models: { "lada-cli/opus": {} },
        });
        ...
        const saved = readSessionStore<{
          ...
          ladaCliSessionId?: string;
        }>(store);
        expect(saved[sessionKey]?.cliSessionIds?.["lada-cli"]).toBeUndefined();
        expect(saved[sessionKey]?.ladaCliSessionId).toBeUndefined();
      });
    } finally {
      vi.mocked(modelSelectionModule.isCliProvider).mockImplementation(() => false);
    }
