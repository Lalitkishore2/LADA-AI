...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.LADA_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("lada gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
