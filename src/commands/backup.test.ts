...
describe("backup commands", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".lada"), { recursive: true });
    delete process.env.LADA_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("lada-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".lada");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.LADA_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "lada.json"), JSON.stringify({}), "utf8");
  ...
  it("collapses default config, credentials, and workspace into the state backup root", async () => {
    const stateDir = path.join(tempHome.home, ".lada");
    const configPath = path.join(stateDir, "lada.json");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".lada");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-workspace-link-"));
    ...
    try {
      ...
      const plan = await resolveBackupPlanFromPaths({
        stateDir,
        configPath: path.join(stateDir, "lada.json"),
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".lada");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "lada-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-backups-"));
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".lada");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".lada");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "lada-backup-cwd-link-"));
