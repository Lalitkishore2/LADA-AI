...
const SEARCH_TOOL = "https://docs.lada.ai/mcp.SearchLADA";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.lada.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('lada docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.lada.ai/");
      runtime.log(`Search: ${formatCliCommand('lada docs "your query"')}`);
    }
    return;
  }
