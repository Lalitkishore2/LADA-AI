...
export async function runExtraUsage(): Promise<ExtraUsageResult> {
  ...
  const hasBillingAccess = hasLADAAiBillingAccess()

  if (!hasBillingAccess && isTeamOrEnterprise) {
    // Mirror apps/lada-ai useHasUnlimitedOverage(): if overage is enabled
  ...
  const url = isTeamOrEnterprise
    ? 'https://lada.ai/admin-settings/usage'
    : 'https://lada.ai/settings/usage'
