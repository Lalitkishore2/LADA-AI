...
type EnvSnapshot = {
  HOME?: string;
  LADA_HOME?: string;
  LADA_STATE_DIR?: string;
  LADA_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    LADA_HOME: process.env.LADA_HOME,
    LADA_STATE_DIR: process.env.LADA_STATE_DIR,
    LADA_OAUTH_DIR: process.env.LADA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: LADAConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: LADAConfig) {
...
async function runStateIntegrityText(cfg: LADAConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "lada-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.LADA_HOME = tempHome;
    process.env.LADA_STATE_DIR = path.join(tempHome, ".lada");
    delete process.env.LADA_OAUTH_DIR;
    fs.mkdirSync(process.env.LADA_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when LADA_OAUTH_DIR is explicitly configured", async () => {
    process.env.LADA_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: LADAConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("prints lada-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: LADAConfig = {};
    ...
    expect(text).toMatch(/lada sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/lada sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /lada sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
