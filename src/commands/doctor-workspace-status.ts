...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("lada tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("lada tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: LADAConfig) {
