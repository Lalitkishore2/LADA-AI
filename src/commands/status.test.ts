...
beforeAll(() => {
  envSnapshot = captureEnv(["LADA_PROFILE"]);
  process.env.LADA_PROFILE = "isolated";
});
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.lada.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.lada.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.LADA_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/lada-root.js", () => ({
  resolveLADAPackageRoot: vi.fn().mockResolvedValue("/tmp/lada"),
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/lada/pnpm-lock.yaml",
      markerPath: "/tmp/lada/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("lada status --all") ||
          line.includes("lada --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("lada --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("LADA_GATEWAY_TOKEN", "abcd1234", async () => {
