...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - LADA Chrome Extension MCP (browser-use via LADA's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **LADA Chrome Extension** - Uses the LADA Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or LADA Chrome Extension**:
   ...
   - For LADA Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.lada/skills/\` directory.** This ensures they are automatically loaded when LADA runs in the project.

Write the skill file to \`.lada/skills/<verifier-name>/SKILL.md\`.
...
1. Where each skill was created (always in \`.lada/skills/\`)
