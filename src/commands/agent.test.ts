...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/lada-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/lada-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "lada-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<LADAConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<LADAConfig["channels"]>["telegram"]>>,
  ...
  const cfg = {
    agents: {
      defaults: {
        ...
        models: { "anthropic/lada-opus-4-6": {} },
        workspace: path.join(home, "lada"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: LADAConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as LADAConfig },
    writeOptions: {},
...
describe("agentCommand", () => {
  ...
  it("persists explicit session-id-only runs with the synthetic session key", async () => {
    await withTempHome(async (home) => {
      ...
      mockConfig(home, store, {
        ...
        models: { "lada-cli/lada-sonnet-4-6": {} },
      });
  ...
  it("uses default fallback list for session model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      vi.mocked(loadModelCatalog).mockResolvedValueOnce([
        { id: "lada-opus-4-6", name: "Opus", provider: "anthropic" },
    ...
    await withTempHome(async (home) => {
      ...
      writeSessionStoreSeed(store, {
        "agent:main:subagent:clear-overrides": {
          ...
          fallbackNoticeSelectedModel: "anthropic/lada-opus-4-6",
          ...
          agentCommand(
            {
              ...
              model: "lada-haiku-4-5",
            },
            runtime,
          ),
        ).rejects.toThrow(
          'Model override "anthropic/lada-haiku-4-5" is not allowed for agent "main".',
        );
      });
    } finally {
  ...
  it("defaults thinking to adaptive for Anthropic LADA 4.6 models", async () => {
    await expectDefaultThinkLevel({
      agentOverrides: {
        model: { primary: "anthropic/lada-opus-4-6" },
        models: { "anthropic/lada-opus-4-6": {} },
