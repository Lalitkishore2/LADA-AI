...
import { getLADAAIOAuthTokens, isLADAAISubscriber } from '../../utils/auth.js';
  ...
  try {
    ...
    if (isLADAAISubscriber()) {
      ...
      if (tokens?.subscriptionType && tokens?.rateLimitTier) {
        isMax20x = tokens.subscriptionType === 'max' && tokens.rateLimitTier === 'default_lada_max_20x';
      } else if (tokens?.accessToken) {
        const profile = await getOauthProfileFromOauthToken(tokens.accessToken);
        isMax20x = profile?.organization?.organization_type === 'lada_max' && profile?.organization?.rate_limit_tier === 'default_lada_max_20x';
      }
      if (isMax20x) {
    ...
    const url = 'https://lada.ai/upgrade/max';
    ...
    setTimeout(onDone, 0, 'Failed to open browser. Please visit https://lada.ai/upgrade/max to upgrade.');
  }
  return null;
}
