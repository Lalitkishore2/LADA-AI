...
export const resolveLADAPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadLADAPlugins: () => createEmptyPluginRegistry(),
}));
...
vi.mock("../infra/lada-root.js", () => ({
  ...
  resolveLADAPackageRootSync: vi.fn(() => "/tmp/lada"),
}));
...
beforeEach(() => {
  ...
  resolveLADAPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.LADA_STATE_DIR;
  originalUpdateInProgress = process.env.LADA_UPDATE_IN_PROGRESS;
  process.env.LADA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-doctor-state-"));
