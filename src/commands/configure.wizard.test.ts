...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.lada/workspace",
  applyWizardMetadata: (cfg: LADAConfig) => cfg,
...
function setupBaseWizardState(config: LADAConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: LADAConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: LADAConfig) =>
