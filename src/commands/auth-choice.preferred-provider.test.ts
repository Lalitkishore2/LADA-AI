...
describe("resolvePreferredProviderForAuthChoice", () => {
  ...
  it("normalizes legacy auth choices before plugin lookup", async () => {
    ...
    await expect(resolvePreferredProviderForAuthChoice({ choice: "lada-cli" })).resolves.toBe(
      "anthropic",
    );
  ...
  it("passes explicit env through legacy auth normalization", async () => {
    const env = { LADA_AUTH_CHOICE_TEST: "1" } as NodeJS.ProcessEnv;
    resolveManifestDeprecatedProviderAuthChoice.mockReturnValue({
      choiceId: "anthropic-cli",
      choiceLabel: "Anthropic LADA CLI",
    });
    ...
    await expect(
      resolvePreferredProviderForAuthChoice({ choice: "lada-cli", env }),
    ).resolves.toBe("anthropic");
    expect(resolveManifestDeprecatedProviderAuthChoice).toHaveBeenCalledWith("lada-cli", { env });
  });
