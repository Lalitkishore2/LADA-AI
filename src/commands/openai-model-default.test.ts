...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: LADAConfig },
  primary: string,
) {
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    const defaultModel = "vercel-ai-gateway/anthropic/lada-opus-4.6";
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: LADAConfig) => config,
      applyDefaultConfig: (config: LADAConfig) => config,
      noteAgentModel,
  ...
  it("adds canonical allowlist key for anthropic aliases", async () => {
    ...
    expect(applied.config.agents?.defaults?.models?.[defaultModel]).toEqual({});
    expect(applied.config.agents?.defaults?.models?.["anthropic/lada-opus-4-6"]).toEqual({});
  });
...
describe("applyOpencodeZenModelDefault", () => {
  ...
  it("overrides existing models", () => {
    const cfg = {
      agents: { defaults: { model: "anthropic/lada-opus-4-6" } },
    } as LADAConfig;
    const applied = applyOpencodeZenModelDefault(cfg);
    expectPrimaryModelChanged(applied, OPENCODE_ZEN_DEFAULT_MODEL);
  });
  ...
  it("no-ops when already legacy opencode-zen default", () => {
    const cfg = {
      agents: { defaults: { model: "opencode-zen/lada-opus-4-5" } },
    } as LADAConfig;
    const applied = applyOpencodeZenModelDefault(cfg);
