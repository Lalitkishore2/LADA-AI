...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveLADAAgentDir: vi.fn().mockReturnValue("/tmp/lada-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/lada-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/lada-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/lada-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/lada-dev/lada.json",
    }),
    loadConfig: vi.fn().mockReturnValue({
      agents: {
        defaults: {
          model: { primary: "anthropic/lada-opus-4-6", fallbacks: [] },
          models: { "anthropic/lada-opus-4-6": { alias: "Opus" } },
        },
      },
      models: { providers: {} },
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveLADAAgentDir: mocks.resolveLADAAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/lada-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveLADAAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/lada-opus-4-6");
    expect(payload.configPath).toBe("/tmp/lada-dev/lada.json");
    expect(payload.auth.storePath).toBe("/tmp/lada-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/lada-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/lada-agent-custom");
  ...
  it("does not report cli backends as missing auth", async () => {
    ...
    mocks.loadConfig.mockReturnValue({
      agents: {
        defaults: {
          model: { primary: "lada-cli/lada-sonnet-4-6", fallbacks: [] },
          models: { "lada-cli/lada-sonnet-4-6": {} },
          cliBackends: { "lada-cli": {} },
        },
      },
      models: { providers: {} },
      env: { shellEnv: { enabled: true } },
    ...
    try {
      await modelsStatusCommand({ json: true }, localRuntime as never);
      const payload = JSON.parse(String((localRuntime.log as Mock).mock.calls[0]?.[0]));
      expect(payload.defaultModel).toBe("lada-cli/lada-sonnet-4-6");
      expect(payload.auth.missingProvidersInUse).toEqual([]);
    } finally {
