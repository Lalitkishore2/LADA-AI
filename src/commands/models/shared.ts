...
export async function loadValidConfigOrThrow(): Promise<LADAConfig> {
...
export async function updateConfig(
  mutator: (cfg: LADAConfig) => LADAConfig,
): Promise<LADAConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: LADAConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LADAConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("lada agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
