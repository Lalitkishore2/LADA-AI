...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.lada", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.lada", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/lada-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/lada/.lada",
    });

    expect(result).toEqual({
      path: "/mnt/slow/lada/.lada",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".lada"), {
      platform: "darwin",
      mountInfo,
    });
