...
describe("legacy web search config", () => {
  it("migrates legacy provider config through bundled web search ownership metadata", () => {
    const res = migrateLegacyWebSearchConfig<LADAConfig>({
      tools: {
        web: {
          search: {
            provider: "grok",
    ...
    expect(findLegacyConfigIssues(rawConfig)).toEqual([
      {
        ...
        message:
          'tools.web.search provider-owned config moved to plugins.entries.<plugin>.config.webSearch. Run "lada doctor --fix".',
      },
    ]);
