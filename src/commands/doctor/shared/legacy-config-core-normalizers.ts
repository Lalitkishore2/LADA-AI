...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as LADAConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as LADAConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<LADAConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<LADAConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<LADAConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as LADAConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyTalkConfig(cfg: LADAConfig, changes: string[]): LADAConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as LADAConfig["talk"]) ?? {};
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<LADAConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<LADAConfig["models"]>["providers"],
    },
  };
}
