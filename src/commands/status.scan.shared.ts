...
export function hasExplicitMemorySearchConfig(cfg: LADAConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: LADAConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: LADAConfig, agentId: string) => { store: { path: string } } | null;
