...
function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<LADAConfig>;
}): ProviderPlugin {
  return {
    ...
    auth: [
      createProviderApiKeyAuthMethod({
        ...
        ...(params.applyConfig ? { applyConfig: () => params.applyConfig as LADAConfig } : {}),
        wizard: {
...
function createDefaultProviderPlugins() {
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as LADAConfig,
  ...
  const cloudflareAiGatewayMethod: ProviderAuthMethod = {
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        defaultModel: "cloudflare-ai-gateway/lada-sonnet-4-5",
      };
    },
  };
  ...
  return [
    ...
    createApiKeyProvider({
      ...
      defaultModel: "litellm/anthropic/lada-opus-4.6",
    }),
    createApiKeyProvider({
      providerId: "minimax",
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("lada-auth-");
  ...
  it("uses existing env API keys for selected providers", async () => {
    const scenarios: Array<{
      ...
      },
      {
        ...
        expectedModel: "vercel-ai-gateway/anthropic/lada-opus-4.6",
      },
      {
        authChoice: "ai-gateway-api-key",
        envKey: "AI_GATEWAY_API_KEY",
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/lada-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("keeps existing default model for explicit provider keys when setDefaultModel=false", async () => {
    const scenarios: Array<{
      ...
      },
      {
        ...
        promptMessage: "Enter OpenCode API key",
        existingPrimary: "anthropic/lada-opus-4-5",
        expectedOverride: "opencode/lada-opus-4-6",
        profileId: "opencode:default",
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireLADAAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
