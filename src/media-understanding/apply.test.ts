...
const TEMP_MEDIA_PREFIX = "lada-media-";
...
function createGroqAudioConfig(): LADAConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): LADAConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): LADAConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: LADAConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredLADATmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        LADA_AGENT_DIR: isolatedAgentDir,
