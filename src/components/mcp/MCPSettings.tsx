...
import { LADAAuthProvider } from '../../services/mcp/auth.js';
import type { McpLADAAIProxyServerConfig, McpHTTPServerConfig, McpSSEServerConfig, McpStdioServerConfig } from '../../services/mcp/types.js';
    ...
    t5 = () => {
      ...
      const prepareServers = async function prepareServers() {
        const serverInfos = await Promise.all(filteredClients.map(async client_0 => {
          ...
          const isLADAAIProxy = client_0.config.type === "ladaai-proxy";
          let isAuthenticated = undefined;
          if (isSSE || isHTTP) {
            const authProvider = new LADAAuthProvider(client_0.name, client_0.config as McpSSEServerConfig | McpHTTPServerConfig);
          ...
          if (isLADAAIProxy) {
            return {
              ...baseInfo,
              transport: "ladaai-proxy" as const,
              isAuthenticated: false,
              config: client_0.config as McpLADAAIProxyServerConfig
            };
          } else {
            if (isSSE) {
              return {
  ...
  if ($[10] !== agentMcpServers.length || $[11] !== filteredClients.length || $[12] !== onComplete || $[13] !== servers.length) {
    t7 = () => {
      ...
      if (servers.length === 0 && agentMcpServers.length === 0) {
        onComplete("No MCP servers configured. Please run /doctor if this is unexpected. Otherwise, run `lada mcp --help` or visit https://code.lada.com/docs/en/mcp to learn more.");
      }
    };
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "ladaai-proxy" ? "lada.ai" : "LADA";
