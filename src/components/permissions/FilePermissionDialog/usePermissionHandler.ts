...
export type PermissionHandlerOptions = {
  ...
  scope?: 'lada-folder' | 'global-lada-folder'
}
...
function handleAcceptSession(
  ...
  // For lada-folder scope, grant session-level access to all .lada/ files
  if (
    options?.scope === 'lada-folder' ||
    options?.scope === 'global-lada-folder'
  ) {
    const pattern =
      options.scope === 'global-lada-folder'
        ? GLOBAL_CLAUDE_FOLDER_PERMISSION_PATTERN
