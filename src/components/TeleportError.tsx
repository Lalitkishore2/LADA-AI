...
import { checkIsGitClean, checkNeedsLADAAiLogin } from 'src/utils/background/remote/preconditions.js';
...
export function TeleportError(t0) {
  ...
  const handleLoginWithLADAAI = t6;
  let t7;
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = value => {
      if (value === "login") {
        handleLoginWithLADAAI();
      } else {
        onCancel();
      }
    };
    ...
    case "needsLogin":
      {
        if (isLoggingIn) {
          ...
          if ($[14] !== handleLoginComplete) {
            t9 = <ConsoleOAuthFlow onDone={handleLoginComplete} mode="login" forceLoginMethod="ladaai" />;
        ...
        if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
          t9 = <Box flexDirection="column"><Text dimColor={true}>Teleport requires a LADA.ai account.</Text><Text dimColor={true}>Your LADA Pro/Max subscription will be used by LADA.</Text></Box>;
          $[16] = t9;
        } else {
          t9 = $[16];
        }
        ...
        if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
          t10 = <Dialog title="Log in to LADA" onCancel={onCancel}>{t9}<Select options={[{
              label: "Login with LADA account",
              value: "login"
            }, {
              label: "Exit",
              value: "exit"
...
export async function getTeleportErrors(): Promise<Set<TeleportLocalErrorType>> {
  ...
  const [needsLogin, isGitClean] = await Promise.all([checkNeedsLADAAiLogin(), checkIsGitClean()]);
  if (needsLogin) {
