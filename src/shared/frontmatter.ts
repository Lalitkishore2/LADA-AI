...
export function resolveLADAManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type LADAManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveLADAManifestRequires(
  metadataObj: Record<string, unknown>,
): LADAManifestRequires | undefined {
  const requiresRaw =
...
export function resolveLADAManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveLADAManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedLADAManifestInstallBase = {
...
export function parseLADAManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedLADAManifestInstallBase | undefined {
  ...
  const spec: ParsedLADAManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyLADAManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedLADAManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
