...
function collectModels(cfg: LADAConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function isGatewayRemotelyExposed(cfg: LADAConfig): boolean {
...
const LEGACY_MODEL_PATTERNS: Array<{ id: string; re: RegExp; label: string }> = [
  ...
  { id: "anthropic.lada2", re: /\blada-(instant|2)\b/i, label: "LADA 2/Instant family" },
...
function isLADAModel(id: string): boolean {
  return /\blada-/i.test(id);
}

function isLADA45OrHigher(id: string): boolean {
  // Match lada-*-4-5+, lada-*-45+, lada-*4.5+, or future 5.x+ majors.
  return /\blada-[^\s/]*?(?:-4-?(?:[5-9]|[1-9]\d)\b|4\.(?:[5-9]|[1-9]\d)\b|-[5-9](?:\b|[.-]))/i.test(
    id,
  );
}
...
function listKnownNodeCommands(cfg: LADAConfig): Set<string> {
  const baseCfg: LADAConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function listGroupPolicyOpen(cfg: LADAConfig): string[] {
  const out: string[] = [];
...
function listPotentialMultiUserSignals(cfg: LADAConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: LADAConfig): {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep LADA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("lada security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer LADA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const ladaGatewayToken =
    typeof env.LADA_GATEWAY_TOKEN === "string" && env.LADA_GATEWAY_TOKEN.trim()
      ? env.LADA_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-lada-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: lada-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  for (const entry of models) {
    ...
    if (isLADAModel(entry.id) && !isLADA45OrHigher(entry.id)) {
      addWeakMatch(entry.id, entry.source, "Below LADA 4.5");
    }
  }
  ...
  if (weakMatches.size > 0) {
    ...
    findings.push({
      ...
      detail:
        ...
        more,
      remediation:
        "Use the latest, top-tier model for any bot with tools or untrusted inboxes. Avoid Haiku tiers; prefer GPT-5+ and LADA 4.5+.",
    });
  }
...
export function collectExposureMatrixFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: LADAConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "LADA's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
