...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("lada status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("lada security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set LADA_GATEWAY_TOKEN/LADA_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("lada security audit --deep")}".`,
    });
  }
  return findings;
}
