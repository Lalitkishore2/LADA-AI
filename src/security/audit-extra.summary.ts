...
function summarizeGroupPolicy(cfg: LADAConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: LADAConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: LADAConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: LADAConfig): boolean {
...
function isBrowserEnabled(cfg: LADAConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: LADAConfig): SecurityAuditFinding[] {
