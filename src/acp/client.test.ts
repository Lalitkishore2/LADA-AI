...
const createTempDir = () => tempDirs.make("lada-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets LADA_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "lada",
    });

    expect(env.LADA_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("lada");
  });

  it("overrides pre-existing LADA_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      LADA_SHELL: "wrong",
    });
    expect(env.LADA_SHELL).toBe("acp-client");
  ...
  it("preserves LADA_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        LADA_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["LADA_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default LADA bridge", () => {
    ...
    expect(env.LADA_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.LADA_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("LADA_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "lada", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "lada", "dist", "entry.js");
    const shimPath = path.join(dir, "lada.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\lada\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/lada-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/lada-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/lada-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
