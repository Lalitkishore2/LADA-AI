...
function setConfiguredFetchProviderKey(configTarget: LADAConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: LADAConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: LADAConfig, provider: ProviderUnderTest): unknown {
