...
type SecretRegistryEntry = {
  id: string;
  configFile: "lada.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.LADA_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_LADA_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForLADATarget(
  config: LADAConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every lada.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "lada.json" &&
        !PLUGIN_OWNED_LADA_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("lada.json", batch);
      const config = {} as LADAConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `LADA_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForLADATarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `LADA_AUTH_SECRET_TARGET_${entry.id}`;
