...
function beginSecretsRuntimeIsolationForTest(): SecretsRuntimeEnvSnapshot {
  const envSnapshot = captureEnv([
    ...
    "LADA_DISABLE_BUNDLED_PLUGINS",
    "LADA_DISABLE_PLUGIN_DISCOVERY_CACHE",
    "LADA_VERSION",
  ]);
  delete process.env.LADA_BUNDLED_PLUGINS_DIR;
  process.env.LADA_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
  delete process.env.LADA_VERSION;
  return envSnapshot;
}
...
describe("secrets runtime snapshot core lanes", () => {
  ...
  it("resolves env-backed auth profile SecretRefs", async () => {
    ...
    expect(snapshot.warnings.map((warning) => warning.path)).toEqual(
      expect.arrayContaining([
        "/tmp/lada-agent-main.auth-profiles.openai:default.key",
        "/tmp/lada-agent-main.auth-profiles.github-copilot:default.token",
      ]),
    );
  ...
  it("activates runtime snapshots for ensureAuthProfileStore", async () => {
    ...
    expect(
      ensureAuthProfileStore("/tmp/lada-agent-main").profiles["openai:default"],
    ).toMatchObject({
      type: "api_key",
      key: "sk-runtime",
    });
