...
export function buildConfigureCandidates(config: LADAConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: LADAConfig;
  authoredLADAConfig?: LADAConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredLADAConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const ladaCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...ladaCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "lada secrets configure",
