...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your LADA.ai account (not Console).';
    case 'no_remote_environment':
      return 'No cloud environment available. Set one up at https://lada.ai/code/onboarding?magic=env-setup';
    ...
    case 'github_app_not_installed':
      return 'The LADA GitHub app must be installed on this repository first.\nhttps://github.com/apps/lada/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. LADA never
 ...
 * lada.ai URL stays a durable record the user can revisit; TTL handles cleanup.
 */
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (!task || task.status !== 'running') {
        ...
        // Session left alive so the lada.ai URL stays valid — the run_hunt.sh
      ...
      // stableIdle is a prompt-mode completion signal (LADA stops writing
      ...
      if (result || sessionDone || reviewTimedOut) {
        ...
        // the formatted findings as an assistant event, so the lada.ai URL
...
/**
 * RemoteAgentTask - Handles remote LADA.ai session execution.
 *
