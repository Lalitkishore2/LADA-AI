...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/lada-root.test.ts"])).toEqual([
      {
        config: "vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/lada-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("lada-vitest-include-");
    expect(spec?.env.LADA_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });
