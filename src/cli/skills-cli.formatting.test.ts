...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "lada-bundled-skills-test-"));
    process.env.LADA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
