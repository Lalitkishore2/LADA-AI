 ...
 * These are dynamically imported only when `lada plugin *` or `lada plugin marketplace *` runs.
 */
...
export async function pluginValidateHandler(
  ...
  try {
    ...
    // If this is a plugin manifest located inside a .lada-plugin directory,
    ...
    if (result.fileType === 'plugin') {
      ...
      if (basename(manifestDir) === '.lada-plugin') {
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `lada plugin install` to install a plugin.',
      )
    }
  }
