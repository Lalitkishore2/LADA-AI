...
const mocks = vi.hoisted(() => ({
  loadLADAPlugins: vi.fn<typeof import("../plugins/loader.js").loadLADAPlugins>(),
...
vi.mock("../plugins/loader.js", () => ({
  loadLADAPlugins: (...args: Parameters<typeof mocks.loadLADAPlugins>) =>
    mocks.loadLADAPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", () => {
    ...
    expect(mocks.loadLADAPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadLADAPlugins).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
  ...
  it("does not treat a pre-seeded partial registry as all scope", () => {
    ...
    expect(mocks.loadLADAPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadLADAPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
