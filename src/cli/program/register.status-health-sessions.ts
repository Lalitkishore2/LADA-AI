...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada status", "Show channel health + session summary."],
          ["lada status --all", "Full diagnosis (read-only)."],
          ["lada status --json", "Machine-readable output."],
          ["lada status --usage", "Show model provider usage/quota snapshots."],
          [
            "lada status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["lada status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.lada.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.lada.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada sessions", "List all sessions."],
          ["lada sessions --agent work", "List sessions for one agent."],
          ["lada sessions --all-agents", "Aggregate sessions across agents."],
          ["lada sessions --active 120", "Only last 2 hours."],
          ["lada sessions --json", "Machine-readable output."],
          ["lada sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.lada.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "lada sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["lada sessions cleanup --enforce", "Apply maintenance now."],
          ["lada sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["lada sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "lada sessions cleanup --enforce --store ./tmp/sessions.json",
