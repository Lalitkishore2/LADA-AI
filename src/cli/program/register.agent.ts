...
${formatHelpExamples([
  ['lada agent --to +15555550123 --message "status update"', "Start a new session."],
  ['lada agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'lada agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'lada agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['lada agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'lada agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.lada.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.lada.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['lada agents set-identity --agent main --name "LADA" --emoji "🦞"', "Set name + emoji."],
  ["lada agents set-identity --agent main --avatar avatars/lada.png", "Set avatar path."],
  [
    "lada agents set-identity --workspace ~/.lada/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "lada agents set-identity --identity-file ~/.lada/workspace/IDENTITY.md --agent main",
