    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada channels list", "List configured channels and auth profiles."],
          ["lada channels status --probe", "Run channel status checks and probes."],
          [
            "lada channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["lada channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.lada.ai/cli/channels",
        )}\n`,
    );
