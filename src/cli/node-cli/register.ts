    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "lada node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["lada node status", "Check node host service status."],
          ["lada node install", "Install the node host service."],
          ["lada node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.lada.ai/cli/node")}\n`,
    );
