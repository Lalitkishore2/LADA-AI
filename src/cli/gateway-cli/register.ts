      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["lada gateway run", "Run the gateway in the foreground."],
            ["lada gateway status", "Show service status and probe reachability."],
            ["lada gateway discover", "Find local and wide-area gateway beacons."],
            ["lada gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.lada.ai/cli/gateway")}\n`,
      ),
  );
