...
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/lada-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/lada-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/lada-control-ui";
  ...
  it.each([
    ...
    ["--lada-cli-logs", "deprecated alias"],
  ])("enables CLI backend log filtering via %s (%s)", async (flag) => {
    delete process.env.LADA_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.LADA_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `lada onboard --mode local` or `lada setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "LADA_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
