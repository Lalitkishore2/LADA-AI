...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.lada${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.LADA_PROFILE = profile;

  const stateDir = env.LADA_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.LADA_STATE_DIR?.trim()) {
    env.LADA_STATE_DIR = stateDir;
  }

  if (!env.LADA_CONFIG_PATH?.trim()) {
    env.LADA_CONFIG_PATH = path.join(stateDir, "lada.json");
  }

  if (profile === "dev" && !env.LADA_GATEWAY_PORT?.trim()) {
    env.LADA_GATEWAY_PORT = "19001";
  }
}
