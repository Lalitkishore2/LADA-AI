...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "lada";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage LADA plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.lada.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "lada"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as LADAConfig;
      const enableResult = enablePluginInConfig(cfg, id);
      let next: LADAConfig = enableResult.config;
  ...
  plugins
    ...
    .option(
      "--marketplace <source>",
      "Install a LADA marketplace plugin from a local repo/path or git/GitHub source",
    )
    .action(
      async (
        raw: string,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.lada.ai/plugin");
      lines.push("");
  ...
  const marketplace = plugins
    .command("marketplace")
    .description("Inspect LADA-compatible plugin marketplaces");
