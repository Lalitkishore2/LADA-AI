...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as LADAConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: LADAConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/lada-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: LADAConfig) =>
    invokeMock<
      [LADAConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: LADAConfig, pluginId: string) =>
    invokeMock<[LADAConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as LADAConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/lada-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: LADAConfig }) => ({
    config,
    warnings: [],
