...
const serveLADAChannelMcp = mocks.serveLADAChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveLADAChannelMcp: mocks.serveLADAChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "lada-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("lada-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("lada-cli-mcp-home-", async () => {
      ...
      expect(serveLADAChannelMcp).toHaveBeenCalledWith({
