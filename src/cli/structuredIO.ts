...
export class StructuredIO {
   ...
   * Used by the bridge to feed permission responses from lada.ai into the
   * SDK permission flow.
   *
   ...
   * stale permission prompt on lada.ai when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt on lada.ai.
        if (
