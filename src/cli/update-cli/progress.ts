...
const STEP_LABELS: Record<string, string> = {
  ...
  "lada doctor entry": "Checking doctor entrypoint",
  "lada doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g lada@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g lada@latest --omit=optional");
  }
