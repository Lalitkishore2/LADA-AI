...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("lada-daemon-install-int-");
    configPath = path.join(tempHome, "lada.json");
    process.env.HOME = tempHome;
    process.env.LADA_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.LADA_GATEWAY_TOKEN).toBeUndefined();
  });
});
