...
import { serveLADAChannelMcp } from "../mcp/channel-server.js";
...
export function registerMcpCli(program: Command) {
  const mcp = program.command("mcp").description("Manage LADA MCP config and channel bridge");

  mcp
    .command("serve")
    .description("Expose LADA channels over MCP stdio")
    ...
    .option(
      "--lada-channel-mode <mode>",
      "LADA channel notification mode: auto, on, or off",
      "auto",
    )
    ...
    .action(async (opts) => {
      try {
        ...
        const ladaChannelMode = String(opts.ladaChannelMode ?? "auto")
          .trim()
          .toLowerCase();
        if (
          ladaChannelMode !== "auto" &&
          ladaChannelMode !== "on" &&
          ladaChannelMode !== "off"
        ) {
          throw new Error("Invalid --lada-channel-mode value. Use auto, on, or off.");
        }
        await serveLADAChannelMcp({
