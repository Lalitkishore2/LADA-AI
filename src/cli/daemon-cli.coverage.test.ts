...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      LADA_GATEWAY_PORT: String(params.port),
      ...(params.token ? { LADA_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.LADA_STATE_DIR = "/tmp/lada-cli-state";
    process.env.LADA_CONFIG_PATH = "/tmp/lada-cli-state/lada.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        LADA_PROFILE: "dev",
        LADA_STATE_DIR: "/tmp/lada-daemon-state",
        LADA_CONFIG_PATH: "/tmp/lada-daemon-state/lada.json",
        LADA_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.lada.gateway.plist",
    });
