...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update LADA and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["lada update", "Update a source checkout (git)"],
        ["lada update --channel beta", "Switch to beta channel (git + npm)"],
        ["lada update --channel dev", "Switch to dev channel (git + npm)"],
        ["lada update --tag beta", "One-off update to a dist-tag or version"],
        ["lada update --tag main", "One-off package install from GitHub main"],
        ["lada update --dry-run", "Preview actions without changing anything"],
        ["lada update --no-restart", "Update without restarting the service"],
        ["lada update --json", "Output result as JSON"],
        ["lada update --yes", "Non-interactive (accept downgrade prompts)"],
        ["lada update wizard", "Interactive update wizard"],
        ["lada --update", "Shorthand for lada update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run lada update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.lada.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada update status", "Show channel + version status."],
          ["lada update status --json", "JSON output."],
          ["lada update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.lada.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
