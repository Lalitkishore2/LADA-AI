...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["lada security audit", "Run a local security audit."],
          ["lada security audit --deep", "Include best-effort live Gateway probe checks."],
          ["lada security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "lada security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["lada security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["lada security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.lada.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("LADA security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("lada security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("lada security audit --fix")}`));
        if (!fixResult) {
