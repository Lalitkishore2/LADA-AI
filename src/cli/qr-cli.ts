...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.LADA_GATEWAY_PASSWORD)) {
    return false;
  }
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.lada.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the LADA mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("lada devices list")}`,
          `  ${theme.command("lada devices approve <requestId>")}`,
        );
