...
const SANDBOX_EXAMPLES = {
  main: [
    ["lada sandbox list", "List all sandbox containers."],
    ["lada sandbox list --browser", "List only browser containers."],
    ["lada sandbox recreate --all", "Recreate all containers."],
    ["lada sandbox recreate --session main", "Recreate a specific session."],
    ["lada sandbox recreate --agent mybot", "Recreate agent containers."],
    ["lada sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["lada sandbox list", "List all sandbox containers."],
    ["lada sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["lada sandbox recreate --all", "Recreate all containers."],
    ["lada sandbox recreate --session main", "Recreate a specific session."],
    ["lada sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["lada sandbox recreate --browser --all", "Recreate only browser containers."],
    ["lada sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["lada sandbox explain", "Show effective sandbox config."],
    ["lada sandbox explain --session agent:main:main", "Explain a specific session."],
    ["lada sandbox explain --agent work", "Explain an agent sandbox."],
    ["lada sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.lada.ai/cli/sandbox")}\n`,
    )
    .action(() => {
