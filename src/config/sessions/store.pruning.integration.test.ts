...
// Keep integration tests deterministic: never read a real lada.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "lada-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.LADA_SESSION_CACHE_TTL_MS;
    process.env.LADA_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.LADA_SESSION_CACHE_TTL_MS;
    } else {
      process.env.LADA_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
