...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "lada-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  it("keeps writes inside an LADA_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".lada", "lada.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "lada.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "lada.json"), "utf-8"),
      ) as {
