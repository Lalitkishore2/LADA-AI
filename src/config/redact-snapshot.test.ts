...
import type { ConfigFileSnapshot } from "./types.lada.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/lada/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/lada/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/lada/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/lada/nickserv-password.txt");
