...
import {
  compareLADAVersions,
  isSameLADAStableFamily,
  parseLADAVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseLADAVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseLADAVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseLADAVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseLADAVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseLADAVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseLADAVersion("2026.3")).toBeNull();
    expect(parseLADAVersion("latest")).toBeNull();
  });
});

describe("compareLADAVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareLADAVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareLADAVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareLADAVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareLADAVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareLADAVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareLADAVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameLADAStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameLADAStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameLADAStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameLADAStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameLADAStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
