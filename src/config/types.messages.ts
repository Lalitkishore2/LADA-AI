...
export type MessagesConfig = {
   ...
   * - `{model}` - short model name (e.g., `lada-opus-4-6`, `gpt-4o`)
   * - `{modelFull}` - full model identifier (e.g., `anthropic/lada-opus-4-6`)
   * - `{provider}` - provider name (e.g., `anthropic`, `openai`)
   * - `{thinkingLevel}` or `{think}` - current thinking level (`high`, `low`, `off`)
   * - `{identity.name}` or `{identityName}` - agent identity name
   *
   * Example: `"[{model} | think:{thinkingLevel}]"` → `"[lada-opus-4-6 | think:high]"`
   *
...
export type CommandsConfig = {
  ...
  config?: boolean;
  /** Allow /mcp command for LADA-managed MCP settings (default: false). */
  mcp?: boolean;
