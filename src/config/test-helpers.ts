...
import { resetConfigRuntimeState, type LADAConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        LADA_DISABLE_PLUGIN_DISCOVERY_CACHE: undefined,
        LADA_DISABLE_PLUGIN_MANIFEST_CACHE: undefined,
        LADA_PLUGIN_CATALOG_PATHS: undefined,
        LADA_MPM_CATALOG_PATHS: undefined,
        LADA_PLUGIN_DISCOVERY_CACHE_MS: undefined,
        LADA_PLUGIN_MANIFEST_CACHE_MS: undefined,
      },
    });
  } finally {
    resetConfigTestRuntimeState();
...
export async function writeLADAConfig(home: string, config: unknown): Promise<string> {
  const configPath = path.join(home, ".lada", "lada.json");
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.LADA_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected LADA_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeLADAConfig(home, config);
