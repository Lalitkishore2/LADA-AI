...
import type { ConfigFileSnapshot, LADAConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => LADAConfig>());
...
function makeSnapshot(params: { valid: boolean; config?: LADAConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/lada.json",
    exists: true,
    raw: "{}",
