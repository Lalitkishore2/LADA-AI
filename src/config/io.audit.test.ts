...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "lada-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".lada", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      formatConfigOverwriteLogMessage({
        configPath: "/tmp/lada.json",
        previousHash: "prev-hash",
        nextHash: "next-hash",
        changedPathCount: 3,
      }),
    ).toBe(
      "Config overwrite: /tmp/lada.json (sha256 prev-hash -> next-hash, backup=/tmp/lada.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      env: {
        LADA_WATCH_MODE: "1",
        LADA_WATCH_SESSION: "watch-session-1",
        LADA_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("appends JSONL audit entries to the resolved audit path", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: createConfigWriteAuditRecordBase({
        configPath: path.join(home, ".lada", "lada.json"),
        env: {} as NodeJS.ProcessEnv,
        existsBefore: true,
        previousHash: "prev-hash",
        nextHash: "next-hash",
    ...
    const auditPath = path.join(home, ".lada", "logs", "config-audit.jsonl");
