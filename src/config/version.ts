...
export function parseLADAVersion(raw: string | null | undefined): LADAVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeLADAVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseLADAVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameLADAStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseLADAVersion(a);
  const parsedB = parseLADAVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareLADAVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseLADAVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseLADAVersion(current);
  const parsedTouched = parseLADAVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameLADAStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareLADAVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: LADAVersion): number {
