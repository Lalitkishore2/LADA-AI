...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("lada-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "lada-sandbox-browser-global" },
      agentBrowser: { network: "lada-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("lada-sandbox-browser-agent");
  });
