...
import type { LADAConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.lada.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: LADAConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: LADAConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: LADAConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LADASchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as LADAConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: LADAConfig | null | undefined;
  ...
  const ensureCompatConfig = (): LADAConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native LADA config schema.
