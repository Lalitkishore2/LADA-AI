...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "lada.plugin.json"),
...
async function writeBundleFixture(params: {
  ...
  const manifestDir = path.join(
    params.dir,
    params.format === "codex" ? ".codex-plugin" : ".lada-plugin",
  );
...
async function writeManifestlessLADABundleFixture(params: { dir: string }) {
  await mkdirSafe(params.dir);
...
describe("config plugin validation", () => {
  ...
  let bundlePluginDir = "";
  let manifestlessLADABundleDir = "";
  const suiteEnv = () =>
    ({
      HOME: suiteHome,
      LADA_HOME: undefined,
      LADA_STATE_DIR: path.join(suiteHome, ".lada"),
      LADA_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lada-config-plugin-validation-"));
    ...
    manifestlessLADABundleDir = path.join(suiteHome, "manifestless-lada-bundle");
    await writeManifestlessLADABundleFixture({
      dir: manifestlessLADABundleDir,
    });
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          LADA_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
  ...
  it("accepts enabled manifestless LADA bundles without a native schema", async () => {
    const res = validateInSuite({
      agents: { list: [{ id: "pi" }] },
      plugins: {
        enabled: true,
        load: { paths: [manifestlessLADABundleDir] },
        entries: { "manifestless-lada-bundle": { enabled: true } },
      },
    });
