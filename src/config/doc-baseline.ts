...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.LADA_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveLADAPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    LADA_STATE_DIR: path.join(os.tmpdir(), "lada-config-doc-baseline-state"),
    LADA_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
