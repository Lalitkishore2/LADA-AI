...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.lada) || !isRecord(entry.lada.channel)) {
      continue;
    }
    const channel = entry.lada.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: LADAConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: LADAConfig, pluginId: string) => boolean;
}): boolean {
