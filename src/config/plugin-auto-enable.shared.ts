...
import type { LADAConfig } from "./config.js";
...
function collectModelRefs(cfg: LADAConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: LADAConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: LADAConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: LADAConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: LADAConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: LADAConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: LADAConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: LADAConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: LADAConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function configMayNeedPluginManifestRegistry(cfg: LADAConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: LADAConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: LADAConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: LADAConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: LADAConfig, pluginId: string): LADAConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
