...
describe("legacy config detection", () => {
  ...
  it("does not auto-migrate lada-cli auth profile mode on load", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".lada", "lada.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        ...
        JSON.stringify(
          {
            auth: {
              profiles: {
                "anthropic:lada-cli": { provider: "anthropic", mode: "token" },
              },
            },
          },
          null,
      ...
      expect(cfg.auth?.profiles?.["anthropic:lada-cli"]?.mode).toBe("token");

      const raw = await fs.readFile(configPath, "utf-8");
      const parsed = JSON.parse(raw) as {
        auth?: { profiles?: Record<string, { mode?: string }> };
      };
      expect(parsed.auth?.profiles?.["anthropic:lada-cli"]?.mode).toBe("token");
    });
  });
