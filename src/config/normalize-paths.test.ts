  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.lada/logs/lada.log" },
        hooks: {
          path: "~/.lada/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".lada", "logs", "lada.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".lada", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".lada", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".lada", "agents", "main"));
