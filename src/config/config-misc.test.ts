...
import { buildWebSearchProviderConfig, withTempHome, writeLADAConfig } from "./test-helpers.js";
import { LADASchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = LADASchema.safeParse({
      $schema: "https://lada.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://lada.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = LADASchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = LADASchema.safeParse({
      cron: {
        enabled: true,
