...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lada-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ LADA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-lada",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-lada#general");
  });
  ...
  it("derives session transcripts dir from LADA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { LADA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when LADA_STATE_DIR differs from stored paths", () => {
  ...
  it("updateSessionStoreEntry merges concurrent patches", async () => {
    ...
    const p1 = updateSessionStoreEntry({
      ...
      update: async () => {
        ...
        return { modelOverride: "anthropic/lada-opus-4-6" };
      },
    });
    ...
    const store = loadSessionStore(storePath);
    expect(store[mainSessionKey]?.modelOverride).toBe("anthropic/lada-opus-4-6");
