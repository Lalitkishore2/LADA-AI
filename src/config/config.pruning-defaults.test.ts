...
describe("config pruning defaults", () => {
  ...
  it("enables cache-ttl pruning + 1h cache TTL for Anthropic API keys", async () => {
    ...
    expect(
      cfg.agents?.defaults?.models?.["anthropic/lada-opus-4-6"]?.params?.cacheRetention,
    ).toBe("short");
  });
  ...
  it("adds cacheRetention defaults for dated Anthropic primary model refs", async () => {
    const cfg = await loadConfigForHome({
      ...
      agents: {
        defaults: {
          model: { primary: "anthropic/lada-sonnet-4-20250514" },
        },
      },
    });
    ...
    expectAnthropicPruningDefaults(cfg);
    expect(
      cfg.agents?.defaults?.models?.["anthropic/lada-sonnet-4-20250514"]?.params?.cacheRetention,
    ).toBe("short");
  });

  it("adds default cacheRetention for Anthropic LADA models on Bedrock", async () => {
    const cfg = await loadConfigForHome({
      auth: {
        profiles: {
          "anthropic:api": { provider: "anthropic", mode: "api_key" },
      ...
      agents: {
        defaults: {
          model: { primary: "amazon-bedrock/us.anthropic.lada-opus-4-6-v1" },
        },
      },
    });

    expect(
      cfg.agents?.defaults?.models?.["amazon-bedrock/us.anthropic.lada-opus-4-6-v1"]?.params
        ?.cacheRetention,
    ).toBe("short");
  });
