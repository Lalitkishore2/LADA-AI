...
function collectConfigEnvVarsByTarget(cfg?: LADAConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: LADAConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: LADAConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: LADAConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.LADA_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
