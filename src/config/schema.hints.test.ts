...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = LADASchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "LADAConfig";
    const hints = mapSensitivePaths(LADASchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(LADASchema, "", isSensitiveUrlConfigPath);
