...
export function makeTempDir(): string {
  return makeTrackedTempDir("lada-plugin-auto-enable", tempDirs);
}

export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const rootDir = makeTempDir();
  return {
    LADA_STATE_DIR: path.join(rootDir, "state"),
    ...overrides,
  };
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "lada.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/lada.plugin.json`,
    })),
    diagnostics: [],
  };
}
