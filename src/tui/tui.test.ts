...
describe("resolveInitialTuiAgentId", () => {
  const cfg: LADAConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/lada" },
        { id: "ops", workspace: "/tmp/lada/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run lada devices list");
    expect(state.pairingHint).toContain("lada devices list");
  });
