...
const testItems = [
  {
    value: "anthropic/lada-3-opus",
    label: "anthropic/lada-3-opus",
    description: "LADA 3 Opus",
  },
  {
    value: "anthropic/lada-3-sonnet",
    label: "anthropic/lada-3-sonnet",
    description: "LADA 3 Sonnet",
  },
...
describe("SearchableSelectList", () => {
  ...
  it("navigates with arrow keys", () => {
    ...
    expect(list.getSelectedItem()?.value).toBe("anthropic/lada-3-opus");

    // Press down arrow (escape sequence for down arrow)
    list.handleInput("\x1b[B");

    expect(list.getSelectedItem()?.value).toBe("anthropic/lada-3-sonnet");
  });
  ...
  it("calls onSelect when enter is pressed", () => {
    ...
    expect(selectedValue).toBe("anthropic/lada-3-opus");
  });
