...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): LADAConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as LADAConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedDefaultAccountCase(input: LADAConfig, expected: string) {
