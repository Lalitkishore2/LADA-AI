...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: LADAConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: LADAConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: LADAConfig, policy: DmPolicy, accountId?: string) => LADAConfig;
