...
let originalLADALogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.LADA_TEST_FILE_LOG;
  originalLADALogLevel = process.env.LADA_LOG_LEVEL;
  delete process.env.LADA_TEST_FILE_LOG;
  delete process.env.LADA_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.LADA_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalLADALogLevel === undefined) {
    delete process.env.LADA_LOG_LEVEL;
  } else {
    process.env.LADA_LOG_LEVEL = originalLADALogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
