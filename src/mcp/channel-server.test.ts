...
import { z } from "zod";
import { createLADAChannelMcpServer, LADAChannelBridge } from "./channel-server.js";

const LADAChannelNotificationSchema = z.object({
  method: z.literal("notifications/lada/channel"),
  params: z.object({
    content: z.string(),
...
const LADAPermissionNotificationSchema = z.object({
  method: z.literal("notifications/lada/channel/permission"),
  params: z.object({
    request_id: z.string(),
...
async function connectMcpWithoutGateway(params?: { ladaChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createLADAChannelMcpServer({
    ladaChannelMode: params?.ladaChannelMode ?? "auto",
    verbose: false,
  });
...
describe("lada channel mcp server", () => {
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("emits LADA channel and permission notifications", async () => {
        const sessionKey = "agent:main:main";
        let mcp: Awaited<ReturnType<typeof connectMcpWithoutGateway>> | null = null;
        try {
          ...
          mcp.client.setNotificationHandler(LADAChannelNotificationSchema, ({ params }) => {
            channelNotifications.push(params);
          });
          mcp.client.setNotificationHandler(LADAPermissionNotificationSchema, ({ params }) => {
          ...
          await (
            ...
            message: {
              role: "user",
              content: [{ type: "text", text: "hello LADA" }],
              timestamp: Date.now(),
            },
          });
          ...
          await mcp.client.notification({
            method: "notifications/lada/channel/permission_request",
            params: {
              request_id: "abcde",
              tool_name: "Bash",
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new LADAChannelBridge({} as never, {
        ladaChannelMode: "off",
        verbose: false,
      });
    ...
    test("swallows notification send errors after channel replies are matched", async () => {
      ...
      (
        bridge as unknown as {
          pendingLADAPermissions: Map<string, Record<string, unknown>>;
          server: { server: { notification: ReturnType<typeof vi.fn> } };
        }
      ).pendingLADAPermissions.set("abcde", {
        toolName: "Bash",
        description: "run npm test",
    ...
    test("waits for queued events through the MCP tool", async () => {
      const mcp = await connectMcpWithoutGateway({ ladaChannelMode: "off" });
      try {
        await (
          mcp.bridge as unknown as {
            handleSessionMessageEvent: (payload: Record<string, unknown>) => Promise<void>;
