...
export class LADAChannelBridge {
  private gateway: GatewayClient | null = null;
  private readonly verbose: boolean;
  private readonly ladaChannelMode: LADAChannelMode;
  private readonly queue: QueueEvent[] = [];
  private readonly pendingWaiters = new Set<PendingWaiter>();
  private readonly pendingLADAPermissions = new Map<string, LADAPermissionRequest>();
  ...
  constructor(
    private readonly cfg: LADAConfig,
    ...
    this.ladaChannelMode = params.ladaChannelMode;
  ...
  async handleLADAPermissionRequest(params: {
    requestId: string;
    toolName: string;
    description: string;
    inputPreview: string;
  }): Promise<void> {
    this.pendingLADAPermissions.set(params.requestId, {
    ...
    if (this.verbose) {
      process.stderr.write(`lada mcp: pending LADA permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `lada mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
  ...
  private async handleSessionMessageEvent(payload: SessionMessagePayload): Promise<void> {
    ...
    if (permissionMatch) {
      ...
      if (requestId && this.pendingLADAPermissions.has(requestId)) {
        this.pendingLADAPermissions.delete(requestId);
        await this.sendNotification({
          method: "notifications/lada/channel/permission",
          params: {
    ...
    if (!this.shouldEmitLADAChannel(role, conversation)) {
      return;
    }
    await this.sendNotification({
      method: "notifications/lada/channel",
      params: {
  ...
  private shouldEmitLADAChannel(
    role: string | undefined,
    conversation: ConversationDescriptor | undefined,
  ): boolean {
    if (this.ladaChannelMode === "off") {
      return false;
    }
    if (role !== "user") {
      return false;
