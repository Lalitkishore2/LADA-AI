...
export type LADAChannelMode = "off" | "on" | "auto";
...
export type LADAPermissionRequest = {
  toolName: string;
  description: string;
  inputPreview: string;
};
...
export const LADAPermissionRequestSchema = z.object({
  method: z.literal("notifications/lada/channel/permission_request"),
  params: z.object({
    request_id: z.string(),
    tool_name: z.string(),
...
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry.__lada && typeof entry.__lada === "object"
      ? toText((entry.__lada as { id?: unknown }).id)
      : undefined)
  );
}
