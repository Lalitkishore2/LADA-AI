...
import { z } from "zod";
import type { LADAChannelBridge } from "./channel-bridge.js";
import {
...
export function getChannelMcpCapabilities(ladaChannelMode: "off" | "on" | "auto") {
  if (ladaChannelMode === "off") {
    return undefined;
  }
  return {
    experimental: {
...
export function registerChannelMcpTools(server: McpServer, bridge: LADAChannelBridge): void {
  server.tool(
    "conversations_list",
    "List LADA channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one LADA conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one LADA conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one LADA conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued LADA conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued LADA conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same LADA conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open LADA exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending LADA exec or plugin approval request.",
    {
