...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getVideoGenerationProvider: vi.fn<
      (providerId: string, config?: LADAConfig) => VideoGenerationProvider | undefined
    >(() => undefined),
    isFailoverError: vi.fn<(err: unknown) => boolean>(() => false),
    listVideoGenerationProviders: vi.fn<(config?: LADAConfig) => VideoGenerationProvider[]>(
      () => [],
    ),
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeVideoGenerationProviders({ config: {} as LADAConfig })).toEqual(
      providers,
    );
    expect(mocks.listVideoGenerationProviders).toHaveBeenCalledWith({} as LADAConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateVideo({ cfg: {} as LADAConfig, prompt: "animate a cat" });
