...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.LADA_BASE_ENV).toBe("base");
    expect(resolved.LADA_TEST_ENV).toBe("ok");
    expect(resolved.LADA_TO_REMOVE).toBeUndefined();
    expect(resolved.LADA_CLI).toBe(LADA_CLI_ENV_VALUE);
  });
