...
describe("canvas host state dir defaults", () => {
  ...
  it("uses LADA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("lada-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("LADA Canvas");
      } finally {
        await handler.close();
      }
    });
