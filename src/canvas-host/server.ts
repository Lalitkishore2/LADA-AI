...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.ladaCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.ladaCanvasA2UIAction &&
        typeof window.ladaCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.ladaSendUserAction === "function";
  ...
  window.addEventListener("lada:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android LADA node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.ladaSendUserAction === "function"
        ? window.ladaSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LADA_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.LADA_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>LADA Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
