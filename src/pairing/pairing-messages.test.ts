...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["LADA_CONTAINER_HINT", "LADA_PROFILE"]);
    delete process.env.LADA_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:lada|lada) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
