...
import { extractLADACodeHints } from '../../utils/ladaCodeHints.js';
...
export const PowerShellTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam({
    ...
    // For image data, format as image content block for LADA
    if (isImage) {
  ...
  async call(input: PowerShellToolInput, toolUseContext: Parameters<Tool['call']>[1], _canUseTool?: CanUseToolFn, _parentMessage?: AssistantMessage, onProgress?: ToolCallProgress<PowerShellProgress>): Promise<{
    ...
    try {
      ...
      if (result.backgroundTaskId) {
        const bgExtracted = extractLADACodeHints(result.stdout || '', input.command);
      ...
      // LADA hints protocol: CLIs/SDKs gated on CLAUDECODE=1 emit a
      // `<lada-code-hint />` tag to stderr (merged into stdout here). Scan,
      // record for useLADACodeHintRecommendation to surface, then strip
      // so the model never sees the tag — a zero-token side channel.
      // Stripping runs unconditionally (subagent output must stay clean too);
      // only the dialog recording is main-thread-only.
      const extracted = extractLADACodeHints(stdout, input.command);
...
async function* runPowerShellCommand({
  ...
  // Handle LADA asking to run it in the background explicitly
