...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .lada is a dangerous directory
  // and internal editable paths live under ~/.lada/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/lada/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.lada; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.lada/settings.json (LADA's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.lada`, validate subsequent statements with
  // cwd='./.lada'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, LADA may only access files in the allowed working directories for this session: ${dirListStr}.`
