...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // lada.ai connectors use a separate auth flow (handleLADAAIAuth in
      ...
      if (config.type === 'ladaai-proxy') {
        return {
          data: {
            ...
            message: `This is a lada.ai MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
